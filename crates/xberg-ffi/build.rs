...
// alef:hash:3e8ca7f31f72b7517acb09ea43dfaf48f8d121dcfbfc975299dbe8821dca42ba
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if std::env::var("CARGO_CFG_TARGET_OS").as_deref() == Ok("macos") {
        println!("cargo:rustc-link-arg-cdylib=-Wl,-install_name,@rpath/libxberg_ffi.dylib");
...
fn export_generated_headers() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut header = String::from_utf8(bytes).map_err(|error| {
        io::Error::new(
            ...
            format!("cbindgen generated invalid UTF-8 for include/xberg.h: {error}"),
        )
    })?;

    stamp_enabled_features(&mut header);
    let destinations = [
        Path::new("include/xberg.h"),
        Path::new("../../packages/go/include/xberg.h"),
    ];
...
fn stamp_enabled_features(header: &mut String) {
    ...
    if header.contains(marker) || !header.contains("XBERG_FEATURE_") {
        return;
    }
    let mut defines: Vec<String> = std::env::vars_os()
        .filter_map(|(key, _)| {
            ...
            Some(format!("#define XBERG_FEATURE_{name} 1"))
        })
        .collect();
    defines.sort();
    defines.dedup();
    let block = format!("\n{marker}\n{}\n", defines.join("\n"));
    let anchor = "#define XBERG_H\n";
