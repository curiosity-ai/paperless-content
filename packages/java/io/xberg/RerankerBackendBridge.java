...
// alef:hash:b966c328b2b0ba45b1ff1abf3fc58a3b73084a0a13347caea76971559aa541c5
...
/**
 * Allocates Panama FFM upcall stubs for an IRerankerBackend implementation,
 * assembles the C vtable in native memory, and provides static
 * registerRerankerBackend/unregisterRerankerBackend helpers.
 */
@SuppressWarnings("PMD")
public final class RerankerBackendBridge implements AutoCloseable {

    private static final Linker LINKER = Linker.nativeLinker();
    private static final MethodHandles.Lookup LOOKUP = MethodHandles.lookup();
    private static final ObjectMapper JSON = new ObjectMapper();
    private static final System.Logger LOGGER = System.getLogger(RerankerBackendBridge.class.getName());

    /** Live registry — keeps Arenas and upcall stubs alive past the register call. */
    private static final ConcurrentHashMap<String, RerankerBackendBridge>
            RERANKER_BACKEND_BRIDGES = new ConcurrentHashMap<>();
    ...
    RerankerBackendBridge(final IRerankerBackend impl) {
        this.impl = impl;
    ...
    private void initializeStubs() throws ReflectiveOperationException {
        ...
        initStubRerank(4L * ValueLayout.ADDRESS.byteSize());
    ...
    private void initStubRerank(long offset) throws ReflectiveOperationException {
        var stubRerank = LINKER.upcallStub(LOOKUP.bind(this, "handleRerank",
            ...
            arena);
        vtable.set(ValueLayout.ADDRESS, offset, stubRerank);
    }
    ...
    private int handleRerank(
        MemorySegment userData,
        MemorySegment query_in,
        MemorySegment documents_in,
    ...
    /** Register a RerankerBackend implementation via Panama FFM upcall stubs. */
    public static void registerRerankerBackend(final IRerankerBackend impl) throws Exception {
        ...
        try {
            try (var nameArena = Arena.ofShared()) {
                ...
                if (rc != 0) {
                    ...
                    throw new RuntimeException("registerRerankerBackend: " + msg);
                }
            }
        } catch (Throwable t) {
            bridge.close();
        ...
        RERANKER_BACKEND_BRIDGES.put(impl.name(), bridge);
    }

    /** Unregister a RerankerBackend implementation by name. */
    public static void unregisterRerankerBackend(String name) throws Exception {
        try {
            try (var nameArena = Arena.ofShared()) {
                ...
                if (rc != 0) {
                    ...
                    throw new RuntimeException("unregisterRerankerBackend: " + msg);
                }
            }
        } catch (Throwable t) {
            if (t instanceof Exception e) {
        ...
        RerankerBackendBridge old = RERANKER_BACKEND_BRIDGES.remove(name);
        if (old != null) { old.close(); }
    }
    /** Clear all registered RerankerBackend implementations. */
    public static void clearRerankerBackends() throws Exception {
        try {
            try (var arena = Arena.ofShared()) {
                ...
                if (rc != 0) {
                    ...
                    String msg = errPtr.equals(MemorySegment.NULL)
                        ? "clear failed (rc=" + rc + ")"
                        : errPtr.reinterpret(Long.MAX_VALUE).getString(0);
                    throw new RuntimeException("clearRerankerBackends: " + msg);
                }
            }
        } catch (Throwable t) {
            if (t instanceof Exception e) {
        ...
        RERANKER_BACKEND_BRIDGES.values().forEach(RerankerBackendBridge::close);
        RERANKER_BACKEND_BRIDGES.clear();
    }
}
