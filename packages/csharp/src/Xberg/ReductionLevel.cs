// This file is auto-generated by alef. DO NOT EDIT.
// alef:hash:e8797b878ba7c0ab877dbdaa6672475ceea8d795529f7061400281ae04a26ff0
#nullable enable

using System;
using System.Text.Json;
using System.Text.Json.Serialization;

namespace Xberg;
/// <summary>
/// Intensity level for the token-reduction pipeline.
/// </summary>
[JsonConverter(typeof(ReductionLevelJsonConverter))]
public enum ReductionLevel {
  /// <summary>
  /// No reduction applied; text is returned as-is.
  /// </summary>
  [JsonPropertyName("Off")] Off,
  /// <summary>
  /// Remove only the most common stopwords.
  /// </summary>
  [JsonPropertyName("Light")] Light,
  /// <summary>
  /// Balanced stopword removal and redundancy filtering.
  /// </summary>
  [JsonPropertyName("Moderate")] Moderate,
  /// <summary>
  /// Aggressive filtering; may remove less common content words.
  /// </summary>
  [JsonPropertyName("Aggressive")] Aggressive,
  /// <summary>
  /// Maximum compression; prioritizes brevity over completeness.
  /// </summary>
  [JsonPropertyName("Maximum")] Maximum,
}

/// <summary>
/// Custom JSON converter for <see cref="ReductionLevel"/> that respects
/// explicit variant names.
/// </summary>
internal sealed class ReductionLevelJsonConverter
    : JsonConverter<ReductionLevel> {
  public override ReductionLevel Read(ref Utf8JsonReader reader,
                                      Type typeToConvert,
                                      JsonSerializerOptions options) {
    var value = reader.GetString();
    return value switch { "Off" => ReductionLevel.Off,
                          "Light" => ReductionLevel.Light,
                          "Moderate" => ReductionLevel.Moderate,
                          "Aggressive" => ReductionLevel.Aggressive,
                          "Maximum" => ReductionLevel.Maximum,
                          _ => throw new JsonException(
                              $"Unknown ReductionLevel value: {value}") };
  }

  public override void Write(Utf8JsonWriter writer, ReductionLevel value,
                             JsonSerializerOptions options) {
    var str = value switch { ReductionLevel.Off => "Off",
                             ReductionLevel.Light => "Light",
                             ReductionLevel.Moderate => "Moderate",
                             ReductionLevel.Aggressive => "Aggressive",
                             ReductionLevel.Maximum => "Maximum",
                             _ => throw new JsonException(
                                 $"Unknown ReductionLevel value: {value}") };
    writer.WriteStringValue(str);
  }
}
