...
// alef:hash:c789b231e146c8dce2c4dc2f090277c7f67b5ebf346018b0efa887ec5db28f69
...
///
/// Defaults to `Characters` (Unicode character count). When using token-based
/// sizing, chunks are sized by token count according to the specified
/// tokenizer.
///
/// Token-based sizing uses HuggingFace tokenizers loaded at runtime, or a
/// tokenizer backend you register yourself. Any tokenizer available on
/// HuggingFace Hub can be used, including OpenAI-compatible tokenizers (e.g.,
/// `Xenova/gpt-4o`, `Xenova/cl100k_base`). To size chunks with your own
/// tokenizer instead (llama.cpp/GGUF vocabularies, SentencePiece models, custom
/// vocabs), register a `TokenizerBackend` with `register_tokenizer_backend` and
/// set `model` to the registered name.
/// </summary>
...
public sealed class ChunkSizingJsonConverter : JsonConverter<ChunkSizing> {
  public override ChunkSizing Read(ref Utf8JsonReader reader,
                                   Type typeToConvert,
    ...
    return tagValue switch {
      ...
      "tokenizer" => JsonSerializer.Deserialize<ChunkSizing.Tokenizer>(
                         flatJson, options) ??
      ...
      _ => throw new JsonException(
          $"Unknown ChunkSizing discriminator: {tagValue}")
    };
  }

  public override void Write(Utf8JsonWriter writer, ChunkSizing value,
                             JsonSerializerOptions options) {
    ...
    case ChunkSizing.Characters _:
      ...
      inner = null;
      break;
    case ChunkSizing.Tokenizer v_tokenizer:
      tag = "tokenizer";
      inner = v_tokenizer;
      break;
    default:
      throw new JsonException(
          $"Unknown ChunkSizing variant: {value.GetType().Name}");
    }
