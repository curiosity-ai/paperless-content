// This file is auto-generated by alef. DO NOT EDIT.
// alef:hash:3d29a8fb7f18dbec15db7e9e48fd9b147b9b8f02b5f63e2141b6231771f9b6e3
#nullable enable

using System;
using System.Text.Json;
using System.Text.Json.Serialization;

namespace Xberg;
/// <summary>
/// PII categories the pattern engine recognises.
/// </summary>
[JsonConverter(typeof(PiiCategoryJsonConverter))]
public enum PiiCategory {
  /// <summary>
  /// Email address (e.g. `user@example.com`).
  /// </summary>
  [JsonPropertyName("email")] Email,
  /// <summary>
  /// Phone number in any common format.
  /// </summary>
  [JsonPropertyName("phone")] Phone,
  /// <summary>
  /// US Social Security Number.
  /// </summary>
  [JsonPropertyName("ssn")] Ssn,
  /// <summary>
  /// Payment card number (Visa, Mastercard, Amex, etc.).
  /// </summary>
  [JsonPropertyName("credit_card")] CreditCard,
  /// <summary>
  /// Postal / ZIP code.
  /// </summary>
  [JsonPropertyName("postal_code")] PostalCode,
  /// <summary>
  /// IPv4 or IPv6 address.
  /// </summary>
  [JsonPropertyName("ip_address")] IpAddress,
  /// <summary>
  /// International Bank Account Number.
  /// </summary>
  [JsonPropertyName("iban")] Iban,
  /// <summary>
  /// SWIFT / BIC bank identifier code.
  /// </summary>
  [JsonPropertyName("swift_bic")] SwiftBic,
  /// <summary>
  /// Date of birth.
  /// </summary>
  [JsonPropertyName("date_of_birth")] DateOfBirth,
  /// <summary>
  /// Person name, surfaced by the optional NER backend.
  /// </summary>
  [JsonPropertyName("person")] Person,
  /// <summary>
  /// Organization name, surfaced by the optional NER backend.
  /// </summary>
  [JsonPropertyName("organization")] Organization,
  /// <summary>
  /// Location, surfaced by the optional NER backend.
  /// </summary>
  [JsonPropertyName("location")] Location,
  /// <summary>
  /// Caller-supplied custom category (e.g. internal employee IDs).
  ///
  /// Surfaced by the redaction engine when a hit comes from
  /// `RedactionConfig.custom_terms`
  /// or `RedactionConfig.custom_patterns`.
  /// The string is the label passed alongside the term/pattern. Use those
  /// fields rather than constructing `Custom` directly via the
  /// `categories` filter — the pattern engine cannot detect arbitrary text
  /// from a category name alone.
  /// </summary>
  [JsonPropertyName("custom")] Custom,
}

/// <summary>
/// Custom JSON converter for <see cref="PiiCategory"/> that respects explicit
/// variant names.
/// </summary>
internal sealed class PiiCategoryJsonConverter : JsonConverter<PiiCategory> {
  public override PiiCategory Read(ref Utf8JsonReader reader,
                                   Type typeToConvert,
                                   JsonSerializerOptions options) {
    var value = reader.GetString();
    return value switch { "email" => PiiCategory.Email,
                          "phone" => PiiCategory.Phone,
                          "ssn" => PiiCategory.Ssn,
                          "credit_card" => PiiCategory.CreditCard,
                          "postal_code" => PiiCategory.PostalCode,
                          "ip_address" => PiiCategory.IpAddress,
                          "iban" => PiiCategory.Iban,
                          "swift_bic" => PiiCategory.SwiftBic,
                          "date_of_birth" => PiiCategory.DateOfBirth,
                          "person" => PiiCategory.Person,
                          "organization" => PiiCategory.Organization,
                          "location" => PiiCategory.Location,
                          "custom" => PiiCategory.Custom,
                          _ => throw new JsonException(
                              $"Unknown PiiCategory value: {value}") };
  }

  public override void Write(Utf8JsonWriter writer, PiiCategory value,
                             JsonSerializerOptions options) {
    var str = value switch { PiiCategory.Email => "email",
                             PiiCategory.Phone => "phone",
                             PiiCategory.Ssn => "ssn",
                             PiiCategory.CreditCard => "credit_card",
                             PiiCategory.PostalCode => "postal_code",
                             PiiCategory.IpAddress => "ip_address",
                             PiiCategory.Iban => "iban",
                             PiiCategory.SwiftBic => "swift_bic",
                             PiiCategory.DateOfBirth => "date_of_birth",
                             PiiCategory.Person => "person",
                             PiiCategory.Organization => "organization",
                             PiiCategory.Location => "location",
                             PiiCategory.Custom => "custom",
                             _ => throw new JsonException(
                                 $"Unknown PiiCategory value: {value}") };
    writer.WriteStringValue(str);
  }
}
