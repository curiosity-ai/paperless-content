...
// alef:hash:e44ab1d8bdbd91ee7eabd5b93301025ec359a68d504d73bb72463f9cf6af2eab
...
/// `tree_sitter_language_pack.DataNode` but flattens its `Span` down to plain
/// byte offsets, so binding generators never need to resolve an external
...
public sealed record CodeDataNode {
  ...
  public List<CodeDataAttribute> Attributes {
    get; init;
  } = [];
  ...
  public List<CodeDataNode> Children {
    get; init;
  } = [];
    ...
    try {
      return JsonSerializer.Deserialize<CodeDataNode>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse CodeDataNode from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse CodeDataNode from JSON: {e.Message}", e);
    }
  }
