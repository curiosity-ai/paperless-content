// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:38951b9ddc6685c9ecd0f82773d2611a6992333ba0b8de853d1b75516c0f63a1
// To regenerate: alef generate
// To verify freshness: alef verify
#nullable enable

using System;
using System.Collections.Generic;
using System.Text.Json;
using System.Text.Json.Serialization;

namespace Xberg;

/// <summary>
/// A single named entity detected in the extracted text.
/// </summary>
public sealed record Entity {
  /// <summary>
  /// Canonical category the entity belongs to (PERSON, ORG, LOCATION, etc.).
  /// </summary>
  [JsonPropertyName("category")]
  public required EntityCategory Category { get; init; }

  /// <summary>
  /// Raw mention text exactly as it appeared in the source.
  /// </summary>
  [JsonPropertyName("text")]
  public required string Text {
    get; init;
  }

  /// <summary>
  /// Byte-offset span in `ExtractedDocument.content` where the mention starts.
  /// </summary>
  [JsonPropertyName("start")]
  public uint Start {
    get; init;
  } = 0;

  /// <summary>
  /// Byte-offset span in `ExtractedDocument.content` where the mention ends
  /// (exclusive).
  /// </summary>
  [JsonPropertyName("end")]
  public uint End {
    get; init;
  } = 0;

  /// <summary>
  /// Backend-reported confidence in `[0.0, 1.0]`. `None` when the backend does
  /// not expose confidence scores.
  /// </summary>
  [JsonPropertyName("confidence")]
  public float
      ? Confidence {
          get; init;
        } = null;

  /// <summary>
  /// Parse a <see cref="Entity"/> from a JSON string.
  /// </summary>
  /// <exception cref="XbergException">When the JSON cannot be
  /// deserialised.</exception>
  public static Entity FromJson(string json) {
    try {
      return JsonSerializer.Deserialize<Entity>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse Entity from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException($"Failed to parse Entity from JSON: {e.Message}",
                               e);
    }
  }

  private static readonly JsonSerializerOptions JsonOptions = new() {
    DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingDefault,
    Converters = { new JsonStringEnumConverter(JsonNamingPolicy.SnakeCaseLower),
                   new ByteArrayJsonConverter() },
  };

  /// <summary>Options for serializing config/input objects to FFI. Strips nulls
  /// (nullable C# fields default to null and would override required Rust
  /// fields with non-deserialisable nulls) but preserves explicit false/0 so
  /// caller intent is kept.</summary>
  private static readonly JsonSerializerOptions JsonSerializationOptions =
      new() {
        DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingNull,
        Converters = { new JsonStringEnumConverter(
                           JsonNamingPolicy.SnakeCaseLower),
                       new ByteArrayJsonConverter() },
      };
}
