...
// alef:hash:c8d939a9403c15b7f882a120a2ae44e376b7c0f149531d053352dc8b9c0f629a
...
/// tree. Root-level nodes have `parent: None`. Use `body_roots()` and
/// `furniture_roots()` to iterate over top-level content by layer.
///
/// # Validation
///
/// Call `validate()` after construction to verify all node indices are in
...
public sealed record DocumentStructure {
  ...
  /// <summary>
  /// Resolved relationships between nodes (footnote refs, citations, anchor
  /// links, etc.).
  ///
  ...
  public List<DocumentRelationship> Relationships {
    get; init;
  } = [];
    ...
    try {
      return JsonSerializer.Deserialize<DocumentStructure>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse DocumentStructure from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse DocumentStructure from JSON: {e.Message}", e);
    }
  }
  ...
  public static DocumentStructure Default() {
    ...
    return JsonSerializer.Deserialize<DocumentStructure>(json ?? "null",
                                                         JsonOptions)!;
  }
}
