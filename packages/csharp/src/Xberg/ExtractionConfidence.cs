...
// alef:hash:eaeaf15258062603fb2f6bedfe0c3bf846dd8df006c47ed9db3c891156912b1d
...
public sealed record ExtractionConfidence {
  ...
  /// <summary>
  /// Mean OCR per-element recognition confidence when OCR ran; `None` when it
  /// did not.
  /// </summary>
  ...
  public required SchemaCompliance SchemaCompliance {
    get; init;
  }

  /// <summary>
  /// Weighted blend in `[0, 1]`.  The value compared against the fallback
  /// threshold.
  /// </summary>
  ...
  public static ExtractionConfidence FromJson(string json) {
    try {
      return JsonSerializer.Deserialize<ExtractionConfidence>(json,
                                                              JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse ExtractionConfidence from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse ExtractionConfidence from JSON: {e.Message}", e);
    }
  }
