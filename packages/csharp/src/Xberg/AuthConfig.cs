...
// alef:hash:f191490263aa90829a17ddc364cff55e32805922b37c44c22cbeedd90ae4fe59
...
public sealed class AuthConfigJsonConverter : JsonConverter<AuthConfig> {
  public override AuthConfig Read(ref Utf8JsonReader reader, Type typeToConvert,
    ...
    return tagValue switch {
      "basic" =>
          JsonSerializer.Deserialize<AuthConfig.Basic>(flatJson, options) ??
          throw new JsonException("Failed to deserialize variant"),
      "bearer" =>
          JsonSerializer.Deserialize<AuthConfig.Bearer>(flatJson, options) ??
          throw new JsonException("Failed to deserialize variant"),
      "header" =>
          JsonSerializer.Deserialize<AuthConfig.Header>(flatJson, options) ??
          throw new JsonException("Failed to deserialize variant"),
      // Unknown discriminator: forward compatibility for unknown variants
      _ => throw new JsonException(
          $"Unknown AuthConfig discriminator: {tagValue}")
    };
  }

  public override void Write(Utf8JsonWriter writer, AuthConfig value,
                             JsonSerializerOptions options) {
    ...
    case AuthConfig.Basic v_basic:
      ...
      inner = v_basic;
      break;
    case AuthConfig.Bearer v_bearer:
      tag = "bearer";
      inner = v_bearer;
      break;
    case AuthConfig.Header v_header:
      tag = "header";
      inner = v_header;
      break;
    default:
      throw new JsonException(
          $"Unknown AuthConfig variant: {value.GetType().Name}");
    }
