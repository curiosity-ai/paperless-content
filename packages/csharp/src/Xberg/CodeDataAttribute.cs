...
// alef:hash:b54921369d08ef95a93cf392d80920a982806cb6663d5928f1d16c02848e09e1
...
///
/// Populated only for `CodeDataNodeKind.Element`; always empty for `KeyValue`
/// and `Sequence` nodes.
/// </summary>
    ...
    try {
      return JsonSerializer.Deserialize<CodeDataAttribute>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse CodeDataAttribute from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse CodeDataAttribute from JSON: {e.Message}", e);
    }
  }
