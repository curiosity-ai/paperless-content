...
// alef:hash:1d2eff04320a4e2dd59f44216bc58aeff74f3c0b0092b88d20143d599fa511e7
...
///
/// Wrapped by `FormatMetadata.Code`. Kept as a named struct (rather than an
/// inline enum-variant body) so serde can tag it under internal tagging and
/// utoipa can emit a referenceable `CodeMetadata` component in the OpenAPI
/// schema.
/// </summary>
    ...
    try {
      return JsonSerializer.Deserialize<CodeMetadata>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse CodeMetadata from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse CodeMetadata from JSON: {e.Message}", e);
    }
  }
