// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:5801e313cc8c3098dfcc46ac0a567b7ce18738e0e4ead58e2b0cb24efbd117be
// To regenerate: alef generate
// To verify freshness: alef verify
#nullable enable

using System;
using System.Collections.Generic;
using System.Text.Json;
using System.Text.Json.Serialization;

namespace Xberg;

/// <summary>
/// HTML preprocessing options for document cleanup before conversion.
/// </summary>
public sealed record PreprocessingOptions {
  /// <summary>
  /// Enable HTML preprocessing globally
  /// </summary>
  [JsonPropertyName("enabled")]
  public bool Enabled { get; init; } = true;

  /// <summary>
  /// Preprocessing preset level (Minimal, Standard, Aggressive)
  /// </summary>
  [JsonPropertyName("preset")]
  public PreprocessingPreset Preset {
    get; init;
  } = PreprocessingPreset.Standard;

  /// <summary>
  /// Remove navigation elements (nav, breadcrumbs, menus, sidebars)
  /// </summary>
  [JsonPropertyName("remove_navigation")]
  public bool RemoveNavigation {
    get; init;
  } = true;

  /// <summary>
  /// Remove form elements (forms, inputs, buttons, etc.)
  /// </summary>
  [JsonPropertyName("remove_forms")]
  public bool RemoveForms {
    get; init;
  } = true;

  /// <summary>
  /// Parse a <see cref="PreprocessingOptions"/> from a JSON string.
  /// </summary>
  /// <exception cref="XbergException">When the JSON cannot be
  /// deserialised.</exception>
  public static PreprocessingOptions FromJson(string json) {
    try {
      return JsonSerializer.Deserialize<PreprocessingOptions>(json,
                                                              JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse PreprocessingOptions from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse PreprocessingOptions from JSON: {e.Message}", e);
    }
  }

  private static readonly JsonSerializerOptions JsonOptions = new() {
    DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingDefault,
    Converters = { new JsonStringEnumConverter(JsonNamingPolicy.SnakeCaseLower),
                   new ByteArrayJsonConverter() },
  };

  /// <summary>Options for serializing config/input objects to FFI. Strips nulls
  /// (nullable C# fields default to null and would override required Rust
  /// fields with non-deserialisable nulls) but preserves explicit false/0 so
  /// caller intent is kept.</summary>
  private static readonly JsonSerializerOptions JsonSerializationOptions =
      new() {
        DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingNull,
        Converters = { new JsonStringEnumConverter(
                           JsonNamingPolicy.SnakeCaseLower),
                       new ByteArrayJsonConverter() },
      };
}
