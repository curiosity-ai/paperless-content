...
// alef:hash:9876e94a9e9a3e0a86d714c2e15300ad040c82ea4633a997e0afaa1cfee1f0a2
...
public sealed record LateInteractionConfig {
  /// <summary>
  /// The late-interaction model to use (defaults to the "gte-moderncolbert"
  /// preset).
  /// </summary>
  ...
  /// reports nothing. Ignored by `LateInteractionModelType.Plugin`, which
  /// downloads no model.
  /// </summary>
  ...
  public static LateInteractionConfig FromJson(string json) {
    try {
      return JsonSerializer.Deserialize<LateInteractionConfig>(json,
                                                               JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse LateInteractionConfig from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse LateInteractionConfig from JSON: {e.Message}", e);
    }
  }
  ...
  public static LateInteractionConfig Default() {
    ...
    return JsonSerializer.Deserialize<LateInteractionConfig>(json ?? "null",
                                                             JsonOptions)!;
  }
}
