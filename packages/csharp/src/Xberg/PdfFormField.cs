// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:368f0e0bd3cd87dfe07335547fc0093b33787d5342e38188d107dbde8b53a03a
// To regenerate: alef generate
// To verify freshness: alef verify
#nullable enable

using System;
using System.Collections.Generic;
using System.Text.Json;
using System.Text.Json.Serialization;

namespace Xberg;

/// <summary>
/// A form field extracted from a PDF's AcroForm or XFA structure.
///
/// Populated by the PDF extractor when `PdfConfig.extract_form_fields` is
/// enabled and the document is a fillable form. Supports both AcroForm
/// (standard) and XFA (XML Forms Architecture) layers. When both are present,
/// AcroForm fields take priority (canonical fallback per PDF spec), and
/// XFA-only fields are appended. The collection is empty for non-form PDFs and
/// for non-PDF formats.
/// </summary>
public sealed record PdfFormField {
  /// <summary>
  /// Partial field name (the leaf name within the field hierarchy).
  /// </summary>
  [JsonPropertyName("name")]
  public required string Name { get; init; }

  /// <summary>
  /// Fully-qualified field name (dotted path from the form root).
  /// </summary>
  [JsonPropertyName("full_name")]
  public required string FullName {
    get; init;
  }

  /// <summary>
  /// Classified field type.
  /// </summary>
  [JsonPropertyName("field_type")]
  public required FormFieldType FieldType {
    get; init;
  }

  /// <summary>
  /// Current field value, if any.
  /// </summary>
  [JsonPropertyName("value")]
  public string
      ? Value {
          get; init;
        } = null;

  /// <summary>
  /// Default field value, if any.
  /// </summary>
  [JsonPropertyName("default_value")]
  public string
      ? DefaultValue {
          get; init;
        } = null;

  /// <summary>
  /// Raw field-flags bitmask (read-only, required, multiline, …).
  /// </summary>
  [JsonPropertyName("flags")]
  public uint Flags {
    get; init;
  } = 0;

  /// <summary>
  /// 1-indexed page the field's widget appears on. Currently always `None` for
  /// AcroForm fields; page assignment is a deferred enhancement requiring
  /// spatial analysis of widget annotations per page.
  /// </summary>
  [JsonPropertyName("page")]
  public uint
      ? Page {
          get; init;
        } = null;

  /// <summary>
  /// Widget bounding box on its page, if known.
  /// </summary>
  [JsonPropertyName("bbox")]
  public BoundingBox
      ? Bbox {
          get; init;
        } = null;

  /// <summary>
  /// Maximum input length for text fields, if specified.
  /// </summary>
  [JsonPropertyName("max_length")]
  public uint
      ? MaxLength {
          get; init;
        } = null;

  /// <summary>
  /// Tooltip / alternate field description, if present.
  /// </summary>
  [JsonPropertyName("tooltip")]
  public string
      ? Tooltip {
          get; init;
        } = null;

  /// <summary>
  /// Parse a <see cref="PdfFormField"/> from a JSON string.
  /// </summary>
  /// <exception cref="XbergException">When the JSON cannot be
  /// deserialised.</exception>
  public static PdfFormField FromJson(string json) {
    try {
      return JsonSerializer.Deserialize<PdfFormField>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse PdfFormField from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse PdfFormField from JSON: {e.Message}", e);
    }
  }

  private static readonly JsonSerializerOptions JsonOptions = new() {
    DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingDefault,
    Converters = { new JsonStringEnumConverter(JsonNamingPolicy.SnakeCaseLower),
                   new ByteArrayJsonConverter() },
  };

  /// <summary>Options for serializing config/input objects to FFI. Strips nulls
  /// (nullable C# fields default to null and would override required Rust
  /// fields with non-deserialisable nulls) but preserves explicit false/0 so
  /// caller intent is kept.</summary>
  private static readonly JsonSerializerOptions JsonSerializationOptions =
      new() {
        DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingNull,
        Converters = { new JsonStringEnumConverter(
                           JsonNamingPolicy.SnakeCaseLower),
                       new ByteArrayJsonConverter() },
      };
}
