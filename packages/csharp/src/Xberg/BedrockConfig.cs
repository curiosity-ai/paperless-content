...
// alef:hash:64b99dbe02b98e58633456ed22797993e7e5a55503d58668b94f8de1daaece69
...
/// `AWS_SECRET_ACCESS_KEY`, `AWS_SESSION_TOKEN`, `BEDROCK_CROSS_REGION`) and
/// the default AWS credential chain. Leave the credential fields unset unless
/// you have an explicit reason to pin them.
/// </summary>
public sealed record BedrockConfig {
  ...
  /// <summary>
  /// Explicit AWS session token for temporary credentials. Secret — never
  /// logged.
  /// </summary>
    ...
    try {
      return JsonSerializer.Deserialize<BedrockConfig>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse BedrockConfig from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse BedrockConfig from JSON: {e.Message}", e);
    }
  }
