// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:3d1d46382ce5a3fc0a6d480c8020e1d6b27a7ad55dd48d90cf6767f7698f8d6b
// To regenerate: alef generate
// To verify freshness: alef verify
#nullable enable

using System;
using System.Collections.Generic;
using System.Text.Json;
using System.Text.Json.Serialization;

namespace Xberg;

/// <summary>
/// PDF-specific metadata.
///
/// Contains metadata fields specific to PDF documents that are not in the
/// common `Metadata` structure. Common fields like title, authors, keywords,
/// and dates are at the `Metadata` level.
/// </summary>
public sealed record PdfMetadata {
  /// <summary>
  /// PDF version (e.g., "1.7", "2.0")
  /// </summary>
  [JsonPropertyName("pdf_version")]
  public string ? PdfVersion { get; init; } = null;

  /// <summary>
  /// PDF producer (application that created the PDF)
  /// </summary>
  [JsonPropertyName("producer")]
  public string
      ? Producer {
          get; init;
        } = null;

  /// <summary>
  /// Whether the PDF is encrypted/password-protected
  /// </summary>
  [JsonPropertyName("is_encrypted")]
  public bool
      ? IsEncrypted {
          get; init;
        } = null;

  /// <summary>
  /// First page width in points (1/72 inch)
  /// </summary>
  [JsonPropertyName("width")]
  public long
      ? Width {
          get; init;
        } = null;

  /// <summary>
  /// First page height in points (1/72 inch)
  /// </summary>
  [JsonPropertyName("height")]
  public long
      ? Height {
          get; init;
        } = null;

  /// <summary>
  /// Total number of pages in the PDF document
  /// </summary>
  [JsonPropertyName("page_count")]
  public uint
      ? PageCount {
          get; init;
        } = null;

  /// <summary>
  /// How strongly the document's most scan-like page resembles a scan, in
  /// `[0.0, 1.0]`.
  ///
  /// `None` when the document could not be inspected. A full-page raster with
  /// no visible text scores at least `0.85`; a born-digital slide with a
  /// full-bleed background image scores `0.50`.
  /// </summary>
  [JsonPropertyName("scanned_confidence")]
  public float
      ? ScannedConfidence {
          get; init;
        } = null;

  /// <summary>
  /// Pages that look like scans (1-indexed), using the default confidence
  /// threshold.
  ///
  /// `None` when the document could not be inspected; empty when no page
  /// qualifies.
  /// </summary>
  [JsonPropertyName("scanned_pages")]
  public List<uint>
      ? ScannedPages {
          get; init;
        } = null;

  /// <summary>
  /// Pages the `auto` layout strategy skipped (1-indexed).
  ///
  /// `None` unless layout detection ran with `LayoutStrategy.Auto`; empty
  /// when the gate selected every page.
  /// </summary>
  [JsonPropertyName("layout_gated_pages")]
  public List<uint>
      ? LayoutGatedPages {
          get; init;
        } = null;

  /// <summary>
  /// Why the `auto` layout gate selected or skipped each page.
  ///
  /// Index `i` is page `i + 1`. Snake_case values such as `multi_column`,
  /// `table_grid`, or `plain_text` (the skip reason). `None` unless layout
  /// detection ran with `LayoutStrategy.Auto`.
  /// </summary>
  [JsonPropertyName("layout_gate_reasons")]
  public List<string>
      ? LayoutGateReasons {
          get; init;
        } = null;

  /// <summary>
  /// Parse a <see cref="PdfMetadata"/> from a JSON string.
  /// </summary>
  /// <exception cref="XbergException">When the JSON cannot be
  /// deserialised.</exception>
  public static PdfMetadata FromJson(string json) {
    try {
      return JsonSerializer.Deserialize<PdfMetadata>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse PdfMetadata from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse PdfMetadata from JSON: {e.Message}", e);
    }
  }

  private static readonly JsonSerializerOptions JsonOptions = new() {
    DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingDefault,
    Converters = { new JsonStringEnumConverter(JsonNamingPolicy.SnakeCaseLower),
                   new ByteArrayJsonConverter() },
  };

  /// <summary>Options for serializing config/input objects to FFI. Strips nulls
  /// (nullable C# fields default to null and would override required Rust
  /// fields with non-deserialisable nulls) but preserves explicit false/0 so
  /// caller intent is kept.</summary>
  private static readonly JsonSerializerOptions JsonSerializationOptions =
      new() {
        DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingNull,
        Converters = { new JsonStringEnumConverter(
                           JsonNamingPolicy.SnakeCaseLower),
                       new ByteArrayJsonConverter() },
      };
}
