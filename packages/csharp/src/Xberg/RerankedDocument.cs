...
// alef:hash:2ace0cae6e39af48f254514fbffdfc6999b713f9f6d06003133711de88bd6fc3
...
/// (e.g. IDs, paths) can be reordered without passing them through the
/// reranker.
/// </summary>
public sealed record RerankedDocument {
  ...
  public required string Document {
    get; init;
  }
    ...
    try {
      return JsonSerializer.Deserialize<RerankedDocument>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse RerankedDocument from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse RerankedDocument from JSON: {e.Message}", e);
    }
  }
