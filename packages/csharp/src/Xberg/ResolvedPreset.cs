...
// alef:hash:e5ed525aac4064e3a3983a994d90325cd20286d5ee61cdd17a749cbf5b6bb32f
...
/// <summary>
/// A preset merged with caller-supplied overrides (custom schema, prompt
/// suffix, context map). Output is what the pipeline orchestrator consumes.
/// </summary>
    ...
    try {
      return JsonSerializer.Deserialize<ResolvedPreset>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse ResolvedPreset from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse ResolvedPreset from JSON: {e.Message}", e);
    }
  }
