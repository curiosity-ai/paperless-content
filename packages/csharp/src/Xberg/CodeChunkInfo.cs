...
// alef:hash:f4605d8268df6d875944e97557ca35797fbe9738e3cb3f6f771ba9b1fac1e2a4
...
///
/// Purpose-built payload owned by xberg — deliberately does not expose the
/// upstream `tree_sitter_language_pack` types, so binding generators never need
/// to resolve an external crate's types across FFI/language boundaries.
/// </summary>
public sealed record CodeChunkInfo {
  ...
  /// <summary>
  /// Hierarchical path of enclosing structural items (e.g. `["MyClass",
  /// "my_method"]`).
  /// </summary>
    ...
    try {
      return JsonSerializer.Deserialize<CodeChunkInfo>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse CodeChunkInfo from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse CodeChunkInfo from JSON: {e.Message}", e);
    }
  }
