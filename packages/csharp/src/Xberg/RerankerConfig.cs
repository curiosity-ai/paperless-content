...
// alef:hash:cd8adb306c4eaa882e41e09fd0ccb53fb6eb609d84bf8a39170138e0aa87af03
...
public sealed record RerankerConfig {
  /// <summary>
  /// The reranker model to use (defaults to "balanced" preset if not
  /// specified).
  /// </summary>
  [JsonPropertyName("model")]
  public required RerankerModelType Model { get; init; }
  ...
  ///
  /// When enabled, transfer progress for the model, tokenizer and config files
  /// is reported at `info` level on the `xberg.model_download` target while
  /// they download (#279). A warm Hugging Face cache transfers nothing and so
  /// reports nothing. Ignored by `RerankerModelType.Llm` and
  /// `RerankerModelType.Plugin`, which download no model.
  /// </summary>
  ...
  ///
  /// Controls which execution provider (CPU, CUDA, CoreML, TensorRT) is used
  /// for local inference. Defaults to `None` (auto-select per platform).
  /// </summary>
    ...
    try {
      return JsonSerializer.Deserialize<RerankerConfig>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse RerankerConfig from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse RerankerConfig from JSON: {e.Message}", e);
    }
  }
  ...
  public static RerankerConfig Default() {
    ...
    return JsonSerializer.Deserialize<RerankerConfig>(json ?? "null",
                                                      JsonOptions)!;
  }
}
