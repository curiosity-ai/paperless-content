...
// alef:hash:fd473590a8924f8ab45c7f54de6f9a41c3bef0fc5b20e0858530b7c9aab604ac
...
public sealed record BrowserConfig {
  ...
  public List<string> BlockUrlPatterns {
    get; init;
  } = [];
  ...
  ///
  /// Scraping captures the native backend result in
  /// `ScrapeResult.browser.eval_result`. Interactions run this script before
  /// page actions on both browser backends but do not include the script result
  ...
  /// <summary>
  /// User-agent used when fetching robots.txt. Defaults to
  /// `BrowserConfig.user_agent` (or crawlberg's default) if unset. Native only.
  /// </summary>
  ...
  public bool CaptureNetworkEvents {
    get; init;
  } = false;
    ...
    try {
      return JsonSerializer.Deserialize<BrowserConfig>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse BrowserConfig from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse BrowserConfig from JSON: {e.Message}", e);
    }
  }
