...
// alef:hash:1b4bb291a31ec18c387d5b1133fbb02c49cdfadd0e2bc37e33a6fd164702d391
...
public sealed record EmailConfig {
  /// <summary>
  /// Windows codepage number to use when an MSG file contains no codepage
  /// property. Defaults to `None`, which falls back to windows-1252.
  ///
    ...
    try {
      return JsonSerializer.Deserialize<EmailConfig>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse EmailConfig from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse EmailConfig from JSON: {e.Message}", e);
    }
  }
