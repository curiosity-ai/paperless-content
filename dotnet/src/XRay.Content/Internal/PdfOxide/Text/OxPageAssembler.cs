...
namespace XRay.Content.Internal.PdfOxide.Text;
...
internal static class OxPageAssembler
{
    ...
    public static PageAssembly Assemble(List<OxTextSpan> spans, float pageWidth)
    {
        ...
        string? rotated = spans.Any(s => !IsUnrotated(s))
            ? XRay.Content.Internal.Pdf.PdfRotationRepair.RepairRotatedPageText(
                XRay.Content.Internal.Pdf.OxSpanBridge.ToPdfSpans(spans))
            : null;
