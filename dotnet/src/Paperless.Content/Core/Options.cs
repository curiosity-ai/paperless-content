...
public sealed class ContentOptions
{
    ...
    /// </summary>
    public static ContentOptions Default { get; set; } = new();
    ...
    /// </para>
    /// <para>
    /// The <c>XBERG_</c> prefix is deliberate and survives this project's rename to
    /// Paperless.Content: these names exist to be shared with the upstream Rust side, which reads
    /// <c>XBERG_*</c>. Renaming them would break the one thing they are for — driving both
    /// implementations from one variable — so they name the harness's contract, not this package.
    /// </para>
    /// </remarks>
    public static ContentOptions FromEnvironment()
    {
        var defaults = new ContentOptions();
