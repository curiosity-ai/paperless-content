...
using Paperless.Content.Internal.Epub;
...
public sealed class EpubExtractor : IExtractor
{
    ...
    private static Paperless.Content.Types.UriKind ClassifyUri(string url)
    {
        if (url.StartsWith("mailto:", StringComparison.Ordinal)) return Paperless.Content.Types.UriKind.Email;
        if (url.StartsWith('#')) return Paperless.Content.Types.UriKind.Anchor;
        return Paperless.Content.Types.UriKind.Hyperlink;
    }
