...
using Paperless.Content.Internal.Markup;
    ...
    private static void BuildOutline(XElement node, byte depth, InternalDocumentBuilder builder)
    {
        ...
        if (text.Length == 0)
        {
            ...
            if (!string.IsNullOrEmpty(emptyNodeNote))
                builder.PushParagraph(ConvertInlineHtml(emptyNodeNote), new List<Paperless.Content.Types.TextAnnotation>(), null, null);
