// Ported from the `outlook-pst` crate's src/encode/{mod,permute,cyclic}.rs.
//
// [MS-PST] obfuscates leaf data blocks with one of two schemes, both driven by the same 768-byte
// table the spec publishes: a straight byte substitution (Permute), or a keyed variant that mixes
// in the block id and advances per byte (Cyclic). Neither is encryption; both are reversible with
// the table alone.

namespace Paperless.Content.Internal.Pst;

internal static class PstEncoding
{
    ...
    /// <summary>Decode a block written with `NDB_CRYPT_PERMUTE`, in place.</summary>
    public static void PermuteDecode(byte[] data)
    {
        for (int i = 0; i < data.Length; i++)
            data[i] = KeyData[512 + data[i]];
    }

    /// <summary>Decode a block written with `NDB_CRYPT_CYCLIC`, in place. The key is the block id.</summary>
    public static void CyclicDecode(byte[] data, uint key)
    {
        ushort k = (ushort)(key ^ (key >> 16));
        for (int i = 0; i < data.Length; i++)
        {
            byte low = (byte)k;
            byte high = (byte)(k >> 8);
