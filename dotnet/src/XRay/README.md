...
A native C# port of the Rust [`xberg`](https://github.com/xberg-io/xberg) extraction engine:
every extractor, type and renderer is reimplemented in managed C#, not wrapped over a native
library.
...
The package id is hyphenated; the API lives under the `XRay` namespace, since a hyphen is not
