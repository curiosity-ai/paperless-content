...
public sealed class EpubExtractor : IExtractor
{
    ...
    private static XRay.Types.UriKind ClassifyUri(string url)
    {
        if (url.StartsWith("mailto:", StringComparison.Ordinal)) return XRay.Types.UriKind.Email;
        if (url.StartsWith('#')) return XRay.Types.UriKind.Anchor;
        return XRay.Types.UriKind.Hyperlink;
    }
