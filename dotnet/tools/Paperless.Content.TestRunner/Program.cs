...
// Paperless.Content C# port — golden parity harness.
//
...
//
// Usage:
//   paperless-content-testrunner <root-dir> [options]
...
ContentOptions.Default = ContentOptions.FromEnvironment();
...
if (args.Length >= 2 && args[0] == "--fetch-layout-model")
{
    ...
    var manager = new Paperless.Content.Internal.Layout.LayoutModelManager(
        args.Length > 2 ? args[2] : null, Paperless.Content.Core.ContentOptions.FromEnvironment());
    try
    {
...
if (args.Length >= 2 && args[0] == "--dump-qr")
{
    ...
    for (int i = 1; i < args.Length; i++)
    {
        ...
        foreach (var code in Paperless.Content.Internal.Qr.QrScanner.Detect(File.ReadAllBytes(args[i])))
...
if (args.Length >= 3 && args[0] == "--dump-yolo")
{
    ...
    for (int i = 2; i < args.Length; i++)
    {
        ...
        {
            "doclaynet" => (Paperless.Content.Internal.Layout.YoloVariant.DocLayNet, 640, 640),
            "docstructbench" => (Paperless.Content.Internal.Layout.YoloVariant.DocStructBench, 1024, 1024),
            "yolox" => (Paperless.Content.Internal.Layout.YoloVariant.Yolox, 768, 1024),
            _ => throw new ArgumentException($"unknown variant {name}"),
        };

        var model = Paperless.Content.Internal.Layout.YoloModel.FromFile(path, variant, width, height, name);
        var found = new System.Text.Json.Nodes.JsonArray();
        foreach (var detection in model.Detect(page))
            found.Add(new System.Text.Json.Nodes.JsonObject
            {
                ["class"] = Paperless.Content.Internal.Layout.LayoutClassExtensions.WireName(detection.ClassName),
...
if (args.Length >= 2 && args[0] == "--dump-wpd")
{
    ...
    for (int i = 1; i < args.Length; i++)
    {
        ...
        try
        {
            var parsed = Paperless.Content.Internal.WordPerfect.WordPerfectReader.Parse(File.ReadAllBytes(args[i]));
            var events = new System.Text.Json.Nodes.JsonArray();
            foreach (var e in parsed.Events)
                events.Add(e.Kind == Paperless.Content.Internal.WordPerfect.WpdEventKind.Text
                    ? $"Text:{e.Text}"
                    : e.Kind.ToString());
            entry["format"] = Paperless.Content.Internal.WordPerfect.WordPerfectReader.Detect(
                File.ReadAllBytes(args[i])).ToString();
            entry["events"] = events;
            entry["plain"] = Paperless.Content.Internal.WordPerfect.WordPerfectReader.RenderPlain(parsed);
        }
        catch (Exception e)
        {
...
{
    var slanet = Paperless.Content.Internal.Layout.SlanetModel.FromFile(args[1]);
...
{
    var tatr = Paperless.Content.Internal.Layout.TatrModel.FromFile(args[1]);
    var pages = new System.Text.Json.Nodes.JsonArray();
    for (int i = 2; i < args.Length; i++)
    {
        using var image = SixLabors.ImageSharp.Image.Load<SixLabors.ImageSharp.PixelFormats.Rgb24>(args[i]);
        ...
        static System.Text.Json.Nodes.JsonArray Render(
            IEnumerable<Paperless.Content.Internal.Layout.TatrDetection> detections)
        {
        ...
        var (grid, structure) = Paperless.Content.Internal.Layout.TatrModel.BuildCellGridWithStructure(
...
{
    var model = Paperless.Content.Internal.Onnx.OnnxModel.Load(args[1]);
    ...
    void Walk(IEnumerable<Paperless.Content.Internal.Onnx.OnnxNode> nodes)
    {
...
{
    var layoutModel = Paperless.Content.Internal.Layout.PpDocLayoutV3Model.FromFile(args[1]);
    var pages = new System.Text.Json.Nodes.JsonArray();
    for (int i = 2; i < args.Length; i++)
    {
        using var image = SixLabors.ImageSharp.Image.Load<SixLabors.ImageSharp.PixelFormats.Rgb24>(args[i]);
        ...
        foreach (var d in layoutModel.Detect(image))
            found.Add(new System.Text.Json.Nodes.JsonObject
            {
                ["class_name"] = Paperless.Content.Internal.Layout.LayoutClassExtensions.WireName(d.ClassName),
                ["confidence"] = d.Confidence,
...
{
    var classifier = Paperless.Content.Internal.Layout.TableClassifier.FromFile(args[1]);
    ...
    for (int i = 2; i < args.Length; i++)
    {
        ...
        {
            ["file"] = args[i],
            ["table_type"] = Paperless.Content.Internal.Layout.TableTypeExtensions.Name(type),
            ["raw_wired"] = rawWired,
...
if (args.Length >= 2 && args[0] == "--check-heuristics")
{
    ...
    static Paperless.Content.Internal.Layout.LayoutClass ParseClass(string label)
    {
        foreach (Paperless.Content.Internal.Layout.LayoutClass value
                 in Enum.GetValues<Paperless.Content.Internal.Layout.LayoutClass>())
            if (Paperless.Content.Internal.Layout.LayoutClassExtensions.WireName(value) == label) return value;
        throw new InvalidOperationException($"unknown layout class '{label}'");
    }

    static List<Paperless.Content.Internal.Layout.LayoutDetection> ReadDetections(System.Text.Json.JsonElement page)
    {
        var list = new List<Paperless.Content.Internal.Layout.LayoutDetection>();
        foreach (var d in page.GetProperty("detections").EnumerateArray())
        {
            var box = d.GetProperty("bbox");
            list.Add(new Paperless.Content.Internal.Layout.LayoutDetection(
                ParseClass(d.GetProperty("class_name").GetString()!),
                d.GetProperty("confidence").GetSingle(),
                new Paperless.Content.Internal.Layout.BBox(
                    box[0].GetSingle(), box[1].GetSingle(), box[2].GetSingle(), box[3].GetSingle())));
        }
        return list;
    }

    static string Render(Paperless.Content.Internal.Layout.LayoutDetection d) =>
        string.Create(System.Globalization.CultureInfo.InvariantCulture,
            $"{Paperless.Content.Internal.Layout.LayoutClassExtensions.WireName(d.ClassName)}@{d.Confidence:F6}{d.Box}");
    ...
    foreach (var entry in probe.RootElement.EnumerateArray())
    {
        ...
        var actual = Paperless.Content.Internal.Layout.LayoutPostprocessing.ApplyHeuristics(
...
static JsonNode? SerializeToNode(object? value)
{
    ...
    var json = JsonSerializer.Serialize(value, value.GetType(), Paperless.Content.Types.Json.Options);
...
{
    Console.WriteLine();
    Console.WriteLine("═══ Paperless.Content C# parity report ═══");
...
static Options? ParseArgs(string[] args)
{
    if (args.Length == 0)
    {
        Console.Error.WriteLine("Usage: paperless-content-testrunner <root-dir> [--filter s] [--ext e] [--show N] [--diff] [--strict-md] [--list-ok]");
        return null;
    }
...
static partial class Cfg
{
    ...
    public static ExtractionConfig Make(OutputFormat fmt, Options o) => new()
    {
        ...
        Options = new ContentOptions { SourceCodeDetection = o.SourceCodeDetection },
    };
