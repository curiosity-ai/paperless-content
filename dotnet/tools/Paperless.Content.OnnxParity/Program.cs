...
using Paperless.Content.Internal.Onnx.Ops;
using System.Globalization;
using System.Text.Json;
using Paperless.Content.Internal.Onnx;

namespace Paperless.Content.OnnxParity;
    ...
    private static int Main(string[] args)
    {
        ...
        if (modelPath is null || (needsReference && referenceDir is null))
        {
            Console.Error.WriteLine(
                "usage: paperless-content-onnx-parity --model MODEL.onnx --reference REF_DIR [--atol A] [--rtol R] [--limit N] [--list-ops]");
            Console.Error.WriteLine(
                "       paperless-content-onnx-parity --model MODEL.onnx --benchmark N   (no reference needed)");
            return 2;
        }
