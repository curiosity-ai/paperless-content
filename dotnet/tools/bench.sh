...
( cd "$REPO/dotnet" && dotnet build tools/Paperless.Content.Bench -c Release 2>&1 | grep -E "error|Error\(s\)" | head -3 ) || { echo "c# build FAILED"; exit 1; }
CS_DLL="$REPO/dotnet/tools/Paperless.Content.Bench/bin/Release/net10.0/paperless-content-bench.dll"
