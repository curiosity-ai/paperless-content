...
// XRay C# port — golden parity harness.
//
...
//
// Usage:
//   xray-testrunner <root-dir> [options]
...
if (args.Length >= 2 && args[0] == "--fetch-layout-model")
{
    ...
    var manager = new XRay.Internal.Layout.LayoutModelManager(
        args.Length > 2 ? args[2] : null, XRay.Core.XRayOptions.FromEnvironment());
    try
    {
...
if (args.Length >= 2 && args[0] == "--dump-qr")
{
    ...
    for (int i = 1; i < args.Length; i++)
    {
        ...
        foreach (var code in XRay.Internal.Qr.QrScanner.Detect(File.ReadAllBytes(args[i])))
...
if (args.Length >= 3 && args[0] == "--dump-yolo")
{
    ...
    for (int i = 2; i < args.Length; i++)
    {
        ...
        {
            "doclaynet" => (XRay.Internal.Layout.YoloVariant.DocLayNet, 640, 640),
            "docstructbench" => (XRay.Internal.Layout.YoloVariant.DocStructBench, 1024, 1024),
            "yolox" => (XRay.Internal.Layout.YoloVariant.Yolox, 768, 1024),
            _ => throw new ArgumentException($"unknown variant {name}"),
        };

        var model = XRay.Internal.Layout.YoloModel.FromFile(path, variant, width, height, name);
        var found = new System.Text.Json.Nodes.JsonArray();
        foreach (var detection in model.Detect(page))
            found.Add(new System.Text.Json.Nodes.JsonObject
            {
                ["class"] = XRay.Internal.Layout.LayoutClassExtensions.WireName(detection.ClassName),
...
if (args.Length >= 2 && args[0] == "--dump-wpd")
{
    ...
    for (int i = 1; i < args.Length; i++)
    {
        ...
        try
        {
            var parsed = XRay.Internal.WordPerfect.WordPerfectReader.Parse(File.ReadAllBytes(args[i]));
            var events = new System.Text.Json.Nodes.JsonArray();
            foreach (var e in parsed.Events)
                events.Add(e.Kind == XRay.Internal.WordPerfect.WpdEventKind.Text
                    ? $"Text:{e.Text}"
                    : e.Kind.ToString());
            entry["format"] = XRay.Internal.WordPerfect.WordPerfectReader.Detect(
                File.ReadAllBytes(args[i])).ToString();
            entry["events"] = events;
            entry["plain"] = XRay.Internal.WordPerfect.WordPerfectReader.RenderPlain(parsed);
        }
        catch (Exception e)
        {
...
{
    var slanet = XRay.Internal.Layout.SlanetModel.FromFile(args[1]);
...
{
    var tatr = XRay.Internal.Layout.TatrModel.FromFile(args[1]);
    var pages = new System.Text.Json.Nodes.JsonArray();
    for (int i = 2; i < args.Length; i++)
    {
        using var image = SixLabors.ImageSharp.Image.Load<SixLabors.ImageSharp.PixelFormats.Rgb24>(args[i]);
        ...
        static System.Text.Json.Nodes.JsonArray Render(
            IEnumerable<XRay.Internal.Layout.TatrDetection> detections)
        {
        ...
        var (grid, structure) = XRay.Internal.Layout.TatrModel.BuildCellGridWithStructure(
...
{
    var model = XRay.Internal.Onnx.OnnxModel.Load(args[1]);
    ...
    void Walk(IEnumerable<XRay.Internal.Onnx.OnnxNode> nodes)
    {
...
{
    var layoutModel = XRay.Internal.Layout.PpDocLayoutV3Model.FromFile(args[1]);
    var pages = new System.Text.Json.Nodes.JsonArray();
    for (int i = 2; i < args.Length; i++)
    {
        using var image = SixLabors.ImageSharp.Image.Load<SixLabors.ImageSharp.PixelFormats.Rgb24>(args[i]);
        ...
        foreach (var d in layoutModel.Detect(image))
            found.Add(new System.Text.Json.Nodes.JsonObject
            {
                ["class_name"] = XRay.Internal.Layout.LayoutClassExtensions.WireName(d.ClassName),
                ["confidence"] = d.Confidence,
...
{
    var classifier = XRay.Internal.Layout.TableClassifier.FromFile(args[1]);
    ...
    for (int i = 2; i < args.Length; i++)
    {
        ...
        {
            ["file"] = args[i],
            ["table_type"] = XRay.Internal.Layout.TableTypeExtensions.Name(type),
            ["raw_wired"] = rawWired,
...
if (args.Length >= 2 && args[0] == "--check-heuristics")
{
    ...
    static XRay.Internal.Layout.LayoutClass ParseClass(string label)
    {
        foreach (XRay.Internal.Layout.LayoutClass value
                 in Enum.GetValues<XRay.Internal.Layout.LayoutClass>())
            if (XRay.Internal.Layout.LayoutClassExtensions.WireName(value) == label) return value;
        throw new InvalidOperationException($"unknown layout class '{label}'");
    }

    static List<XRay.Internal.Layout.LayoutDetection> ReadDetections(System.Text.Json.JsonElement page)
    {
        var list = new List<XRay.Internal.Layout.LayoutDetection>();
        foreach (var d in page.GetProperty("detections").EnumerateArray())
        {
            var box = d.GetProperty("bbox");
            list.Add(new XRay.Internal.Layout.LayoutDetection(
                ParseClass(d.GetProperty("class_name").GetString()!),
                d.GetProperty("confidence").GetSingle(),
                new XRay.Internal.Layout.BBox(
                    box[0].GetSingle(), box[1].GetSingle(), box[2].GetSingle(), box[3].GetSingle())));
        }
        return list;
    }

    static string Render(XRay.Internal.Layout.LayoutDetection d) =>
        string.Create(System.Globalization.CultureInfo.InvariantCulture,
            $"{XRay.Internal.Layout.LayoutClassExtensions.WireName(d.ClassName)}@{d.Confidence:F6}{d.Box}");
    ...
    foreach (var entry in probe.RootElement.EnumerateArray())
    {
        ...
        var actual = XRay.Internal.Layout.LayoutPostprocessing.ApplyHeuristics(
...
static JsonNode? SerializeToNode(object? value)
{
    ...
    var json = JsonSerializer.Serialize(value, value.GetType(), XRay.Types.Json.Options);
...
{
    Console.WriteLine();
    Console.WriteLine("═══ XRay C# parity report ═══");
...
static Options? ParseArgs(string[] args)
{
    if (args.Length == 0)
    {
        Console.Error.WriteLine("Usage: xray-testrunner <root-dir> [--filter s] [--ext e] [--show N] [--diff] [--strict-md] [--list-ok]");
        return null;
    }
