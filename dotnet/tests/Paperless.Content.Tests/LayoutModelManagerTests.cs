...
using Paperless.Content.Internal.Layout;
using Xunit;
...
public class LayoutModelManagerTests : IDisposable
{
    ...
    public void AnExplicitDirectoryIsUsedAsGiven()
    {
        ...
        Assert.Equal(LayoutModelManager.DefaultHfCacheDir(),
                     new LayoutModelManager(null, new ContentOptions()).CacheDir);
    }
    ...
    {
        var options = new ContentOptions { ModelCacheDirectory = _dir };
    ...
    {
        var manager = new LayoutModelManager(_dir, new ContentOptions { ModelDownloadsDisabled = true });
