using Paperless.Content.Rendering;
...
public sealed class ListSourceLabelTests
{
    ...
    {
        string html = new StyledHtmlRenderer(new Paperless.Content.Core.HtmlOutputConfig())
    ...
    public void AParenthesizedQuantityClarificationIsNotAListItem(string line) =>
        Assert.False(Paperless.Content.Internal.Pdf.PdfStructure.LooksLikeListItem(line));
    ...
    public void GenuineParenthesizedMarkersRemainListItems(string line) =>
        Assert.True(Paperless.Content.Internal.Pdf.PdfStructure.LooksLikeListItem(line));
