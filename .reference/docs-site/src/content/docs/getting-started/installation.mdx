...
import { Content as Snip_go_getting_started_hello_world } from "../../../snippets/go/getting-started/hello_world.md";
import { Content as Snip_java_getting_started_basic_usage } from "../../../snippets/java/getting-started/basic_usage.md";
import { Content as Snip_php_quickstart_basic_extraction_oop } from "../../../snippets/php/quickstart/basic_extraction_oop.md";
import { Content as Snip_python_getting_started_hello_world } from "../../../snippets/python/getting-started/hello_world.md";
import { Content as Snip_ruby_getting_started_install_verify } from "../../../snippets/ruby/getting-started/install_verify.md";
import { Content as Snip_elixir_getting_started_basic_usage } from "../../../snippets/elixir/getting-started/basic_usage.md";
import { Content as Snip_wasm_getting_started_basic_extract } from "../../../snippets/wasm/getting-started/basic-extract.md";
...
<TabItem label="Docker (CLI image)">
...
<TabItem label="Docker (full image)">
...
[CLI Usage](/cli/usage/)
[API Server Guide](/guides/api-server/)

</div>

:::caution[x86_64 CPU — AVX/AVX2 instruction set required]
...
:::caution[Windows — ONNX Runtime required for Go, Elixir, and C/C++]
...
:::caution[C# / .NET in a Linux container — publish with a RID and set `LD_LIBRARY_PATH`]
The `XbergIo.Xberg` NuGet package ships its native library as a per-RID runtime package (`XbergIo.Xberg.runtime.<rid>`) resolved through a `runtime.json` graph, the same pattern as `Microsoft.Data.Sqlite`. Two container gotchas:
...
:::caution[Intel Macs (x86_64) — prebuilt binaries are limited]
Apple Silicon (arm64) macOS is fully supported across every package. On **Intel Macs (x86_64)**, prebuilt binaries ship only for the **CLI** (install script, Homebrew, and the release tarball) and the **Python** wheel. The TypeScript/Node, Go, Java, Kotlin, C#, Dart, C/C++, Ruby, PHP, and Elixir packages ship macOS prebuilts for **Apple Silicon only** — on an Intel Mac they fall back to building the native library from source (a Rust toolchain is required). Intel-macOS is a legacy target: GitHub retires its last x86_64 macOS CI runner in 2027 and the bundled ONNX Runtime has no Intel-macOS build, so Intel support is best-effort and will be removed. Prefer Apple Silicon where you can.
:::
...
-  **Python**
  ...
  ***
  ...
  [API Reference](/reference/api-python/)
...
-  **TypeScript (Node.js / Bun)**
  ...
  ***
  ...
  [API Reference](/reference/api-typescript/)
...
-  **TypeScript (Browser / Edge)**
  ...
  ***
  ...
  [API Reference](/reference/api-wasm/)
  [Quick Start](#typescript)
...
-  **Rust**
  ...
  ***
  ...
  [API Reference](/reference/api-rust/)
  [Quick Start](/getting-started/quickstart/)
...
-  **Go**
  ...
  ***
  ...
  [API Reference](/reference/api-go/)
  [Quick Start](/getting-started/quickstart/)
...
-  **Java**
  ...
  ***
  ...
  ```groovy
  implementation 'io.xberg:xberg:1.1.3'
  ```

  [API Reference](/reference/api-java/)
  [Quick Start](#java)
...
-  **Kotlin Android**
  ...
  ***
  ...
  ```kotlin
  implementation("io.xberg:xberg-android:1.1.3")
  ```

  [API Reference](/reference/api-kotlin-android/)
  [Quick Start](#kotlin)
...
-  **Swift**
  ...
  ***
  ...
  ```swift
  .package(url: "https://github.com/xberg-io/xberg.git", from: "1.1.3")
  ```

  [API Reference](/reference/api-swift/)
  [Quick Start](#swift)
...
-  **C# / .NET**
  ...
  ***
  ...
  [API Reference](/reference/api-csharp/)
  [Quick Start](/reference/api-csharp/)
...
-  **PHP**
  ...
  ***
  ...
  [API Reference](/reference/api-php/)
  [Quick Start](/getting-started/quickstart/)
...
-  **Elixir**
  ...
  ***
  ...
  [API Reference](/reference/api-elixir/)
  [Quick Start](#elixir)
...
-  **Dart / Flutter**
  ...
  ***
  ...
  [API Reference](/reference/api-dart/)
  [Quick Start](#dart)
...
-  **Zig**
  ...
  ***
  ...
  ```bash
  zig fetch --save https://github.com/xberg-io/xberg/archive/refs/tags/v1.1.3.tar.gz
  ```

  [API Reference](/reference/api-zig/)
  [Quick Start](#zig)
...
<Snip_go_getting_started_hello_world />
...
<Snip_php_quickstart_basic_extraction_oop />

</TabItem>
<TabItem label="Python">

<Snip_python_getting_started_hello_world />

</TabItem>
<TabItem label="Ruby">

<Snip_ruby_getting_started_install_verify />
...
<Snip_wasm_getting_started_basic_extract />
...
(`.heics`), HEIF sequences (`.heifs`), `.hif`, and AVCS requires the **`heic`
Cargo feature** plus the system
...
See [AccelerationConfig](/reference/configuration/#accelerationconfig) and [ORT_DYLIB_PATH](/reference/environment-variables/#ort_dylib_path) for details.
...
:::note[Wasm — package layout (`web` target only)]
`@xberg-io/xberg-wasm` publishes a single wasm-pack **`web`** target — a standard ES module you
`import` directly in the browser, over a CDN (jsDelivr/unpkg), in Deno via `npm:`, in Cloudflare
Workers, and in Node.js 22+ (ESM). Dedicated `bundler`, Node-CommonJS, and Deno target builds are
**not** shipped: each is a full copy of the ~100MB wasm binary, and shipping all four pushed the
npm package past jsDelivr's 150MB per-package CDN limit (which broke the live demo's CDN load).
Modern bundlers (Vite, webpack 5, Rollup, esbuild) consume the `web` target directly. Initialize
with the package's default `init()` export before calling `extract`.
:::

:::caution[Wasm Platform Limitations]
...
:::caution[Not a Go module]
The repository root is not a Go module — `go get github.com/xberg-io/xberg` (without the
`/packages/go` suffix) fails, resolving against a stale Go module-proxy cache entry. Always
target the `/packages/go` subdirectory as shown above.
:::
