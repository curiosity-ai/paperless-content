...
The Python binding has no standalone `rerank`/`rerank_sync` entry point — only reranker-backend
registration (`register_reranker_backend`, `list_reranker_backends`, `unregister_reranker_backend`)
and the `RerankerConfig`/`RerankerModelType` types are exposed. To run the dispatcher shown below,
call it from Rust (or a language whose binding does expose it):

```python
from xberg import RerankerConfig, RerankerModelType

# NOTE: The Python binding has no standalone reranking entry point — RerankerConfig
# and RerankerModelType are exposed so you can build config for a plugin backend, but
# there is no rerank()/rerank_sync() function to call it with. See the "In-process
# plugin backend" section below to register your own model instead.
...
```typescript
// NOTE: The Node/TypeScript binding has no standalone rerank()/rerankSync() entry
// point — only reranker-backend registration (registerRerankerBackend,
// listRerankerBackends, unregisterRerankerBackend) and the RerankerConfig type are
// exposed. See the "In-process plugin backend" section below to register your own
// model instead of calling a dispatcher directly.
import type { RerankerConfig } from "@xberg-io/xberg";
...
```go
// NOTE: The Go binding has no standalone Rerank() entry point — only reranker-backend
// registration (RegisterRerankerBackend, ListRerankerBackends, UnregisterRerankerBackend)
// and the RerankerConfig type are exposed. See the "In-process plugin backend" section
// below to register your own model instead of calling a dispatcher directly.
...
```java
// NOTE: The Java binding has no standalone Xberg.rerank() entry point — only
// reranker-backend registration (Xberg.registerRerankerBackend, listRerankerBackends,
// unregisterRerankerBackend) and the RerankerConfig type are exposed. See the
// "In-process plugin backend" section below to register your own model instead of
// calling a dispatcher directly.
import io.xberg.*;
...
```python
# NOTE: The Python binding has no standalone rerank()/rerank_sync() entry point —
# RerankerConfig/RerankerModelType are exposed to build config, but there is no
# function to run the dispatcher shown here directly from Python.
from xberg import RerankerConfig, RerankerModelType
...
```python
# NOTE: The Python binding has no standalone rerank()/rerank_sync() entry point —
# RerankerConfig/RerankerModelType/LlmConfig are exposed to build config, but there
# is no function to run the dispatcher shown here directly from Python.
import os
from xberg import RerankerConfig, RerankerModelType, LlmConfig
...
```python
# NOTE: register_reranker_backend() is real and registers your model in xberg's
# Rust-side registry, but the Python binding exposes no rerank()/rerank_sync() to
# invoke it afterward — invocation must come from a caller that does have a rerank
# entry point (currently only the Rust crate).
from collections.abc import Iterable

from xberg import register_reranker_backend, RerankerConfig, RerankerModelType
...
- [Retrieval](/guides/retrieval/) — build the candidate set this reranker sharpens
- [Retrieval Modes](/concepts/retrieval/) — where reranking sits relative to dense, sparse and late-interaction
