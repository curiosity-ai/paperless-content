...
<!-- alef:hash:708e0d5085af67402d2958f6c8e5c123c2be7ac990ebf2e4ae2e10dc3bffe5ea -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Zig API Reference <span class="version-badge">v1.1.3</span>
...
```zig
pub fn extract(input: []const u8, config: []const u8) Error![]u8
```

**Example:**

```zig
const result = try extract(.{}, .{});
```
...
|------|------|----------|-------------|
| `input` | `\[\]const u8` | Yes | The input data |
...
```zig
pub fn extract_batch(inputs: []const u8, config: []const u8) Error![]u8
```

**Example:**

```zig
const result = try extract_batch(&[_]u8{}, .{});
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `inputs` | `\[\]const u8` | Yes | The inputs |
| `config` | `\[\]const u8` | Yes | The configuration options |
...
```zig
pub fn map_url(uri: []const u8, config: []const u8) Error![]u8
```

**Example:**

```zig
const result = try map_url("value", .{});
```
...
```zig
const result = list_supported_formats();
```
...
```zig
const result = try list_embedding_backends();
```
...
```zig
pub fn register_document_extractor(extractor: DocumentExtractor) XbergError!void
```

**Example:**

```zig
try register_document_extractor(.{});
```
...
```zig
pub fn unregister_document_extractor(name: []const u8) XbergError!void
```

**Example:**

```zig
try unregister_document_extractor("value");
```
...
```zig
const result = try list_document_extractors();
```
...
```zig
const result = try list_ocr_backends();
```
...
```zig
const result = try list_post_processors();
```
...
```zig
pub fn register_renderer(renderer: Renderer) XbergError!void
```

**Example:**

```zig
try register_renderer(.{});
```
...
```zig
pub fn unregister_renderer(name: []const u8) XbergError!void
```

**Example:**

```zig
try unregister_renderer("value");
```
...
```zig
const result = try list_renderers();
```
...
```zig
const result = try list_reranker_backends();
```
...
```zig
const result = try list_tokenizer_backends();
```
...
```zig
const result = try list_validators();
```
...
```zig
const result = try classify_chunks_owned(.{}, .{});
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `result` | `\[\]const u8` | Yes | The extracted document |
| `config` | `\[\]const u8` | Yes | The configuration options |
...
```zig
pub fn find_unmarked_claims(markdown: []const u8) []u8
```

**Example:**

```zig
const result = find_unmarked_claims("value");
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `markdown` | `\[\]const u8` | Yes | The markdown text to search |
...
```zig
pub fn verify_excerpt(excerpt: []const u8, source_text: []const u8) bool
```

**Example:**

```zig
const result = verify_excerpt("value", "value");
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `excerpt` | `\[\]const u8` | Yes | The text snippet to find |
| `source_text` | `\[\]const u8` | Yes | The full source text to search |
...
```zig
pub fn max_sim_score(query: []const u8, doc: []const u8) f64
```

**Example:**

```zig
const result = max_sim_score(.{}, .{});
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `query` | `\[\]const u8` | Yes | The multi vector embedding |
| `doc` | `\[\]const u8` | Yes | The multi vector embedding |
...
```zig
pub fn max_sim_rank(query: []const u8, docs: []const u8) []u8
```

**Example:**

```zig
const result = max_sim_rank(.{}, &[_]u8{});
```
...
```zig
pub fn doctor(config: []const u8) []u8
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `path` | `\[\]const u8` | — | Archive-relative file path (e.g. "folder/document.pdf"). |
| `mime_type` | `\[\]const u8` | — | Detected MIME type of the file. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `format` | `\[\]const u8` | — | Archive format ("ZIP", "TAR", "7Z", etc.) |
| `file_count` | `u32` | — | Total number of files in the archive |
| `file_list` | `\[\]const \[\]const u8` | `[]` | List of file paths within the archive |
| `total_size` | `u64` | — | Total uncompressed size in bytes |
| `compressed_size` | `u64?` | `null` | Compressed size in bytes (if available) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `id` | `\[\]const u8?` | `null` | Element ID (#identifier) |
| `classes` | `\[\]const \[\]const u8` | `[]` | CSS classes (.class1 .class2) |
| `key_values` | `\[\]const KeyValueAttribute` | `[]` | Key-value pairs (key="value") |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `duration_ms` | `u64?` | `null` | Duration in milliseconds derived from the decoded audio stream. |
| `codec` | `\[\]const u8?` | `null` | Audio codec (e.g. "mp3", "aac", "opus", "flac"). |
| `container` | `\[\]const u8?` | `null` | Container format (e.g. "mpeg", "mp4", "ogg", "wav"). |
| `sample_rate_hz` | `u32?` | `null` | Sample rate in Hz after decode (always 16000 when resampled for Whisper). |
...
| `access_key_id` | `\[\]const u8?` | `null` | Explicit AWS access key ID. Secret — never logged. |
| `secret_access_key` | `\[\]const u8?` | `null` | Explicit AWS secret access key. Secret — never logged. |
| `session_token` | `\[\]const u8?` | `null` | Explicit AWS session token for temporary credentials. Secret — never logged. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `entry_count` | `u64` | — | Number of entries in the bibliography. |
| `citation_keys` | `\[\]const \[\]const u8` | `[]` | BibTeX citation keys (e.g. `"knuth1984"`) for all entries. |
| `authors` | `\[\]const \[\]const u8` | `[]` | Author names collected across all bibliography entries. |
| `year_range` | `YearRange?` | `null` | Earliest and latest publication years found in the bibliography. |
| `entry_types` | `std.StringHashMap(u64)?` | `{}` | Count of entries grouped by BibTeX entry type (e.g. `"article"` → 5). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `total_files` | `u64` | — | Total number of files currently in the cache directory. |
...
| `prompt` | `\[\]const u8?` | `null` | Optional custom caption prompt. `null` uses the default `RegionKind.Caption` prompt that ships with `crate.llm.region_extractor`. |
...
| `from` | `\[\]const u8` | — | Value before the change. |
| `to` | `\[\]const u8` | — | Value after the change. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `content` | `\[\]const u8` | — | The text content of this chunk. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `prompt_template` | `\[\]const u8?` | `null` | Minijinja prompt template. Receives `{{ definitions }}` (rendered label + description list) and `{{ chunks }}` (a numbered list of chunk texts in the current batch) variables. `null` lets the backend pick a sensible default. |
| `definitions` | `\[\]const ChunkClassificationDefinition` | — | The set of label definitions the classifier may emit. Must contain at least one entry. |
| `llm` | `LlmConfig` | — | LLM configuration used for classification. |
| `batch_size` | `u64` | `10` | Number of chunks batched into a single LLM request. Larger batches amortize the fixed prompt cost (definitions block) across more chunks, at the risk of exceeding the model's context window for very large taxonomies or chunk texts. Defaults to `DEFAULT_BATCH_SIZE`. |
| `max_concurrency` | `u64` | `4` | Maximum number of in-flight batch requests. Bounds concurrency against the configured LLM provider. Defaults to `DEFAULT_MAX_CONCURRENCY`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `label` | `\[\]const u8` | — | Label name returned in `ChunkMetadata.classifications`. |
| `description` | `\[\]const u8` | — | Semantic description of when this label applies. Injected verbatim into the classification prompt next to the label name. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `byte_start` | `u64` | — | Byte offset where this chunk starts in the original text (UTF-8 valid boundary). |
| `byte_end` | `u64` | — | Byte offset where this chunk ends in the original text (UTF-8 valid boundary). |
| `token_count` | `u64?` | `null` | Number of tokens in this chunk (if available). This is calculated by the embedding model's tokenizer if embeddings are enabled. |
| `chunk_index` | `u64` | — | Zero-based index of this chunk in the document. |
| `total_chunks` | `u64` | — | Total number of chunks in the document. |
| `first_page` | `u32?` | `null` | First page number this chunk spans (1-indexed). Only populated when page tracking is enabled in extraction configuration. |
| `last_page` | `u32?` | `null` | Last page number this chunk spans (1-indexed, equal to first_page for single-page chunks). Only populated when page tracking is enabled in extraction configuration. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `max_characters` | `u64` | `1000` | Maximum size per chunk (in units determined by `sizing`). When `sizing` is `Characters` (default), this is the max character count. When using token-based sizing, this is the max token count. Default: 1000 |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `label` | `\[\]const u8` | — | The label of the citation (e.g., "src1" in `[^src1]: ...`). |
| `source` | `\[\]const u8` | — | The source reference (path, URL, or identifier). |
| `locator` | `\[\]const u8?` | `null` | Optional locator within the source (e.g., "page 3" or "section 2.1"). |
| `excerpt` | `\[\]const u8?` | `null` | Optional excerpt — quoted text from the source. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `citation_count` | `u64` | — | Total number of citation records in the file. |
| `format` | `\[\]const u8?` | `null` | Detected citation file format (e.g. `"ris"`, `"pubmed"`, `"endnote"`). |
| `authors` | `\[\]const \[\]const u8` | `[]` | Author names collected across all citation records. |
| `year_range` | `YearRange?` | `null` | Earliest and latest publication years found in the file. |
| `dois` | `\[\]const \[\]const u8` | `[]` | DOI identifiers found in the citation records. |
| `keywords` | `\[\]const \[\]const u8` | `[]` | Keywords collected from all citation records. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `label` | `\[\]const u8` | — | Label name as configured in `PageClassificationConfig.labels`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `text` | `\[\]const u8` | — | The raw source text of this chunk. |
| `context_path` | `\[\]const \[\]const u8` | — | Hierarchical path of enclosing structural items (e.g. `["MyClass", "my_method"]`). |
| `node_types` | `\[\]const \[\]const u8` | — | Tree-sitter node kinds that appear at the top level of this chunk (e.g. `"function_definition"`, `"class_definition"`). |
| `byte_start` | `u64` | — | Inclusive start byte offset of this chunk in the original source. |
| `byte_end` | `u64` | — | Exclusive end byte offset of this chunk in the original source. |
...
| `byte_start` | `u64` | — | Inclusive start byte offset of the `name="value"` attribute token. |
| `byte_end` | `u64` | — | Exclusive end byte offset of the `name="value"` attribute token. |
...
| `key` | `\[\]const u8?` | `/* serde(default) */` | Key, attribute name, tag name, or positional index (`"0"`, `"1"`, …). `null` at the document root. |
| `value` | `\[\]const u8?` | `/* serde(default) */` | Leaf scalar value, if any. `null` for containers (objects, arrays, XML elements with child elements). |
| `attributes` | `\[\]const CodeDataAttribute` | `/* serde(default) */` | Attributes on element-shape nodes (XML `STag` attributes). Empty for all other kinds. |
| `children` | `\[\]const CodeDataNode` | `/* serde(default) */` | Children for nested containers and XML element bodies. |
| `byte_start` | `u64` | — | Inclusive start byte offset of this node in the original source. |
| `byte_end` | `u64` | — | Exclusive end byte offset of this node in the original source. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `chunks` | `\[\]const CodeChunkInfo` | `[]` | Structural code chunks (function/class/module boundaries). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `\[\]const u8` | — | Contributor display name. |
| `role` | `\[\]const u8?` | `null` | Contributor role (e.g. `"author"`, `"editor"`). |
...
| `list_indent_width` | `u64` | `2` | Number of spaces (or tabs) to use for each level of list indentation. |
| `bullets` | `\[\]const u8` | `"-*+"` | Bullet character(s) to use for unordered list items (e.g. `"-"`, `"*"`). |
| `strong_em_symbol` | `\[\]const u8` | `"*"` | Character used for bold/italic emphasis markers (`*` or `_`). |
...
| `code_language` | `\[\]const u8` | `""` | Default language annotation for fenced code blocks that have no language hint. |
...
| `wrap_width` | `u64` | `80` | Maximum output line width in characters when `wrap` is `true` (default `80`). Lines are broken at word boundaries so that no line exceeds this length. A value of `0` is treated as "no limit" — equivalent to leaving `wrap` disabled. Has no effect when `wrap` is `false`. |
| `convert_as_inline` | `bool` | `false` | Treat the entire document as inline content (no block-level wrappers). |
| `sub_symbol` | `\[\]const u8` | `""` | Markdown notation for subscript text (e.g. `"~"`). |
| `sup_symbol` | `\[\]const u8` | `""` | Markdown notation for superscript text (e.g. `"^"`). |
...
| `keep_inline_images_in` | `\[\]const \[\]const u8` | `[]` | HTML tag names whose `<img>` children are kept inline instead of block. |
...
| `encoding` | `\[\]const u8` | `"utf-8"` | Expected character encoding of the input HTML (default `"utf-8"`). |
| `debug` | `bool` | `false` | Emit debug information during conversion. |
| `strip_tags` | `\[\]const \[\]const u8` | `[]` | HTML tag names whose content is stripped from the output entirely. |
| `preserve_tags` | `\[\]const \[\]const u8` | `[]` | HTML tag names that are preserved verbatim in the output. |
...
| `max_depth` | `u64?` | `null` | Maximum DOM traversal depth. `null` uses the library's internal native-stack safety limit. Explicit values above that safety limit are clamped to prevent process-aborting stack overflows on pathologically deep DOM trees. |
| `exclude_selectors` | `\[\]const \[\]const u8` | `[]` | CSS selectors for elements to exclude entirely (element + all content). Unlike `strip_tags` (which removes the tag wrapper but keeps children), excluded elements and all their descendants are dropped from the output. Supports any CSS selector that `tl` supports: tag names, `.class`, `#id`, `[attribute]`, etc. Invalid selectors are silently skipped at conversion time. Example: `[".cookie-banner", "#ad-container", "[role='complementary']"]` |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `title` | `\[\]const u8?` | `null` | Document title |
| `subject` | `\[\]const u8?` | `null` | Document subject/topic |
| `creator` | `\[\]const u8?` | `null` | Document creator/author |
| `keywords` | `\[\]const u8?` | `null` | Keywords or tags |
| `description` | `\[\]const u8?` | `null` | Document description/abstract |
| `last_modified_by` | `\[\]const u8?` | `null` | User who last modified the document |
| `revision` | `\[\]const u8?` | `null` | Revision number |
| `created` | `\[\]const u8?` | `null` | Creation timestamp (ISO 8601) |
| `modified` | `\[\]const u8?` | `null` | Last modification timestamp (ISO 8601) |
| `category` | `\[\]const u8?` | `null` | Document category |
| `content_status` | `\[\]const u8?` | `null` | Content status (Draft, Final, etc.) |
| `language` | `\[\]const u8?` | `null` | Document language |
| `identifier` | `\[\]const u8?` | `null` | Unique identifier |
| `version` | `\[\]const u8?` | `null` | Document version |
| `last_printed` | `\[\]const u8?` | `null` | Last print timestamp (ISO 8601) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `delimiter` | `\[\]const u8?` | `null` | Field delimiter, as a single-character string (e.g. `","`, `";"`, `"\t"`, `"\|"`). When `null` (default), the delimiter is auto-detected from a sample of the file. Must be exactly one ASCII byte when set — `ExtractionConfig.validate` rejects an empty string or a multi-byte value with a helpful error. The TSV MIME type (`text/tab-separated-values`) always forces `\t` regardless of this setting. |
| `comment_prefixes` | `\[\]const \[\]const u8` | `[]` | Line prefixes that mark a comment line to skip entirely during row parsing (e.g. `["#"]`). A line is treated as a comment when its trimmed start matches any of these prefixes exactly. Default: empty, meaning no line is treated as a comment (matches the pre-existing extractor behavior). |
...
| `delimiter` | `\[\]const u8?` | `null` | Field delimiter character (e.g. `","` or `"\t"`). |
| `has_header` | `bool` | — | Whether the first row was treated as a header. |
| `column_types` | `\[\]const \[\]const u8?` | `[]` | Inferred data type for each column (e.g. `"string"`, `"integer"`, `"float"`). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `\[\]const u8` | — | Field (column) name. |
| `field_type` | `\[\]const u8` | — | dBASE field type character (e.g. `"C"` for character, `"N"` for numeric). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `record_count` | `u64` | — | Total number of data records in the DBF file. |
| `field_count` | `u64` | — | Number of field (column) definitions. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `mime_type` | `\[\]const u8` | — | Detected MIME type |
| `filename` | `\[\]const u8?` | `null` | Original filename (if provided) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `from_line` | `u64` | — | Starting line number in the old content (0-indexed). |
| `from_count` | `u64` | — | Number of lines from the old content in this hunk. |
| `to_line` | `u64` | — | Starting line number in the new content (0-indexed). |
| `to_count` | `u64` | — | Number of lines from the new content in this hunk. |
...
| `max_content_chars` | `u64?` | `null` | Truncate content to this many characters before diffing. Useful for very large documents where only the first N characters matter. `null` means no truncation. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `identifier` | `\[\]const u8` | — | Element identifier used by the Djot attribute map. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `plain_text` | `\[\]const u8` | — | Plain text representation for backwards compatibility |
...
| `attributes` | `\[\]const DjotAttributeGroup` | `/* serde(default) */` | Attributes mapped by element identifier (if present) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `src` | `\[\]const u8` | — | Image source URL or path |
| `alt` | `\[\]const u8` | — | Alternative text |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `url` | `\[\]const u8` | — | Link URL |
| `text` | `\[\]const u8` | — | Link text content |
| `title` | `\[\]const u8?` | `null` | Optional title |
| `attributes` | `Attributes?` | `null` | Element attributes |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `\[\]const u8` | — | Check identifier, e.g. `ocr.tesseract` or `layout.rtdetr`. |
| `status` | `ProbeStatus` | — | Pass / warn / fail / skip verdict. |
| `message` | `\[\]const u8` | — | One-line reason or detail (e.g. missing language, resolved path, error). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `checks` | `\[\]const DoctorCheck` | `[]` | Individual check verdicts, in execution order. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `pages` | `u64` | — | Total pages in the source document (`0` when not page-addressable). |
| `tables` | `u64` | — | Tables detected in the document. |
| `images` | `u64` | — | Images detected in the document. |
...
```zig
pub fn extract(self: *const DocumentExtractor, input: []const u8, config: []const u8) XbergError![]u8
```

**Example:**

```zig
const result = try instance.extract(.{}, .{});
```
...
```zig
pub fn supported_mime_types(self: *const DocumentExtractor) []u8
```

**Example:**

```zig
const result = instance.supported_mime_types();
```
...
```zig
pub fn can_handle(self: *const DocumentExtractor, path: []const u8, mime_type: []const u8) bool
```

**Example:**

```zig
const result = instance.can_handle("value", "value");
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `path` | `\[\]const u8` | Yes | The  path |
| `mime_type` | `\[\]const u8` | Yes | The  mime type |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `mime_type` | `\[\]const u8` | — | MIME type of the document. |
| `size_bytes` | `u64` | — | File size in bytes. |
| `page_count` | `u32?` | `null` | Page count (if known, e.g., from previous analysis). |
| `force_ocr` | `bool` | — | Whether OCR is forced regardless of text layer. |
| `user_chunk_config` | `UserChunkConfig?` | `null` | User-provided chunk configuration overrides. |
...
| `page_end` | `u32?` | `null` | Page number where this node ends (for multi-page tables/sections). |
...
| `attributes` | `std.StringHashMap(\[\]const u8)?` | `null` | Format-specific key-value attributes. Extensible bag for miscellaneous data without a dedicated typed field: CSS classes, LaTeX environment names, Excel cell formulas, slide layout names, etc. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `revision_id` | `\[\]const u8` | — | Format-specific revision identifier. For DOCX this is the `w:id` attribute value on the change element (e.g. `"42"`). When the attribute is absent a synthetic fallback is generated (`"docx-ins-0"`, `"docx-del-3"`, …). |
| `author` | `\[\]const u8?` | `null` | Display name of the author who made this change, when available. |
| `timestamp` | `\[\]const u8?` | `null` | ISO-8601 timestamp of the change, when available. Stored as a plain string so this type remains FFI-friendly and unconditionally available without the `chrono` optional dep. DOCX populates this from the `w:date` attribute (e.g. `"2024-03-15T10:30:00Z"`). |
...
| `source_format` | `\[\]const u8?` | `null` | Origin format identifier (e.g. "docx", "pptx", "html", "pdf"). Allows renderers to apply format-aware heuristics when converting the document tree to output formats. |
...
| `node_types` | `\[\]const \[\]const u8` | `[]` | Sorted, deduplicated list of node type names present in this document. Each value is the snake_case `node_type` tag of the corresponding `NodeContent` variant (e.g. `"paragraph"`, `"heading"`, `"table"`, …). Computed from `nodes` via `DocumentStructure.finalize_node_types`. Empty until that method is called (internal construction paths call it at the end of derivation). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `text` | `\[\]const u8` | — | Summary text (plain prose). |
| `strategy` | `SummaryStrategy` | — | Strategy that produced this summary. |
| `token_count` | `u32?` | `null` | Approximate token count of the summary, when known. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `application` | `\[\]const u8?` | `null` | Application name (e.g., "Microsoft Office Word") |
| `app_version` | `\[\]const u8?` | `null` | Application version |
| `template` | `\[\]const u8?` | `null` | Template filename |
| `total_time` | `i32?` | `null` | Total editing time in minutes |
| `pages` | `i32?` | `null` | Number of pages |
| `words` | `i32?` | `null` | Number of words |
| `characters` | `i32?` | `null` | Number of characters (excluding spaces) |
| `characters_with_spaces` | `i32?` | `null` | Number of characters (including spaces) |
| `lines` | `i32?` | `null` | Number of lines |
| `paragraphs` | `i32?` | `null` | Number of paragraphs |
| `company` | `\[\]const u8?` | `null` | Company name |
| `doc_security` | `i32?` | `null` | Document security level |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `core_properties` | `CoreProperties?` | `null` | Core properties from docProps/core.xml (Dublin Core metadata) Contains title, creator, subject, keywords, dates, etc. Shared format across DOCX/PPTX/XLSX documents. |
| `app_properties` | `DocxAppProperties?` | `null` | Application properties from docProps/app.xml (Word-specific statistics) Contains word count, page count, paragraph count, editing time, etc. DOCX-specific variant of Office application properties. |
| `custom_properties` | `std.StringHashMap(\[\]const u8)?` | `{}` | Custom properties from docProps/custom.xml (user-defined properties) Contains key-value pairs defined by users or applications. Values can be strings, numbers, booleans, or dates. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `element_id` | `\[\]const u8` | `/* serde(default) */` | Deterministic element identifier. Empty only when deserializing legacy payloads that predate this field's wire representation. |
| `element_type` | `ElementType` | — | Semantic type of this element |
| `text` | `\[\]const u8` | — | Text content of the element |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `page_number` | `u32?` | `null` | Page number (1-indexed) |
| `filename` | `\[\]const u8?` | `null` | Source filename or document name |
| `coordinates` | `BoundingBox?` | `null` | Bounding box coordinates if available |
| `element_index` | `u64?` | `null` | Position index in the element sequence |
| `additional` | `std.StringHashMap(\[\]const u8)` | — | Additional custom metadata |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `\[\]const u8?` | `null` | Attachment name (from Content-Disposition header) |
| `filename` | `\[\]const u8?` | `null` | Filename of the attachment |
| `mime_type` | `\[\]const u8?` | `null` | MIME type of the attachment |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `msg_fallback_codepage` | `u32?` | `null` | Windows codepage number to use when an MSG file contains no codepage property. Defaults to `null`, which falls back to windows-1252. If an unrecognized or invalid codepage number is supplied (including 0), the behavior silently falls back to windows-1252 — the same as when the MSG file itself contains an unrecognized codepage. No error or warning is emitted. Users should verify output when supplying unusual values. Common values: - 1250: Central European (Polish, Czech, Hungarian, etc.) - 1251: Cyrillic (Russian, Ukrainian, Bulgarian, etc.) - 1252: Western European (default) - 1253: Greek - 1254: Turkish - 1255: Hebrew - 1256: Arabic - 932:  Japanese (Shift-JIS) - 936:  Simplified Chinese (GBK) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `subject` | `\[\]const u8?` | `null` | Email subject line |
| `from_email` | `\[\]const u8?` | `null` | Sender email address |
| `to_emails` | `\[\]const \[\]const u8` | — | Primary recipient email addresses |
| `cc_emails` | `\[\]const \[\]const u8` | — | CC recipient email addresses |
| `bcc_emails` | `\[\]const \[\]const u8` | — | BCC recipient email addresses |
| `date` | `\[\]const u8?` | `null` | Email date/timestamp |
| `message_id` | `\[\]const u8?` | `null` | Message-ID header value |
| `plain_text` | `\[\]const u8?` | `null` | Plain text version of the email body |
| `html_content` | `\[\]const u8?` | `null` | HTML version of the email body |
| `content` | `\[\]const u8` | — | Cleaned/processed text content. Aliased as `cleaned_text` for back-compat. |
| `attachments` | `\[\]const EmailAttachment` | — | List of email attachments |
| `metadata` | `std.StringHashMap(\[\]const u8)` | — | Additional email headers and metadata |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `from_email` | `\[\]const u8?` | `null` | Sender's email address |
| `from_name` | `\[\]const u8?` | `null` | Sender's display name |
| `to_emails` | `\[\]const \[\]const u8` | `[]` | Primary recipients |
| `cc_emails` | `\[\]const \[\]const u8` | `[]` | CC recipients |
| `bcc_emails` | `\[\]const \[\]const u8` | `[]` | BCC recipients |
| `message_id` | `\[\]const u8?` | `null` | Message-ID header value |
| `attachments` | `\[\]const \[\]const u8` | `[]` | List of attachment filenames |
...
| `changed` | `\[\]const EmbeddedDiff` | `[]` | Children present in both but with differing content (matched by `path`). Each entry holds the diff of the nested `ExtractedDocument`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `path` | `\[\]const u8` | — | Archive-relative path identifying this entry. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `\[\]const u8` | — | The filename as stored in the PDF name tree. |
| `data` | `\[\]const u8` | — | Raw file bytes from the embedded stream (already decompressed by lopdf). |
| `compressed_size` | `u64` | — | Compressed byte count of the original stream (before decompression). Used by callers to compute the decompression ratio and detect zip-bomb-style attacks that embed a tiny compressed stream expanding to gigabytes of data. |
| `mime_type` | `\[\]const u8?` | `null` | MIME type if specified in the filespec, otherwise `null`. |
...
```zig
pub fn embed(self: *const EmbeddingBackend, texts: []const u8) XbergError![]u8
```

**Example:**

```zig
const result = try instance.embed(&[_]u8{});
```
...
| `batch_size` | `u64` | `32` | Batch size for embedding generation |
...
| `cache_dir` | `\[\]const u8?` | `null` | Optional alternate Hugging Face cache root for model files. When unset, hf-hub follows `HF_HUB_CACHE`, `HUGGINGFACE_HUB_CACHE`, `HF_HOME`, XDG, and platform defaults. Prefer those environment variables when configuring the cache process-wide. |
...
| `text` | `\[\]const u8` | — | Raw mention text exactly as it appeared in the source. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `coverage` | `\[\]const u8?` | `null` | Dublin Core `coverage` field (geographic or temporal scope). |
| `dc_format` | `\[\]const u8?` | `null` | Dublin Core `format` field (media type of the resource). |
| `relation` | `\[\]const u8?` | `null` | Dublin Core `relation` field (related resource identifier). |
| `source` | `\[\]const u8?` | `null` | Dublin Core `source` field (origin resource identifier). |
| `dc_type` | `\[\]const u8?` | `null` | Dublin Core `type` field (nature or genre of the resource). |
| `cover_image` | `\[\]const u8?` | `null` | Path or identifier of the cover image within the EPUB container. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `error_type` | `\[\]const u8` | — | Machine-readable error type identifier (e.g. "UnsupportedFormat"). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `sheet_count` | `u32?` | `null` | Number of sheets in the workbook. |
| `sheet_names` | `\[\]const \[\]const u8?` | `[]` | Names of all sheets in the workbook. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `\[\]const u8` | — | Sheet name as it appears in Excel |
| `markdown` | `\[\]const u8` | — | Sheet content converted to Markdown tables |
| `row_count` | `u64` | — | Number of rows |
| `col_count` | `u64` | — | Number of columns |
| `cell_count` | `u64` | — | Total number of non-empty cells |
| `table_cells` | `\[\]const \[\]const \[\]const u8?` | `null` | Pre-extracted table cells (2D vector of cell values) Populated during markdown generation to avoid re-parsing markdown. None for empty sheets. |
...
| `metadata` | `std.StringHashMap(\[\]const u8)` | — | Workbook-level metadata (author, creation date, etc.) |
...
| `bytes` | `\[\]const u8?` | `null` | Raw bytes for `kind = "bytes"`. |
| `uri` | `\[\]const u8?` | `null` | Local path, `file://` URI, or HTTP(S) URL for `kind = "uri"`. |
| `mime_type` | `\[\]const u8?` | `null` | MIME type hint. |
| `filename` | `\[\]const u8?` | `null` | Filename hint used for MIME detection and metadata. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `content` | `\[\]const u8` | — | Plain-text representation of the extracted document content. |
| `mime_type` | `\[\]const u8` | — | MIME type of the source document (e.g. `"application/pdf"`). |
| `metadata` | `Metadata` | — | Document-level metadata (author, title, dates, format-specific fields). |
| `extraction_method` | `ExtractionMethod?` | `null` | Extraction strategy used to produce the returned text. Populated when the extractor can reliably distinguish native text extraction, OCR-only extraction, or mixed native/OCR output. |
...
| `detected_languages` | `\[\]const \[\]const u8?` | `[]` | ISO 639-1 language codes detected in the document content. |
| `detected_language_confidences` | `\[\]const LanguageConfidence?` | `[]` | Structured per-language detection results: confidence, document share, script, and reliability, alongside the ISO-code-only `detected_languages` (#261). One entry per language in `detected_languages`, in the same order. `null` under the same conditions as `detected_languages`: detection disabled, empty input text, or no language met the configured `min_confidence`. |
...
| `djot_content` | `DjotContent?` | `null` | Rich Djot content structure (when extracting Djot documents). When extracting Djot documents with structured extraction enabled, this field contains the full semantic structure including: - Block-level elements with nesting - Inline formatting with attributes - Links, images, footnotes - Math expressions - Complete attribute information The `content` field still contains plain text for backward compatibility. Always `null` for non-Djot documents. |
| `ocr_elements` | `\[\]const OcrElement?` | `[]` | OCR elements with full spatial and confidence metadata. When OCR is performed with element extraction enabled, this field contains the structured representation of detected text including: - Bounding geometry (rectangles or quadrilaterals) - Confidence scores (detection and recognition) - Rotation information - Hierarchical relationships (Tesseract only) This field preserves all metadata that would otherwise be lost when converting to plain text or markdown output formats. Only populated when `OcrElementConfig.include_elements` is true. |
...
| `structured_output` | `\[\]const u8?` | `null` | Structured extraction output from LLM-based JSON schema extraction. When `structured_extraction` is configured in `ExtractionConfig`, the extracted document content is sent to a VLM with the provided JSON schema. The response is parsed and stored here as a JSON value matching the schema. |
| `code_intelligence` | `\[\]const u8?` | `null` | Code intelligence results from tree-sitter analysis. Populated when extracting source code files with the `tree-sitter` feature. Contains metrics, structural analysis, imports/exports, comments, docstrings, symbols, diagnostics, and optionally chunked code segments. Stored as an opaque JSON value so that all language bindings (Go, Java, C#, …) can deserialize it as a raw JSON object rather than a typed struct. The underlying type is `tree_sitter_language_pack.ProcessResult`. |
| `llm_usage` | `\[\]const LlmUsage?` | `[]` | LLM token usage and cost data for all LLM calls made during this extraction. Contains one entry per LLM call. Multiple entries are produced when VLM OCR, structured extraction, or LLM embeddings run during the same extraction. `null` when no LLM was used. |
| `entities` | `\[\]const Entity?` | `[]` | Named entities detected in `content` by the NER post-processor. `null` when no NER backend is configured. Populated by the `xberg-gliner` ONNX backend or the LLM-driven backend (see `crates/xberg/src/text/ner/`). |
...
| `page_classifications` | `\[\]const PageClassification?` | `[]` | Per-page classifications produced by the page-classification post-processor. `null` when classification is not configured. |
| `redaction_report` | `RedactionReport?` | `null` | Audit report of redactions applied by the redaction post-processor. The redaction processor rewrites `content`, `formatted_content`, every chunk's text, and the textual fields of `entities` / `summary` / `translation` / `page_classifications` in place. This report describes what was found and how it was replaced. `null` when redaction is not configured. |
| `formulas` | `\[\]const Formula` | `[]` | Mathematical formulas recognized in the document. Populated from every source that produces formulas: layout-guided OCR (with geometry), VLM OCR (text only), and markup extraction (DOCX, PPTX, ODT, EPUB, HTML, JATS, LaTeX, Markdown, and related formats, without geometry). Empty when the document contains no formulas. |
| `form_fields` | `\[\]const PdfFormField` | `[]` | Form fields extracted from a PDF's AcroForm or XFA structure. Populated by the PDF extractor when `PdfConfig.extract_form_fields` is enabled (default) and the document is a fillable form. Empty otherwise. |
...
| `format` | `\[\]const u8` | — | Image format (e.g., "jpeg", "png", "webp") Uses Cow<'static, str> to avoid allocation for static literals. |
| `image_index` | `u32` | — | Zero-indexed position of this image in the document/page |
| `page_number` | `u32?` | `null` | Page/slide number where image was found (1-indexed) |
| `width` | `u32?` | `null` | Image width in pixels |
| `height` | `u32?` | `null` | Image height in pixels |
| `colorspace` | `\[\]const u8?` | `null` | Colorspace information (e.g., "RGB", "CMYK", "Gray") |
| `bits_per_component` | `u32?` | `null` | Bits per color component (e.g., 8, 16) |
| `is_mask` | `bool` | — | Whether this image is a mask image |
| `description` | `\[\]const u8?` | `null` | Optional description of the image |
| `ocr_result` | `ExtractedDocument?` | `null` | Nested OCR extraction result (if image was OCRed) When OCR is performed on this image, the result is embedded here rather than in a separate collection, making the relationship explicit. |
| `bounding_box` | `BoundingBox?` | `null` | Bounding box of the image on the page (PDF coordinates: x0=left, y0=bottom, x1=right, y1=top). Only populated for PDF-extracted images when position data is available from the PDF extractor. |
| `source_path` | `\[\]const u8?` | `null` | Original source path of the image within the document archive (e.g., "media/image1.png" in DOCX). Used for rendering image references when the binary data is not extracted. |
| `image_kind` | `ImageKind?` | `null` | Heuristic classification of what this image likely depicts. `null` if classification was disabled or inconclusive. |
| `kind_confidence` | `f32?` | `null` | Confidence score for `image_kind`, in the range 0.0 to 1.0. |
| `cluster_id` | `u32?` | `null` | Identifier shared across images that form a single logical figure (e.g. all raster tiles of one technical drawing). `null` for singletons. |
| `caption` | `\[\]const u8?` | `null` | VLM-generated caption describing the image, when captioning is configured. Populated by the captioning post-processor (`crates/xberg/src/plugins/processor/builtin/captioning.rs`), which routes each image through `crate.llm.region_extractor.extract_region_with_vlm` in caption mode. `null` when captioning is disabled or the VLM declined to caption. |
| `qr_codes` | `\[\]const QrCode?` | `[]` | QR codes decoded from this image, when QR detection is enabled. Populated by the QR post-processor (`crates/xberg/src/extractors/qr.rs`) via the pure-Rust `rqrr` decoder. `null` when QR detection is disabled; an empty `Some([])` when detection ran but found nothing. |
| `data_base64` | `\[\]const u8?` | `null` | Base64-encoded copy of `data`; populated when `ImageExtractionConfig.include_data_base64` is `true`. Omitted from JSON by default; use instead of `data` in JSON-only clients. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `url` | `\[\]const u8` | — | The URL or path string. |
| `label` | `\[\]const u8?` | `null` | Optional display text / label for the link. |
...
| `ocr_aggregate` | `f32?` | `null` | Mean OCR per-element recognition confidence when OCR ran; `null` when it did not. |
...
| `force_ocr_pages` | `\[\]const u32?` | `null` | Force OCR on specific pages only (1-indexed page numbers, must be >= 1). When set, only the listed pages are OCR'd regardless of text layer quality. Unlisted pages use native text extraction. Ignored when `force_ocr` is `true`. Only applies to PDF documents. Duplicates are automatically deduplicated. An `ocr` config is recommended for backend/language selection; defaults are used if absent. |
...
| `content_filter` | `ContentFilterConfig?` | `null` | Content filtering configuration (None = use extractor defaults). Controls whether document "furniture" (headers, footers, watermarks, repeating text) is included in or stripped from extraction results. See `ContentFilterConfig` for per-field documentation. |
| `images` | `ImageExtractionConfig?` | `null` | Image extraction configuration (None = no image extraction) |
| `pdf_options` | `PdfConfig?` | `null` | PDF-specific options (None = use defaults) |
| `token_reduction` | `TokenReductionOptions?` | `null` | Token reduction configuration (None = no token reduction) |
| `language_detection` | `LanguageDetectionConfig?` | `null` | Language detection configuration (None = no language detection) |
...
| `html_options` | `ConversionOptions?` | `null` | HTML to Markdown conversion options (None = use defaults) Configure how HTML documents are converted to Markdown, including heading styles, list formatting, code block styles, and preprocessing options. |
| `html_output` | `HtmlOutputConfig?` | `null` | Styled HTML output configuration. When set alongside `output_format = OutputFormat.Html`, the extraction pipeline uses `StyledHtmlRenderer` which emits stable `kb-*` CSS class hooks on every structural element and optionally embeds theme CSS or user-supplied CSS in a `<style>` block. When `null`, the existing plain comrak-based HTML renderer is used. |
| `extraction_timeout_secs` | `u64?` | `600` | Default per-file timeout in seconds for batch extraction. When set, each file in a batch will be canceled after this duration unless overridden by `FileExtractionConfig.timeout_secs`. Defaults to `Some(600)` (10 minutes) to prevent pathological files (e.g. deeply nested archives, documents with millions of cells) from running indefinitely and exhausting caller resources, while still giving slow paths (VLM-based OCR, large scanned documents) enough headroom to finish. Set to `null` to disable the timeout for trusted input or long-running workloads. |
| `max_concurrent_extractions` | `u64?` | `null` | Maximum concurrent document extractions in batch operations. This is a ceiling within the configured total thread budget, not an independent pool size. When unset, the scheduler derives document and per-document concurrency from `ConcurrencyConfig.max_threads`. |
...
| `cache_namespace` | `\[\]const u8?` | `null` | Cache namespace for tenant isolation. When set, cache entries are stored under `{cache_dir}/{namespace}/`. Must be alphanumeric, hyphens, or underscores only (max 64 chars). Different namespaces have isolated cache spaces on the same filesystem. |
| `cache_ttl_secs` | `u64?` | `null` | Per-request cache TTL in seconds. Overrides the global `max_age_days` for this specific extraction. When `0`, caching is completely skipped (no read or write). When `null`, the global TTL applies. |
...
| `max_archive_depth` | `u64` | `3` | Maximum recursion depth for archive extraction (default: 3). Set to 0 to disable recursive extraction (legacy behavior). |
| `tree_sitter` | `TreeSitterConfig?` | `null` | Tree-sitter language pack configuration (None = tree-sitter disabled). When set, enables code file extraction using tree-sitter parsers. Controls grammar download behavior and code analysis options. |
| `structured_extraction` | `StructuredExtractionConfig?` | `null` | Structured extraction via LLM (None = disabled). When set, the extracted document content is sent to an LLM with the provided JSON schema. The structured response is stored in `ExtractedDocument.structured_output`. |
...
| `page_classification` | `PageClassificationConfig?` | `null` | Per-page classification configuration. When set, the classification post-processor runs at the Middle stage and populates `ExtractedDocument.page_classifications`. |
| `chunk_classification` | `ChunkClassificationConfig?` | `null` | Per-chunk multi-label classification configuration. When set, the chunk-classification post-processor runs at the Middle stage (after chunking) and populates `ChunkMetadata.classifications` on every chunk. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `content_diff` | `\[\]const DiffHunk` | `[]` | Unified-diff hunks for the `content` field. Empty when the content is identical. |
| `tables_added` | `\[\]const Table` | `[]` | Tables present in `b` but not in `a` (by index position, excess right-side tables). |
| `tables_removed` | `\[\]const Table` | `[]` | Tables present in `a` but not in `b` (by index position, excess left-side tables). |
| `tables_changed` | `\[\]const TableDiff` | `[]` | Cell-level changes for table pairs that share the same index and dimensions. |
| `metadata_changed` | `\[\]const u8` | — | Metadata difference, encoded as a JSON object with three top-level keys: `added` (keys present in `b` but not `a`), `removed` (keys present in `a` but not `b`), and `changed` (keys whose values differ — each entry is `{ "from": <value-in-a>, "to": <value-in-b> }`). This is NOT RFC 6902 JSON Patch — we deliberately chose a flatter shape to avoid pulling in a json-patch crate. If you need RFC 6902 semantics (with JSON Pointer paths) feed `a.metadata` and `b.metadata` to your preferred json-patch impl directly. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `index` | `u64` | — | Input index in the original request. |
| `code` | `u32` | — | Stable numeric error code. |
| `error_type` | `\[\]const u8` | — | Stable snake_case error kind. |
| `source` | `\[\]const u8` | — | Best-effort source identifier. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `results` | `\[\]const ExtractedDocument` | `[]` | Extracted documents in discovery order. |
| `errors` | `\[\]const ExtractionErrorItem` | `[]` | Non-fatal per-input errors. |
| `summary` | `ExtractionSummary` | — | Aggregate counts for the operation. |
| `crawl_final_urls` | `\[\]const \[\]const u8` | `[]` | Final URLs reached after redirects during URL ingestion. |
| `crawl_redirect_count` | `u64` | — | Total redirects followed while fetching or crawling URLs. |
| `crawl_unique_normalized_urls` | `\[\]const \[\]const u8` | `[]` | Unique normalized URLs discovered by crawls. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `inputs` | `u64` | — | Number of inputs submitted by the caller. |
| `results` | `u64` | — | Number of extraction results produced. |
| `errors` | `u64` | — | Number of per-input errors. |
| `remote_urls` | `u64` | — | Number of URI inputs that resolved to remote HTTP(S) URLs. |
| `pages_crawled` | `u64` | — | Number of HTML pages crawled or scraped. |
| `documents_downloaded` | `u64` | — | Number of downloaded non-HTML documents extracted from URLs. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `genres` | `\[\]const \[\]const u8` | `[]` | Genre tags as declared in the FB2 `<genre>` elements. |
| `sequences` | `\[\]const \[\]const u8` | `[]` | Book series (sequence) names, if any. |
| `annotation` | `\[\]const u8?` | `null` | Short annotation / summary from the FB2 `<annotation>` element. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `mime_detection_policy` | `MimeDetectionPolicy?` | `null` | Override MIME inference policy for this file. |
...
| `ocr_strategy` | `OcrStrategy?` | `null` | Override the OCR page-selection strategy for this file. |
| `force_ocr_pages` | `\[\]const u32?` | `[]` | Override force OCR pages for this file (1-indexed page numbers). |
...
| `content_filter` | `ContentFilterConfig?` | `null` | Override content filtering configuration for this file. |
| `images` | `ImageExtractionConfig?` | `null` | Override image extraction configuration for this file. |
| `pdf_options` | `PdfConfig?` | `null` | Override PDF options for this file. |
| `token_reduction` | `TokenReductionOptions?` | `null` | Override token reduction for this file. |
| `language_detection` | `LanguageDetectionConfig?` | `null` | Override language detection for this file. |
...
| `html_options` | `ConversionOptions?` | `null` | Override HTML conversion options for this file. |
| `html_output` | `HtmlOutputConfig?` | `null` | Override styled HTML output configuration for this file. |
| `result_format` | `ResultFormat?` | `null` | Override result format for this file. |
| `output_format` | `OutputFormat?` | `null` | Override output content format for this file. |
...
| `timeout_secs` | `u64?` | `null` | Override per-file extraction timeout in seconds. When set, the extraction for this file will be canceled after the specified duration. A timed-out file produces an error result without affecting other files in the batch. |
| `tree_sitter` | `TreeSitterConfig?` | `null` | Override tree-sitter configuration for this file. |
| `structured_extraction` | `StructuredExtractionConfig?` | `null` | Override structured extraction configuration for this file. When set, enables LLM-based structured extraction with a JSON schema for this specific file. The extracted content is sent to a VLM/LLM and the response is parsed according to the provided schema. |
...
| `page_classification` | `PageClassificationConfig?` | `null` | Override per-page classification configuration for this file. |
| `chunk_classification` | `ChunkClassificationConfig?` | `null` | Override per-chunk classification configuration for this file. |
| `captioning` | `CaptioningConfig?` | `null` | Override VLM captioning configuration for this file. |
| `qr_codes` | `bool?` | `null` | Override QR-code detection for this file. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `label` | `\[\]const u8` | — | The label of the footnote reference (e.g., "1" in `[^1]`). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `label` | `\[\]const u8` | — | The label of the footnote (e.g., "1" in `[^1]: ...`). |
| `content` | `\[\]const u8` | — | The full content of the footnote definition. |
...
| `inline_content` | `\[\]const InlineElement` | — | Inline content within the block |
| `attributes` | `Attributes?` | `null` | Element attributes (classes, IDs, key-value pairs) |
| `language` | `\[\]const u8?` | `null` | Language identifier for code blocks |
| `code` | `\[\]const u8?` | `null` | Raw code content for code blocks |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `latex` | `\[\]const u8` | — | LaTeX source of the formula, without surrounding `$$` delimiters. Markup converters and formula OCR produce real LaTeX. The native PDF layout path stores the plain text of a detected formula region, which keeps the original Unicode math characters instead of LaTeX commands. To render the formula in Markdown or other formats, wrap it in `$$..$$`. |
...
| `page` | `u32?` | `/* serde(default) */` | 1-indexed page number the formula appears on. `null` when the source format has no page concept. The C FFI reports an absent page as `0`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `content` | `\[\]const u8` | — | Cell text content. |
...
| `text` | `\[\]const u8` | — | Normalized text content of the header |
| `id` | `\[\]const u8?` | `null` | HTML id attribute if present |
...
| `text` | `\[\]const u8` | — | The text content of the heading. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `text` | `\[\]const u8` | — | The text content of this block |
| `font_size` | `f32` | — | The font size of the text in this block |
| `level` | `\[\]const u8` | — | The hierarchy level of this block (H1-H6 or Body) Levels correspond to HTML heading tags: - "h1": Top-level heading - "h2": Secondary heading - "h3": Tertiary heading - "h4": Quaternary heading - "h5": Quinary heading - "h6": Senary heading - "body": Body text (no heading level) |
...
| `k_clusters` | `u64` | `3` | Number of font size clusters to use for hierarchy levels (1-7) Default: 3, which provides two heading levels plus body text. Larger values create more fine-grained hierarchy levels. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `title` | `\[\]const u8?` | `null` | Document title from `<title>` tag |
| `description` | `\[\]const u8?` | `null` | Document description from `<meta name="description">` tag |
| `keywords` | `\[\]const \[\]const u8` | `[]` | Document keywords from `<meta name="keywords">` tag, split on commas |
| `author` | `\[\]const u8?` | `null` | Document author from `<meta name="author">` tag |
| `canonical_url` | `\[\]const u8?` | `null` | Canonical URL from `<link rel="canonical">` tag |
| `base_href` | `\[\]const u8?` | `null` | Base URL from `<base href="">` tag for resolving relative URLs |
| `language` | `\[\]const u8?` | `null` | Document language from `lang` attribute |
| `text_direction` | `TextDirection?` | `null` | Document text direction from `dir` attribute |
| `open_graph` | `std.StringHashMap(\[\]const u8)` | `{}` | Open Graph metadata (og:* properties) for social media Keys like "title", "description", "image", "url", etc. |
| `twitter_card` | `std.StringHashMap(\[\]const u8)` | `{}` | Twitter Card metadata (twitter:* properties) Keys like "card", "site", "creator", "title", "description", "image", etc. |
| `meta_tags` | `std.StringHashMap(\[\]const u8)` | `{}` | Additional meta tags not covered by specific fields Keys are meta name/property attributes, values are content |
...
| `structured_data` | `\[\]const StructuredData` | `[]` | Extracted structured data blocks |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `css` | `\[\]const u8?` | `null` | Inline CSS string injected into the output after the theme stylesheet. Concatenated after `css_file` content when both are set. |
| `css_file` | `\[\]const u8?` | `null` | Path to a CSS file loaded once at renderer construction time. Concatenated before `css` when both are set. |
| `theme` | `HtmlTheme` | `HtmlTheme.unstyled` | Built-in colour/typography theme. Default: `HtmlTheme.Unstyled`. |
| `class_prefix` | `\[\]const u8` | `"kb-"` | CSS class prefix applied to every emitted class name. Default: `"kb-"`. Change this if your host application already uses classes that start with `kb-`. |
...
| `format` | `\[\]const u8` | — | Image format (e.g., "PNG", "JPEG", "TIFF") |
| `exif` | `std.StringHashMap(\[\]const u8)` | `{}` | EXIF metadata tags |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `src` | `\[\]const u8` | — | Image source (URL, data URI, or SVG content) |
| `alt` | `\[\]const u8?` | `null` | Alternative text from alt attribute |
| `title` | `\[\]const u8?` | `null` | Title attribute |
| `dimensions` | `ImageDimensions?` | `null` | Image dimensions if available. |
| `image_type` | `ImageType` | — | Image type classification |
| `attributes` | `\[\]const KeyValueAttribute` | — | Additional attributes as key-value pairs. |
...
| `binarization_method` | `\[\]const u8` | `"otsu"` | Binarization method: "none" (alias "off"), "otsu", "sauvola", or "adaptive". `deskew` must be `false` when this is `none` or `off`. |
...
| `new_dimensions` | `PixelDimensions?` | `null` | New dimensions after resizing (if resized). |
| `resample_method` | `\[\]const u8` | — | Resampling algorithm used ("LANCZOS3", "CATMULLROM", etc.) |
| `dimension_clamped` | `bool` | — | Whether dimensions were clamped to max_image_dimension |
| `calculated_dpi` | `i32?` | `null` | Calculated optimal DPI (if auto_adjust_dpi enabled) |
| `skipped_resize` | `bool` | — | Whether resize was skipped (dimensions already optimal) |
| `resize_error` | `\[\]const u8?` | `null` | Error message if resize failed |
...
| `attributes` | `Attributes?` | `null` | Element attributes |
| `metadata` | `std.StringHashMap(\[\]const u8)?` | `null` | Additional metadata (e.g., href for links, src/alt for images) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `copyright` | `\[\]const u8?` | `null` | Copyright statement from the article's `<permissions>` element. |
| `license` | `\[\]const u8?` | `null` | Open-access license URI from the article's `<license>` element. |
| `history_dates` | `std.StringHashMap(\[\]const u8)` | `{}` | Publication history dates keyed by event type (e.g. `"received"`, `"accepted"`). |
| `contributor_roles` | `\[\]const ContributorRole` | `[]` | Authors and contributors with their stated roles. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `text` | `\[\]const u8` | — | The keyword text. |
...
| `max_keywords` | `u64` | `10` | Maximum number of keywords to extract (default: 10). |
...
| `language` | `\[\]const u8?` | `"en"` | Language code for stopword filtering (e.g., "en", "de", "fr"). If None, no stopword filtering is applied. |
| `yake_params` | `YakeParams?` | `null` | YAKE-specific tuning parameters. |
| `rake_params` | `RakeParams?` | `null` | RAKE-specific tuning parameters. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `language` | `\[\]const u8` | — | ISO 639-3 language code, matching the corresponding entry in `detected_languages`. |
...
| `script` | `\[\]const u8` | — | Writing system whatlang detected for this language (e.g. `"Latin"`, `"Cyrillic"`). |
...
| `batch_size` | `u64` | `16` | Batch size for local ONNX inference. ColBERT emits a `[seq, dim]` multi-vector embedding per document, so memory scales with batch size — keep this modest. |
| `max_length` | `u64` | `512` | Maximum token sequence length for the tokenizer (documents). |
| `query_max_length` | `u64` | `32` | Fixed padded length for query augmentation. ColBERT queries are padded (with the mask token, kept attention-live) to exactly this many tokens rather than truncated/left as-is — this is the "query augmentation" trick from the ColBERT paper. |
...
| `cache_dir` | `\[\]const u8?` | `null` | Optional alternate Hugging Face cache root for model files. When unset, hf-hub follows the standard Hugging Face environment and platform cache conventions. |
| `acceleration` | `AccelerationConfig?` | `null` | Hardware acceleration for the late-interaction ONNX model. |
| `max_embed_duration_secs` | `u64?` | `60` | Maximum wall-clock duration (in seconds) for a single embed call when using `LateInteractionModelType.Plugin`. `null` disables the timeout. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `index` | `u64` | — | Position of this document in the original input slice. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `\[\]const u8` | — | Stable preset name referenced from config. |
| `model_repo` | `\[\]const u8` | — | HuggingFace repository hosting the ONNX model. |
| `model_file` | `\[\]const u8` | — | Path to the ONNX file within the repo. |
| `additional_files` | `\[\]const \[\]const u8` | — | Sibling files that must be downloaded alongside `model_file`. |
| `max_length` | `u64` | — | Maximum document token sequence length. |
| `query_max_length` | `u64` | — | Fixed padded query length (ColBERT query augmentation). |
| `dim` | `u64` | — | Per-token embedding dimensionality. |
...
| `confidence_threshold` | `f32?` | `null` | Confidence threshold override (None = use model default). |
...
| `formula_model` | `FormulaModel?` | `null` | Formula recognition model for layout-detected formula regions. `null` (the default) keeps the plain OCR text of the region. Setting a model converts each formula region crop to LaTeX. Requires the `formula-recognition` feature; without it the setting is ignored. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `class_name` | `\[\]const u8` | — | Layout class name (e.g. "picture", "table", "text", "section_header"). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `href` | `\[\]const u8` | — | The href URL value |
| `text` | `\[\]const u8` | — | Link text content (normalized) |
| `title` | `\[\]const u8?` | `null` | Optional title attribute |
| `link_type` | `LinkType` | — | Link type classification |
| `rel` | `\[\]const \[\]const u8` | — | Rel attribute values |
| `attributes` | `\[\]const KeyValueAttribute` | — | Additional attributes as key-value pairs. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `model` | `\[\]const u8` | — | Provider/model string using liter-llm routing format. Examples: `"openai/gpt-4o"`, `"anthropic/claude-sonnet-4-20250514"`, `"groq/llama-3.1-70b-versatile"`. |
| `api_key` | `\[\]const u8?` | `null` | API key for the provider. When `null`, liter-llm falls back to the provider's standard environment variable (e.g., `OPENAI_API_KEY`). |
| `base_url` | `\[\]const u8?` | `null` | Custom base URL override for the provider endpoint. |
| `timeout_secs` | `u64?` | `null` | Request timeout in seconds. When `null`, liter-llm's built-in 60s default applies, except the VLM OCR path which uses a 300s default (a single page image transcription routinely exceeds 60s). Set explicitly to override. |
| `max_retries` | `u32?` | `null` | Maximum retry attempts (default: 3). |
| `temperature` | `f64?` | `null` | Sampling temperature for generation tasks. |
| `max_tokens` | `u64?` | `null` | Maximum tokens to generate. |
| `top_p` | `f64?` | `null` | Nucleus sampling parameter for generation tasks, applied to individual requests built from this config. Restricts sampling to the smallest set of tokens whose cumulative probability mass is at least this value; lower is more focused. Validated to `[0.0, 1.0]` by `LlmConfig.validate`. Mirrors liter-llm's `ChatCompletionRequest.top_p`. A request-time parameter like `temperature`/`max_tokens` above, not a client-level setting. |
| `stop` | `\[\]const \[\]const u8?` | `[]` | Stop sequence(s) that halt token generation, applied to individual requests built from this config. Mirrors liter-llm's `ChatCompletionRequest.stop` (`types.common.StopSequence`), which liter-llm represents as either a single string or a list of strings via an untagged enum. Always expressed here as a list — even one stop sequence is `["..."]` — so the field has a single, FFI-friendly shape across every language binding instead of a single-or-list union type. Converted to liter-llm's `StopSequence.Multiple` at each request-building call site; see `llm.client.to_stop_sequence`. |
| `seed` | `i64?` | `null` | Random seed for reproducible outputs, applied to individual requests built from this config. Provider support varies — some silently ignore it. Mirrors liter-llm's `ChatCompletionRequest.seed`. |
| `presence_penalty` | `f64?` | `null` | Presence penalty for generation tasks, applied to individual requests built from this config. Positive values discourage the model from repeating topics already present in the conversation. Validated to `[-2.0, 2.0]` by `LlmConfig.validate`. Mirrors liter-llm's `ChatCompletionRequest.presence_penalty`. |
| `frequency_penalty` | `f64?` | `null` | Frequency penalty for generation tasks, applied to individual requests built from this config. Positive values discourage the model from repeating the same tokens verbatim. Validated to `[-2.0, 2.0]` by `LlmConfig.validate`. Mirrors liter-llm's `ChatCompletionRequest.frequency_penalty`. |
| `reasoning_effort` | `\[\]const u8?` | `null` | Reasoning effort level for extended-thinking models, applied to individual requests built from this config. Mirrors liter-llm's `ChatCompletionRequest.reasoning_effort` (`types.chat.ReasoningEffort`). A request-time parameter like `temperature`/ `max_tokens` above, not a client-level setting — `into_client_builder` does not map it. Accepted as a plain string — one of `"low"`, `"medium"`, `"high"`, `"minimal"`, `"max"` (case-insensitive; liter-llm's own `#[serde(rename_all = "lowercase")]` spelling) — rather than importing liter-llm's enum, because this module compiles even when the `liter-llm` feature is disabled. See `llm.client.parse_reasoning_effort` for the conversion into `liter_llm.ReasoningEffort`. |
| `extra_body` | `\[\]const u8?` | `null` | Provider-specific extra parameters merged into the request body (guardrails, safety settings, grounding config, etc.), applied to individual requests built from this config. Mirrors liter-llm's `ChatCompletionRequest.extra_body`. A request-time parameter like `temperature`/`max_tokens` above, not a client-level setting. |
| `load_env` | `bool?` | `null` | Whether liter-llm should load provider credentials from environment variables. Mirrors liter-llm's `ClientConfigBuilder.load_env`. When `null`, liter-llm's own default behavior applies. |
| `headers` | `std.StringHashMap(\[\]const u8)?` | `{}` | Extra HTTP headers sent with every request to the provider. Mirrors liter-llm's `ClientConfigBuilder.header`, for gateways or providers that require custom auth/routing headers. |
| `providers` | `\[\]const LlmProviderConfig?` | `[]` | Custom provider configurations, in addition to liter-llm's built-in providers. Mirrors liter-llm's `LlmConfig.providers`, for OpenAI-compatible gateways and self-hosted model servers that are not in the built-in provider catalog. |
...
| `rate_limit` | `LlmRateLimitConfig?` | `null` | Per-model rate limiting configuration. Mirrors liter-llm's `LlmConfig.rate_limit`. Only takes effect when liter-llm's `tower` feature is compiled in; otherwise the value is accepted but unused. |
| `cost_tracking` | `bool?` | `null` | Enable per-request cost tracking. Mirrors liter-llm's `LlmConfig.cost_tracking`. Only takes effect when liter-llm's `tower` feature is compiled in; otherwise the value is accepted but unused. |
...
| `cooldown_secs` | `u64?` | `null` | Cooldown duration after transient errors, in seconds. Mirrors liter-llm's `LlmConfig.cooldown_secs`. Only takes effect when liter-llm's `tower` feature is compiled in; otherwise the value is accepted but unused. |
| `health_check_secs` | `u64?` | `null` | Background health check interval, in seconds. Mirrors liter-llm's `LlmConfig.health_check_secs`. Only takes effect when liter-llm's `tower` feature is compiled in; otherwise the value is accepted but unused. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `model` | `\[\]const u8` | — | The LLM model identifier (e.g. "openai/gpt-4o", "anthropic/claude-sonnet-4-20250514"). |
| `source` | `\[\]const u8` | — | The pipeline stage that triggered this LLM call (e.g. "vlm_ocr", "structured_extraction", "embeddings"). |
| `input_tokens` | `u64?` | `null` | Number of input/prompt tokens consumed. |
| `output_tokens` | `u64?` | `null` | Number of output/completion tokens generated. |
| `total_tokens` | `u64?` | `null` | Total tokens (input + output). |
| `estimated_cost` | `f64?` | `null` | Estimated cost in USD based on the provider's published pricing. |
| `finish_reason` | `\[\]const u8?` | `null` | Why the model stopped generating (e.g. "stop", "length", "content_filter"). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `language` | `\[\]const u8` | — | Declared language identifier, or an empty string when absent. |
| `code` | `\[\]const u8` | — | Code block content. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `text` | `\[\]const u8` | — | Visible link text. |
| `url` | `\[\]const u8` | — | Link destination. |
...
```zig
pub fn compile_meta_schema(meta_schema_json: []const u8) LoadError!MetaSchema
```

**Example:**

```zig
const result = try compile_meta_schema("value");
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `meta_schema_json` | `\[\]const u8` | Yes | The meta schema json |
...
```zig
pub fn parse_preset(self: *const MetaSchema, path: []const u8, raw: []const u8) LoadError![]u8
```

**Example:**

```zig
const result = try instance.parse_preset("value", "data");
```
...
|-------|------|---------|-------------|
| `title` | `\[\]const u8?` | `null` | Document title |
| `subject` | `\[\]const u8?` | `null` | Document subject or description |
| `authors` | `\[\]const \[\]const u8?` | `[]` | Primary author(s) - always Vec for consistency |
| `keywords` | `\[\]const \[\]const u8?` | `[]` | Keywords/tags - always Vec for consistency |
| `language` | `\[\]const u8?` | `null` | Primary language (ISO 639 code) |
| `created_at` | `\[\]const u8?` | `null` | Creation timestamp (ISO 8601 format) |
| `modified_at` | `\[\]const u8?` | `null` | Last modification timestamp (ISO 8601 format) |
| `created_by` | `\[\]const u8?` | `null` | User who created the document |
| `modified_by` | `\[\]const u8?` | `null` | User who last modified the document |
...
| `image_preprocessing` | `ImagePreprocessingMetadata?` | `null` | Image preprocessing metadata (when OCR preprocessing was applied) |
| `json_schema` | `\[\]const u8?` | `null` | JSON schema (for structured data extraction) |
| `error` | `ErrorMetadata?` | `null` | Error metadata (for batch operations) |
| `extraction_duration_ms` | `u64?` | `null` | Extraction duration in milliseconds (for benchmarking). This field is populated by batch extraction to provide per-file timing information. It's `null` for single-file extraction (which uses external timing). |
| `category` | `\[\]const u8?` | `null` | Document category (from frontmatter or classification). |
| `tags` | `\[\]const \[\]const u8?` | `[]` | Document tags (from frontmatter). |
| `document_version` | `\[\]const u8?` | `null` | Document version string (from frontmatter). |
| `abstract_text` | `\[\]const u8?` | `null` | Abstract or summary text (from frontmatter). |
| `output_format` | `\[\]const u8?` | `null` | Output format identifier (e.g., "markdown", "html", "text"). Set by the output format pipeline stage when format conversion is applied. Previously stored in `metadata.additional["output_format"]`. |
...
| `additional` | `std.StringHashMap(\[\]const u8)` | `{}` | Additional custom fields from postprocessors. Serialized as a nested `"additional"` object (not flattened at root level). Uses `Cow<'static, str>` keys so static string keys avoid allocation. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `det_model` | `\[\]const u8` | — | Exact path to the detection ONNX model in the Hugging Face snapshot. |
| `cls_model` | `\[\]const u8` | — | Exact path to the classification ONNX model in the Hugging Face snapshot. |
| `rec_model` | `\[\]const u8` | — | Exact path to the recognition ONNX model in the Hugging Face snapshot. |
| `dict_file` | `\[\]const u8` | — | Path to the character dictionary file. |
...
| `data` | `\[\]const f32` | — | Flat row-major buffer, length `num_tokens * dim`. |
...
| `pages` | `\[\]const PageSignals` | — | Per-page signals extracted from the PDF. |
...
| `model` | `\[\]const u8?` | `null` | Override the default model — only used by `NerBackendKind.Onnx`. `null` lets the backend pick its pinned default xberg GLiNER model alias. |
...
| `custom_labels` | `\[\]const \[\]const u8` | `[]` | Arbitrary user-supplied entity labels for zero-shot detection. `xberg-gliner` natively supports zero-shot inference over caller-supplied labels. The LLM backend also honours these labels by including them in the structured-output schema. Custom labels surface as `EntityCategory.Custom` in the resulting `Entity` stream. Use this when you need domain-specific entity types (e.g. `"Treatment"`, `"Product"`, `"Vessel"`) without forking GLiNER's taxonomy. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `min` | `u64` | `1` | Minimum number of words in a candidate. |
| `max` | `u64` | `3` | Maximum number of words in a candidate. |
...
```zig
pub fn process_image(self: *const OcrBackend, image_bytes: []const u8, config: []const u8) XbergError![]u8
```

**Example:**

```zig
const result = try instance.process_image("data", .{});
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `image_bytes` | `\[\]const u8` | Yes | Raw image data (JPEG, PNG, TIFF, etc.) |
| `config` | `\[\]const u8` | Yes | OCR configuration (language, PSM mode, etc.) |
...
```zig
pub fn process_image_file(self: *const OcrBackend, path: []const u8, config: []const u8) XbergError![]u8
```

**Example:**

```zig
const result = try instance.process_image_file("value", .{});
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `path` | `\[\]const u8` | Yes | Path to the image file |
| `config` | `\[\]const u8` | Yes | OCR configuration |
...
```zig
pub fn supports_language(self: *const OcrBackend, lang: []const u8) bool
```

**Example:**

```zig
const result = instance.supports_language("value");
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `lang` | `\[\]const u8` | Yes | ISO 639-2/3 language code (e.g., "eng", "deu", "fra") |
...
```zig
pub fn backend_type(self: *const OcrBackend) OcrBackendType
```

**Example:**

```zig
const result = instance.backend_type();
```
...
```zig
pub fn supported_languages(self: *const OcrBackend) []u8
```

**Example:**

```zig
const result = instance.supported_languages();
```
...
```zig
pub fn supports_table_detection(self: *const OcrBackend) bool
```

**Example:**

```zig
const result = instance.supports_table_detection();
```
...
```zig
pub fn supports_document_processing(self: *const OcrBackend) bool
```

**Example:**

```zig
const result = instance.supports_document_processing();
```
...
```zig
pub fn emits_structured_markdown(self: *const OcrBackend) bool
```

**Example:**

```zig
const result = instance.emits_structured_markdown();
```
...
```zig
pub fn confidence_semantics(self: *const OcrBackend) ConfidenceSemantics
```

**Example:**

```zig
const result = instance.confidence_semantics();
```
...
```zig
pub fn page_orientation_handling(self: *const OcrBackend) PageOrientationHandling
```

**Example:**

```zig
const result = instance.page_orientation_handling();
```
...
```zig
pub fn process_document(self: *const OcrBackend, path: []const u8, config: []const u8) XbergError![]u8
```

**Example:**

```zig
const result = try instance.process_document("value", .{});
```
...
|------|------|----------|-------------|
| `path` | `\[\]const u8` | Yes | The  path |
| `config` | `\[\]const u8` | Yes | The ocr config |
...
| `vlm_fallback` | `VlmFallbackPolicy` | `VlmFallbackPolicy.disabled` | Ergonomic VLM fallback policy. When set to anything other than `VlmFallbackPolicy.Disabled` and `OcrConfig.pipeline` is `null`, a multi-stage pipeline is synthesised automatically: - `VlmFallbackPolicy.OnLowQuality` → `[classical_stage, vlm_stage]` with the `quality_threshold` mapped onto `OcrQualityThresholds.pipeline_min_quality`. - `VlmFallbackPolicy.Always` → `[vlm_stage]` only. Requires `OcrConfig.vlm_config` to be `Some` when not `Disabled`. When `OcrConfig.pipeline` is explicitly set, this field is ignored. |
| `vlm_config` | `LlmConfig?` | `null` | VLM (Vision Language Model) OCR configuration. Required when `backend` is `"vlm"` or when `vlm_fallback` is not `VlmFallbackPolicy.Disabled`. Uses liter-llm to send page images to a vision model for text extraction. |
| `vlm_prompt` | `\[\]const u8?` | `null` | Custom Jinja2 prompt template for VLM OCR. When `null`, uses the default template. Available variables: - `{{ language }}` — The document language code (e.g., "eng", "deu"). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `text` | `\[\]const u8` | — | The recognized text content. |
...
| `parent_id` | `\[\]const u8?` | `null` | Parent element ID for hierarchical relationships. When hierarchy output is enabled, this resolves to another emitted element's `backend_metadata["element_id"]` value. |
| `backend_metadata` | `std.StringHashMap(\[\]const u8)` | `{}` | Backend-specific metadata that doesn't fit the unified schema. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `content` | `\[\]const u8` | — | Recognized text content |
| `mime_type` | `\[\]const u8` | — | Original MIME type of the processed image |
| `metadata` | `std.StringHashMap(\[\]const u8)` | `{}` | OCR processing metadata (confidence scores, language, etc.) |
| `tables` | `\[\]const OcrTable` | `[]` | Tables detected and extracted via OCR |
| `ocr_elements` | `\[\]const OcrElement?` | `[]` | Structured OCR elements with bounding boxes and confidence scores. Available when TSV output is requested or table detection is enabled. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `language` | `\[\]const u8` | — | OCR language code(s) used |
| `psm` | `i32` | — | Tesseract Page Segmentation Mode (PSM) |
| `output_format` | `\[\]const u8` | — | Output format (e.g., "text", "hocr") |
| `table_count` | `u32` | — | Number of tables detected |
| `table_rows` | `u32?` | `null` | Number of rows in the detected table (if a single table was found). |
| `table_cols` | `u32?` | `null` | Number of columns in the detected table (if a single table was found). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `min_total_non_whitespace` | `u64` | `64` | Minimum total non-whitespace characters to consider text substantive. |
| `min_non_whitespace_per_page` | `f64` | `32` | Minimum non-whitespace characters per page on average. |
| `min_meaningful_word_len` | `u64` | `4` | Minimum character count for a word to be "meaningful". |
| `min_meaningful_words` | `u64` | `3` | Minimum count of meaningful words before text is accepted. |
| `min_alnum_ratio` | `f64` | `0.3` | Minimum alphanumeric ratio (non-whitespace chars that are alphanumeric). |
| `min_garbage_chars` | `u64` | `5` | Minimum Unicode replacement characters (U+FFFD) to trigger OCR fallback. |
...
| `min_words_for_ocr_output_check` | `u64` | `20` | Minimum word count before `Self.max_ocr_output_fragmented_word_ratio` may report a page. Short pages (a signature block, an exhibit title) are legitimately dominated by short words, so the ratio is not meaningful on them and the signal stays disabled. |
...
| `min_words_for_avg_length_check` | `u64` | `50` | Minimum word count before average word length check applies. |
| `min_consecutive_repeat_ratio` | `f64` | `0.08` | Minimum consecutive word repetition ratio to detect column scrambling. |
| `min_words_for_repeat_check` | `u64` | `50` | Minimum word count before consecutive repetition check is applied. |
| `substantive_min_chars` | `u64` | `100` | Minimum character count for "substantive markdown" OCR skip gate. |
| `non_text_min_chars` | `u64` | `20` | Minimum character count for "non-text content" OCR skip gate. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `cells` | `\[\]const \[\]const \[\]const u8` | — | Table cells as a 2D vector (rows × columns) |
| `markdown` | `\[\]const u8` | — | Markdown representation of the table |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `language` | `\[\]const u8` | `"en"` | Language code (e.g., "en", "ch", "jpn", "kor", "deu", "fra") |
| `cache_dir` | `\[\]const u8?` | `null` | Optional Hugging Face Hub cache root for model files. When unset, the standard `HF_HUB_CACHE`, legacy `HUGGINGFACE_HUB_CACHE`, and `HF_HOME` conventions are used. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `byte_start` | `u64` | — | Byte offset where this page starts in the content string (UTF-8 valid boundary, inclusive) |
| `byte_end` | `u64` | — | Byte offset where this page ends in the content string (UTF-8 valid boundary, exclusive) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `prompt_template` | `\[\]const u8?` | `null` | Minijinja prompt template. Receives `{{ labels }}` (joined list), `{{ page_text }}` and `{{ multi_label }}` variables. `null` lets the backend pick a sensible default. |
| `labels` | `\[\]const \[\]const u8` | — | The set of labels the classifier may emit. Must contain at least one entry. |
...
| `marker_format` | `\[\]const u8` | `"<!-- PAGE {page_num} -->"` | Page marker format (use {page_num} placeholder) Default: "\n\n<!-- PAGE {page_num} -->\n\n" |
...
|-------|------|---------|-------------|
| `page_number` | `u32` | — | Page number (1-indexed) |
| `content` | `\[\]const u8` | — | Text content for this page |
...
| `image_indices` | `\[\]const u32` | `/* serde(default) */` | Indices into `ExtractedDocument.images` for images found on this page. Each value is a zero-based index into the top-level `images` collection. Only populated when `extract_images = true` in the extraction config. |
| `image_preprocessing` | `ImagePreprocessingMetadata?` | `/* serde(default) */` | OCR image preprocessing applied to this page's raster. |
...
| `layout_regions` | `\[\]const LayoutRegion?` | `null` | Layout detection regions for this page (when layout detection is enabled). Contains detected layout regions with class, confidence, bounding box, and area fraction. Only populated when layout detection is configured. |
| `speaker_notes` | `\[\]const u8?` | `null` | Speaker notes for this slide (PPTX only). Contains the text from the slide's notes pane (`ppt/notesSlides/notesSlide{N}.xml`). Only populated when the source is a PPTX file and notes are present. |
| `section_name` | `\[\]const u8?` | `null` | Section name this slide belongs to (PPTX only). PowerPoint sections group slides into logical chapters (`<p:sectionLst>` in `ppt/presentation.xml`). Only populated when the source is a PPTX file and the slide belongs to a named section. |
| `sheet_name` | `\[\]const u8?` | `null` | Sheet name for this page (XLSX/ODS only). Each spreadsheet sheet maps to one `PageContent` entry. This field carries the sheet's display name as it appears in the workbook. `null` for all non-spreadsheet formats and for sheets with an empty name. |
| `ocr_confidence` | `PageOcrConfidence?` | `/* serde(default) */` | Aggregate OCR confidence for this page. `null` when the page was not OCR'd. |
...
| `title` | `\[\]const u8?` | `null` | Page title (usually for presentations) |
| `dimensions` | `PageDimensions?` | `/* serde(default) */` | Dimensions in points (PDF) or pixels (images). |
| `image_count` | `u32?` | `null` | Number of images on this page |
| `table_count` | `u32?` | `null` | Number of tables on this page |
...
| `backend` | `\[\]const u8` | — | Name of the OCR backend that produced the page text. |
...
| `text_excerpt` | `\[\]const u8` | — | First ~500 characters of extracted text. |
...
| `text` | `\[\]const u8` | — | Matched substring (owned copy — pattern engine returns owned data so the caller can free the original text if needed before replacement). |
...
| `content` | `\[\]const u8?` | `null` | Text content of the annotation (e.g., comment text, link URL). |
| `page_number` | `u32` | — | Page number where the annotation appears (1-indexed). |
| `bounding_box` | `BoundingBox?` | `null` | Bounding box of the annotation on the page. |
| `author` | `\[\]const u8?` | `/* serde(default) */` | Author/creator of the annotation (PDF `/T` entry). |
| `modified` | `\[\]const u8?` | `/* serde(default) */` | Last modification date of the annotation (PDF `/M` entry), as a raw PDF date string (e.g. `"D:20240115120000Z"`). |
| `color` | `\[\]const u8?` | `/* serde(default) */` | Annotation colour (PDF `/C` entry), normalised to a CSS-compatible `#rrggbb` hex string. Gray and CMYK colour spaces are converted to RGB. |
| `subject` | `\[\]const u8?` | `/* serde(default) */` | Subject of the annotation (PDF `/Subj` entry). |
| `quad_points` | `\[\]const BoundingBox?` | `/* serde(default) */` | Per-line bounding boxes derived from the annotation's `/QuadPoints` entry. Present for text markup annotations (Highlight, Underline, StrikeOut, Squiggly), one box per marked line/run of text. |
| `marked_text` | `\[\]const u8?` | `/* serde(default) */` | The document text covered by `Self.quad_points`, recovered from the page content underneath the marked-up region. Populated for Highlight, Underline, StrikeOut, and Squiggly annotations when the underlying text could be recovered. |
...
| `passwords` | `\[\]const \[\]const u8?` | `null` | List of passwords to try when opening encrypted PDFs |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `\[\]const u8` | — | Partial field name (the leaf name within the field hierarchy). |
| `full_name` | `\[\]const u8` | — | Fully-qualified field name (dotted path from the form root). |
| `field_type` | `FormFieldType` | — | Classified field type. |
| `value` | `\[\]const u8?` | `/* serde(default) */` | Current field value, if any. |
| `default_value` | `\[\]const u8?` | `/* serde(default) */` | Default field value, if any. |
| `flags` | `u32` | `/* serde(default) */` | Raw field-flags bitmask (read-only, required, multiline, …). |
| `page` | `u32?` | `/* serde(default) */` | 1-indexed page the field's widget appears on. Currently always `null` for AcroForm fields; page assignment is a deferred enhancement requiring spatial analysis of widget annotations per page. |
| `bbox` | `BoundingBox?` | `/* serde(default) */` | Widget bounding box on its page, if known. |
| `max_length` | `u32?` | `/* serde(default) */` | Maximum input length for text fields, if specified. |
| `tooltip` | `\[\]const u8?` | `/* serde(default) */` | Tooltip / alternate field description, if present. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `pdf_version` | `\[\]const u8?` | `null` | PDF version (e.g., "1.7", "2.0") |
| `producer` | `\[\]const u8?` | `null` | PDF producer (application that created the PDF) |
...
| `page_count` | `u32?` | `null` | Total number of pages in the PDF document |
| `scanned_confidence` | `f32?` | `null` | How strongly the document's most scan-like page resembles a scan, in `[0.0, 1.0]`. `null` when the document could not be inspected. A full-page raster with no visible text scores at least `0.85`; a born-digital slide with a full-bleed background image scores `0.50`. |
| `scanned_pages` | `\[\]const u32?` | `[]` | Pages that look like scans (1-indexed), using the default confidence threshold. `null` when the document could not be inspected; empty when no page qualifies. |
| `layout_gated_pages` | `\[\]const u32?` | `[]` | Pages the `auto` layout strategy skipped (1-indexed). `null` unless layout detection ran with `LayoutStrategy.Auto`; empty when the gate selected every page. |
| `layout_gate_reasons` | `\[\]const \[\]const u8?` | `[]` | Why the `auto` layout gate selected or skipped each page. Index `i` is page `i + 1`. Snake_case values such as `multi_column`, `table_grid`, or `plain_text` (the skip reason). `null` unless layout detection ran with `LayoutStrategy.Auto`. |
...
```zig
pub fn name(self: *const Plugin) []u8
```
...
```zig
pub fn version(self: *const Plugin) []u8
```
...
```zig
pub fn initialize(self: *const Plugin) XbergError!void
```
...
```zig
pub fn shutdown(self: *const Plugin) XbergError!void
```
...
```zig
pub fn description(self: *const Plugin) []u8
```
...
```zig
pub fn author(self: *const Plugin) []u8
```
...
```zig
pub fn process(self: *const PostProcessor, result: []const u8, config: []const u8) XbergError!void
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `result` | `\[\]const u8` | Yes | Mutable reference to the extraction result to process |
| `config` | `\[\]const u8` | Yes | Extraction configuration |
...
```zig
pub fn processing_stage(self: *const PostProcessor) ProcessingStage
```

**Example:**

```zig
const result = instance.processing_stage();
```
...
```zig
pub fn should_process(self: *const PostProcessor, result: []const u8, config: []const u8) bool
```

**Example:**

```zig
const result = instance.should_process(.{}, .{});
```
...
|------|------|----------|-------------|
| `result` | `\[\]const u8` | Yes | The extracted document |
| `config` | `\[\]const u8` | Yes | The extraction config |
...
```zig
pub fn estimated_duration_ms(self: *const PostProcessor, result: []const u8) u64
```

**Example:**

```zig
const result = instance.estimated_duration_ms(.{});
```
...
| `enabled_processors` | `\[\]const \[\]const u8?` | `null` | Whitelist of processor names to run (None = all enabled) |
| `disabled_processors` | `\[\]const \[\]const u8?` | `null` | Blacklist of processor names to skip (None = none disabled) |
| `enabled_set` | `\[\]const \[\]const u8?` | `null` | Pre-computed AHashSet for O(1) enabled processor lookup |
| `disabled_set` | `\[\]const \[\]const u8?` | `null` | Pre-computed AHashSet for O(1) disabled processor lookup |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `application` | `\[\]const u8?` | `null` | Application name (e.g., "Microsoft Office PowerPoint") |
| `app_version` | `\[\]const u8?` | `null` | Application version |
| `total_time` | `i32?` | `null` | Total editing time in minutes |
| `company` | `\[\]const u8?` | `null` | Company name |
| `doc_security` | `i32?` | `null` | Document security level |
...
| `hidden_slides` | `i32?` | `null` | Number of hidden slides |
| `multimedia_clips` | `i32?` | `null` | Number of multimedia clips |
| `presentation_format` | `\[\]const u8?` | `null` | Presentation format (e.g., "Widescreen", "Standard") |
| `slide_titles` | `\[\]const \[\]const u8` | `[]` | Slide titles |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `content` | `\[\]const u8` | — | Extracted text content from all slides |
| `metadata` | `PptxMetadata` | — | Presentation metadata |
| `slide_count` | `u64` | — | Total number of slides |
| `image_count` | `u64` | — | Total number of embedded images |
| `table_count` | `u64` | — | Total number of tables |
| `images` | `\[\]const ExtractedImage` | — | Extracted images from the presentation |
| `page_structure` | `PageStructure?` | `null` | Slide structure with boundaries (when page tracking is enabled) |
| `page_contents` | `\[\]const PageContent?` | `null` | Per-slide content (when page tracking is enabled) |
| `document` | `DocumentStructure?` | `null` | Structured document representation |
| `hyperlinks` | `\[\]const PresentationHyperlink` | `/* serde(default) */` | Hyperlinks discovered in slides. |
| `office_metadata` | `std.StringHashMap(\[\]const u8)` | `/* serde(default) */` | Office metadata extracted from docProps/core.xml and docProps/app.xml. Contains keys like "title", "author", "created_by", "subject", "keywords", "modified_by", "created_at", "modified_at", etc. |
...
| `slide_names` | `\[\]const \[\]const u8` | `[]` | Names of slides (if available) |
| `image_count` | `u32?` | `null` | Number of embedded images |
| `table_count` | `u32?` | `null` | Number of tables |
...
|-------|------|---------|-------------|
| `url` | `\[\]const u8` | — | Link destination. |
| `label` | `\[\]const u8?` | `null` | Optional visible label. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `id` | `\[\]const u8` | — | Stable, URL-safe preset identifier (lowercase snake_case). |
| `version` | `\[\]const u8` | — | Monotonic version string (e.g. `v1`). |
| `schema_name` | `\[\]const u8` | — | Human-readable schema name forwarded to the LLM as the response/tool name. |
| `description` | `\[\]const u8` | — | One-line preset description shown in the registry UI. |
| `category` | `PresetCategory` | — | Top-level category for grouping in the playground. |
| `tags` | `\[\]const \[\]const u8` | `/* serde(default) */` | Free-form tags used for search/filtering. May be empty. |
| `schema` | `\[\]const u8` | — | JSON Schema (Draft 2020-12) describing the structured output shape. |
| `system_prompt` | `\[\]const u8` | — | Instruction primer sent to the model. |
| `context_template` | `\[\]const u8?` | `/* serde(default) */` | Optional mustache-style template merged with caller-supplied context. |
...
| `fingerprint` | `\[\]const u8` | `/* serde(default) */` | Stable sha256 fingerprint of the canonical preset file contents. Populated at registry load — not present in the on-disk JSON files. Used as a cache-invalidation token by the worker pipeline. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `input_path` | `\[\]const u8` | — | Path to the sample input file, relative to the preset directory. |
| `output_path` | `\[\]const u8` | — | Path to the reference structured output, relative to the preset directory. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `id` | `\[\]const u8` | — | Preset identifier matching `Preset.id`. |
| `version` | `\[\]const u8` | — | Preset version matching `Preset.version`. |
| `schema_name` | `\[\]const u8` | — | Schema name matching `Preset.schema_name`. |
| `description` | `\[\]const u8` | — | One-line preset description. |
| `category` | `PresetCategory` | — | Top-level category. |
| `tags` | `\[\]const \[\]const u8` | — | Free-form tags. |
| `preferred_call_mode` | `CallMode` | — | Default call mode. |
| `emit_citations` | `bool` | — | Whether the preset prompts the model for citations. |
| `fingerprint` | `\[\]const u8` | — | Stable fingerprint matching `Preset.fingerprint`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `source` | `\[\]const u8` | — | The pipeline stage or feature that produced this warning (e.g., "embedding", "chunking", "language_detection", "output_format"). |
| `message` | `\[\]const u8` | — | Human-readable description of what went wrong. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `\[\]const u8` | — | Property name, such as `"bold"`, `"italic"`, `"font_size"`, or `"font_color"`. |
| `from` | `\[\]const u8?` | `null` | Value before the change, when available. |
| `to` | `\[\]const u8?` | `null` | Value after the change, when available. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `message_count` | `u64` | — | Total number of email messages found in the PST archive. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `payload` | `\[\]const u8` | — | Decoded payload (text, URL, vCard string, …). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `min_word_length` | `u64` | `1` | Minimum word length to consider (default: 1). |
| `max_words_per_phrase` | `u64` | `3` | Maximum words in a keyword phrase (default: 3). |
...
| `cells` | `\[\]const \[\]const \[\]const u8` | — | Table cells as a 2D vector (rows × columns). |
| `markdown` | `\[\]const u8` | — | Rendered markdown table. |
...
| `custom_terms` | `\[\]const RedactionTerm` | `[]` | Arbitrary user-supplied literal terms to redact. Each term is treated as a regex hit against the document, surfacing as `PiiCategory.Custom(label)` in `RedactionFinding` where `label` is the per-term label (defaulting to the literal value itself). Case-insensitive by default; set `RedactionTerm.case_sensitive` for exact match. Use this when you need to redact tenant-specific tokens (employee IDs, project codes, internal product names) without writing a custom plugin. |
| `custom_patterns` | `\[\]const RedactionPattern` | `[]` | Arbitrary user-supplied regex patterns to redact. Same surfacing semantics as `custom_terms`: each hit becomes a `PiiCategory.Custom(label)` finding. Patterns are validated at config-construction time via `RedactionConfig.validate`. |
...
| `replacement_token` | `\[\]const u8` | — | String that replaced the original mention. Always present; for `Drop` the replacement is the empty string. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `label` | `\[\]const u8` | — | Custom category label surfaced in `RedactionFinding.category`. |
| `pattern` | `\[\]const u8` | — | Regex pattern (Rust `regex` crate dialect — no look-around). |
...
|-------|------|---------|-------------|
| `label` | `\[\]const u8` | — | Custom category label surfaced in `RedactionFinding.category`. |
| `value` | `\[\]const u8` | — | Literal value to match. Regex metacharacters are escaped automatically. |
...
```zig
pub fn load_embedded_registry() LoadError!Registry
```

**Example:**

```zig
const result = try load_embedded_registry();
```
...
```zig
pub fn get(self: *const Registry, id: []const u8) ?[]u8
```
...
```zig
pub fn summaries(self: *const Registry) []u8
```
...
```zig
pub fn len(self: *const Registry) u64
```
...
```zig
pub fn is_empty(self: *const Registry) bool
```
...
```zig
pub fn sample_bytes(self: *const Registry, preset_id: []const u8, name: []const u8) ?[]u8
```

**Example:**

```zig
const result = instance.sample_bytes("value", "value");
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `preset_id` | `\[\]const u8` | Yes | The preset id |
| `name` | `\[\]const u8` | Yes | The name |
...
```zig
pub fn extend_from_dir(self: *const Registry, dir: []const u8) LoadError!u64
```

**Example:**

```zig
const result = try instance.extend_from_dir("value");
```
...
```zig
pub fn render_result(self: *const Renderer, result: []const u8) XbergError![]u8
```

**Example:**

```zig
const result = try instance.render_result(.{});
```
...
| `document` | `\[\]const u8` | — | The document text. |
...
```zig
pub fn rerank(self: *const RerankerBackend, query: []const u8, documents: []const u8) XbergError![]u8
```

**Example:**

```zig
const result = try instance.rerank("value", &[_]u8{});
```
...
| `documents` | `\[\]const u8` | Yes | The documents |
...
| `top_k` | `u64?` | `null` | Return at most this many documents. `null` returns all. Applied after sorting by score, so the highest-scoring documents are kept. |
| `batch_size` | `u64` | `32` | Batch size for local ONNX cross-encoder inference. |
...
| `max_rerank_duration_secs` | `u64?` | `60` | Maximum wall-clock duration (in seconds) for a single `rerank()` call when using `RerankerModelType.Plugin`. Applies only to the in-process plugin path — protects against hung host-language backends. On timeout, the dispatcher returns `Plugin` instead of blocking forever. `null` disables the timeout. The default (60 seconds) is conservative for common in-process inference; increase for large document sets on slow hardware. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `id` | `\[\]const u8` | — | Source preset identifier. |
| `version` | `\[\]const u8` | — | Source preset version. |
| `fingerprint` | `\[\]const u8` | — | Fingerprint of the source preset file, used as a cache token. |
| `schema_name` | `\[\]const u8` | — | Schema name forwarded to the LLM. |
| `schema` | `\[\]const u8` | — | Effective JSON Schema (caller override or the preset's own). |
| `system_prompt` | `\[\]const u8` | — | System prompt with rendered context appended. |
...
| `table_changes` | `\[\]const CellChange` | `[]` | Cell-level table changes for this revision. |
| `property_changes` | `\[\]const PropertyChange` | `[]` | Formatting or metadata property changes for this revision. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `host` | `\[\]const u8` | `"127.0.0.1"` | Server host address (e.g., "127.0.0.1", "0.0.0.0") |
| `port` | `u16` | `8000` | Server port number |
| `cors_origins` | `\[\]const \[\]const u8` | `[]` | CORS allowed origins. Empty vector means allow all origins. If this is an empty vector, the server will accept requests from any origin. If populated with specific origins (e.g., `"<https://example.com>"`), only those origins will be allowed. |
| `max_request_body_bytes` | `u64` | `104857600` | Maximum size of request body in bytes (default: 100 MB) |
| `max_multipart_field_bytes` | `u64` | `104857600` | Maximum size of multipart fields in bytes (default: 100 MB) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `indices` | `\[\]const u32` | — | Vocabulary token ids with non-zero weight, ascending. |
| `values` | `\[\]const f32` | — | Weights parallel to `SparseEmbedding.indices`. |
...
| `batch_size` | `u64` | `16` | Batch size for local ONNX inference. SPLADE emits a `[seq, vocab]` logit tensor per document, so memory scales with batch size — keep this modest. |
| `max_length` | `u64` | `256` | Maximum token sequence length for the tokenizer. |
...
| `max_embed_duration_secs` | `u64?` | `60` | Maximum wall-clock duration (in seconds) for a single embed call when using `SparseEmbeddingModelType.Plugin`. `null` disables the timeout. |
...
| `model_file` | `\[\]const u8` | — | Path to the ONNX file within the repo. |
| `additional_files` | `\[\]const \[\]const u8` | — | Sibling files that must be downloaded alongside `model_file`. |
| `max_length` | `u64` | — | Maximum token sequence length. |
| `description` | `\[\]const u8` | — | Human-readable description. |
...
| `scheme_allowlist` | `\[\]const \[\]const u8` | `["http", "https"]` | Allowed URI schemes. Default: `["http", "https"]`. Only `http` and `https` are supported. An empty list denies every URL. |
...
| `raw_json` | `\[\]const u8` | — | Raw JSON string representation |
| `schema_type` | `\[\]const u8?` | `null` | Schema type if detectable (e.g., "Article", "Event", "Product") |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `content` | `\[\]const u8` | — | The extracted text content, formatted for readability. |
| `format` | `\[\]const u8` | — | The source format identifier (e.g. `"json"`, `"yaml"`, `"toml"`). |
| `metadata` | `std.StringHashMap(\[\]const u8)` | — | Key-value metadata extracted from recognized text fields. |
| `text_fields` | `\[\]const \[\]const u8` | — | JSON paths of fields that were classified as text-bearing. |
| `value` | `\[\]const u8?` | `null` | The parsed document as a canonical `serde_json.Value` tree, when the source format could be represented as one. `null` only for TOML inputs whose `toml.Value` fails to round-trip through `serde_json.Value` (xberg-io/xberg#155): the extractor falls back to a raw code block in that case. |
| `flattened` | `\[\]const \[\]const u8` | — | Flattened `path: value` renderings for every leaf field, in traversal order. Previously computed and discarded (xberg-io/xberg#166); now surfaced so callers get a full-text view even when the structured renderer only emits headings/lists for a subset of fields. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `schema` | `\[\]const u8` | — | JSON Schema defining the desired output structure. |
| `schema_name` | `\[\]const u8` | `"extraction"` | Schema name passed to the LLM's structured output mode. |
| `schema_description` | `\[\]const u8?` | `/* serde(default) */` | Optional schema description for the LLM. |
| `strict` | `bool` | `/* serde(default) */` | Enable strict mode — output must exactly match the schema. |
| `prompt` | `\[\]const u8?` | `/* serde(default) */` | Custom Jinja2 extraction prompt template. When `null`, a default template is used. Available template variables: - `{{ content }}` — The extracted document text. - `{{ schema }}` — The JSON schema as a formatted string. - `{{ schema_name }}` — The schema name. - `{{ schema_description }}` — The schema description (may be empty). |
...
| `max_tokens` | `u32?` | `null` | Maximum summary length in tokens. `null` lets the backend pick a default. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `extension` | `\[\]const u8` | — | File extension (without leading dot), e.g., "pdf", "docx" |
| `mime_type` | `\[\]const u8` | — | MIME type string, e.g., "application/pdf" |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `content` | `\[\]const u8` | — | Cell content as text |
...
| `heading_level` | `u8?` | `null` | Outline level 1-6 when the style resolves to a heading, otherwise `null`. |
| `style_name` | `\[\]const u8?` | `null` | Human-readable style name, e.g. `heading 2`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `from_index` | `u64` | — | Zero-based index of the table in both `a.tables` and `b.tables`. |
| `to_index` | `u64` | — | Zero-based index in `b.tables` (equal to `from_index` for same-dimension tables). |
| `cell_changes` | `\[\]const CellChange` | — | Cell-level changes within the table. |
...
| `tessedit_char_whitelist` | `\[\]const u8` | `""` | Whitelist of allowed characters (empty = all allowed) |
| `tessedit_char_blacklist` | `\[\]const u8` | `""` | Blacklist of forbidden characters (empty = none forbidden) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `content` | `\[\]const u8` | — | Extracted text content |
| `line_count` | `u64` | — | Number of lines |
| `word_count` | `u64` | — | Number of words |
| `character_count` | `u64` | — | Number of characters |
| `headers` | `\[\]const \[\]const u8?` | `null` | Markdown headers (text only, Markdown files only) |
| `links` | `\[\]const MarkdownLink?` | `null` | Markdown links (Markdown files only). |
| `code_blocks` | `\[\]const MarkdownCodeBlock?` | `null` | Code blocks (Markdown files only). |
...
| `headers` | `\[\]const \[\]const u8?` | `[]` | Markdown headers (headings text only, for Markdown files) |
| `links` | `\[\]const MarkdownLink?` | `[]` | Markdown links (for Markdown files). |
| `code_blocks` | `\[\]const MarkdownCodeBlock?` | `[]` | Code blocks (for Markdown files). |
...
```zig
const result = new_token_counter();
```
...
| `language_hint` | `\[\]const u8?` | `null` | ISO 639-1 language code hint for stopword selection (e.g. `"en"`, `"de"`). |
...
| `custom_stopwords` | `std.StringHashMap(\[\]const \[\]const u8)?` | `null` | Per-language custom stopword lists (`language_code → stopword_list`). |
| `preserve_patterns` | `\[\]const \[\]const u8` | `[]` | Regex patterns whose matched text is always preserved unchanged. |
| `target_reduction` | `f32?` | `null` | Target fraction of text to retain (0.0–1.0); `null` = no fixed target. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `mode` | `\[\]const u8` | `"off"` | Reduction mode: "off", "light", "moderate", "aggressive", "maximum" |
...
```zig
pub fn count_tokens(self: *const TokenizerBackend, text: []const u8) u64
```

**Example:**

```zig
const result = instance.count_tokens("value");
```
...
| `language` | `\[\]const u8?` | `null` | Optional language hint (ISO-639-1 code, e.g. "en", "de"). When `null` (default), the current engine falls back to English. For deterministic production output, always set this explicitly. |
...
| `max_duration_ms` | `u64?` | `1800000` | Hard safety limit on input duration (milliseconds). Files longer than this are rejected after decode, before model work. Default: 30 minutes. Set to `null` to disable (not recommended for untrusted input). |
| `max_bytes` | `u64?` | `536870912` | Hard safety limit on input size (bytes). Default: 512 MiB. Protects against pathological or malicious uploads. |
| `timeout_ms` | `u64?` | `600000` | Wall-clock timeout for the entire transcription operation (ms). Bounds audio decode, model resolution/download, and inference together. On expiry the extraction fails with an `XbergError.Transcription`. `null` disables the bound and lets the operation run unbounded (not recommended for untrusted input). Enforced on the async extraction path only; the size and duration caps (`max_bytes`, `max_duration_ms`) are checked on every path. Default: 10 minutes. |
| `model_cache_dir` | `\[\]const u8?` | `null` | Optional alternate Hugging Face cache root for Whisper models. When unset, hf-hub follows `HF_HUB_CACHE`, `HUGGINGFACE_HUB_CACHE`, `HF_HOME`, XDG, and platform defaults. Files remain in the standard content-addressed snapshot layout and are not copied into an Xberg cache. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `target_lang` | `\[\]const u8` | — | BCP-47 language tag the translation was produced into (e.g. `"de"`, `"fr-CA"`). |
| `source_lang` | `\[\]const u8?` | `null` | BCP-47 source language. `null` when the translation backend was asked to detect. |
| `content` | `\[\]const u8` | — | Translated plain-text body. Matches the shape of `ExtractedDocument.content`. |
| `formatted_content` | `\[\]const u8?` | `null` | Translated markup body (Markdown / HTML / etc.) when `preserve_markup` was enabled on the config. `null` otherwise. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `target_lang` | `\[\]const u8` | — | BCP-47 language tag for the target language (e.g. `"de"`, `"fr-CA"`). |
| `source_lang` | `\[\]const u8?` | `null` | Optional explicit source language. `null` asks the backend to auto-detect. |
...
| `chunk_max_size` | `u64?` | `null` | Maximum chunk size in bytes. `null` disables chunking. |
...
| `document_url_pattern` | `\[\]const u8?` | `null` | Optional regex filter for document-discovered URLs. |
| `max_document_urls_per_result` | `u32?` | `100` | Maximum URLs to follow per extraction result. |
| `max_total_urls` | `u32?` | `1000` | Maximum URLs followed across the whole extraction call. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `page_ranges` | `\[\]const PageRange?` | `[]` | User-specified page ranges (overrides automatic chunking). |
| `pages_per_chunk` | `u32?` | `null` | User-specified pages per chunk (overrides automatic calculation). |
...
```zig
pub fn validate(self: *const Validator, result: []const u8, config: []const u8) XbergError!void
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `result` | `\[\]const u8` | Yes | The extraction result to validate |
| `config` | `\[\]const u8` | Yes | Extraction configuration |
...
```zig
pub fn should_validate(self: *const Validator, result: []const u8, config: []const u8) bool
```

**Example:**

```zig
const result = instance.should_validate(.{}, .{});
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `application` | `\[\]const u8?` | `null` | Application name (e.g., "Microsoft Excel") |
| `app_version` | `\[\]const u8?` | `null` | Application version |
| `doc_security` | `i32?` | `null` | Document security level |
| `scale_crop` | `bool?` | `null` | Scale crop flag |
| `links_up_to_date` | `bool?` | `null` | Links up to date flag |
...
| `hyperlinks_changed` | `bool?` | `null` | Hyperlinks changed flag |
| `company` | `\[\]const u8?` | `null` | Company name |
| `worksheet_names` | `\[\]const \[\]const u8` | `[]` | Worksheet names |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `content` | `\[\]const u8` | — | Extracted text content (XML structure filtered out) |
| `element_count` | `u64` | — | Total number of XML elements processed |
| `unique_elements` | `\[\]const \[\]const u8` | — | List of unique element names found (sorted) |
...
| `unique_elements` | `\[\]const \[\]const u8` | `[]` | List of unique element tag names (sorted) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `window_size` | `u64` | `2` | Window size for co-occurrence analysis (default: 2). Controls the context window for computing co-occurrence statistics. |
...
| `custom` | Custom renderer registered via the RendererRegistry. The string is the renderer name (e.g., "docx", "latex"). — Fields: `0`: `[]const u8` |
...
| Value | Description |
|-------|-------------|
| `preset` | Use a preset ColBERT model (recommended). — Fields: `name`: `[]const u8` |
| `custom` | Use a custom ColBERT ONNX model from HuggingFace. — Fields: `model_id`: `[]const u8`, `model_file`: `[]const u8`, `additional_files`: `[]const []const u8`, `max_length`: `i64` |
| `plugin` | In-process late-interaction backend registered via the plugin system. — Fields: `name`: `[]const u8` |
...
| Value | Description |
|-------|-------------|
| `azure_ad` | Azure AD OAuth2 client-credentials flow (Azure OpenAI / Azure Cognitive Services). — Fields: `tenant_id`: `[]const u8`, `client_id`: `[]const u8`, `client_secret`: `[]const u8`, `scope`: `[]const u8` |
| `vertex_oauth2` | Google Vertex AI OAuth2 via a service-account JSON key file on disk. Points at a file path rather than embedding the key inline: the key file contains an RSA private key — stronger secret material than an API key — and `LlmConfig` must never carry that directly, matching the credential-handling policy the rest of this module follows. — Fields: `service_account_key_file`: `[]const u8`, `scope`: `[]const u8` |
| `vertex_adc` | Google Vertex AI Application Default Credentials, resolved from the GCE/GKE/Cloud Run metadata server. Carries no secret material at all. — Fields: `scope`: `[]const u8` |
| `bedrock_web_identity` | AWS STS `AssumeRoleWithWebIdentity` (EKS IRSA / OIDC federation) for Bedrock. — Fields: `role_arn`: `[]const u8`, `token_file`: `[]const u8`, `session_name`: `[]const u8`, `region`: `[]const u8` |
...
| `tokenizer` | Size measured in tokens from a HuggingFace tokenizer or a registered tokenizer backend. — Fields: `model`: `[]const u8`, `cache_dir`: `[]const u8` |
...
| Value | Description |
|-------|-------------|
| `preset` | Use a preset model configuration (recommended) — Fields: `name`: `[]const u8` |
| `custom` | Use a custom ONNX model from HuggingFace — Fields: `model_id`: `[]const u8`, `dimensions`: `u64` |
...
| Value | Description |
|-------|-------------|
| `preset` | Use a preset cross-encoder model (recommended). — Fields: `name`: `[]const u8` |
| `custom` | Use a custom ONNX cross-encoder from HuggingFace. — Fields: `model_id`: `[]const u8`, `model_file`: `[]const u8`, `additional_files`: `[]const []const u8`, `max_length`: `i64`, `head`: `RerankerHead` |
...
| Value | Description |
|-------|-------------|
| `preset` | Use a preset SPLADE model (recommended). — Fields: `name`: `[]const u8` |
| `custom` | Use a custom SPLADE (`BertForMaskedLM`) ONNX model from HuggingFace. — Fields: `model_id`: `[]const u8`, `model_file`: `[]const u8`, `additional_files`: `[]const []const u8`, `max_length`: `i64` |
| `plugin` | In-process sparse-embedding backend registered via the plugin system. — Fields: `name`: `[]const u8` |
...
| Value | Description |
|-------|-------------|
| `title` | Document title. — Fields: `text`: `[]const u8` |
| `heading` | Section heading with level (1-6). — Fields: `level`: `u8`, `text`: `[]const u8` |
| `paragraph` | Body text paragraph. — Fields: `text`: `[]const u8` |
| `list` | List container — children are `ListItem` nodes. — Fields: `ordered`: `bool` |
| `list_item` | Individual list item. — Fields: `text`: `[]const u8` |
| `table` | Table with structured cell grid. — Fields: `grid`: `TableGrid` |
| `image` | Image reference. — Fields: `description`: `[]const u8`, `image_index`: `u32`, `src`: `[]const u8` |
| `code` | Code block. — Fields: `text`: `[]const u8`, `language`: `[]const u8` |
| `quote` | Block quote — container, children carry the quoted content. |
| `formula` | Mathematical formula / equation. — Fields: `text`: `[]const u8` |
| `footnote` | Footnote reference content. — Fields: `text`: `[]const u8` |
| `comment` | Reviewer/editor comment content (e.g. DOCX comments). Distinct from `NodeContent.Footnote` (xberg-io/xberg#300): comments and footnotes both reach the internal document via a marker/definition pair, but a consumer needs to tell a reviewer comment apart from an authored footnote. — Fields: `text`: `[]const u8` — **Since:** `v1.1` |
| `group` | Logical grouping container (section, key-value area). `heading_level` + `heading_text` capture the section heading directly rather than relying on a first-child positional convention. — Fields: `label`: `[]const u8`, `heading_level`: `u8`, `heading_text`: `[]const u8` |
| `page_break` | Page break marker. |
| `slide` | Presentation slide container — children are the slide's content nodes. — Fields: `number`: `u32`, `title`: `[]const u8` |
| `definition_list` | Definition list container — children are `DefinitionItem` nodes. |
| `definition_item` | Individual definition list entry with term and definition. — Fields: `term`: `[]const u8`, `definition`: `[]const u8` |
| `citation` | Citation or bibliographic reference. — Fields: `key`: `[]const u8`, `text`: `[]const u8` |
| `admonition` | Admonition / callout container (note, warning, tip, etc.). Children carry the admonition body content. — Fields: `kind`: `[]const u8`, `title`: `[]const u8` |
| `raw_block` | Raw block preserved verbatim from the source format. Used for content that cannot be mapped to a semantic node type (e.g. JSX in MDX, raw LaTeX in markdown, embedded HTML). — Fields: `format`: `[]const u8`, `content`: `[]const u8` |
| `metadata_block` | Structured metadata block (email headers, YAML frontmatter, etc.). — Fields: `entries`: `[]const KeyValueAttribute` |
...
| `subscript` | Subscript text. |
| `superscript` | Superscript text. |
| `link` | Hyperlink annotation. — Fields: `url`: `[]const u8`, `title`: `[]const u8` |
| `highlight` | Highlighted text (PDF highlights, HTML `<mark>`). |
| `color` | Text color (CSS-compatible value, e.g. "#ff0000", "red"). — Fields: `value`: `[]const u8` |
| `font_size` | Font size with units (e.g. "12pt", "1.2em", "16px"). — Fields: `value`: `[]const u8` |
| `custom` | Extensible annotation for format-specific styling. — Fields: `name`: `[]const u8`, `value`: `[]const u8` |
...
| `url` | A URL or URI. |
| `custom` | A caller-supplied custom category label. — Fields: `0`: `[]const u8` |
...
| `quadrilateral` | 4-point quadrilateral for rotated/skewed text (PaddleOCR). Points are in clockwise order starting from top-left: `[top_left, top_right, bottom_right, bottom_left]` — Fields: `points`: `[]const OcrPoint` |
...
| Value | Description |
|-------|-------------|
| `context` | Unchanged context line. — Fields: `0`: `[]const u8` |
| `added` | Line added in the "after" version. — Fields: `0`: `[]const u8` |
| `removed` | Line removed from the "before" version. — Fields: `0`: `[]const u8` |
...
| Value | Description |
|-------|-------------|
| `paragraph` | Body paragraph, identified by its zero-based index in the document flow. — Fields: `index`: `u64` |
| `table_cell` | Cell inside a table. — Fields: `row`: `u64`, `col`: `u64`, `table_index`: `u64` |
...
| `sheet` | Spreadsheet cell or range, identified by sheet index and optional name. — Fields: `index`: `u64`, `name`: `[]const u8` |
...
| Value | Description |
|-------|-------------|
| `small_file` | File is below size threshold. — Fields: `size_bytes`: `u64`, `threshold_bytes`: `u64` |
| `few_pages` | Document has fewer pages than threshold. — Fields: `page_count`: `u32`, `threshold`: `u32` |
| `text_layer_detected` | PDF has substantial text layer (OCR not needed). — Fields: `text_coverage`: `f32`, `avg_chars_per_page`: `u32` |
| `format_not_chunkable` | Document format does not support chunking. — Fields: `mime_type`: `[]const u8` |
...
| Value | Description |
|-------|-------------|
| `large_file` | File exceeds size threshold. — Fields: `size_bytes`: `u64`, `threshold_bytes`: `u64` |
| `many_pages` | Document has many pages. — Fields: `page_count`: `u32`, `threshold`: `u32` |
| `ocr_required` | PDF requires OCR and is large. — Fields: `page_count`: `u32`, `force_ocr`: `bool` |
| `large_and_many_pages` | Both size and page count exceed thresholds. — Fields: `size_bytes`: `u64`, `page_count`: `u32` |
...
|-------|----------|------------|-------|
| `OcrBackend` | register_ocr_backend | unregister_ocr_backend | clear_ocr_backends |
| `PostProcessor` | register_post_processor | unregister_post_processor | clear_post_processors |
| `Validator` | register_validator | unregister_validator | clear_validators |
| `DocumentExtractor` | register_document_extractor | unregister_document_extractor | clear_document_extractors |
| `EmbeddingBackend` | register_embedding_backend | unregister_embedding_backend | clear_embedding_backends |
| `Renderer` | register_renderer | unregister_renderer | clear_renderers |
| `RerankerBackend` | register_reranker_backend | unregister_reranker_backend | clear_reranker_backends |
| `TokenizerBackend` | register_tokenizer_backend | unregister_tokenizer_backend | clear_tokenizer_backends |
