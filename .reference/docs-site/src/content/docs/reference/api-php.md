...
<!-- alef:hash:eb00510c44f45e333beba54c4d07c8b70249d2f9855ff950b513dfd82446f534 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## PHP API Reference <span class="version-badge">v1.1.3</span>
...
```php
$result = extract(new ExtractInput(), new ExtractionConfig());
```
...
```php
public static function extractBatch(array<ExtractInput> $inputs, ExtractionConfig $config): ExtractionResult
```

**Example:**

```php
$result = extractBatch([], new ExtractionConfig());
```
...
```php
$result = mapUrl("value", new UrlExtractionConfig());
```
...
```php
public static function registerDocumentExtractor(DocumentExtractor $extractor): void
```
...
```php
public static function listTokenizerBackends(): array<string>
```

**Example:**

```php
$result = listTokenizerBackends();
```
...
```php
public static function classifyChunksOwned(ExtractedDocument $result, ChunkClassificationConfig $config): ExtractedDocument
```

**Example:**

```php
$result = classifyChunksOwned(new ExtractedDocument(), new ChunkClassificationConfig());
```
...
```php
public static function findUnmarkedClaims(string $markdown): array<string>
```
...
```php
public static function verifyExcerpt(string $excerpt, string $sourceText): bool
```
...
```php
$result = maxSimScore(new MultiVectorEmbedding(), new MultiVectorEmbedding());
```
...
```php
public static function maxSimRank(MultiVectorEmbedding $query, array<MultiVectorEmbedding> $docs): array<LateInteractionMatch>
```

**Example:**

```php
$result = maxSimRank(new MultiVectorEmbedding(), []);
```
...
```php
$result = doctor(new ExtractionConfig());
```
...
| `entryTypes` | `?array<string, int>` | `{}` | Count of entries grouped by BibTeX entry type (e.g. `"article"` → 5). |
...
| `totalSizeMb` | `float` | — | Combined size of all cache files in megabytes. |
| `availableSpaceMb` | `float` | — | Free disk space available on the cache volume, in megabytes. |
| `oldestFileAgeDays` | `float` | — | Age of the oldest cache file in days (0.0 if the cache is empty). |
| `newestFileAgeDays` | `float` | — | Age of the most recently written cache file in days (0.0 if the cache is empty). |
...
| `batchSize` | `int` | `10` | Number of chunks batched into a single LLM request. Larger batches amortize the fixed prompt cost (definitions block) across more chunks, at the risk of exceeding the model's context window for very large taxonomies or chunk texts. Defaults to `DEFAULT_BATCH_SIZE`. |
| `maxConcurrency` | `int` | `4` | Maximum number of in-flight batch requests. Bounds concurrency against the configured LLM provider. Defaults to `DEFAULT_MAX_CONCURRENCY`. |
...
| `nodeTypes` | `array<string>` | — | Tree-sitter node kinds that appear at the top level of this chunk (e.g. `"function_definition"`, `"class_definition"`). |
...
| `maxDepth` | `?int` | `null` | Maximum DOM traversal depth. `null` uses the library's internal native-stack safety limit. Explicit values above that safety limit are clamped to prevent process-aborting stack overflows on pathologically deep DOM trees. |
...
| `maxLinksPerPage` | `?int` | `null` | Maximum links enqueued from a single page. Defaults to 10000. Bounds the work one hostile or pathological page can create; links past the cap are dropped and a warning is logged. |
...
| `bm25Threshold` | `?float` | `null` | Minimum BM25 score a page must reach to be kept. Defaults to `0.0`. |
...
| `columnTypes` | `?array<string>` | `[]` | Inferred data type for each column (e.g. `"string"`, `"integer"`, `"float"`). |
...
```php
$result = $instance->extract(new ExtractInput(), new ExtractionConfig());
```
...
| `children` | `array<int>` | `/* serde(default) */` | Child node indices in reading order. |
...
| `attributes` | `?array<string, string>` | `null` | Format-specific key-value attributes. Extensible bag for miscellaneous data without a dedicated typed field: CSS classes, LaTeX environment names, Excel cell formulas, slide layout names, etc. |
...
| `sheetNames` | `?array<string>` | `[]` | Names of all sheets in the workbook. |
...
| `bytes` | `?string` | `null` | Raw bytes for `kind = "bytes"`. |
...
| `detectedLanguages` | `?array<string>` | `[]` | ISO 639-1 language codes detected in the document content. |
| `detectedLanguageConfidences` | `?array<LanguageConfidence>` | `[]` | Structured per-language detection results: confidence, document share, script, and reliability, alongside the ISO-code-only `detected_languages` (#261). One entry per language in `detected_languages`, in the same order. `null` under the same conditions as `detected_languages`: detection disabled, empty input text, or no language met the configured `min_confidence`. |
...
| `codeIntelligence` | `?mixed` | `null` | Code intelligence results from tree-sitter analysis. Populated when extracting source code files with the `tree-sitter` feature. Contains metrics, structural analysis, imports/exports, comments, docstrings, symbols, diagnostics, and optionally chunked code segments. Stored as an opaque JSON value so that all language bindings (Go, Java, C#, …) can deserialize it as a raw JSON object rather than a typed struct. The underlying type is `tree_sitter_language_pack::ProcessResult`. |
| `llmUsage` | `?array<LlmUsage>` | `[]` | LLM token usage and cost data for all LLM calls made during this extraction. Contains one entry per LLM call. Multiple entries are produced when VLM OCR, structured extraction, or LLM embeddings run during the same extraction. `null` when no LLM was used. |
| `entities` | `?array<Entity>` | `[]` | Named entities detected in `content` by the NER post-processor. `null` when no NER backend is configured. Populated by the `xberg-gliner` ONNX backend or the LLM-driven backend (see `crates/xberg/src/text/ner/`). |
...
| `pageClassifications` | `?array<PageClassification>` | `[]` | Per-page classifications produced by the page-classification post-processor. `null` when classification is not configured. |
...
| `qrCodes` | `?array<QrCode>` | `[]` | QR codes decoded from this image, when QR detection is enabled. Populated by the QR post-processor (`crates/xberg/src/extractors/qr.rs`) via the pure-Rust `rqrr` decoder. `null` when QR detection is disabled; an empty `Some([])` when detection ran but found nothing. |
...
| `maxConcurrentExtractions` | `?int` | `null` | Maximum concurrent document extractions in batch operations. This is a ceiling within the configured total thread budget, not an independent pool size. When unset, the scheduler derives document and per-document concurrency from `ConcurrencyConfig::max_threads`. |
...
| `maxArchiveDepth` | `int` | `3` | Maximum recursion depth for archive extraction (default: 3). Set to 0 to disable recursive extraction (legacy behavior). |
...
| `kClusters` | `int` | `3` | Number of font size clusters to use for hierarchy levels (1-7) Default: 3, which provides two heading levels plus body text. Larger values create more fine-grained hierarchy levels. |
...
| `historyDates` | `array<string, string>` | `{}` | Publication history dates keyed by event type (e.g. `"received"`, `"accepted"`). |
...
| `batchSize` | `int` | `16` | Batch size for local ONNX inference. ColBERT emits a `[seq, dim]` multi-vector embedding per document, so memory scales with batch size — keep this modest. |
| `maxLength` | `int` | `512` | Maximum token sequence length for the tokenizer (documents). |
| `queryMaxLength` | `int` | `32` | Fixed padded length for query augmentation. ColBERT queries are padded (with the mask token, kept attention-live) to exactly this many tokens rather than truncated/left as-is — this is the "query augmentation" trick from the ColBERT paper. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `globalLimit` | `?float` | `null` | Global spend limit in USD. |
| `modelLimits` | `?array<string, float>` | `{}` | Per-model spend limits in USD, keyed by model name. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `maxEntries` | `?int` | `null` | Maximum number of cached entries. |
...
| `backendConfig` | `?array<string, string>` | `{}` | Backend-specific configuration key/value pairs. |
...
| `presencePenalty` | `?float` | `null` | Presence penalty for generation tasks, applied to individual requests built from this config. Positive values discourage the model from repeating topics already present in the conversation. Validated to `[-2.0, 2.0]` by `LlmConfig::validate`. Mirrors liter-llm's `ChatCompletionRequest::presence_penalty`. |
| `frequencyPenalty` | `?float` | `null` | Frequency penalty for generation tasks, applied to individual requests built from this config. Positive values discourage the model from repeating the same tokens verbatim. Validated to `[-2.0, 2.0]` by `LlmConfig::validate`. Mirrors liter-llm's `ChatCompletionRequest::frequency_penalty`. |
...
| `headers` | `?array<string, string>` | `{}` | Extra HTTP headers sent with every request to the provider. Mirrors liter-llm's `ClientConfigBuilder::header`, for gateways or providers that require custom auth/routing headers. |
...
```php
public static function compile(string $metaSchemaJson): MetaSchema
```
...
```php
$result = $instance->parsePreset("value", "data");
```
...
```php
public function processImage(string $imageBytes, OcrConfig $config): ExtractedDocument
```
...
```php
public function processImageFile(string $path, OcrConfig $config): ExtractedDocument
```
...
```php
public function emitsStructuredMarkdown(): bool
```
...
```php
public function confidenceSemantics(): ConfidenceSemantics
```
...
```php
public function pageOrientationHandling(): PageOrientationHandling
```
...
```php
public function processDocument(string $path, OcrConfig $config): ExtractedDocument
```
...
| `ocrElements` | `?array<OcrElement>` | `[]` | Structured OCR elements with bounding boxes and confidence scores. Available when TSV output is requested or table detection is enabled. |
...
| `minProvenanceFallbackRatio` | `float` | `0.5` | Minimum fraction of a page's non-whitespace characters with `MappingProvenance::Fallback` provenance before the page is treated as having a fabricated text layer and routed to OCR (issue #1254). Gated by `min_total_non_whitespace` so a short page with a stray fallback character cannot trip it. Only used when `enable_provenance_ocr_routing` is `true`. |
...
| `imageIndices` | `array<int>` | `/* serde(default) */` | Indices into `ExtractedDocument.images` for images found on this page. Each value is a zero-based index into the top-level `images` collection. Only populated when `extract_images = true` in the extraction config. |
...
| `quadPoints` | `?array<BoundingBox>` | `/* serde(default) */` | Per-line bounding boxes derived from the annotation's `/QuadPoints` entry. Present for text markup annotations (Highlight, Underline, StrikeOut, Squiggly), one box per marked line/run of text. |
...
| `scannedPages` | `?array<int>` | `[]` | Pages that look like scans (1-indexed), using the default confidence threshold. `null` when the document could not be inspected; empty when no page qualifies. |
| `layoutGatedPages` | `?array<int>` | `[]` | Pages the `auto` layout strategy skipped (1-indexed). `null` unless layout detection ran with `LayoutStrategy::Auto`; empty when the gate selected every page. |
| `layoutGateReasons` | `?array<string>` | `[]` | Why the `auto` layout gate selected or skipped each page. Index `i` is page `i + 1`. Snake_case values such as `multi_column`, `table_grid`, or `plain_text` (the skip reason). `null` unless layout detection ran with `LayoutStrategy::Auto`. |
...
```php
public function process(ExtractedDocument $result, ExtractionConfig $config): void
```
...
```php
public function shouldProcess(ExtractedDocument $result, ExtractionConfig $config): bool
```

**Example:**

```php
$result = $instance->shouldProcess(new ExtractedDocument(), new ExtractionConfig());
```
...
```php
public function estimatedDurationMs(ExtractedDocument $result): int
```

**Example:**

```php
$result = $instance->estimatedDurationMs(new ExtractedDocument());
```
...
| `officeMetadata` | `array<string, string>` | `/* serde(default) */` | Office metadata extracted from docProps/core.xml and docProps/app.xml. Contains keys like "title", "author", "created_by", "subject", "keywords", "modified_by", "created_at", "modified_at", etc. |
| `revisions` | `?array<DocumentRevision>` | `/* serde(default) */` | Slide comments as revisions. Each `<p:cm>` element in `ppt/comments/comment{N}.xml` becomes a `DocumentRevision { kind: Comment }` with author (resolved from `ppt/commentAuthors.xml`), ISO-8601 timestamp, and `RevisionAnchor::Slide { index }`. `null` when no comment XML parts exist. |
...
| `schema` | `mixed` | — | JSON Schema (Draft 2020-12) describing the structured output shape. |
...
```php
public function summaries(): array<PresetSummary>
```
...
```php
public function sampleBytes(string $presetId, string $name): ?string
```
...
```php
$result = $instance->extendFromDir("value");
```
...
```php
public function renderResult(ExtractedDocument $result): string
```

**Example:**

```php
$result = $instance->renderResult(new ExtractedDocument());
```
...
| `topK` | `?int` | `null` | Return at most this many documents. `null` returns all. Applied after sorting by score, so the highest-scoring documents are kept. |
...
| `schema` | `mixed` | — | Effective JSON Schema (caller override or the preset's own). |
...
| `maxNestingDepth` | `int` | `1024` | Maximum nesting depth for structures (1024) |
...
| `maxRequestBodyBytes` | `int` | `104857600` | Maximum size of request body in bytes (default: 100 MB) |
| `maxMultipartFieldBytes` | `int` | `104857600` | Maximum size of multipart fields in bytes (default: 100 MB) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `indices` | `array<int>` | — | Vocabulary token ids with non-zero weight, ascending. |
...
| `batchSize` | `int` | `16` | Batch size for local ONNX inference. SPLADE emits a `[seq, vocab]` logit tensor per document, so memory scales with batch size — keep this modest. |
| `maxLength` | `int` | `256` | Maximum token sequence length for the tokenizer. |
...
| `metadata` | `array<string, string>` | — | Key-value metadata extracted from recognized text fields. |
| `textFields` | `array<string>` | — | JSON paths of fields that were classified as text-bearing. |
| `value` | `?mixed` | `null` | The parsed document as a canonical `serde_json::Value` tree, when the source format could be represented as one. `null` only for TOML inputs whose `toml::Value` fails to round-trip through `serde_json::Value` (xberg-io/xberg#155): the extractor falls back to a raw code block in that case. |
...
| `columns` | `?array<string>` | `[]` | Header cells for this fragment, i.e. the first row of `cells`. Populated even when this fragment's own header row was merged away or physically lives in a sibling fragment (see `table_id`), so a single fragment is interpretable in isolation. `null` when no header row could be determined. |
...
| `links` | `?array<MarkdownLink>` | `null` | Markdown links (Markdown files only). |
| `codeBlocks` | `?array<MarkdownCodeBlock>` | `null` | Code blocks (Markdown files only). |
...
| `codeBlocks` | `?array<MarkdownCodeBlock>` | `[]` | Code blocks (for Markdown files). |
...
| `customStopwords` | `?array<string, array<string>>` | `null` | Per-language custom stopword lists (`language_code → stopword_list`). |
...
| `languages` | `?array<string>` | `null` | Languages to pre-download on init (e.g., `["python", "rust"]`). Consumed only by the CLI's `tree-sitter download --from-config` and `cache warm` commands as a pre-download hint. Extraction itself does not read this field: a given source file always processes with a single, already auto-detected language, so there is nothing for a language allowlist to gate at extraction time. |
| `groups` | `?array<string>` | `null` | Language groups to pre-download (e.g., `["web", "systems", "scripting"]`). Consumed only by the CLI's `tree-sitter download --from-config` and `cache warm` commands, for the same reason as `languages` above. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `pageRanges` | `?array<PageRange>` | `[]` | User-specified page ranges (overrides automatic chunking). |
...
```php
public function validate(ExtractedDocument $result, ExtractionConfig $config): void
```
...
```php
public function shouldValidate(ExtractedDocument $result, ExtractionConfig $config): bool
```

**Example:**

```php
$result = $instance->shouldValidate(new ExtractedDocument(), new ExtractionConfig());
```
...
| `years` | `array<int>` | `/* serde(default) */` | All individual years present in the collection. |
...
| `Custom` | Use a custom ColBERT ONNX model from HuggingFace. — Fields: `modelId`: `string`, `modelFile`: `string`, `additionalFiles`: `array<string>`, `maxLength`: `int` |
...
| `Custom` | Use a custom ONNX cross-encoder from HuggingFace. — Fields: `modelId`: `string`, `modelFile`: `string`, `additionalFiles`: `array<string>`, `maxLength`: `int`, `head`: `RerankerHead` |
...
| `Custom` | Use a custom SPLADE (`BertForMaskedLM`) ONNX model from HuggingFace. — Fields: `modelId`: `string`, `modelFile`: `string`, `additionalFiles`: `array<string>`, `maxLength`: `int` |
...
| Value | Description |
|-------|-------------|
| `Legibility` | Validated to track legibility on a known scale — usable as an absolute quality gate. — Fields: `scaleMax`: `float` |
...
|-------|----------|------------|-------|
| `OcrBackend` | Xberg::registerOcrBackend | Xberg::unregisterOcrBackend | Xberg::clearOcrBackends |
| `PostProcessor` | Xberg::registerPostProcessor | Xberg::unregisterPostProcessor | Xberg::clearPostProcessors |
| `Validator` | Xberg::registerValidator | Xberg::unregisterValidator | Xberg::clearValidators |
| `DocumentExtractor` | Xberg::registerDocumentExtractor | Xberg::unregisterDocumentExtractor | Xberg::clearDocumentExtractors |
| `EmbeddingBackend` | Xberg::registerEmbeddingBackend | Xberg::unregisterEmbeddingBackend | Xberg::clearEmbeddingBackends |
| `Renderer` | Xberg::registerRenderer | Xberg::unregisterRenderer | Xberg::clearRenderers |
| `RerankerBackend` | Xberg::registerRerankerBackend | Xberg::unregisterRerankerBackend | Xberg::clearRerankerBackends |
| `TokenizerBackend` | Xberg::registerTokenizerBackend | Xberg::unregisterTokenizerBackend | Xberg::clearTokenizerBackends |
