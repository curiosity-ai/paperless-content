...
<!-- alef:hash:0a67c7968dfc71e41e82c54429d270a198446e9caf987281dc379173629afae4 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Dart API Reference <span class="version-badge">v1.1.3</span>
...
```dart
Future<ExtractionResult> extract(ExtractInput input, ExtractionConfig config)
```

**Example:**

```dart
final result = await extract(ExtractInput(), ExtractionConfig());
```
...
```dart
Future<ExtractionResult> extractBatch(List<ExtractInput> inputs, ExtractionConfig config)
```

**Example:**

```dart
final result = await extractBatch([], ExtractionConfig());
```
...
```dart
Future<MapResult> mapUrl(String uri, UrlExtractionConfig config)
```

**Example:**

```dart
final result = await mapUrl("value", UrlExtractionConfig());
```
...
```dart
final result = listEmbeddingBackends();
```
...
```dart
Future<void> registerDocumentExtractor(DocumentExtractor extractor)
```

**Example:**

```dart
registerDocumentExtractor(DocumentExtractor());
```
...
```dart
Future<void> unregisterDocumentExtractor(String name)
```
...
```dart
Future<void> registerRenderer(Renderer renderer)
```
...
```dart
Future<void> unregisterRenderer(String name)
```
...
```dart
final result = listRerankerBackends();
```
...
```dart
final result = listTokenizerBackends();
```
...
```dart
Future<ExtractedDocument> classifyChunksOwned(ExtractedDocument result, ChunkClassificationConfig config)
```

**Example:**

```dart
final result = await classifyChunksOwned(ExtractedDocument(), ChunkClassificationConfig());
```
...
```dart
Future<List<String>> findUnmarkedClaims(String markdown)
```

**Example:**

```dart
final result = findUnmarkedClaims("value");
```
...
```dart
Future<bool> verifyExcerpt(String excerpt, String sourceText)
```

**Example:**

```dart
final result = verifyExcerpt("value", "value");
```
...
```dart
Future<double> maxSimScore(MultiVectorEmbedding query, MultiVectorEmbedding doc)
```

**Example:**

```dart
final result = maxSimScore(MultiVectorEmbedding(), MultiVectorEmbedding());
```
...
```dart
Future<List<LateInteractionMatch>> maxSimRank(MultiVectorEmbedding query, List<MultiVectorEmbedding> docs)
```

**Example:**

```dart
final result = maxSimRank(MultiVectorEmbedding(), []);
```
...
```dart
Future<DoctorReport> doctor(ExtractionConfig config)
```

**Example:**

```dart
final result = doctor(ExtractionConfig());
```
...
| `classes` | `List<String>` | `[]` | CSS classes (.class1 .class2) |
| `keyValues` | `List<KeyValueAttribute>` | `[]` | Key-value pairs (key="value") |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `durationMs` | `int?` | `null` | Duration in milliseconds derived from the decoded audio stream. |
...
| `channels` | `int?` | `null` | Number of audio channels (1 = mono, 2 = stereo). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `x1` | `double` | — | Left edge (x-coordinate of the top-left corner). |
| `y1` | `double` | — | Top edge (y-coordinate of the top-left corner). |
| `x2` | `double` | — | Right edge (x-coordinate of the bottom-right corner). |
| `y2` | `double` | — | Bottom edge (y-coordinate of the bottom-right corner). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `entryCount` | `PlatformInt64` | — | Number of entries in the bibliography. |
...
| `entryTypes` | `Map<String, PlatformInt64>?` | `{}` | Count of entries grouped by BibTeX entry type (e.g. `"article"` → 5). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `totalFiles` | `PlatformInt64` | — | Total number of files currently in the cache directory. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `row` | `PlatformInt64` | — | Zero-based row index. |
| `col` | `PlatformInt64` | — | Zero-based column index. |
...
| `embedding` | `List<double>?` | `null` | Optional embedding vector for this chunk. Only populated when `EmbeddingConfig` is provided in chunking configuration. The dimensionality depends on the chosen embedding model. |
...
| `batchSize` | `PlatformInt64` | `10` | Number of chunks batched into a single LLM request. Larger batches amortize the fixed prompt cost (definitions block) across more chunks, at the risk of exceeding the model's context window for very large taxonomies or chunk texts. Defaults to `DEFAULT_BATCH_SIZE`. |
| `maxConcurrency` | `PlatformInt64` | `4` | Maximum number of in-flight batch requests. Bounds concurrency against the configured LLM provider. Defaults to `DEFAULT_MAX_CONCURRENCY`. |
...
| `estimatedTimeMs` | `int` | — | Estimated processing time for this chunk in milliseconds. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `byteStart` | `PlatformInt64` | — | Byte offset where this chunk starts in the original text (UTF-8 valid boundary). |
| `byteEnd` | `PlatformInt64` | — | Byte offset where this chunk ends in the original text (UTF-8 valid boundary). |
| `tokenCount` | `PlatformInt64?` | `null` | Number of tokens in this chunk (if available). This is calculated by the embedding model's tokenizer if embeddings are enabled. |
| `chunkIndex` | `PlatformInt64` | — | Zero-based index of this chunk in the document. |
| `totalChunks` | `PlatformInt64` | — | Total number of chunks in the document. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `maxCharacters` | `PlatformInt64` | `1000` | Maximum size per chunk (in units determined by `sizing`). When `sizing` is `Characters` (default), this is the max character count. When using token-based sizing, this is the max token count. Default: 1000 |
| `overlap` | `PlatformInt64` | `200` | Overlap between chunks (in units determined by `sizing`). Default: 200 |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `citationCount` | `PlatformInt64` | — | Total number of citation records in the file. |
...
| `confidence` | `double?` | `null` | Backend-reported confidence in `[0.0, 1.0]`. `null` when the backend (e.g. an LLM prompt without explicit confidence schema) did not report one. |
...
| `byteStart` | `PlatformInt64` | — | Inclusive start byte offset of this chunk in the original source. |
| `byteEnd` | `PlatformInt64` | — | Exclusive end byte offset of this chunk in the original source. |
...
| `byteStart` | `PlatformInt64` | — | Inclusive start byte offset of the `name="value"` attribute token. |
| `byteEnd` | `PlatformInt64` | — | Exclusive end byte offset of the `name="value"` attribute token. |
...
| `byteStart` | `PlatformInt64` | — | Inclusive start byte offset of this node in the original source. |
| `byteEnd` | `PlatformInt64` | — | Exclusive end byte offset of this node in the original source. |
...
| `stripTags` | `List<String>` | `[]` | HTML tag names to strip (render children only, remove the tag wrapper). Default: `[]`. |
...
| `maxDepth` | `PlatformInt64?` | `null` | Max DOM traversal depth. Prevents stack overflow on deeply nested HTML. |
| `wrap` | `bool` | `false` | Enable line wrapping. Default: `false`. |
| `wrapWidth` | `PlatformInt64` | `80` | Wrap width when `wrap` is enabled. Default: `80`. |
...
| `listIndentWidth` | `PlatformInt64` | `2` | Number of spaces (or tabs) to use for each level of list indentation. |
...
| `wrapWidth` | `PlatformInt64` | `80` | Maximum output line width in characters when `wrap` is `true` (default `80`). Lines are broken at word boundaries so that no line exceeds this length. A value of `0` is treated as "no limit" — equivalent to leaving `wrap` disabled. Has no effect when `wrap` is `false`. |
...
| `maxDepth` | `PlatformInt64?` | `null` | Maximum DOM traversal depth. `null` uses the library's internal native-stack safety limit. Explicit values above that safety limit are clamped to prevent process-aborting stack overflows on pathologically deep DOM trees. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `maxDepth` | `PlatformInt64?` | `null` | Maximum crawl depth (number of link hops from the start URL). |
| `maxPages` | `PlatformInt64?` | `null` | Maximum number of pages to crawl. |
| `maxLinksPerPage` | `PlatformInt64?` | `null` | Maximum links enqueued from a single page. Defaults to 10000. Bounds the work one hostile or pathological page can create; links past the cap are dropped and a warning is logged. |
| `maxConcurrent` | `PlatformInt64?` | `null` | Maximum number of concurrent requests. |
...
| `mapLimit` | `PlatformInt64?` | `null` | Maximum number of URLs to return from a map operation. |
...
| `maxAssetSize` | `PlatformInt64?` | `null` | Maximum size in bytes for individual asset downloads. |
...
| `documentUrlDepth` | `int?` | `null` | Maximum document-depth (from the seed URL through document links only) when `follow_document_urls` is true. `null` means inherit `max_depth`. Independent of `max_depth`: a document URL is enqueued only if BOTH the outer `max_depth` and (if set) `document_url_depth` permit it. |
...
| `documentMaxSize` | `PlatformInt64?` | `52428800` | Maximum size in bytes for document downloads. Defaults to 50 MB. |
...
| `commentPrefixes` | `List<String>` | `[]` | Line prefixes that mark a comment line to skip entirely during row parsing (e.g. `["#"]`). A line is treated as a comment when its trimmed start matches any of these prefixes exactly. Default: empty, meaning no line is treated as a comment (matches the pre-existing extractor behavior). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `recordCount` | `PlatformInt64` | — | Total number of data records in the DBF file. |
| `fieldCount` | `PlatformInt64` | — | Number of field (column) definitions. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `modelPath` | `String?` | `null` | Local DeepSeek-OCR model directory. The backend requires this option. |
| `device` | `CandleDevicePreference?` | `null` | Optional per-call device override. |
| `version` | `int?` | `null` | DeepSeek-OCR model generation, either 1 or 2. Defaults to 2. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `fromLine` | `PlatformInt64` | — | Starting line number in the old content (0-indexed). |
| `fromCount` | `PlatformInt64` | — | Number of lines from the old content in this hunk. |
| `toLine` | `PlatformInt64` | — | Starting line number in the new content (0-indexed). |
| `toCount` | `PlatformInt64` | — | Number of lines from the new content in this hunk. |
...
| `maxContentChars` | `PlatformInt64?` | `null` | Truncate content to this many characters before diffing. Useful for very large documents where only the first N characters matter. `null` means no truncation. |
...
| `confidence` | `double` | — | Confidence in this boundary, `[0.0, 1.0]`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `pages` | `PlatformInt64` | — | Total pages in the source document (`0` when not page-addressable). |
| `tables` | `PlatformInt64` | — | Tables detected in the document. |
| `images` | `PlatformInt64` | — | Images detected in the document. |
...
```dart
final result = await instance.extract(ExtractInput(), ExtractionConfig());
```
...
```dart
final result = instance.supportedMimeTypes();
```
...
```dart
final result = instance.priority();
```
...
```dart
final result = instance.canHandle("value", "value");
```
...
| `elementIndex` | `PlatformInt64?` | `null` | Position index in the element sequence |
...
| `size` | `PlatformInt64?` | `null` | Size in bytes |
| `isImage` | `bool` | — | Whether this attachment is an image |
| `data` | `Uint8List?` | `null` | Attachment data (if extracted). Uses `bytes.Bytes` for cheap cloning of large buffers. |
...
| `data` | `Uint8List` | — | Raw file bytes from the embedded stream (already decompressed by lopdf). |
| `compressedSize` | `PlatformInt64` | — | Compressed byte count of the original stream (before decompression). Used by callers to compute the decompression ratio and detect zip-bomb-style attacks that embed a tiny compressed stream expanding to gigabytes of data. |
...
```dart
Future<PlatformInt64> dimensions()
```

**Example:**

```dart
final result = instance.dimensions();
```
...
```dart
final result = await instance.embed([]);
```
...
| `batchSize` | `PlatformInt64` | `32` | Batch size for embedding generation |
...
| `confidence` | `double?` | `null` | Backend-reported confidence in `[0.0, 1.0]`. `null` when the backend does not expose confidence scores. |
...
| `rowCount` | `PlatformInt64` | — | Number of rows |
| `colCount` | `PlatformInt64` | — | Number of columns |
| `cellCount` | `PlatformInt64` | — | Total number of non-empty cells |
...
| `bytes` | `Uint8List?` | `null` | Raw bytes for `kind = "bytes"`. |
...
| `detectedLanguageConfidences` | `List<LanguageConfidence>?` | `[]` | Structured per-language detection results: confidence, document share, script, and reliability, alongside the ISO-code-only `detected_languages` (#261). One entry per language in `detected_languages`, in the same order. `null` under the same conditions as `detected_languages`: detection disabled, empty input text, or no language met the configured `min_confidence`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `data` | `Uint8List` | — | Raw image data (PNG, JPEG, WebP, etc. bytes). Uses `bytes.Bytes` for cheap cloning of large buffers. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `textCoverage` | `double` | — | Fraction of pages with a usable text layer. |
| `ocrAggregate` | `double?` | `null` | Mean OCR per-element recognition confidence when OCR ran; `null` when it did not. |
| `schemaCompliance` | `SchemaCompliance` | — | Whether the merged output validates against the preset schema. |
| `combined` | `double` | — | Weighted blend in `[0, 1]`.  The value compared against the fallback threshold. |
...
| `extractionTimeoutSecs` | `int?` | `600` | Default per-file timeout in seconds for batch extraction. When set, each file in a batch will be canceled after this duration unless overridden by `FileExtractionConfig.timeout_secs`. Defaults to `Some(600)` (10 minutes) to prevent pathological files (e.g. deeply nested archives, documents with millions of cells) from running indefinitely and exhausting caller resources, while still giving slow paths (VLM-based OCR, large scanned documents) enough headroom to finish. Set to `null` to disable the timeout for trusted input or long-running workloads. |
| `maxConcurrentExtractions` | `PlatformInt64?` | `null` | Maximum concurrent document extractions in batch operations. This is a ceiling within the configured total thread budget, not an independent pool size. When unset, the scheduler derives document and per-document concurrency from `ConcurrencyConfig.max_threads`. |
...
| `maxArchiveDepth` | `PlatformInt64` | `3` | Maximum recursion depth for archive extraction (default: 3). Set to 0 to disable recursive extraction (legacy behavior). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `index` | `PlatformInt64` | — | Input index in the original request. |
...
| `crawlRedirectCount` | `PlatformInt64` | — | Total redirects followed while fetching or crawling URLs. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `inputs` | `PlatformInt64` | — | Number of inputs submitted by the caller. |
| `results` | `PlatformInt64` | — | Number of extraction results produced. |
| `errors` | `PlatformInt64` | — | Number of per-input errors. |
| `remoteUrls` | `PlatformInt64` | — | Number of URI inputs that resolved to remote HTTP(S) URLs. |
| `pagesCrawled` | `PlatformInt64` | — | Number of HTML pages crawled or scraped. |
| `documentsDownloaded` | `PlatformInt64` | — | Number of downloaded non-HTML documents extracted from URLs. |
...
| `offset` | `PlatformInt64` | — | Byte offset of the anchor in the markdown text. |
...
| `offset` | `PlatformInt64` | — | Byte offset of the definition line in the markdown text. |
...
| `level` | `PlatformInt64?` | `null` | Heading level (1-6) for headings, or nesting level for lists |
...
| `page` | `int?` | `/* serde(default) */` | 1-indexed page number the formula appears on. `null` when the source format has no page concept. The C FFI reports an absent page as `0`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `task` | `GlmOcrTaskKind?` | `null` | Optional recognition task; the backend constructor's task is used when absent. |
| `device` | `CandleDevicePreference?` | `null` | Optional per-call device override. |
| `layoutMode` | `GlmOcrLayoutMode?` | `null` | Optional whole-page or layout-paired dispatch mode. |
| `enableChartUnderstanding` | `bool?` | `null` | Whether chart regions use chart understanding instead of captioning. |
| `cacheDir` | `String?` | `null` | Optional Hugging Face cache root. |
...
| `textLayerThreshold` | `double` | `0.7` | Minimum fraction of pages that must have text to skip chunking. Range `0.0..=1.0`. Default: `0.7` (70 % of pages). |
| `fileSizeThresholdBytes` | `int` | `10485760` | File size threshold in bytes for considering chunking. Files smaller than this are processed without chunking. Default: 10 MiB (10 × 1 024 × 1 024). |
...
| `diskProcessingThresholdBytes` | `int` | `52428800` | File size threshold for disk-based processing. Files larger than this are buffered to disk to prevent OOM. Default: 50 MiB (50 × 1 024 × 1 024). |
...
| `maxXlsxWorkbookCells` | `int` | `5000000` | Maximum cell count (sheets × rows × columns approximation) in an XLSX workbook. Default: 5 000 000 (≈ 200 sheets × 25 k cells). |
...
| `fontSize` | `double` | — | The font size of the text in this block |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `left` | `double` | — | Left coordinate. |
| `top` | `double` | — | Top coordinate. |
| `right` | `double` | — | Right coordinate. |
| `bottom` | `double` | — | Bottom coordinate. |
...
| `kClusters` | `PlatformInt64` | `3` | Number of font size clusters to use for hierarchy levels (1-7) Default: 3, which provides two heading levels plus body text. Larger values create more fine-grained hierarchy levels. |
...
| `score` | `double` | — | Relevance score (higher is better, algorithm-specific range). |
| `algorithm` | `KeywordAlgorithm` | — | Algorithm that extracted this keyword. |
| `positions` | `List<PlatformInt64>?` | `null` | Optional positions where keyword appears in text (character offsets). |
...
| `maxKeywords` | `PlatformInt64` | `10` | Maximum number of keywords to extract (default: 10). |
| `minScore` | `double` | `0` | Minimum score threshold (0.0-1.0, default: 0.0). Keywords with scores below this threshold are filtered out. Note: Score ranges differ between algorithms. |
...
| `batchSize` | `PlatformInt64` | `16` | Batch size for local ONNX inference. ColBERT emits a `[seq, dim]` multi-vector embedding per document, so memory scales with batch size — keep this modest. |
| `maxLength` | `PlatformInt64` | `512` | Maximum token sequence length for the tokenizer (documents). |
| `queryMaxLength` | `PlatformInt64` | `32` | Fixed padded length for query augmentation. ColBERT queries are padded (with the mask token, kept attention-live) to exactly this many tokens rather than truncated/left as-is — this is the "query augmentation" trick from the ColBERT paper. |
...
| `maxEmbedDurationSecs` | `int?` | `60` | Maximum wall-clock duration (in seconds) for a single embed call when using `LateInteractionModelType.Plugin`. `null` disables the timeout. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `index` | `PlatformInt64` | — | Position of this document in the original input slice. |
| `score` | `double` | — | MaxSim relevance score. Higher means more relevant to the query. |
...
| `maxLength` | `PlatformInt64` | — | Maximum document token sequence length. |
| `queryMaxLength` | `PlatformInt64` | — | Fixed padded query length (ColBERT query augmentation). |
| `dim` | `PlatformInt64` | — | Per-token embedding dimensionality. |
...
| `confidence` | `double` | — | Detection confidence score in `[0.0, 1.0]`. |
...
| `confidenceThreshold` | `double?` | `null` | Confidence threshold override (None = use model default). |
...
| `modelLimits` | `Map<String, double>?` | `{}` | Per-model spend limits in USD, keyed by model name. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `maxEntries` | `PlatformInt64?` | `null` | Maximum number of cached entries. |
| `ttlSeconds` | `int?` | `null` | Cache entry time-to-live, in seconds. |
| `backend` | `String?` | `null` | Cache backend name (e.g. `"memory"`, or an `opendal` scheme). |
| `backendConfig` | `Map<String, String>?` | `{}` | Backend-specific configuration key/value pairs. |
...
| `timeoutSecs` | `int?` | `null` | Request timeout in seconds. When `null`, liter-llm's built-in 60s default applies, except the VLM OCR path which uses a 300s default (a single page image transcription routinely exceeds 60s). Set explicitly to override. |
...
| `stop` | `List<String>?` | `[]` | Stop sequence(s) that halt token generation, applied to individual requests built from this config. Mirrors liter-llm's `ChatCompletionRequest.stop` (`types.common.StopSequence`), which liter-llm represents as either a single string or a list of strings via an untagged enum. Always expressed here as a list — even one stop sequence is `["..."]` — so the field has a single, FFI-friendly shape across every language binding instead of a single-or-list union type. Converted to liter-llm's `StopSequence.Multiple` at each request-building call site; see `llm.client.to_stop_sequence`. |
| `seed` | `int?` | `null` | Random seed for reproducible outputs, applied to individual requests built from this config. Provider support varies — some silently ignore it. Mirrors liter-llm's `ChatCompletionRequest.seed`. |
...
| `headers` | `Map<String, String>?` | `{}` | Extra HTTP headers sent with every request to the provider. Mirrors liter-llm's `ClientConfigBuilder.header`, for gateways or providers that require custom auth/routing headers. |
| `providers` | `List<LlmProviderConfig>?` | `[]` | Custom provider configurations, in addition to liter-llm's built-in providers. Mirrors liter-llm's `LlmConfig.providers`, for OpenAI-compatible gateways and self-hosted model servers that are not in the built-in provider catalog. |
...
| `cooldownSecs` | `int?` | `null` | Cooldown duration after transient errors, in seconds. Mirrors liter-llm's `LlmConfig.cooldown_secs`. Only takes effect when liter-llm's `tower` feature is compiled in; otherwise the value is accepted but unused. |
| `healthCheckSecs` | `int?` | `null` | Background health check interval, in seconds. Mirrors liter-llm's `LlmConfig.health_check_secs`. Only takes effect when liter-llm's `tower` feature is compiled in; otherwise the value is accepted but unused. |
...
| `modelPrefixes` | `List<String>` | `[]` | Model name prefixes routed to this provider (e.g. `["my-provider/"]`). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `rpm` | `int?` | `null` | Requests per minute limit. |
| `tpm` | `int?` | `null` | Tokens per minute limit. |
| `windowSeconds` | `int?` | `null` | Rate limit window, in seconds. |
...
```dart
static Future<MetaSchema> compile(String metaSchemaJson)
```

**Example:**

```dart
final result = MetaSchema.compile("value");
```
...
```dart
Future<Preset> parsePreset(String path, Uint8List raw)
```

**Example:**

```dart
final result = instance.parsePreset("value", Uint8List.fromList([100, 97, 116, 97]));
```
...
| `data` | `List<double>` | — | Flat row-major buffer, length `num_tokens * dim`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `densityShiftThreshold` | `double` | `0.3` | Text density difference threshold for `DensityShift` detection. Default: 0.3. |
| `bigramOverlapMin` | `double` | `0.1` | Minimum bigram-overlap ratio below which a density shift is promoted to a `DensityShift` boundary.  Default: 0.1 (10 % overlap). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `min` | `PlatformInt64` | `1` | Minimum number of words in a candidate. |
| `max` | `PlatformInt64` | `3` | Maximum number of words in a candidate. |
...
```dart
final result = await instance.processImage(Uint8List.fromList([100, 97, 116, 97]), OcrConfig());
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `imageBytes` | `Uint8List` | Yes | Raw image data (JPEG, PNG, TIFF, etc.) |
...
```dart
final result = await instance.processImageFile("value", OcrConfig());
```
...
```dart
final result = instance.supportsLanguage("value");
```
...
```dart
final result = instance.backendType();
```
...
```dart
final result = instance.supportedLanguages();
```
...
```dart
final result = instance.supportsTableDetection();
```
...
```dart
final result = instance.supportsDocumentProcessing();
```
...
```dart
final result = instance.emitsStructuredMarkdown();
```
...
```dart
Future<ConfidenceSemantics> confidenceSemantics()
```

**Example:**

```dart
final result = instance.confidenceSemantics();
```
...
```dart
Future<PageOrientationHandling> pageOrientationHandling()
```

**Example:**

```dart
final result = instance.pageOrientationHandling();
```
...
```dart
final result = await instance.processDocument("value", OcrConfig());
```
...
| `backendMetadata` | `Map<String, String>` | `{}` | Backend-specific metadata that doesn't fit the unified schema. |
...
| `metadata` | `Map<String, String>` | `{}` | OCR processing metadata (confidence scores, language, etc.) |
| `tables` | `List<OcrTable>` | `[]` | Tables detected and extracted via OCR |
| `ocrElements` | `List<OcrElement>?` | `[]` | Structured OCR elements with bounding boxes and confidence scores. Available when TSV output is requested or table detection is enabled. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `minTotalNonWhitespace` | `PlatformInt64` | `64` | Minimum total non-whitespace characters to consider text substantive. |
| `minNonWhitespacePerPage` | `double` | `32` | Minimum non-whitespace characters per page on average. |
| `minMeaningfulWordLen` | `PlatformInt64` | `4` | Minimum character count for a word to be "meaningful". |
| `minMeaningfulWords` | `PlatformInt64` | `3` | Minimum count of meaningful words before text is accepted. |
| `minAlnumRatio` | `double` | `0.3` | Minimum alphanumeric ratio (non-whitespace chars that are alphanumeric). |
| `minGarbageChars` | `PlatformInt64` | `5` | Minimum Unicode replacement characters (U+FFFD) to trigger OCR fallback. |
...
| `minWordsForOcrOutputCheck` | `PlatformInt64` | `20` | Minimum word count before `Self.max_ocr_output_fragmented_word_ratio` may report a page. Short pages (a signature block, an exhibit title) are legitimately dominated by short words, so the ratio is not meaningful on them and the signal stays disabled. |
...
| `minWordsForAvgLengthCheck` | `PlatformInt64` | `50` | Minimum word count before average word length check applies. |
| `minConsecutiveRepeatRatio` | `double` | `0.08` | Minimum consecutive word repetition ratio to detect column scrambling. |
| `minWordsForRepeatCheck` | `PlatformInt64` | `50` | Minimum word count before consecutive repetition check is applied. |
| `substantiveMinChars` | `PlatformInt64` | `100` | Minimum character count for "substantive markdown" OCR skip gate. |
| `nonTextMinChars` | `PlatformInt64` | `20` | Minimum character count for "non-text content" OCR skip gate. |
...
| `detDbThresh` | `double` | `0.3` | Database threshold for text detection (default: 0.3) Range: 0.0-1.0, higher values require more confident detections |
| `detDbBoxThresh` | `double` | `0.5` | Box threshold for text bounding box refinement (default: 0.5) Range: 0.0-1.0 |
| `detDbUnclipRatio` | `double` | `1.6` | Unclip ratio for expanding text bounding boxes (default: 1.6) Controls the expansion of detected text regions |
...
| `dropScore` | `double` | `0.5` | Minimum recognition confidence score for text lines (default: 0.5). Text regions with recognition confidence below this threshold are discarded. Matches PaddleOCR Python's `drop_score` parameter. Range: 0.0-1.0 |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `task` | `PaddleOcrVlTaskKind?` | `null` | Optional per-call recognition task; the backend constructor's task is used when absent. |
| `modelPath` | `String?` | `null` | Optional local model directory, which takes precedence over `model_id`. |
| `modelId` | `String?` | `null` | Optional Hugging Face repository identifier. |
| `hfRevision` | `String?` | `null` | Optional immutable Hugging Face model revision. |
| `cacheDir` | `String?` | `null` | Optional Hugging Face cache root. |
| `device` | `CandleDevicePreference?` | `null` | Optional per-call device override. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `byteStart` | `PlatformInt64` | — | Byte offset where this page starts in the content string (UTF-8 valid boundary, inclusive) |
| `byteEnd` | `PlatformInt64` | — | Byte offset where this page ends in the content string (UTF-8 valid boundary, exclusive) |
...
| `imageIndices` | `List<int>` | `/* serde(default) */` | Indices into `ExtractedDocument.images` for images found on this page. Each value is a zero-based index into the top-level `images` collection. Only populated when `extract_images = true` in the extraction config. |
...
| `layoutTextDensity` | `double` | — | Text density: characters per page area, normalised to `[0.0, 1.0]`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `start` | `PlatformInt64` | — | Inclusive byte-offset start of the match in the source text. |
| `end` | `PlatformInt64` | — | Exclusive byte-offset end of the match. |
...
| `scannedConfidence` | `double?` | `null` | How strongly the document's most scan-like page resembles a scan, in `[0.0, 1.0]`. `null` when the document could not be inspected. A full-page raster with no visible text scores at least `0.85`; a born-digital slide with a full-bleed background image scores `0.50`. |
| `scannedPages` | `List<int>?` | `[]` | Pages that look like scans (1-indexed), using the default confidence threshold. `null` when the document could not be inspected; empty when no page qualifies. |
| `layoutGatedPages` | `List<int>?` | `[]` | Pages the `auto` layout strategy skipped (1-indexed). `null` unless layout detection ran with `LayoutStrategy.Auto`; empty when the gate selected every page. |
| `layoutGateReasons` | `List<String>?` | `[]` | Why the `auto` layout gate selected or skipped each page. Index `i` is page `i + 1`. Snake_case values such as `multi_column`, `table_grid`, or `plain_text` (the skip reason). `null` unless layout detection ran with `LayoutStrategy.Auto`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `width` | `PlatformInt64` | — | Width in pixels. |
| `height` | `PlatformInt64` | — | Height in pixels. |
...
```dart
final result = instance.description();
```
...
```dart
await instance.process(ExtractedDocument(), ExtractionConfig());
```
...
```dart
final result = instance.processingStage();
```
...
```dart
final result = instance.shouldProcess(ExtractedDocument(), ExtractionConfig());
```
...
```dart
final result = instance.estimatedDurationMs(ExtractedDocument());
```
...
| `slideCount` | `PlatformInt64` | — | Total number of slides |
| `imageCount` | `PlatformInt64` | — | Total number of embedded images |
| `tableCount` | `PlatformInt64` | — | Total number of tables |
...
| `schema` | `String` | — | JSON Schema (Draft 2020-12) describing the structured output shape. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `messageCount` | `PlatformInt64` | — | Total number of email messages found in the PST archive. |
...
| `confidence` | `double?` | `null` | Detector-reported confidence in `[0.0, 1.0]`. `null` when the decoder does not expose confidence (the default `rqrr` backend always reports `Some` because successful decode implies high confidence). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `minWordLength` | `PlatformInt64` | `1` | Minimum word length to consider (default: 1). |
| `maxWordsPerPhrase` | `PlatformInt64` | `3` | Maximum words in a keyword phrase (default: 3). |
...
```dart
final result = Registry.loadEmbedded();
```
...
```dart
Future<Preset?> get_(String id)
```

**Example:**

```dart
final result = instance.get_("value");
```
...
```dart
final result = instance.summaries();
```
...
```dart
Future<Uint8List?> sampleBytes(String presetId, String name)
```

**Example:**

```dart
final result = instance.sampleBytes("value", "value");
```
...
```dart
Future<PlatformInt64> extendFromDir(String dir)
```

**Example:**

```dart
final result = instance.extendFromDir("value");
```
...
```dart
final result = instance.renderResult(ExtractedDocument());
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `index` | `PlatformInt64` | — | Position of this document in the original input `documents` slice. |
| `score` | `double` | — | Relevance score in `[0, 1]`. Higher means more relevant to the query. |
...
```dart
Future<List<double>> rerank(String query, List<String> documents)
```

**Example:**

```dart
final result = await instance.rerank("value", []);
```
...
| `topK` | `PlatformInt64?` | `null` | Return at most this many documents. `null` returns all. Applied after sorting by score, so the highest-scoring documents are kept. |
| `batchSize` | `PlatformInt64` | `32` | Batch size for local ONNX cross-encoder inference. |
...
| `maxRerankDurationSecs` | `int?` | `60` | Maximum wall-clock duration (in seconds) for a single `rerank()` call when using `RerankerModelType.Plugin`. Applies only to the in-process plugin path — protects against hung host-language backends. On timeout, the dispatcher returns `Plugin` instead of blocking forever. `null` disables the timeout. The default (60 seconds) is conservative for common in-process inference; increase for large document sets on slow hardware. |
...
| `schema` | `String` | — | Effective JSON Schema (caller override or the preset's own). |
...
| `propertyChanges` | `List<PropertyChange>` | `[]` | Formatting or metadata property changes for this revision. |
...
| `maxRequestBodyBytes` | `PlatformInt64` | `104857600` | Maximum size of request body in bytes (default: 100 MB) |
| `maxMultipartFieldBytes` | `PlatformInt64` | `104857600` | Maximum size of multipart fields in bytes (default: 100 MB) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `indices` | `List<int>` | — | Vocabulary token ids with non-zero weight, ascending. |
| `values` | `List<double>` | — | Weights parallel to `SparseEmbedding.indices`. |
...
| `batchSize` | `PlatformInt64` | `16` | Batch size for local ONNX inference. SPLADE emits a `[seq, vocab]` logit tensor per document, so memory scales with batch size — keep this modest. |
| `maxLength` | `PlatformInt64` | `256` | Maximum token sequence length for the tokenizer. |
...
| `maxEmbedDurationSecs` | `int?` | `60` | Maximum wall-clock duration (in seconds) for a single embed call when using `SparseEmbeddingModelType.Plugin`. `null` disables the timeout. |
...
| `modelFile` | `String` | — | Path to the ONNX file within the repo. |
| `additionalFiles` | `List<String>` | — | Sibling files that must be downloaded alongside `model_file`. |
| `maxLength` | `PlatformInt64` | — | Maximum token sequence length. |
| `description` | `String` | — | Human-readable description. |
...
| `maxRedirects` | `int` | `5` | Maximum number of HTTP redirects to follow during validation. |
...
| `value` | `String?` | `null` | The parsed document as a canonical `serde_json.Value` tree, when the source format could be represented as one. `null` only for TOML inputs whose `toml.Value` fails to round-trip through `serde_json.Value` (xberg-io/xberg#155): the extractor falls back to a raw code block in that case. |
...
| `renderDpi` | `double` | `96` | Target DPI when rasterizing SVG to a pixel-based format (PNG, JPEG, WebP, HEIF).  The tree's viewBox is scaled by `render_dpi / 96.0` before the pixel buffer is allocated.  Defaults to `96.0` (1× CSS pixel density). |
...
| `headingLevel` | `int?` | `null` | Outline level 1-6 when the style resolves to a heading, otherwise `null`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `fromIndex` | `PlatformInt64` | — | Zero-based index of the table in both `a.tables` and `b.tables`. |
| `toIndex` | `PlatformInt64` | — | Zero-based index in `b.tables` (equal to `from_index` for same-dimension tables). |
...
| `lineCount` | `PlatformInt64` | — | Number of lines |
| `wordCount` | `PlatformInt64` | — | Number of words |
| `characterCount` | `PlatformInt64` | — | Number of characters |
...
| `links` | `List<MarkdownLink>?` | `[]` | Markdown links (for Markdown files). |
| `codeBlocks` | `List<MarkdownCodeBlock>?` | `[]` | Code blocks (for Markdown files). |
...
```dart
static Future<TokenCounter> create()
```

**Example:**

```dart
final result = TokenCounter.create();
```
...
| `semanticThreshold` | `double` | `0.3` | Cosine similarity threshold below which sentences are considered dissimilar. |
...
| `targetReduction` | `double?` | `null` | Target fraction of text to retain (0.0–1.0); `null` = no fixed target. |
...
```dart
Future<PlatformInt64> countTokens(String text)
```

**Example:**

```dart
final result = instance.countTokens("value");
```
...
| `maxDurationMs` | `int?` | `1800000` | Hard safety limit on input duration (milliseconds). Files longer than this are rejected after decode, before model work. Default: 30 minutes. Set to `null` to disable (not recommended for untrusted input). |
| `maxBytes` | `int?` | `536870912` | Hard safety limit on input size (bytes). Default: 512 MiB. Protects against pathological or malicious uploads. |
| `timeoutMs` | `int?` | `600000` | Wall-clock timeout for the entire transcription operation (ms). Bounds audio decode, model resolution/download, and inference together. On expiry the extraction fails with an `XbergError.Transcription`. `null` disables the bound and lets the operation run unbounded (not recommended for untrusted input). Enforced on the async extraction path only; the size and duration caps (`max_bytes`, `max_duration_ms`) are checked on every path. Default: 10 minutes. |
...
| `chunkMaxSize` | `PlatformInt64?` | `null` | Maximum chunk size in bytes. `null` disables chunking. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `variant` | `CandleTrocrVariant?` | `null` | Optional model variant; the backend constructor's variant is used when absent. |
| `device` | `CandleDevicePreference?` | `null` | Optional per-call device override. |
| `cacheDir` | `String?` | `null` | Optional Hugging Face cache root. |
| `hfRevision` | `String?` | `null` | Optional immutable Hugging Face model revision. |
...
| `maxDocumentUrlsPerResult` | `int?` | `100` | Maximum URLs to follow per extraction result. |
| `maxTotalUrls` | `int?` | `1000` | Maximum URLs followed across the whole extraction call. |
...
```dart
await instance.validate(ExtractedDocument(), ExtractionConfig());
```
...
```dart
final result = instance.shouldValidate(ExtractedDocument(), ExtractionConfig());
```
...
| `elementCount` | `PlatformInt64` | — | Total number of XML elements processed |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `windowSize` | `PlatformInt64` | `2` | Window size for co-occurrence analysis (default: 2). Controls the context window for computing co-occurrence statistics. |
...
| `Custom` | Use a custom ColBERT ONNX model from HuggingFace. — Fields: `modelId`: `String`, `modelFile`: `String`, `additionalFiles`: `List<String>`, `maxLength`: `int` |
...
| `Custom` | Use a custom ONNX model from HuggingFace — Fields: `modelId`: `String`, `dimensions`: `PlatformInt64` |
...
| `Custom` | Use a custom ONNX cross-encoder from HuggingFace. — Fields: `modelId`: `String`, `modelFile`: `String`, `additionalFiles`: `List<String>`, `maxLength`: `int`, `head`: `RerankerHead` |
...
| `Custom` | Use a custom SPLADE (`BertForMaskedLM`) ONNX model from HuggingFace. — Fields: `modelId`: `String`, `modelFile`: `String`, `additionalFiles`: `List<String>`, `maxLength`: `int` |
...
| Value | Description |
|-------|-------------|
| `Paragraph` | Body paragraph, identified by its zero-based index in the document flow. — Fields: `index`: `PlatformInt64` |
| `TableCell` | Cell inside a table. — Fields: `row`: `PlatformInt64`, `col`: `PlatformInt64`, `tableIndex`: `PlatformInt64` |
| `Page` | Page, identified by its zero-based index. — Fields: `index`: `PlatformInt64` |
| `Slide` | Presentation slide, identified by its zero-based index. — Fields: `index`: `PlatformInt64` |
| `Sheet` | Spreadsheet cell or range, identified by sheet index and optional name. — Fields: `index`: `PlatformInt64`, `name`: `String` |
...
| Value | Description |
|-------|-------------|
| `SmallFile` | File is below size threshold. — Fields: `sizeBytes`: `int`, `thresholdBytes`: `int` |
| `FewPages` | Document has fewer pages than threshold. — Fields: `pageCount`: `int`, `threshold`: `int` |
| `TextLayerDetected` | PDF has substantial text layer (OCR not needed). — Fields: `textCoverage`: `double`, `avgCharsPerPage`: `int` |
...
| Value | Description |
|-------|-------------|
| `LargeFile` | File exceeds size threshold. — Fields: `sizeBytes`: `int`, `thresholdBytes`: `int` |
| `ManyPages` | Document has many pages. — Fields: `pageCount`: `int`, `threshold`: `int` |
| `OcrRequired` | PDF requires OCR and is large. — Fields: `pageCount`: `int`, `forceOcr`: `bool` |
| `LargeAndManyPages` | Both size and page count exceed thresholds. — Fields: `sizeBytes`: `int`, `pageCount`: `int` |
...
|-------|----------|------------|-------|
| `OcrBackend` | XbergBridge.registerOcrBackend | XbergBridge.unregisterOcrBackend | XbergBridge.clearOcrBackends |
| `PostProcessor` | XbergBridge.registerPostProcessor | XbergBridge.unregisterPostProcessor | XbergBridge.clearPostProcessors |
| `Validator` | XbergBridge.registerValidator | XbergBridge.unregisterValidator | XbergBridge.clearValidators |
| `DocumentExtractor` | XbergBridge.registerDocumentExtractor | XbergBridge.unregisterDocumentExtractor | XbergBridge.clearDocumentExtractors |
| `EmbeddingBackend` | XbergBridge.registerEmbeddingBackend | XbergBridge.unregisterEmbeddingBackend | XbergBridge.clearEmbeddingBackends |
| `Renderer` | XbergBridge.registerRenderer | XbergBridge.unregisterRenderer | XbergBridge.clearRenderers |
| `RerankerBackend` | XbergBridge.registerRerankerBackend | XbergBridge.unregisterRerankerBackend | XbergBridge.clearRerankerBackends |
| `TokenizerBackend` | XbergBridge.registerTokenizerBackend | XbergBridge.unregisterTokenizerBackend | XbergBridge.clearTokenizerBackends |
