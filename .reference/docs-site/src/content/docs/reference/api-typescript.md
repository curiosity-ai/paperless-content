...
<!-- alef:hash:3913cafdff5c571543999ee2ba586b9929e3f575f81a5d45de2bc37ffa789a91 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## TypeScript API Reference <span class="version-badge">v1.1.3</span>
...
```typescript
function classifyChunksOwned(result: ExtractedDocument, config: ChunkClassificationConfig): Promise<ExtractedDocument>
```

**Example:**

```typescript
const result = await classifyChunksOwned(new ExtractedDocument(), new ChunkClassificationConfig());
```
...
```typescript
function maxSimScore(query: MultiVectorEmbedding, doc: MultiVectorEmbedding): number
```

**Example:**

```typescript
const result = maxSimScore(new MultiVectorEmbedding(), new MultiVectorEmbedding());
```
...
```typescript
function maxSimRank(query: MultiVectorEmbedding, docs: Array<MultiVectorEmbedding>): Array<LateInteractionMatch>
```

**Example:**

```typescript
const result = maxSimRank(new MultiVectorEmbedding(), []);
```
...
```typescript
function doctor(config: ExtractionConfig): DoctorReport
```

**Example:**

```typescript
const result = doctor(new ExtractionConfig());
```
...
| `byteStart` | `number` | — | Inclusive start byte offset of this chunk in the original source. |
| `byteEnd` | `number` | — | Exclusive end byte offset of this chunk in the original source. |
...
| `byteStart` | `number` | — | Inclusive start byte offset of the `name="value"` attribute token. |
| `byteEnd` | `number` | — | Exclusive end byte offset of the `name="value"` attribute token. |
...
| `key` | `string \| null` | `/* serde(default) */` | Key, attribute name, tag name, or positional index (`"0"`, `"1"`, …). `null` at the document root. |
| `value` | `string \| null` | `/* serde(default) */` | Leaf scalar value, if any. `null` for containers (objects, arrays, XML elements with child elements). |
...
| `byteStart` | `number` | — | Inclusive start byte offset of this node in the original source. |
| `byteEnd` | `number` | — | Exclusive end byte offset of this node in the original source. |
...
| `data` | `CodeDataNode \| null` | `null` | Hierarchical key/value data tree extracted from data-format source (JSON, YAML, TOML, XML, CSV, etc.), when data extraction was enabled. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `modelPath` | `string \| null` | `null` | Local DeepSeek-OCR model directory. The backend requires this option. |
| `device` | `CandleDevicePreference \| null` | `null` | Optional per-call device override. |
| `version` | `number \| null` | `null` | DeepSeek-OCR model generation, either 1 or 2. Defaults to 2. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `task` | `GlmOcrTaskKind \| null` | `null` | Optional recognition task; the backend constructor's task is used when absent. |
| `device` | `CandleDevicePreference \| null` | `null` | Optional per-call device override. |
| `layoutMode` | `GlmOcrLayoutMode \| null` | `null` | Optional whole-page or layout-paired dispatch mode. |
| `enableChartUnderstanding` | `boolean \| null` | `null` | Whether chart regions use chart understanding instead of captioning. |
| `cacheDir` | `string \| null` | `null` | Optional Hugging Face cache root. |
...
```typescript
const result = instance.parsePreset("value", Buffer.from("data"));
```
...
```typescript
const result = await instance.processImage(Buffer.from("data"), new OcrConfig());
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `task` | `PaddleOcrVlTaskKind \| null` | `null` | Optional per-call recognition task; the backend constructor's task is used when absent. |
| `modelPath` | `string \| null` | `null` | Optional local model directory, which takes precedence over `model_id`. |
| `modelId` | `string \| null` | `null` | Optional Hugging Face repository identifier. |
| `hfRevision` | `string \| null` | `null` | Optional immutable Hugging Face model revision. |
| `cacheDir` | `string \| null` | `null` | Optional Hugging Face cache root. |
| `device` | `CandleDevicePreference \| null` | `null` | Optional per-call device override. |
...
| `maxRequestBodyBytes` | `number` | `104857600` | Maximum size of request body in bytes (default: 100 MB) |
| `maxMultipartFieldBytes` | `number` | `104857600` | Maximum size of multipart fields in bytes (default: 100 MB) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `variant` | `CandleTrocrVariant \| null` | `null` | Optional model variant; the backend constructor's variant is used when absent. |
| `device` | `CandleDevicePreference \| null` | `null` | Optional per-call device override. |
| `cacheDir` | `string \| null` | `null` | Optional Hugging Face cache root. |
| `hfRevision` | `string \| null` | `null` | Optional immutable Hugging Face model revision. |
