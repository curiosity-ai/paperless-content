...
<!-- alef:hash:5ef418c7b09e2c6140643235eda8e2992cec37e8552f6b46bb833fa71713babc -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Swift API Reference <span class="version-badge">v1.1.3</span>
...
```swift
let result = try extract(ExtractInput(), ExtractionConfig())
```
...
```swift
let result = try extractBatch([], ExtractionConfig())
```
...
```swift
public static func mapUrl(uri: String, config: UrlExtractionConfig) throws -> MapResult
```

**Example:**

```swift
let result = try mapUrl("value", UrlExtractionConfig())
```
...
```swift
public static func clearTokenizerBackends() throws
```

**Example:**

```swift
try clearTokenizerBackends()
```
...
```swift
public static func listTokenizerBackends() throws -> [String]
```

**Example:**

```swift
let result = try listTokenizerBackends()
```
...
```swift
public static func classifyChunksOwned(result: ExtractedDocument, config: ChunkClassificationConfig) throws -> ExtractedDocument
```

**Example:**

```swift
let result = try classifyChunksOwned(ExtractedDocument(), ChunkClassificationConfig())
```
...
```swift
public static func findUnmarkedClaims(markdown: String) -> [String]
```

**Example:**

```swift
let result = findUnmarkedClaims("value")
```
...
```swift
public static func verifyExcerpt(excerpt: String, sourceText: String) -> Bool
```

**Example:**

```swift
let result = verifyExcerpt("value", "value")
```
...
```swift
public static func maxSimScore(query: MultiVectorEmbedding, doc: MultiVectorEmbedding) -> Double
```

**Example:**

```swift
let result = maxSimScore(MultiVectorEmbedding(), MultiVectorEmbedding())
```
...
```swift
public static func maxSimRank(query: MultiVectorEmbedding, docs: [MultiVectorEmbedding]) -> [LateInteractionMatch]
```

**Example:**

```swift
let result = maxSimRank(MultiVectorEmbedding(), [])
```
...
```swift
public static func doctor(config: ExtractionConfig) -> DoctorReport
```

**Example:**

```swift
let result = doctor(ExtractionConfig())
```
...
| `compressedSize` | `UInt64?` | `null` | Compressed size in bytes (if available) |
...
| `classes` | `\[String\]` | `[]` | CSS classes (.class1 .class2) |
| `keyValues` | `\[KeyValueAttribute\]` | `[]` | Key-value pairs (key="value") |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `durationMs` | `UInt64?` | `null` | Duration in milliseconds derived from the decoded audio stream. |
| `codec` | `String?` | `null` | Audio codec (e.g. "mp3", "aac", "opus", "flac"). |
| `container` | `String?` | `null` | Container format (e.g. "mpeg", "mp4", "ogg", "wav"). |
| `sampleRateHz` | `UInt32?` | `null` | Sample rate in Hz after decode (always 16000 when resampled for Whisper). |
| `channels` | `UInt16?` | `null` | Number of audio channels (1 = mono, 2 = stereo). |
| `bitrate` | `UInt32?` | `null` | Audio bitrate in kbps from the source file tags/properties. |
...
| `citationKeys` | `\[String\]` | `[]` | BibTeX citation keys (e.g. `"knuth1984"`) for all entries. |
| `authors` | `\[String\]` | `[]` | Author names collected across all bibliography entries. |
| `yearRange` | `YearRange?` | `null` | Earliest and latest publication years found in the bibliography. |
| `entryTypes` | `\[String: UInt\]?` | `{}` | Count of entries grouped by BibTeX entry type (e.g. `"article"` → 5). |
...
| `tokenCount` | `UInt?` | `null` | Number of tokens in this chunk (if available). This is calculated by the embedding model's tokenizer if embeddings are enabled. |
...
| `authors` | `\[String\]` | `[]` | Author names collected across all citation records. |
| `yearRange` | `YearRange?` | `null` | Earliest and latest publication years found in the file. |
| `dois` | `\[String\]` | `[]` | DOI identifiers found in the citation records. |
| `keywords` | `\[String\]` | `[]` | Keywords collected from all citation records. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `chunks` | `\[CodeChunkInfo\]` | `[]` | Structural code chunks (function/class/module boundaries). |
...
| `keepInlineImagesIn` | `\[String\]` | `[]` | HTML tag names whose `<img>` children are kept inline instead of block. |
...
| `stripTags` | `\[String\]` | `[]` | HTML tag names whose content is stripped from the output entirely. |
| `preserveTags` | `\[String\]` | `[]` | HTML tag names that are preserved verbatim in the output. |
...
| `maxDepth` | `UInt?` | `null` | Maximum DOM traversal depth. `null` uses the library's internal native-stack safety limit. Explicit values above that safety limit are clamped to prevent process-aborting stack overflows on pathologically deep DOM trees. |
...
| `commentPrefixes` | `\[String\]` | `[]` | Line prefixes that mark a comment line to skip entirely during row parsing (e.g. `["#"]`). A line is treated as a comment when its trimmed start matches any of these prefixes exactly. Default: empty, meaning no line is treated as a comment (matches the pre-existing extractor behavior). |
...
| `columnTypes` | `\[String\]?` | `[]` | Inferred data type for each column (e.g. `"string"`, `"integer"`, `"float"`). |
...
| `fields` | `\[DbfFieldInfo\]` | `[]` | Descriptor for each field in the table schema. |
...
| `version` | `UInt32?` | `null` | DeepSeek-OCR model generation, either 1 or 2. Defaults to 2. |
...
| `maxContentChars` | `UInt?` | `null` | Truncate content to this many characters before diffing. Useful for very large documents where only the first N characters matter. `null` means no truncation. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `checks` | `\[DoctorCheck\]` | `[]` | Individual check verdicts, in execution order. |
...
| `pageCount` | `UInt32?` | `null` | Page count (if known, e.g., from previous analysis). |
...
| `tokenCount` | `UInt32?` | `null` | Approximate token count of the summary, when known. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `pageNumber` | `UInt32?` | `null` | Page number (1-indexed) |
| `filename` | `String?` | `null` | Source filename or document name |
| `coordinates` | `BoundingBox?` | `null` | Bounding box coordinates if available |
| `elementIndex` | `UInt?` | `null` | Position index in the element sequence |
| `additional` | `\[String: String\]` | — | Additional custom metadata |
...
| `size` | `UInt?` | `null` | Size in bytes |
| `isImage` | `Bool` | — | Whether this attachment is an image |
| `data` | `Data?` | `null` | Attachment data (if extracted). Uses `bytes.Bytes` for cheap cloning of large buffers. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `msgFallbackCodepage` | `UInt32?` | `null` | Windows codepage number to use when an MSG file contains no codepage property. Defaults to `null`, which falls back to windows-1252. If an unrecognized or invalid codepage number is supplied (including 0), the behavior silently falls back to windows-1252 — the same as when the MSG file itself contains an unrecognized codepage. No error or warning is emitted. Users should verify output when supplying unusual values. Common values: - 1250: Central European (Polish, Czech, Hungarian, etc.) - 1251: Cyrillic (Russian, Ukrainian, Bulgarian, etc.) - 1252: Western European (default) - 1253: Greek - 1254: Turkish - 1255: Hebrew - 1256: Arabic - 932:  Japanese (Shift-JIS) - 936:  Simplified Chinese (GBK) |
...
| `metadata` | `\[String: String\]` | — | Additional email headers and metadata |
...
| `data` | `Data` | — | Raw file bytes from the embedded stream (already decompressed by lopdf). |
...
| `cacheDir` | `URL?` | `null` | Optional alternate Hugging Face cache root for model files. When unset, hf-hub follows `HF_HUB_CACHE`, `HUGGINGFACE_HUB_CACHE`, `HF_HOME`, XDG, and platform defaults. Prefer those environment variables when configuring the cache process-wide. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `sheetCount` | `UInt32?` | `null` | Number of sheets in the workbook. |
| `sheetNames` | `\[String\]?` | `[]` | Names of all sheets in the workbook. |
...
| `tableCells` | `\[\[String\]\]?` | `null` | Pre-extracted table cells (2D vector of cell values) Populated during markdown generation to avoid re-parsing markdown. None for empty sheets. |
...
| `bytes` | `Data?` | `null` | Raw bytes for `kind = "bytes"`. |
...
| `detectedLanguageConfidences` | `\[LanguageConfidence\]?` | `[]` | Structured per-language detection results: confidence, document share, script, and reliability, alongside the ISO-code-only `detected_languages` (#261). One entry per language in `detected_languages`, in the same order. `null` under the same conditions as `detected_languages`: detection disabled, empty input text, or no language met the configured `min_confidence`. |
...
| `formulas` | `\[Formula\]` | `[]` | Mathematical formulas recognized in the document. Populated from every source that produces formulas: layout-guided OCR (with geometry), VLM OCR (text only), and markup extraction (DOCX, PPTX, ODT, EPUB, HTML, JATS, LaTeX, Markdown, and related formats, without geometry). Empty when the document contains no formulas. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `data` | `Data` | — | Raw image data (PNG, JPEG, WebP, etc. bytes). Uses `bytes.Bytes` for cheap cloning of large buffers. |
...
| `pageNumber` | `UInt32?` | `null` | Page/slide number where image was found (1-indexed) |
| `width` | `UInt32?` | `null` | Image width in pixels |
| `height` | `UInt32?` | `null` | Image height in pixels |
...
| `qrCodes` | `\[QrCode\]?` | `[]` | QR codes decoded from this image, when QR detection is enabled. Populated by the QR post-processor (`crates/xberg/src/extractors/qr.rs`) via the pure-Rust `rqrr` decoder. `null` when QR detection is disabled; an empty `Some([])` when detection ran but found nothing. |
...
| `page` | `UInt32?` | `null` | Optional page number where the URI was found (1-indexed). |
...
| `maxConcurrentExtractions` | `UInt?` | `null` | Maximum concurrent document extractions in batch operations. This is a ceiling within the configured total thread budget, not an independent pool size. When unset, the scheduler derives document and per-document concurrency from `ConcurrencyConfig.max_threads`. |
...
| `crawlUniqueNormalizedUrls` | `\[String\]` | `[]` | Unique normalized URLs discovered by crawls. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `genres` | `\[String\]` | `[]` | Genre tags as declared in the FB2 `<genre>` elements. |
| `sequences` | `\[String\]` | `[]` | Book series (sequence) names, if any. |
...
| `forceOcrPages` | `\[UInt32\]?` | `[]` | Override force OCR pages for this file (1-indexed page numbers). |
...
| `level` | `UInt?` | `null` | Heading level (1-6) for headings, or nesting level for lists |
...
| `page` | `UInt32?` | `/* serde(default) */` | 1-indexed page number the formula appears on. `null` when the source format has no page concept. The C FFI reports an absent page as `0`. |
...
| `keywords` | `\[String\]` | `[]` | Document keywords from `<meta name="keywords">` tag, split on commas |
...
| `openGraph` | `\[String: String\]` | `{}` | Open Graph metadata (og:* properties) for social media Keys like "title", "description", "image", "url", etc. |
| `twitterCard` | `\[String: String\]` | `{}` | Twitter Card metadata (twitter:* properties) Keys like "card", "site", "creator", "title", "description", "image", etc. |
| `metaTags` | `\[String: String\]` | `{}` | Additional meta tags not covered by specific fields Keys are meta name/property attributes, values are content |
| `headers` | `\[HeaderMetadata\]` | `[]` | Extracted header elements with hierarchy |
| `links` | `\[LinkMetadata\]` | `[]` | Extracted hyperlinks with type classification |
| `images` | `\[ImageMetadataType\]` | `[]` | Extracted images with source and dimensions |
| `structuredData` | `\[StructuredData\]` | `[]` | Extracted structured data blocks |
...
| `cssFile` | `URL?` | `null` | Path to a CSS file loaded once at renderer construction time. Concatenated before `css` when both are set. |
...
| `positions` | `\[UInt\]?` | `null` | Optional positions where keyword appears in text (character offsets). |
...
| `cacheDir` | `URL?` | `null` | Optional alternate Hugging Face cache root for model files. When unset, hf-hub follows the standard Hugging Face environment and platform cache conventions. |
| `acceleration` | `AccelerationConfig?` | `null` | Hardware acceleration for the late-interaction ONNX model. |
| `maxEmbedDurationSecs` | `UInt64?` | `60` | Maximum wall-clock duration (in seconds) for a single embed call when using `LateInteractionModelType.Plugin`. `null` disables the timeout. |
...
| `modelLimits` | `\[String: Double\]?` | `{}` | Per-model spend limits in USD, keyed by model name. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `maxEntries` | `UInt?` | `null` | Maximum number of cached entries. |
| `ttlSeconds` | `UInt64?` | `null` | Cache entry time-to-live, in seconds. |
| `backend` | `String?` | `null` | Cache backend name (e.g. `"memory"`, or an `opendal` scheme). |
| `backendConfig` | `\[String: String\]?` | `{}` | Backend-specific configuration key/value pairs. |
...
| `timeoutSecs` | `UInt64?` | `null` | Request timeout in seconds. When `null`, liter-llm's built-in 60s default applies, except the VLM OCR path which uses a 300s default (a single page image transcription routinely exceeds 60s). Set explicitly to override. |
| `maxRetries` | `UInt32?` | `null` | Maximum retry attempts (default: 3). |
| `temperature` | `Double?` | `null` | Sampling temperature for generation tasks. |
| `maxTokens` | `UInt64?` | `null` | Maximum tokens to generate. |
...
| `stop` | `\[String\]?` | `[]` | Stop sequence(s) that halt token generation, applied to individual requests built from this config. Mirrors liter-llm's `ChatCompletionRequest.stop` (`types.common.StopSequence`), which liter-llm represents as either a single string or a list of strings via an untagged enum. Always expressed here as a list — even one stop sequence is `["..."]` — so the field has a single, FFI-friendly shape across every language binding instead of a single-or-list union type. Converted to liter-llm's `StopSequence.Multiple` at each request-building call site; see `llm.client.to_stop_sequence`. |
| `seed` | `Int64?` | `null` | Random seed for reproducible outputs, applied to individual requests built from this config. Provider support varies — some silently ignore it. Mirrors liter-llm's `ChatCompletionRequest.seed`. |
...
| `headers` | `\[String: String\]?` | `{}` | Extra HTTP headers sent with every request to the provider. Mirrors liter-llm's `ClientConfigBuilder.header`, for gateways or providers that require custom auth/routing headers. |
| `providers` | `\[LlmProviderConfig\]?` | `[]` | Custom provider configurations, in addition to liter-llm's built-in providers. Mirrors liter-llm's `LlmConfig.providers`, for OpenAI-compatible gateways and self-hosted model servers that are not in the built-in provider catalog. |
...
| `cooldownSecs` | `UInt64?` | `null` | Cooldown duration after transient errors, in seconds. Mirrors liter-llm's `LlmConfig.cooldown_secs`. Only takes effect when liter-llm's `tower` feature is compiled in; otherwise the value is accepted but unused. |
| `healthCheckSecs` | `UInt64?` | `null` | Background health check interval, in seconds. Mirrors liter-llm's `LlmConfig.health_check_secs`. Only takes effect when liter-llm's `tower` feature is compiled in; otherwise the value is accepted but unused. |
...
| `modelPrefixes` | `\[String\]` | `[]` | Model name prefixes routed to this provider (e.g. `["my-provider/"]`). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `rpm` | `UInt32?` | `null` | Requests per minute limit. |
| `tpm` | `UInt64?` | `null` | Tokens per minute limit. |
| `windowSeconds` | `UInt64?` | `null` | Rate limit window, in seconds. |
...
| `inputTokens` | `UInt64?` | `null` | Number of input/prompt tokens consumed. |
| `outputTokens` | `UInt64?` | `null` | Number of output/completion tokens generated. |
| `totalTokens` | `UInt64?` | `null` | Total tokens (input + output). |
...
```swift
public static func compile(metaSchemaJson: String) throws -> MetaSchema
```

**Example:**

```swift
let result = try MetaSchema.compile("value")
```
...
```swift
public func parsePreset(path: String, raw: Data) throws -> Preset
```

**Example:**

```swift
let result = try instance.parsePreset("value", Data("data".utf8))
```
...
| `authors` | `\[String\]?` | `[]` | Primary author(s) - always Vec for consistency |
| `keywords` | `\[String\]?` | `[]` | Keywords/tags - always Vec for consistency |
...
| `extractionDurationMs` | `UInt64?` | `null` | Extraction duration in milliseconds (for benchmarking). This field is populated by batch extraction to provide per-file timing information. It's `null` for single-file extraction (which uses external timing). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `detModel` | `URL` | — | Exact path to the detection ONNX model in the Hugging Face snapshot. |
| `clsModel` | `URL` | — | Exact path to the classification ONNX model in the Hugging Face snapshot. |
| `recModel` | `URL` | — | Exact path to the recognition ONNX model in the Hugging Face snapshot. |
| `dictFile` | `URL` | — | Path to the character dictionary file. |
...
| `data` | `\[Float\]` | — | Flat row-major buffer, length `num_tokens * dim`. |
...
```swift
public func emitsStructuredMarkdown() -> Bool
```

**Example:**

```swift
let result = instance.emitsStructuredMarkdown()
```
...
```swift
public func confidenceSemantics() -> ConfidenceSemantics
```

**Example:**

```swift
let result = instance.confidenceSemantics()
```
...
```swift
public func pageOrientationHandling() -> PageOrientationHandling
```

**Example:**

```swift
let result = instance.pageOrientationHandling()
```
...
| `backendMetadata` | `\[String: String\]` | `{}` | Backend-specific metadata that doesn't fit the unified schema. |
...
| `ocrElements` | `\[OcrElement\]?` | `[]` | Structured OCR elements with bounding boxes and confidence scores. Available when TSV output is requested or table detection is enabled. |
...
| `tableRows` | `UInt32?` | `null` | Number of rows in the detected table (if a single table was found). |
| `tableCols` | `UInt32?` | `null` | Number of columns in the detected table (if a single table was found). |
...
| `cacheDir` | `URL?` | `null` | Optional Hugging Face Hub cache root for model files. When unset, the standard `HF_HUB_CACHE`, legacy `HUGGINGFACE_HUB_CACHE`, and `HF_HOME` conventions are used. |
...
| `passwords` | `\[String\]?` | `null` | List of passwords to try when opening encrypted PDFs |
...
| `page` | `UInt32?` | `/* serde(default) */` | 1-indexed page the field's widget appears on. Currently always `null` for AcroForm fields; page assignment is a deferred enhancement requiring spatial analysis of widget annotations per page. |
...
| `pageCount` | `UInt32?` | `null` | Total number of pages in the PDF document |
...
| `scannedPages` | `\[UInt32\]?` | `[]` | Pages that look like scans (1-indexed), using the default confidence threshold. `null` when the document could not be inspected; empty when no page qualifies. |
| `layoutGatedPages` | `\[UInt32\]?` | `[]` | Pages the `auto` layout strategy skipped (1-indexed). `null` unless layout detection ran with `LayoutStrategy.Auto`; empty when the gate selected every page. |
| `layoutGateReasons` | `\[String\]?` | `[]` | Why the `auto` layout gate selected or skipped each page. Index `i` is page `i + 1`. Snake_case values such as `multi_column`, `table_grid`, or `plain_text` (the skip reason). `null` unless layout detection ran with `LayoutStrategy.Auto`. |
...
| `enabledProcessors` | `\[String\]?` | `null` | Whitelist of processor names to run (None = all enabled) |
| `disabledProcessors` | `\[String\]?` | `null` | Blacklist of processor names to skip (None = none disabled) |
...
| `sharedDoc` | `Bool?` | `null` | Shared document flag |
| `hyperlinksChanged` | `Bool?` | `null` | Hyperlinks changed flag |
| `slides` | `Int32?` | `null` | Number of slides |
| `notes` | `Int32?` | `null` | Number of notes |
| `hiddenSlides` | `Int32?` | `null` | Number of hidden slides |
| `multimediaClips` | `Int32?` | `null` | Number of multimedia clips |
| `presentationFormat` | `String?` | `null` | Presentation format (e.g., "Widescreen", "Standard") |
| `slideTitles` | `\[String\]` | `[]` | Slide titles |
...
| `imageCount` | `UInt32?` | `null` | Number of embedded images |
| `tableCount` | `UInt32?` | `null` | Number of tables |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `categories` | `\[PiiCategory\]` | `[]` | Categories to redact. Empty means "every category supported by the engine." |
...
```swift
public static func loadEmbedded() throws -> Registry
```

**Example:**

```swift
let result = try Registry.loadEmbedded()
```
...
```swift
public static func global() -> Registry
```
...
```swift
public func get(id: String) -> Preset?
```
...
```swift
public func summaries() -> [PresetSummary]
```
...
```swift
public func sampleBytes(presetId: String, name: String) -> Data?
```

**Example:**

```swift
let result = instance.sampleBytes("value", "value")
```
...
```swift
public func extendFromDir(dir: URL) throws -> UInt
```

**Example:**

```swift
let result = try instance.extendFromDir("value")
```
...
| `topK` | `UInt?` | `null` | Return at most this many documents. `null` returns all. Applied after sorting by score, so the highest-scoring documents are kept. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `content` | `\[DiffLine\]` | `[]` | Line-level content changes for this revision. |
| `tableChanges` | `\[CellChange\]` | `[]` | Cell-level table changes for this revision. |
| `propertyChanges` | `\[PropertyChange\]` | `[]` | Formatting or metadata property changes for this revision. |
...
| `values` | `\[Float\]` | — | Weights parallel to `SparseEmbedding.indices`. |
...
| `maxEmbedDurationSecs` | `UInt64?` | `60` | Maximum wall-clock duration (in seconds) for a single embed call when using `SparseEmbeddingModelType.Plugin`. `null` disables the timeout. |
...
| `metadata` | `\[String: String\]` | — | Key-value metadata extracted from recognized text fields. |
...
| `maxTokens` | `UInt32?` | `null` | Maximum summary length in tokens. `null` lets the backend pick a default. |
...
| `headingLevel` | `UInt8?` | `null` | Outline level 1-6 when the style resolves to a heading, otherwise `null`. |
...
| `links` | `\[MarkdownLink\]?` | `null` | Markdown links (Markdown files only). |
| `codeBlocks` | `\[MarkdownCodeBlock\]?` | `null` | Code blocks (Markdown files only). |
...
| `links` | `\[MarkdownLink\]?` | `[]` | Markdown links (for Markdown files). |
| `codeBlocks` | `\[MarkdownCodeBlock\]?` | `[]` | Code blocks (for Markdown files). |
...
| `customStopwords` | `\[String: \[String\]\]?` | `null` | Per-language custom stopword lists (`language_code → stopword_list`). |
| `preservePatterns` | `\[String\]` | `[]` | Regex patterns whose matched text is always preserved unchanged. |
...
```swift
public func countTokens(text: String) -> UInt
```

**Example:**

```swift
let result = instance.countTokens("value")
```
...
| `maxDurationMs` | `UInt64?` | `1800000` | Hard safety limit on input duration (milliseconds). Files longer than this are rejected after decode, before model work. Default: 30 minutes. Set to `null` to disable (not recommended for untrusted input). |
...
| `modelCacheDir` | `URL?` | `null` | Optional alternate Hugging Face cache root for Whisper models. When unset, hf-hub follows `HF_HUB_CACHE`, `HUGGINGFACE_HUB_CACHE`, `HF_HOME`, XDG, and platform defaults. Files remain in the standard content-addressed snapshot layout and are not copied into an Xberg cache. |
...
| `chunkMaxSize` | `UInt?` | `null` | Maximum chunk size in bytes. `null` disables chunking. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `pageRanges` | `\[PageRange\]?` | `[]` | User-specified page ranges (overrides automatic chunking). |
| `pagesPerChunk` | `UInt32?` | `null` | User-specified pages per chunk (overrides automatic calculation). |
...
| `hyperlinksChanged` | `Bool?` | `null` | Hyperlinks changed flag |
| `company` | `String?` | `null` | Company name |
| `worksheetNames` | `\[String\]` | `[]` | Worksheet names |
...
| `Tokenizer` | Size measured in tokens from a HuggingFace tokenizer or a registered tokenizer backend. — Fields: `model`: `String`, `cacheDir`: `URL` |
...
| `TextLayerDetected` | PDF has substantial text layer (OCR not needed). — Fields: `textCoverage`: `Float`, `avgCharsPerPage`: `UInt32` |
