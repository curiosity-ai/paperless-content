...
<!-- alef:hash:e6dc73e828e5b9f9114a4e2393b242e4c5719cda2a956c1614a466f961e2ddba -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Elixir API Reference <span class="version-badge">v1.1.3</span>
...
```elixir
@spec extract(input, config) :: {:ok, term()} | {:error, term()}
...
```elixir
@spec extract_batch(inputs, config) :: {:ok, term()} | {:error, term()}
def extract_batch(inputs, config)
```

**Example:**

```elixir
{:ok, result} = extract_batch([], %{})
```
...
```elixir
@spec map_url(uri, config) :: {:ok, term()} | {:error, term()}
def map_url(uri, config)
```

**Example:**

```elixir
{:ok, result} = map_url("value", %{})
```
...
- `Ok(list(String.t()))` - Vector of post-processor names
...
```elixir
@spec clear_tokenizer_backends() :: {:ok, term()} | {:error, term()}
def clear_tokenizer_backends()
```

**Example:**

```elixir
:ok = clear_tokenizer_backends()
```
...
```elixir
@spec list_tokenizer_backends() :: {:ok, term()} | {:error, term()}
def list_tokenizer_backends()
```

**Example:**

```elixir
{:ok, result} = list_tokenizer_backends()
```
...
```elixir
@spec classify_chunks_owned(result, config) :: {:ok, term()} | {:error, term()}
def classify_chunks_owned(result, config)
```

**Example:**

```elixir
{:ok, result} = classify_chunks_owned(%{}, %{})
```
...
```elixir
@spec max_sim_score(query, doc) :: {:ok, term()} | {:error, term()}
def max_sim_score(query, doc)
```

**Example:**

```elixir
{:ok, result} = max_sim_score(%{}, %{})
```
...
```elixir
@spec max_sim_rank(query, docs) :: {:ok, term()} | {:error, term()}
def max_sim_rank(query, docs)
```

**Example:**

```elixir
{:ok, result} = max_sim_rank(%{}, [])
```
...
```elixir
@spec doctor(config) :: {:ok, term()} | {:error, term()}
def doctor(config)
```
...
| `access_key_id` | `String.t() \| nil` | `nil` | Explicit AWS access key ID. Secret — never logged. |
| `secret_access_key` | `String.t() \| nil` | `nil` | Explicit AWS secret access key. Secret — never logged. |
| `session_token` | `String.t() \| nil` | `nil` | Explicit AWS session token for temporary credentials. Secret — never logged. |
...
| `min_image_area` | `integer()` | `1000` | Skip images whose `width * height` is below this threshold (in pixels). Default `1_000` filters out icons and decorations. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `prompt_template` | `String.t() \| nil` | `nil` | Minijinja prompt template. Receives `{{ definitions }}` (rendered label + description list) and `{{ chunks }}` (a numbered list of chunk texts in the current batch) variables. `nil` lets the backend pick a sensible default. |
...
| `batch_size` | `integer()` | `10` | Number of chunks batched into a single LLM request. Larger batches amortize the fixed prompt cost (definitions block) across more chunks, at the risk of exceeding the model's context window for very large taxonomies or chunk texts. Defaults to `DEFAULT_BATCH_SIZE`. |
| `max_concurrency` | `integer()` | `4` | Maximum number of in-flight batch requests. Bounds concurrency against the configured LLM provider. Defaults to `DEFAULT_MAX_CONCURRENCY`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `label` | `String.t()` | — | Label name returned in `ChunkMetadata.classifications`. |
| `description` | `String.t()` | — | Semantic description of when this label applies. Injected verbatim into the classification prompt next to the label name. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `text` | `String.t()` | — | The raw source text of this chunk. |
| `context_path` | `list(String.t())` | — | Hierarchical path of enclosing structural items (e.g. `["MyClass", "my_method"]`). |
| `node_types` | `list(String.t())` | — | Tree-sitter node kinds that appear at the top level of this chunk (e.g. `"function_definition"`, `"class_definition"`). |
| `byte_start` | `integer()` | — | Inclusive start byte offset of this chunk in the original source. |
| `byte_end` | `integer()` | — | Exclusive end byte offset of this chunk in the original source. |
...
| `byte_start` | `integer()` | — | Inclusive start byte offset of the `name="value"` attribute token. |
| `byte_end` | `integer()` | — | Exclusive end byte offset of the `name="value"` attribute token. |
...
| `key` | `String.t() \| nil` | `/* serde(default) */` | Key, attribute name, tag name, or positional index (`"0"`, `"1"`, …). `nil` at the document root. |
| `value` | `String.t() \| nil` | `/* serde(default) */` | Leaf scalar value, if any. `nil` for containers (objects, arrays, XML elements with child elements). |
...
| `byte_start` | `integer()` | — | Inclusive start byte offset of this node in the original source. |
| `byte_end` | `integer()` | — | Exclusive end byte offset of this node in the original source. |
...
| `data` | `CodeDataNode \| nil` | `nil` | Hierarchical key/value data tree extracted from data-format source (JSON, YAML, TOML, XML, CSV, etc.), when data extraction was enabled. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `max_threads` | `integer() \| nil` | `nil` | Maximum number of threads for all internal thread pools. Caps Rayon global pool size, ONNX Runtime intra-op threads, and the combined document/inner-task budget for batch extraction. When `nil`, the effective budget is `min(detected_cpu_cores, 8)` unless a Linux cgroup CPU quota is present, in which case the quota is used as the ceiling instead. On hosts with more than 8 cores and no cgroup quota, set `max_threads` explicitly to use the additional cores — the default will not scale past 8 on its own. |
...
| `include_footnotes` | `boolean()` | `false` | Include footnote bodies in extraction output. - PDF: Prevents the layout model from treating `Footnote`-classified regions as furniture, so footnote bodies survive alongside the main text instead of being silently dropped. - Other formats: No effect currently. Default: `false` (footnotes are stripped), matching the existing `include_headers` / `include_footers` defaults. |
...
| `output_format` | `HtmlToMarkdownRsOutputFormat` | `:markdown` | Target output format (Markdown, plain text, etc.). |
...
| `max_depth` | `integer() \| nil` | `nil` | Maximum DOM traversal depth. `nil` uses the library's internal native-stack safety limit. Explicit values above that safety limit are clamped to prevent process-aborting stack overflows on pathologically deep DOM trees. |
...
| `max_links_per_page` | `integer() \| nil` | `nil` | Maximum links enqueued from a single page. Defaults to 10000. Bounds the work one hostile or pathological page can create; links past the cap are dropped and a warning is logged. |
| `max_concurrent` | `integer() \| nil` | `nil` | Maximum number of concurrent requests. |
| `crawl_strategy` | `CrawlStrategyKind` | `:bfs` | Traversal order. Defaults to breadth-first. A frontier or strategy set explicitly on `CrawlEngineBuilder` takes precedence over this field. |
| `content_filter` | `ContentFilterKind \| nil` | `nil` | Content filter applied to each page. `nil` keeps every page. A content filter set explicitly on `CrawlEngineBuilder` takes precedence. |
| `bm25_query` | `String.t() \| nil` | `nil` | Query the BM25 content filter scores pages against. Required by `ContentFilterKind.Bm25`. |
| `bm25_threshold` | `float() \| nil` | `nil` | Minimum BM25 score a page must reach to be kept. Defaults to `0.0`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `delimiter` | `String.t() \| nil` | `nil` | Field delimiter, as a single-character string (e.g. `","`, `";"`, `"\t"`, `"\|"`). When `nil` (default), the delimiter is auto-detected from a sample of the file. Must be exactly one ASCII byte when set — `ExtractionConfig.validate` rejects an empty string or a multi-byte value with a helpful error. The TSV MIME type (`text/tab-separated-values`) always forces `\t` regardless of this setting. |
| `comment_prefixes` | `list(String.t())` | `[]` | Line prefixes that mark a comment line to skip entirely during row parsing (e.g. `["#"]`). A line is treated as a comment when its trimmed start matches any of these prefixes exactly. Default: empty, meaning no line is treated as a comment (matches the pre-existing extractor behavior). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `model_path` | `String.t() \| nil` | `nil` | Local DeepSeek-OCR model directory. The backend requires this option. |
| `device` | `CandleDevicePreference \| nil` | `nil` | Optional per-call device override. |
| `version` | `integer() \| nil` | `nil` | DeepSeek-OCR model generation, either 1 or 2. Defaults to 2. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `identifier` | `String.t()` | — | Element identifier used by the Djot attribute map. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `String.t()` | — | Check identifier, e.g. `ocr.tesseract` or `layout.rtdetr`. |
| `status` | `ProbeStatus` | — | Pass / warn / fail / skip verdict. |
| `message` | `String.t()` | — | One-line reason or detail (e.g. missing language, resolved path, error). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `pages` | `integer()` | — | Total pages in the source document (`0` when not page-addressable). |
| `tables` | `integer()` | — | Tables detected in the document. |
| `images` | `integer()` | — | Images detected in the document. |
...
```elixir
def extract(obj, input, config)
```

**Example:**

```elixir
{:ok, result} = instance.extract(%{}, %{})
```
...
```elixir
def can_handle(obj, path, mime_type)
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `mime_type` | `String.t()` | — | MIME type of the document. |
| `size_bytes` | `integer()` | — | File size in bytes. |
| `page_count` | `integer() \| nil` | `nil` | Page count (if known, e.g., from previous analysis). |
| `force_ocr` | `boolean()` | — | Whether OCR is forced regardless of text layer. |
| `user_chunk_config` | `UserChunkConfig \| nil` | `nil` | User-provided chunk configuration overrides. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `element_id` | `String.t()` | `/* serde(default) */` | Deterministic element identifier. Empty only when deserializing legacy payloads that predate this field's wire representation. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `model` | `EmbeddingModelType` | `:preset { name: "balanced" }` | The embedding model to use (defaults to "gte-modernbert-base" preset if not specified) |
...
| `show_download_progress` | `boolean()` | `false` | Show model download progress. When enabled, transfer progress for the model, tokenizer and config files is reported at `info` level on the `xberg.model_download` target while they download (#279). Covers both local backends (ONNX and static/model2vec). A warm Hugging Face cache transfers nothing and so reports nothing. Ignored by `EmbeddingModelType.Llm` and `EmbeddingModelType.Plugin`, which download no model. |
| `cache_dir` | `String.t() \| nil` | `nil` | Optional alternate Hugging Face cache root for model files. When unset, hf-hub follows `HF_HUB_CACHE`, `HUGGINGFACE_HUB_CACHE`, `HF_HOME`, XDG, and platform defaults. Prefer those environment variables when configuring the cache process-wide. |
...
| `start` | `integer()` | — | Byte-offset span in `ExtractedDocument.content` where the mention starts. |
| `end` | `integer()` | — | Byte-offset span in `ExtractedDocument.content` where the mention ends (exclusive). |
...
| `bytes` | `binary() \| nil` | `nil` | Raw bytes for `kind = "bytes"`. |
| `uri` | `String.t() \| nil` | `nil` | Local path, `file://` URI, or HTTP(S) URL for `kind = "uri"`. |
| `mime_type` | `String.t() \| nil` | `nil` | MIME type hint. |
| `filename` | `String.t() \| nil` | `nil` | Filename hint used for MIME detection and metadata. |
| `config` | `FileExtractionConfig \| nil` | `nil` | Per-input extraction overrides. |
...
| `detected_language_confidences` | `list(LanguageConfidence) \| nil` | `[]` | Structured per-language detection results: confidence, document share, script, and reliability, alongside the ISO-code-only `detected_languages` (#261). One entry per language in `detected_languages`, in the same order. `nil` under the same conditions as `detected_languages`: detection disabled, empty input text, or no language met the configured `min_confidence`. |
...
| `ocr_result` | `ExtractedDocument \| nil` | `nil` | Nested OCR extraction result (if image was OCRed) When OCR is performed on this image, the result is embedded here rather than in a separate collection, making the relationship explicit. |
...
| `caption` | `String.t() \| nil` | `nil` | VLM-generated caption describing the image, when captioning is configured. Populated by the captioning post-processor (`crates/xberg/src/plugins/processor/builtin/captioning.rs`), which routes each image through `crate.llm.region_extractor.extract_region_with_vlm` in caption mode. `nil` when captioning is disabled or the VLM declined to caption. |
| `qr_codes` | `list(QrCode) \| nil` | `[]` | QR codes decoded from this image, when QR detection is enabled. Populated by the QR post-processor (`crates/xberg/src/extractors/qr.rs`) via the pure-Rust `rqrr` decoder. `nil` when QR detection is disabled; an empty `Some([])` when detection ran but found nothing. |
| `data_base64` | `String.t() \| nil` | `nil` | Base64-encoded copy of `data`; populated when `ImageExtractionConfig.include_data_base64` is `true`. Omitted from JSON by default; use instead of `data` in JSON-only clients. |
...
| `ocr_aggregate` | `float() \| nil` | `nil` | Mean OCR per-element recognition confidence when OCR ran; `nil` when it did not. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `mime_detection_policy` | `MimeDetectionPolicy` | `:prefer_content` | Controls whether MIME inference prefers content, a supported extension, or content alone. |
...
| `ocr` | `OcrConfig \| nil` | `nil` | OCR configuration. `nil` does not run OCR for documents that already have usable text. Under `OcrStrategy.Auto`, a PDF with no text layer at all (a scan) is still routed to OCR with default settings so it is not returned empty (#1338). Set `Self.disable_ocr` to hard-disable OCR regardless of the detected content. |
| `force_ocr` | `boolean()` | `false` | Force OCR even for searchable PDFs |
| `ocr_strategy` | `OcrStrategy` | `:auto` | Which pages get OCR'd when neither `force_ocr` nor `force_ocr_pages` applies. Defaults to `OcrStrategy.Auto`, which OCRs only pages whose native text fails a quality check. Only applies to PDF documents. Cannot be `OcrStrategy.ScannedPages` while `disable_ocr` is `true`. |
...
| `disable_ocr` | `boolean()` | `false` | Disable OCR entirely, even for images. When `true`, OCR is skipped for all document types. Images return metadata only (dimensions, format, EXIF) without text extraction. PDFs use only native text extraction without OCR fallback. Cannot be `true` simultaneously with `force_ocr`. |
...
| `extraction_timeout_secs` | `integer() \| nil` | `600` | Default per-file timeout in seconds for batch extraction. When set, each file in a batch will be canceled after this duration unless overridden by `FileExtractionConfig.timeout_secs`. Defaults to `Some(600)` (10 minutes) to prevent pathological files (e.g. deeply nested archives, documents with millions of cells) from running indefinitely and exhausting caller resources, while still giving slow paths (VLM-based OCR, large scanned documents) enough headroom to finish. Set to `nil` to disable the timeout for trusted input or long-running workloads. |
| `max_concurrent_extractions` | `integer() \| nil` | `nil` | Maximum concurrent document extractions in batch operations. This is a ceiling within the configured total thread budget, not an independent pool size. When unset, the scheduler derives document and per-document concurrency from `ConcurrencyConfig.max_threads`. |
...
| `csv` | `CsvConfig \| nil` | `nil` | CSV/TSV extraction configuration (None = use defaults). Lets callers set an explicit delimiter and declare comment-line prefixes to skip, instead of relying solely on delimiter auto-detection. See `CsvConfig` for details. |
| `geojson` | `GeoJsonExtractionConfig \| nil` | `nil` | GeoJSON extraction configuration (None = bounded summary). By default, GeoJSON coordinates are replaced by aggregate counts and bounds so large geometry arrays do not become unbounded rendered output. Set `include_full_coordinates` explicitly to retain the legacy full-coordinate output. |
| `concurrency` | `ConcurrencyConfig \| nil` | `nil` | Concurrency limits for constrained environments (None = use defaults). Controls Rayon thread pool size, ONNX Runtime intra-op threads, and the combined document/inner-task budget for batch extraction. See `ConcurrencyConfig` for details. |
| `url` | `UrlExtractionConfig` | — | URL ingestion and crawl configuration. |
| `max_archive_depth` | `integer()` | `3` | Maximum recursion depth for archive extraction (default: 3). Set to 0 to disable recursive extraction (legacy behavior). |
...
| `structured_extraction` | `StructuredExtractionConfig \| nil` | `nil` | Structured extraction via LLM (None = disabled). When set, the extracted document content is sent to an LLM with the provided JSON schema. The structured response is stored in `ExtractedDocument.structured_output`. |
| `ner` | `NerConfig \| nil` | `nil` | Named-entity recognition configuration. When set, the NER post-processor runs at the Middle stage and populates `ExtractedDocument.entities`. |
| `redaction` | `RedactionConfig \| nil` | `nil` | Redaction / anonymisation configuration. When set, the redaction post-processor runs at the Late stage and rewrites every textual field in `ExtractedDocument`, emitting an audit trail in `ExtractedDocument.redaction_report`. |
| `summarization` | `SummarizationConfig \| nil` | `nil` | Summarisation configuration. When set, the summarisation post-processor runs at the Middle stage and populates `ExtractedDocument.summary`. |
| `translation` | `TranslationConfig \| nil` | `nil` | Translation configuration. When set, the translation post-processor runs at the Middle stage and populates `ExtractedDocument.translation`. |
| `page_classification` | `PageClassificationConfig \| nil` | `nil` | Per-page classification configuration. When set, the classification post-processor runs at the Middle stage and populates `ExtractedDocument.page_classifications`. |
| `chunk_classification` | `ChunkClassificationConfig \| nil` | `nil` | Per-chunk multi-label classification configuration. When set, the chunk-classification post-processor runs at the Middle stage (after chunking) and populates `ChunkMetadata.classifications` on every chunk. |
...
| `error_type` | `String.t()` | — | Stable snake_case error kind. |
| `source` | `String.t()` | — | Best-effort source identifier. |
...
| `crawl_final_urls` | `list(String.t())` | `[]` | Final URLs reached after redirects during URL ingestion. |
| `crawl_redirect_count` | `integer()` | — | Total redirects followed while fetching or crawling URLs. |
| `crawl_unique_normalized_urls` | `list(String.t())` | `[]` | Unique normalized URLs discovered by crawls. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `mime_detection_policy` | `MimeDetectionPolicy \| nil` | `nil` | Override MIME inference policy for this file. |
...
| `ocr_strategy` | `OcrStrategy \| nil` | `nil` | Override the OCR page-selection strategy for this file. |
...
| `html_output` | `HtmlOutputConfig \| nil` | `nil` | Override styled HTML output configuration for this file. |
...
| `transcription` | `TranscriptionConfig \| nil` | `nil` | Transcription configuration (see ExtractionConfig for docs). |
...
| `url` | `UrlExtractionConfig \| nil` | `nil` | Override URL ingestion and crawl configuration for this file. |
| `ner` | `NerConfig \| nil` | `nil` | Override named-entity recognition configuration for this file. |
| `redaction` | `RedactionConfig \| nil` | `nil` | Override redaction configuration for this file. |
| `summarization` | `SummarizationConfig \| nil` | `nil` | Override summarization configuration for this file. |
| `translation` | `TranslationConfig \| nil` | `nil` | Override translation configuration for this file. |
| `page_classification` | `PageClassificationConfig \| nil` | `nil` | Override per-page classification configuration for this file. |
| `chunk_classification` | `ChunkClassificationConfig \| nil` | `nil` | Override per-chunk classification configuration for this file. |
| `captioning` | `CaptioningConfig \| nil` | `nil` | Override VLM captioning configuration for this file. |
| `qr_codes` | `boolean() \| nil` | `nil` | Override QR-code detection for this file. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `latex` | `String.t()` | — | LaTeX source of the formula, without surrounding `$$` delimiters. Markup converters and formula OCR produce real LaTeX. The native PDF layout path stores the plain text of a detected formula region, which keeps the original Unicode math characters instead of LaTeX commands. To render the formula in Markdown or other formats, wrap it in `$$..$$`. |
| `bbox` | `BoundingBox \| nil` | `/* serde(default) */` | Bounding box of the formula region on its page. `nil` for markup sources. PDF OCR sources report PDF point coordinates with the origin at the bottom-left of the page, comparable to native PDF geometry. Image sources, and PDF pages whose geometry is unavailable, report pixels of the image the OCR backend saw. The C FFI reports an absent bbox as a null pointer. |
| `page` | `integer() \| nil` | `/* serde(default) */` | 1-indexed page number the formula appears on. `nil` when the source format has no page concept. The C FFI reports an absent page as `0`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `include_full_coordinates` | `boolean()` | — | Include every coordinate in rendered content and `flattened_fields` metadata. Defaults to `false`. When false, extraction reports feature, property, geometry, position-count, bounds, truncation, and discarded-category metadata and emits a `ProcessingWarning` for every GeoJSON input. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `task` | `GlmOcrTaskKind \| nil` | `nil` | Optional recognition task; the backend constructor's task is used when absent. |
| `device` | `CandleDevicePreference \| nil` | `nil` | Optional per-call device override. |
| `layout_mode` | `GlmOcrLayoutMode \| nil` | `nil` | Optional whole-page or layout-paired dispatch mode. |
| `enable_chart_understanding` | `boolean() \| nil` | `nil` | Whether chart regions use chart understanding instead of captioning. |
| `cache_dir` | `String.t() \| nil` | `nil` | Optional Hugging Face cache root. |
...
| `row_span` | `integer()` | `1` | Number of rows this cell spans. |
| `col_span` | `integer()` | `1` | Number of columns this cell spans. |
...
| `bbox` | `HierarchicalBoundingBox \| nil` | `nil` | Bounding box information for the block Contains left, top, right, and bottom coordinates in PDF units. |
...
| `k_clusters` | `integer()` | `3` | Number of font size clusters to use for hierarchy levels (1-7) Default: 3, which provides two heading levels plus body text. Larger values create more fine-grained hierarchy levels. |
...
| `class_prefix` | `String.t()` | `"kb-"` | CSS class prefix applied to every emitted class name. Default: `"kb-"`. Change this if your host application already uses classes that start with `kb-`. |
...
| `inject_placeholders` | `boolean()` | `true` | Whether to inject image reference placeholders into markdown output. When `true` (default), image references like `!Image 1` are appended to the markdown. Set to `false` to extract images as data without polluting the markdown output. |
...
| `dimensions` | `ImageDimensions \| nil` | `nil` | Image dimensions if available. |
...
| `contrast_enhance` | `boolean()` | `false` | Enhance contrast for better text visibility. With `none` or `off` binarization, this applies background normalization and sharpening while preserving grayscale pixels. |
| `binarization_method` | `String.t()` | `"otsu"` | Binarization method: "none" (alias "off"), "otsu", "sauvola", or "adaptive". `deskew` must be `false` when this is `none` or `off`. |
...
| `new_dimensions` | `PixelDimensions \| nil` | `nil` | New dimensions after resizing (if resized). |
...
| `language` | `String.t() \| nil` | `"en"` | Language code for stopword filtering (e.g., "en", "de", "fr"). If None, no stopword filtering is applied. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `language` | `String.t()` | — | ISO 639-3 language code, matching the corresponding entry in `detected_languages`. |
| `confidence` | `float()` | — | Confidence for this language, in `[0.0, 1.0]`. In single-language mode this is whatlang's `Info.confidence()` for the whole document. In multi-language mode this is the average whatlang confidence across the document's 200-character chunks that were classified as this language. |
| `proportion` | `float()` | — | Share of the document's analyzed content classified as this language, in `[0.0, 1.0]`. In single-language mode this is always `1.0`. In multi-language mode this is the fraction of 200-character chunks classified as this language (chunks that did not meet `min_confidence` for any language are excluded from the count but still count toward the denominator). |
| `script` | `String.t()` | — | Writing system whatlang detected for this language (e.g. `"Latin"`, `"Cyrillic"`). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `model` | `LateInteractionModelType` | `:preset` | The late-interaction model to use (defaults to the "gte-moderncolbert" preset). |
| `batch_size` | `integer()` | `16` | Batch size for local ONNX inference. ColBERT emits a `[seq, dim]` multi-vector embedding per document, so memory scales with batch size — keep this modest. |
| `max_length` | `integer()` | `512` | Maximum token sequence length for the tokenizer (documents). |
| `query_max_length` | `integer()` | `32` | Fixed padded length for query augmentation. ColBERT queries are padded (with the mask token, kept attention-live) to exactly this many tokens rather than truncated/left as-is — this is the "query augmentation" trick from the ColBERT paper. |
| `show_download_progress` | `boolean()` | `false` | Show model download progress (local ONNX path only). When enabled, transfer progress for the model, tokenizer and config files is reported at `info` level on the `xberg.model_download` target while they download (#279). A warm Hugging Face cache transfers nothing and so reports nothing. Ignored by `LateInteractionModelType.Plugin`, which downloads no model. |
| `cache_dir` | `String.t() \| nil` | `nil` | Optional alternate Hugging Face cache root for model files. When unset, hf-hub follows the standard Hugging Face environment and platform cache conventions. |
| `acceleration` | `AccelerationConfig \| nil` | `nil` | Hardware acceleration for the late-interaction ONNX model. |
| `max_embed_duration_secs` | `integer() \| nil` | `60` | Maximum wall-clock duration (in seconds) for a single embed call when using `LateInteractionModelType.Plugin`. `nil` disables the timeout. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `index` | `integer()` | — | Position of this document in the original input slice. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `String.t()` | — | Stable preset name referenced from config. |
| `model_repo` | `String.t()` | — | HuggingFace repository hosting the ONNX model. |
| `model_file` | `String.t()` | — | Path to the ONNX file within the repo. |
| `additional_files` | `list(String.t())` | — | Sibling files that must be downloaded alongside `model_file`. |
| `max_length` | `integer()` | — | Maximum document token sequence length. |
| `query_max_length` | `integer()` | — | Fixed padded query length (ColBERT query augmentation). |
| `dim` | `integer()` | — | Per-token embedding dimensionality. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `strategy` | `LayoutStrategy` | `:always` | Which pages the layout model runs on. Defaults to `LayoutStrategy.Always`, the historical behavior: every page is rendered and inferred. `LayoutStrategy.Auto` pre-screens pages with cheap signals and skips the model where it cannot help. |
...
| `formula_model` | `FormulaModel \| nil` | `nil` | Formula recognition model for layout-detected formula regions. `nil` (the default) keeps the plain OCR text of the region. Setting a model converts each formula region crop to LaTeX. Requires the `formula-recognition` feature; without it the setting is ignored. |
| `table_overlap_preference` | `TableOverlapPreference` | `:content` | How to resolve overlapping native vs layout tables. When a native table and a layout (TATR/SLANeXT) table overlap on the same region, this controls which one is kept. Defaults to `TableOverlapPreference.Content` (historical behavior: keep the table with more content). Set to `TableOverlapPreference.Native` to favor source reading order (higher text F1) over the model's cell reflow. |
...
| `timeout_secs` | `integer() \| nil` | `nil` | Request timeout in seconds. When `nil`, liter-llm's built-in 60s default applies, except the VLM OCR path which uses a 300s default (a single page image transcription routinely exceeds 60s). Set explicitly to override. |
...
| `frequency_penalty` | `float() \| nil` | `nil` | Frequency penalty for generation tasks, applied to individual requests built from this config. Positive values discourage the model from repeating the same tokens verbatim. Validated to `[-2.0, 2.0]` by `LlmConfig.validate`. Mirrors liter-llm's `ChatCompletionRequest.frequency_penalty`. |
| `reasoning_effort` | `String.t() \| nil` | `nil` | Reasoning effort level for extended-thinking models, applied to individual requests built from this config. Mirrors liter-llm's `ChatCompletionRequest.reasoning_effort` (`types.chat.ReasoningEffort`). A request-time parameter like `temperature`/ `max_tokens` above, not a client-level setting — `into_client_builder` does not map it. Accepted as a plain string — one of `"low"`, `"medium"`, `"high"`, `"minimal"`, `"max"` (case-insensitive; liter-llm's own `#[serde(rename_all = "lowercase")]` spelling) — rather than importing liter-llm's enum, because this module compiles even when the `liter-llm` feature is disabled. See `llm.client.parse_reasoning_effort` for the conversion into `liter_llm.ReasoningEffort`. |
| `extra_body` | `term() \| nil` | `nil` | Provider-specific extra parameters merged into the request body (guardrails, safety settings, grounding config, etc.), applied to individual requests built from this config. Mirrors liter-llm's `ChatCompletionRequest.extra_body`. A request-time parameter like `temperature`/`max_tokens` above, not a client-level setting. |
| `load_env` | `boolean() \| nil` | `nil` | Whether liter-llm should load provider credentials from environment variables. Mirrors liter-llm's `ClientConfigBuilder.load_env`. When `nil`, liter-llm's own default behavior applies. |
| `headers` | `map() \| nil` | `%{}` | Extra HTTP headers sent with every request to the provider. Mirrors liter-llm's `ClientConfigBuilder.header`, for gateways or providers that require custom auth/routing headers. |
| `providers` | `list(LlmProviderConfig) \| nil` | `[]` | Custom provider configurations, in addition to liter-llm's built-in providers. Mirrors liter-llm's `LlmConfig.providers`, for OpenAI-compatible gateways and self-hosted model servers that are not in the built-in provider catalog. |
...
| `budget` | `LlmBudgetConfig \| nil` | `nil` | Budget enforcement configuration. Mirrors liter-llm's `LlmConfig.budget`. Only takes effect when liter-llm's `tower` feature is compiled in; otherwise the value is accepted but unused. |
| `rate_limit` | `LlmRateLimitConfig \| nil` | `nil` | Per-model rate limiting configuration. Mirrors liter-llm's `LlmConfig.rate_limit`. Only takes effect when liter-llm's `tower` feature is compiled in; otherwise the value is accepted but unused. |
| `cost_tracking` | `boolean() \| nil` | `nil` | Enable per-request cost tracking. Mirrors liter-llm's `LlmConfig.cost_tracking`. Only takes effect when liter-llm's `tower` feature is compiled in; otherwise the value is accepted but unused. |
| `tracing` | `boolean() \| nil` | `nil` | Enable OpenTelemetry-compatible tracing spans. Mirrors liter-llm's `LlmConfig.tracing`. Only takes effect when liter-llm's `tower` feature is compiled in; otherwise the value is accepted but unused. |
| `cooldown_secs` | `integer() \| nil` | `nil` | Cooldown duration after transient errors, in seconds. Mirrors liter-llm's `LlmConfig.cooldown_secs`. Only takes effect when liter-llm's `tower` feature is compiled in; otherwise the value is accepted but unused. |
| `health_check_secs` | `integer() \| nil` | `nil` | Background health check interval, in seconds. Mirrors liter-llm's `LlmConfig.health_check_secs`. Only takes effect when liter-llm's `tower` feature is compiled in; otherwise the value is accepted but unused. |
| `bedrock` | `BedrockConfig \| nil` | `nil` | AWS Bedrock settings (region, cross-region routing, explicit credentials). Only consulted for `bedrock/`-prefixed models. When `nil` — or when an individual field inside it is `nil` — liter-llm falls back to the standard AWS environment variables and the default credential chain. |
| `credential_provider` | `CredentialProviderConfig \| nil` | `nil` | Managed OAuth2/STS credential provider for auth modes liter-llm cannot express via a static `api_key` — Azure AD, Vertex AI OAuth2, Vertex AI Application Default Credentials, and AWS STS Web Identity (EKS IRSA) for Bedrock. Mirrors liter-llm's `client.ClientConfigBuilder.credential_provider`, which takes an `Arc<dyn liter_llm.auth.CredentialProvider>` trait object — that cannot appear in a serde DTO. Every `CredentialProviderConfig` variant is plain data instead, so it round-trips through TOML/JSON/YAML and every language binding like the rest of `LlmConfig`. Managed credential providers are unavailable on `wasm32`, where liter-llm uses browser HTTP rather than its native authentication modules. `LlmConfig.validate` rejects a configured provider on that target instead of silently ignoring it. GitHub Copilot's device-flow provider has no variant here: it takes no configuration at all (`liter_llm.auth.github_copilot.GithubCopilotCredentialProvider.new` accepts only an HTTP client) and drives an interactive terminal prompt, so it cannot be expressed as data. A Rust embedder who needs it — or any other fully custom `CredentialProvider` — can call `xberg.llm.client.create_client_with_credential_provider` directly with a `liter-llm` dependency of their own. |
| `max_concurrency` | `integer() \| nil` | `nil` | Maximum number of simultaneously in-flight requests to the LLM provider this config resolves to. This is a real, global bound on *provider* concurrency, not a per-extraction allowance: `xberg.llm.client.create_client` shares one process-wide client instance per distinct resolved config, so every concurrent extraction that resolves to the same config shares the one in-flight-request limit this value configures, instead of each minting its own (GH#1465). `nil` means unlimited. PDF and image OCR batch sizing are **not** derived from this field, even when the configured OCR backend or `vlm_fallback` policy can reach a VLM — those call sites mix CPU-bound raster/OCR work with, at most, occasional remote requests, so they size their batches from the general thread budget (`max_threads`) unconditionally (GH#1465). Captioning is the one feature that *additionally* uses this value to bound its own per-extraction async request fan-out (issuing only VLM requests, with no CPU batching to protect), on top of the global provider-side limit described above; that per-extraction bound clamps a value below 1 up to 1. The global provider-side limit does not clamp: `Some(0)` reaches liter-llm as-is, and liter-llm rejects it when building the client (zero permitted in-flight requests is never useful), so it surfaces as a `create_client` error rather than a silent clamp to 1. This field is intentionally last to preserve positional constructor compatibility in generated language bindings. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `language` | `String.t()` | — | Declared language identifier, or an empty string when absent. |
| `code` | `String.t()` | — | Code block content. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `text` | `String.t()` | — | Visible link text. |
| `url` | `String.t()` | — | Link destination. |
...
```elixir
def compile(meta_schema_json)
```

**Example:**

```elixir
{:ok, result} = MetaSchema.compile("value")
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `meta_schema_json` | `String.t()` | Yes | The meta schema json |
...
```elixir
def parse_preset(obj, path, raw)
```

**Example:**

```elixir
{:ok, result} = instance.parse_preset("value", <<100, 97, 116, 97>>)
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `det_model` | `String.t()` | — | Exact path to the detection ONNX model in the Hugging Face snapshot. |
| `cls_model` | `String.t()` | — | Exact path to the classification ONNX model in the Hugging Face snapshot. |
| `rec_model` | `String.t()` | — | Exact path to the recognition ONNX model in the Hugging Face snapshot. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `num_tokens` | `integer()` | — | Number of attention-live token rows (padding rows are dropped, not zeroed — see `engine.normalize_tokens`). |
| `dim` | `integer()` | — | Dimensionality of each per-token vector. |
...
| `llm` | `LlmConfig \| nil` | `nil` | Optional LLM configuration — only used by `NerBackendKind.Llm`. Token usage for LLM backends is recorded in `ExtractedDocument.llm_usage`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `min` | `integer()` | `1` | Minimum number of words in a candidate. |
| `max` | `integer()` | `3` | Maximum number of words in a candidate. |
...
```elixir
def process_image(obj, image_bytes, config)
```
...
```elixir
def process_image_file(obj, path, config)
```
...
```elixir
def supports_language(obj, lang)
```
...
```elixir
def supports_table_detection(obj)
```
...
```elixir
def supports_document_processing(obj)
```
...
```elixir
def emits_structured_markdown(obj)
```

**Example:**

```elixir
{:ok, result} = instance.emits_structured_markdown()
```
...
```elixir
def confidence_semantics(obj)
```

**Example:**

```elixir
{:ok, result} = instance.confidence_semantics()
```
...
```elixir
def page_orientation_handling(obj)
```

**Example:**

```elixir
{:ok, result} = instance.page_orientation_handling()
```
...
```elixir
def process_document(obj, path, config)
```
...
| `backend` | `String.t()` | `"tesseract"` | OCR backend: tesseract, paddleocr, paddle-ocr, sceptre, or vlm. Sceptre uses ONNX Runtime on desktop/server and tract on supported mobile builds. Browser WebAssembly uses the separate byte-fed Sceptre worker API. |
| `language` | `list(String.t())` | `["eng"]` | Language code(s) for OCR recognition. Defaults to `["eng"]`. For Tesseract, languages are joined with "+". A list is the canonical form and the only form accepted by the binding object APIs (Python, Node, PHP, WASM, etc.): `["eng", "deu"]`. When deserializing from a config file, JSON body, or the REST/MCP API, a single string is also accepted, either as one code ("eng") or "+"-joined ("eng+deu"). |
...
| `security_limits` | `SecurityLimits \| nil` | `nil` | Security limits applied when decoding raw image bytes for OCR (GH#1554). Not user-configurable via config files — injected at runtime from `ExtractionConfig.security_limits` before each `process_image` call, the same pattern `Self.acceleration` uses. `ExtractionConfig.security_limits` is the single source of truth: this field only ever holds a copy the caller placed here immediately before dispatch, so the two cannot drift. A backend consulting a `backend_options` override for this call may still let that override win, but in the absence of one this field is what backends should fall back to instead of `SecurityLimits.default()`. `nil` means "use `SecurityLimits.default()`", never "disable the check". |
...
| `parent_id` | `String.t() \| nil` | `nil` | Parent element ID for hierarchical relationships. When hierarchy output is enabled, this resolves to another emitted element's `backend_metadata["element_id"]` value. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `include_elements` | `boolean()` | — | Whether to include OCR elements in the extraction result. When true, the `ocr_elements` field in `ExtractedDocument` will be populated. |
...
| `build_hierarchy` | `boolean()` | — | Whether to build hierarchical relationships between elements. When true, emitted elements receive an `element_id` metadata value and `parent_id` references are populated only when a spatially containing parent is also emitted. |
...
| `ocr_elements` | `list(OcrElement) \| nil` | `[]` | Structured OCR elements with bounding boxes and confidence scores. Available when TSV output is requested or table detection is enabled. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `backend` | `String.t()` | — | Backend name: "tesseract", "paddleocr", "paddle-ocr", "sceptre", "vlm", or a custom registered name. Sceptre uses ONNX Runtime on desktop/server and tract on Android/iOS; browser WebAssembly has a separate byte-fed engine because the normal async OCR registry assumes native model storage. |
| `priority` | `integer()` | `100` | Priority weight (higher = tried first). Stages are sorted by priority descending. |
| `language` | `list(String.t()) \| nil` | `/* serde(default) */` | Language override for this stage (None = use parent OcrConfig.language). A list is the canonical form and the only form accepted by the binding object APIs: `["eng", "deu"]`. When deserializing from a config file, JSON body, or the REST/MCP API, a single string is also accepted, either as one code ("eng") or "+"-joined ("eng+deu"). |
...
| `min_words_for_ocr_output_check` | `integer()` | `20` | Minimum word count before `Self.max_ocr_output_fragmented_word_ratio` may report a page. Short pages (a signature block, an exhibit title) are legitimately dominated by short words, so the ratio is not meaningful on them and the signal stays disabled. |
...
| `discard_suspected_ocr_noise` | `boolean()` | `false` | Discard non-empty OCR text when any configured recognition-noise signal fires. Defaults to `false`: suspected noise is retained and surfaced through `processing_warnings`. Set to `true` to preserve the legacy destructive behavior. |
...
| `det_limit_side_len` | `integer()` | `1024` | Maximum side length for detection image (default: 1024) Larger images may be resized to this limit for faster inference |
| `rec_batch_num` | `integer()` | `6` | Batch size for recognition inference (default: 6) Number of text regions to process simultaneously |
| `padding` | `integer()` | `10` | Padding in pixels added around the image before detection (default: 10). Large values can include surrounding content like table gridlines. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `task` | `PaddleOcrVlTaskKind \| nil` | `nil` | Optional per-call recognition task; the backend constructor's task is used when absent. |
| `model_path` | `String.t() \| nil` | `nil` | Optional local model directory, which takes precedence over `model_id`. |
| `model_id` | `String.t() \| nil` | `nil` | Optional Hugging Face repository identifier. |
| `hf_revision` | `String.t() \| nil` | `nil` | Optional immutable Hugging Face model revision. |
| `cache_dir` | `String.t() \| nil` | `nil` | Optional Hugging Face cache root. |
| `device` | `CandleDevicePreference \| nil` | `nil` | Optional per-call device override. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `extract_pages` | `boolean()` | `false` | Extract pages as separate array (ExtractedDocument.pages) |
...
| `image_indices` | `list(integer())` | `/* serde(default) */` | Indices into `ExtractedDocument.images` for images found on this page. Each value is a zero-based index into the top-level `images` collection. Only populated when `extract_images = true` in the extraction config. |
| `image_preprocessing` | `ImagePreprocessingMetadata \| nil` | `/* serde(default) */` | OCR image preprocessing applied to this page's raster. |
...
| `ocr_confidence` | `PageOcrConfidence \| nil` | `/* serde(default) */` | Aggregate OCR confidence for this page. `nil` when the page was not OCR'd. |
...
| `dimensions` | `PageDimensions \| nil` | `/* serde(default) */` | Dimensions in points (PDF) or pixels (images). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `score` | `float() \| nil` | `nil` | Aggregate legibility score in `0.0..=1.0`, or `nil` when the backend that produced this page does not report a calibrated legibility scale. Backends differ in what their confidence numbers mean (see `OcrConfidence`'s per-backend normalization), and not every backend maps onto a 0.0-1.0 legibility scale at all. When a backend has no such calibrated scale, this is `nil` rather than a misleading number, and scores must never be compared across backends. |
| `word_count` | `integer()` | — | Number of words the score was averaged over, AFTER noise filtering. A small `word_count` means the average is based on little evidence, so a high `score` next to a small `word_count` is not representative of the whole page. |
| `backend` | `String.t()` | — | Name of the OCR backend that produced the page text. |
...
| `text_excerpt` | `String.t()` | — | First ~500 characters of extracted text. |
...
| `bbox` | `BoundingBox \| nil` | `nil` | Bounding box on this page, if known. |
...
| `author` | `String.t() \| nil` | `/* serde(default) */` | Author/creator of the annotation (PDF `/T` entry). |
| `modified` | `String.t() \| nil` | `/* serde(default) */` | Last modification date of the annotation (PDF `/M` entry), as a raw PDF date string (e.g. `"D:20240115120000Z"`). |
| `color` | `String.t() \| nil` | `/* serde(default) */` | Annotation colour (PDF `/C` entry), normalised to a CSS-compatible `#rrggbb` hex string. Gray and CMYK colour spaces are converted to RGB. |
| `subject` | `String.t() \| nil` | `/* serde(default) */` | Subject of the annotation (PDF `/Subj` entry). |
| `quad_points` | `list(BoundingBox) \| nil` | `/* serde(default) */` | Per-line bounding boxes derived from the annotation's `/QuadPoints` entry. Present for text markup annotations (Highlight, Underline, StrikeOut, Squiggly), one box per marked line/run of text. |
| `marked_text` | `String.t() \| nil` | `/* serde(default) */` | The document text covered by `Self.quad_points`, recovered from the page content underneath the marked-up region. Populated for Highlight, Underline, StrikeOut, and Squiggly annotations when the underlying text could be recovered. |
...
| `extract_tables` | `boolean()` | `true` | Extract tables from PDF. When `true` (default), runs the native engine's grid detector and, if it finds nothing, falls back to the heuristic text-layer reconstruction in `pdf.native.table.extract_tables_heuristic`. Set to `false` to skip both passes — `tables` will then be empty in the result. |
...
| `ocr_inline_images` | `boolean()` | `false` | Perform OCR on inline images extracted from PDF pages and attach the recognized text to each `ExtractedImage.ocr_result`. Uses the backend selected by `ExtractionConfig.ocr`, or the default OCR backend when no OCR configuration is supplied. Requires the `ocr` or `ocr-pipeline` feature. Per-image failures degrade gracefully (the image is returned without OCR text rather than failing the whole extraction). Default: `false`. |
| `extract_form_fields` | `boolean()` | `true` | Extract AcroForm and XFA form fields into `ExtractedDocument.form_fields`. When `true` (default), reads the document's interactive form structure (field names, types, values, widget geometry). Cheap and strictly additive — non-form PDFs simply yield an empty list. Set to `false` to skip the form pass entirely. |
| `reading_order` | `boolean()` | `false` | Reorder extracted text by layout-detected reading order. When `true`, projects text spans onto layout-detected regions, performs column detection, and emits spans in natural reading order (important for multi-column academic PDFs). It also repairs 90/180/270-degree rotated text runs — sideways tables and captions — that otherwise read word-reversed and glued (GH#1358); see `crate.extractors.pdf.reading_order` for the rotation-handling details and its limits. Requires the `layout-detection` feature and a page for which layout detection actually produces hints: a page with no detected regions falls back to the original, unrepaired extraction order even with this enabled. Independent of `LayoutStrategy`, which only controls whether layout detection runs at all — enabling `Always` or `Auto` alone does not turn reordering on. Defaults to `false`. |
| `backend` | `PdfBackend` | `:native` | Which engine parses and renders this PDF. Defaults to `PdfBackend.Native`. Selecting `PdfBackend.Pdfium` requires the `pdf-pdfium` feature and is rejected otherwise; the pdfium engine is also deliberately narrower in scope than `Native` -- see `PdfBackend` and `extractors.pdf.pdfium_engine` for details. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `String.t()` | — | Partial field name (the leaf name within the field hierarchy). |
| `full_name` | `String.t()` | — | Fully-qualified field name (dotted path from the form root). |
| `field_type` | `FormFieldType` | — | Classified field type. |
| `value` | `String.t() \| nil` | `/* serde(default) */` | Current field value, if any. |
| `default_value` | `String.t() \| nil` | `/* serde(default) */` | Default field value, if any. |
| `flags` | `integer()` | `/* serde(default) */` | Raw field-flags bitmask (read-only, required, multiline, …). |
| `page` | `integer() \| nil` | `/* serde(default) */` | 1-indexed page the field's widget appears on. Currently always `nil` for AcroForm fields; page assignment is a deferred enhancement requiring spatial analysis of widget annotations per page. |
| `bbox` | `BoundingBox \| nil` | `/* serde(default) */` | Widget bounding box on its page, if known. |
| `max_length` | `integer() \| nil` | `/* serde(default) */` | Maximum input length for text fields, if specified. |
| `tooltip` | `String.t() \| nil` | `/* serde(default) */` | Tooltip / alternate field description, if present. |
...
| `scanned_confidence` | `float() \| nil` | `nil` | How strongly the document's most scan-like page resembles a scan, in `[0.0, 1.0]`. `nil` when the document could not be inspected. A full-page raster with no visible text scores at least `0.85`; a born-digital slide with a full-bleed background image scores `0.50`. |
| `scanned_pages` | `list(integer()) \| nil` | `[]` | Pages that look like scans (1-indexed), using the default confidence threshold. `nil` when the document could not be inspected; empty when no page qualifies. |
| `layout_gated_pages` | `list(integer()) \| nil` | `[]` | Pages the `auto` layout strategy skipped (1-indexed). `nil` unless layout detection ran with `LayoutStrategy.Auto`; empty when the gate selected every page. |
| `layout_gate_reasons` | `list(String.t()) \| nil` | `[]` | Why the `auto` layout gate selected or skipped each page. Index `i` is page `i + 1`. Snake_case values such as `multi_column`, `table_grid`, or `plain_text` (the skip reason). `nil` unless layout detection ran with `LayoutStrategy.Auto`. |
...
```elixir
def should_process(obj, result, config)
```
...
```elixir
def estimated_duration_ms(obj, result)
```
...
|-------|------|---------|-------------|
| `url` | `String.t()` | — | Link destination. |
| `label` | `String.t() \| nil` | `nil` | Optional visible label. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `id` | `String.t()` | — | Stable, URL-safe preset identifier (lowercase snake_case). |
| `version` | `String.t()` | — | Monotonic version string (e.g. `v1`). |
| `schema_name` | `String.t()` | — | Human-readable schema name forwarded to the LLM as the response/tool name. |
| `description` | `String.t()` | — | One-line preset description shown in the registry UI. |
| `category` | `PresetCategory` | — | Top-level category for grouping in the playground. |
| `tags` | `list(String.t())` | `/* serde(default) */` | Free-form tags used for search/filtering. May be empty. |
| `schema` | `term()` | — | JSON Schema (Draft 2020-12) describing the structured output shape. |
| `system_prompt` | `String.t()` | — | Instruction primer sent to the model. |
| `context_template` | `String.t() \| nil` | `/* serde(default) */` | Optional mustache-style template merged with caller-supplied context. |
...
| `sample` | `PresetSample \| nil` | `/* serde(default) */` | Optional bundled sample (input file + reference output) for preview. |
| `fingerprint` | `String.t()` | `/* serde(default) */` | Stable sha256 fingerprint of the canonical preset file contents. Populated at registry load — not present in the on-disk JSON files. Used as a cache-invalidation token by the worker pipeline. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `input_path` | `String.t()` | — | Path to the sample input file, relative to the preset directory. |
| `output_path` | `String.t()` | — | Path to the reference structured output, relative to the preset directory. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `id` | `String.t()` | — | Preset identifier matching `Preset.id`. |
| `version` | `String.t()` | — | Preset version matching `Preset.version`. |
| `schema_name` | `String.t()` | — | Schema name matching `Preset.schema_name`. |
| `description` | `String.t()` | — | One-line preset description. |
| `category` | `PresetCategory` | — | Top-level category. |
| `tags` | `list(String.t())` | — | Free-form tags. |
| `preferred_call_mode` | `CallMode` | — | Default call mode. |
| `emit_citations` | `boolean()` | — | Whether the preset prompts the model for citations. |
| `fingerprint` | `String.t()` | — | Stable fingerprint matching `Preset.fingerprint`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `String.t()` | — | Property name, such as `"bold"`, `"italic"`, `"font_size"`, or `"font_color"`. |
| `from` | `String.t() \| nil` | `nil` | Value before the change, when available. |
| `to` | `String.t() \| nil` | `nil` | Value after the change, when available. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `start` | `integer()` | — | Byte-offset start in the original (pre-redaction) `ExtractedDocument.content`. |
| `end` | `integer()` | — | Byte-offset end (exclusive) in the original `ExtractedDocument.content`. |
...
| `case_sensitive` | `boolean()` | `false` | When `true`, match case-sensitively; otherwise prepend `(?i)` to the regex. |
...
| `case_sensitive` | `boolean()` | `false` | When `true`, match the value as-is; otherwise match ASCII-case-insensitively. |
...
```elixir
{:ok, result} = Registry.load_embedded()
```
...
```elixir
def sample_bytes(obj, preset_id, name)
```

**Example:**

```elixir
{:ok, result} = instance.sample_bytes("value", "value")
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `preset_id` | `String.t()` | Yes | The preset id |
...
```elixir
def extend_from_dir(obj, dir)
```

**Example:**

```elixir
{:ok, result} = instance.extend_from_dir("value")
```
...
```elixir
def render_result(obj, result)
```

**Example:**

```elixir
{:ok, result} = instance.render_result(%{})
```
...
```elixir
def rerank(obj, query, documents)
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `model` | `RerankerModelType` | `:preset { name: "balanced" }` | The reranker model to use (defaults to "balanced" preset if not specified). |
...
| `max_rerank_duration_secs` | `integer() \| nil` | `60` | Maximum wall-clock duration (in seconds) for a single `rerank()` call when using `RerankerModelType.Plugin`. Applies only to the in-process plugin path — protects against hung host-language backends. On timeout, the dispatcher returns `Plugin` instead of blocking forever. `nil` disables the timeout. The default (60 seconds) is conservative for common in-process inference; increase for large document sets on slow hardware. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `id` | `String.t()` | — | Source preset identifier. |
| `version` | `String.t()` | — | Source preset version. |
| `fingerprint` | `String.t()` | — | Fingerprint of the source preset file, used as a cache token. |
| `schema_name` | `String.t()` | — | Schema name forwarded to the LLM. |
| `schema` | `term()` | — | Effective JSON Schema (caller override or the preset's own). |
| `system_prompt` | `String.t()` | — | System prompt with rendered context appended. |
...
| `max_nesting_depth` | `integer()` | `1024` | Maximum nesting depth for structures (1024) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `host` | `String.t()` | `"127.0.0.1"` | Server host address (e.g., "127.0.0.1", "0.0.0.0") |
...
| `max_request_body_bytes` | `integer()` | `104857600` | Maximum size of request body in bytes (default: 100 MB) |
| `max_multipart_field_bytes` | `integer()` | `104857600` | Maximum size of multipart fields in bytes (default: 100 MB) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `model` | `SparseEmbeddingModelType` | `:preset` | The sparse-embedding model to use (defaults to the "opensearch-v3-distill" preset). |
| `batch_size` | `integer()` | `16` | Batch size for local ONNX inference. SPLADE emits a `[seq, vocab]` logit tensor per document, so memory scales with batch size — keep this modest. |
| `max_length` | `integer()` | `256` | Maximum token sequence length for the tokenizer. |
| `show_download_progress` | `boolean()` | `false` | Show model download progress (local ONNX path only). When enabled, transfer progress for the model, tokenizer and config files is reported at `info` level on the `xberg.model_download` target while they download (#279). A warm Hugging Face cache transfers nothing and so reports nothing. Ignored by `SparseEmbeddingModelType.Plugin`, which downloads no model. |
| `cache_dir` | `String.t() \| nil` | `nil` | Optional alternate Hugging Face cache root for model files. When unset, hf-hub follows the standard Hugging Face environment and platform cache conventions. |
| `acceleration` | `AccelerationConfig \| nil` | `nil` | Hardware acceleration for the sparse-embedding ONNX model. |
| `max_embed_duration_secs` | `integer() \| nil` | `60` | Maximum wall-clock duration (in seconds) for a single embed call when using `SparseEmbeddingModelType.Plugin`. `nil` disables the timeout. |
...
| `model_file` | `String.t()` | — | Path to the ONNX file within the repo. |
| `additional_files` | `list(String.t())` | — | Sibling files that must be downloaded alongside `model_file`. |
| `max_length` | `integer()` | — | Maximum token sequence length. |
| `description` | `String.t()` | — | Human-readable description. |
...
| `scheme_allowlist` | `list(String.t())` | `["http", "https"]` | Allowed URI schemes. Default: `["http", "https"]`. Only `http` and `https` are supported. An empty list denies every URL. |
...
| `value` | `term() \| nil` | `nil` | The parsed document as a canonical `serde_json.Value` tree, when the source format could be represented as one. `nil` only for TOML inputs whose `toml.Value` fails to round-trip through `serde_json.Value` (xberg-io/xberg#155): the extractor falls back to a raw code block in that case. |
| `flattened` | `list(String.t())` | — | Flattened `path: value` renderings for every leaf field, in traversal order. Previously computed and discarded (xberg-io/xberg#166); now surfaced so callers get a full-text view even when the structured renderer only emits headings/lists for a subset of fields. |
...
| `schema_name` | `String.t()` | `"extraction"` | Schema name passed to the LLM's structured output mode. |
...
| `columns` | `list(String.t()) \| nil` | `[]` | Header cells for this fragment, i.e. the first row of `cells`. Populated even when this fragment's own header row was merged away or physically lives in a sibling fragment (see `table_id`), so a single fragment is interpretable in isolation. `nil` when no header row could be determined. |
...
| `row_span` | `integer()` | `1` | Row span (number of rows this cell spans) |
| `col_span` | `integer()` | `1` | Column span (number of columns this cell spans) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `row` | `integer()` | — | Zero-indexed row of the cell this style belongs to. |
| `col` | `integer()` | — | Zero-indexed column of the cell this style belongs to. |
| `heading_level` | `integer() \| nil` | `nil` | Outline level 1-6 when the style resolves to a heading, otherwise `nil`. |
| `style_name` | `String.t() \| nil` | `nil` | Human-readable style name, e.g. `heading 2`. |
...
`i32`/`list(String.t())`).** They are two independent struct definitions bridged only by
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `language` | `list(String.t())` | `["eng"]` | Language code(s) for OCR recognition. For Tesseract, languages are joined with "+". A list is the canonical form and the only form accepted by the binding object APIs (Python, Node, PHP, WASM, etc.): `["eng", "deu"]`. When deserializing from a config file, JSON body, or the REST/MCP API, a single string is also accepted, either as one code ("eng") or "+"-joined ("eng+deu"). |
| `psm` | `integer() \| nil` | `nil` | Page Segmentation Mode (1-13). PSM 0 is rejected: Tesseract's `PSM_OSD_ONLY` performs orientation and script detection with no character recognition, so it cannot satisfy a text-extraction request and previously yielded an empty document (GH#1586). `nil` (the default) means the caller made no explicit choice: the extraction pipeline applies its own context-appropriate PSM (whole-image PSM 11, vertical- language PSM 5, layout-region PSM 6, or the sparse-text retry's PSM 3) exactly as it would with no `TesseractConfig` at all — see issue #1573. Setting any other field on this struct no longer changes that behaviour. Common explicit values: - 3: Fully automatic page segmentation (native engine default) - 6: Assume a single uniform block of text (WASM engine default — avoids layout-analysis hang) - 11: Sparse text with no particular order |
...
| `language_model_ngram_on` | `boolean()` | `true` | Enable N-gram language model. Kept on by default (see `Self.default` and `ocr.types.TesseractConfig.language_model_ngram_on` for the rationale); keep this field's default in sync with the internal struct's. |
...
| `links` | `list(MarkdownLink) \| nil` | `nil` | Markdown links (Markdown files only). |
| `code_blocks` | `list(MarkdownCodeBlock) \| nil` | `nil` | Code blocks (Markdown files only). |
...
| `links` | `list(MarkdownLink) \| nil` | `[]` | Markdown links (for Markdown files). |
| `code_blocks` | `list(MarkdownCodeBlock) \| nil` | `[]` | Code blocks (for Markdown files). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `mode` | `String.t()` | `"off"` | Reduction mode: "off", "light", "moderate", "aggressive", "maximum" |
...
```elixir
def count_tokens(obj, text)
```

**Example:**

```elixir
{:ok, result} = instance.count_tokens("value")
```
...
| `language` | `String.t() \| nil` | `nil` | Optional language hint (ISO-639-1 code, e.g. "en", "de"). When `nil` (default), the current engine falls back to English. For deterministic production output, always set this explicitly. |
...
| `max_duration_ms` | `integer() \| nil` | `1800000` | Hard safety limit on input duration (milliseconds). Files longer than this are rejected after decode, before model work. Default: 30 minutes. Set to `nil` to disable (not recommended for untrusted input). |
| `max_bytes` | `integer() \| nil` | `536870912` | Hard safety limit on input size (bytes). Default: 512 MiB. Protects against pathological or malicious uploads. |
| `timeout_ms` | `integer() \| nil` | `600000` | Wall-clock timeout for the entire transcription operation (ms). Bounds audio decode, model resolution/download, and inference together. On expiry the extraction fails with an `XbergError.Transcription`. `nil` disables the bound and lets the operation run unbounded (not recommended for untrusted input). Enforced on the async extraction path only; the size and duration caps (`max_bytes`, `max_duration_ms`) are checked on every path. Default: 10 minutes. |
| `model_cache_dir` | `String.t() \| nil` | `nil` | Optional alternate Hugging Face cache root for Whisper models. When unset, hf-hub follows `HF_HUB_CACHE`, `HUGGINGFACE_HUB_CACHE`, `HF_HOME`, XDG, and platform defaults. Files remain in the standard content-addressed snapshot layout and are not copied into an Xberg cache. |
...
| `content` | `String.t()` | — | Translated plain-text body. Matches the shape of `ExtractedDocument.content`. |
...
| `data_extraction` | `boolean()` | `false` | Extract a hierarchical key/value data tree from data-format files (JSON, YAML, TOML, XML, CSV, etc.). Default: false. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `variant` | `CandleTrocrVariant \| nil` | `nil` | Optional model variant; the backend constructor's variant is used when absent. |
| `device` | `CandleDevicePreference \| nil` | `nil` | Optional per-call device override. |
| `cache_dir` | `String.t() \| nil` | `nil` | Optional Hugging Face cache root. |
| `hf_revision` | `String.t() \| nil` | `nil` | Optional immutable Hugging Face model revision. |
...
| `max_document_urls_per_result` | `integer() \| nil` | `100` | Maximum URLs to follow per extraction result. |
| `max_total_urls` | `integer() \| nil` | `1000` | Maximum URLs followed across the whole extraction call. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `page_ranges` | `list(PageRange) \| nil` | `[]` | User-specified page ranges (overrides automatic chunking). |
| `pages_per_chunk` | `integer() \| nil` | `nil` | User-specified pages per chunk (overrides automatic calculation). |
...
```elixir
def validate(obj, result, config)
```
...
```elixir
def should_validate(obj, result, config)
```
...
| Value | Description |
|-------|-------------|
| `preset` | Use a preset ColBERT model (recommended). — Fields: `name`: `String.t()` |
| `custom` | Use a custom ColBERT ONNX model from HuggingFace. — Fields: `model_id`: `String.t()`, `model_file`: `String.t()`, `additional_files`: `list(String.t())`, `max_length`: `integer()` |
| `plugin` | In-process late-interaction backend registered via the plugin system. — Fields: `name`: `String.t()` |
...
| Value | Description |
|-------|-------------|
| `azure_ad` | Azure AD OAuth2 client-credentials flow (Azure OpenAI / Azure Cognitive Services). — Fields: `tenant_id`: `String.t()`, `client_id`: `String.t()`, `client_secret`: `String.t()`, `scope`: `String.t()` |
| `vertex_oauth2` | Google Vertex AI OAuth2 via a service-account JSON key file on disk. Points at a file path rather than embedding the key inline: the key file contains an RSA private key — stronger secret material than an API key — and `LlmConfig` must never carry that directly, matching the credential-handling policy the rest of this module follows. — Fields: `service_account_key_file`: `String.t()`, `scope`: `String.t()` |
| `vertex_adc` | Google Vertex AI Application Default Credentials, resolved from the GCE/GKE/Cloud Run metadata server. Carries no secret material at all. — Fields: `scope`: `String.t()` |
| `bedrock_web_identity` | AWS STS `AssumeRoleWithWebIdentity` (EKS IRSA / OIDC federation) for Bedrock. — Fields: `role_arn`: `String.t()`, `token_file`: `String.t()`, `session_name`: `String.t()`, `region`: `String.t()` |
...
| `tokenizer` | Size measured in tokens from a HuggingFace tokenizer or a registered tokenizer backend. — Fields: `model`: `String.t()`, `cache_dir`: `String.t()` |
...
| `custom` | Use a custom ONNX cross-encoder from HuggingFace. — Fields: `model_id`: `String.t()`, `model_file`: `String.t()`, `additional_files`: `list(String.t())`, `max_length`: `integer()`, `head`: `RerankerHead` |
...
| Value | Description |
|-------|-------------|
| `preset` | Use a preset SPLADE model (recommended). — Fields: `name`: `String.t()` |
| `custom` | Use a custom SPLADE (`BertForMaskedLM`) ONNX model from HuggingFace. — Fields: `model_id`: `String.t()`, `model_file`: `String.t()`, `additional_files`: `list(String.t())`, `max_length`: `integer()` |
| `plugin` | In-process sparse-embedding backend registered via the plugin system. — Fields: `name`: `String.t()` |
...
| Value | Description |
|-------|-------------|
| `legibility` | Validated to track legibility on a known scale — usable as an absolute quality gate. — Fields: `scale_max`: `float()` |
...
| `format_not_chunkable` | Document format does not support chunking. — Fields: `mime_type`: `String.t()` |
...
| Value | Description |
|-------|-------------|
| `base_printed` | Base printed |
| `large_printed` | Large printed |
| `base_handwritten` | Base handwritten |
| `large_handwritten` | Large handwritten |
...
`nil` (the `CrawlConfig.document_content_encoding` default) produces neither — unlike
...
| Value | Description |
|-------|-------------|
| `exact` | Exact hostname match (case-insensitive). — Fields: `value`: `String.t()` |
| `suffix` | Suffix match: ".xberg.io" matches "api.xberg.io" and "xberg.io". — Fields: `value`: `String.t()` |
| `cidr` | CIDR match: "10.0.0.0/8" matches IP addresses in that range. — Fields: `value`: `String.t()` |
...
0-4 (`nil`/`Conversion`/`Unknown`/`Panic`/`InvalidHandle`).
...
|-------|----------|------------|-------|
| `OcrBackend` | Xberg.register_ocr_backend | Xberg.unregister_ocr_backend | Xberg.clear_ocr_backends |
| `PostProcessor` | Xberg.register_post_processor | Xberg.unregister_post_processor | Xberg.clear_post_processors |
| `Validator` | Xberg.register_validator | Xberg.unregister_validator | Xberg.clear_validators |
| `DocumentExtractor` | Xberg.register_document_extractor | Xberg.unregister_document_extractor | Xberg.clear_document_extractors |
| `EmbeddingBackend` | Xberg.register_embedding_backend | Xberg.unregister_embedding_backend | Xberg.clear_embedding_backends |
| `Renderer` | Xberg.register_renderer | Xberg.unregister_renderer | Xberg.clear_renderers |
| `RerankerBackend` | Xberg.register_reranker_backend | Xberg.unregister_reranker_backend | Xberg.clear_reranker_backends |
| `TokenizerBackend` | Xberg.register_tokenizer_backend | Xberg.unregister_tokenizer_backend | Xberg.clear_tokenizer_backends |
