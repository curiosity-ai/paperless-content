...
# alef:hash:37542d3b713cef2ffe7b8df8609510b4452823d5024f05b4f85b4205f5e932f8
# To regenerate: alef generate
# To verify freshness: alef verify
defmodule Xberg do
  @moduledoc "High-level API for xberg"

  @doc "Classify a document's chunks and return the updated document."
  @spec classify_chunks_owned(String.t() | nil, map()) ::
          {:ok, map()} | {:error, atom, String.t()}
  def classify_chunks_owned(result \\ nil, config) do
    Xberg.Native.classify_chunks_owned_async(
      cond do
        is_nil(result) -> nil
        is_binary(result) -> result
        true -> Jason.encode!(result)
      end,
      config
    )
  end
  ...
  @doc "Probe the backends and settings in `config` and report what will actually execute on this host."
  @spec doctor() :: map()
  def doctor do
    case Xberg.Native.doctor(nil) do
      {:ok, value} -> value
  ...
  @doc "Probe the backends and settings in `config` and report what will actually execute on this host."
  @spec doctor(String.t() | nil) :: map()
  def doctor(config) do
    case Xberg.Native.doctor(
           cond do
             is_nil(config) -> nil
  ...
  def max_sim_rank(query, docs) do
    case Xberg.Native.max_sim_rank(
           query,
           cond do
             is_nil(docs) -> nil
             is_binary(docs) -> docs
             true -> Jason.encode!(docs)
           end
         ) do
      {:ok, value} -> value
      {:error, error} -> raise ArgumentError, error
  ...
  def metaschema_compile(meta_schema_json) do
    case Xberg.Native.metaschema_compile(meta_schema_json) do
      {:ok, ref} -> {:ok, %Xberg.MetaSchema{ref: ref}}
  ...
  def metaschema_parse_preset(obj, path, raw) do
    Xberg.Native.metaschema_parse_preset(obj.ref, path, raw)
  end

  @doc "Build the registry from preset files embedded at compile time under"
  def registry_load_embedded do
    case Xberg.Native.registry_load_embedded() do
      {:ok, ref} -> {:ok, %Xberg.Registry{ref: ref}}
      {:error, error} -> {:error, error}
    end
  end
  ...
  defmodule OcrBackend.Host do
    ...
    @doc "Handle the `confidence_semantics` trait call."
    @callback confidence_semantics() :: map()

    @doc "Handle the `page_orientation_handling` trait call."
    @callback page_orientation_handling() :: map()
    ...
    @optional_callbacks [
      process_image_file: 2,
      supported_languages: 0,
      supports_table_detection: 0,
      supports_document_processing: 0,
      emits_structured_markdown: 0,
      confidence_semantics: 0,
      page_orientation_handling: 0,
      process_document: 2,
      initialize: 0,
      shutdown: 0
    ]
