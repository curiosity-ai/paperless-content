...
// alef:hash:d4f6232d4225fd7e3f169f51b85b0f3cab0eeba9d3e892822a856ab129c6aeda
...
public sealed record TreeSitterProcessConfig {
  /// <summary>
  /// Extract structural items (functions, classes, structs, etc.). Default:
  /// true.
  /// </summary>
  ...
  public CodeContentMode ContentMode {
    get; init;
  ...
  public static TreeSitterProcessConfig FromJson(string json) {
    try {
      return JsonSerializer.Deserialize<TreeSitterProcessConfig>(json,
                                                                 JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse TreeSitterProcessConfig from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse TreeSitterProcessConfig from JSON: {e.Message}", e);
    }
  }
  ...
  public static TreeSitterProcessConfig Default() {
    ...
    return JsonSerializer.Deserialize<TreeSitterProcessConfig>(json ?? "null",
                                                               JsonOptions)!;
  }
}
