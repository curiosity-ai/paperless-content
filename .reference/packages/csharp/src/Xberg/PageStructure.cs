...
// alef:hash:ba799afd7e42362de229e938ea23d13877cd2a29489567522ea00db502ef9694
...
public sealed record PageStructure {
  ...
  public required PageUnitType UnitType {
    get; init;
  }
    ...
    try {
      return JsonSerializer.Deserialize<PageStructure>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse PageStructure from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse PageStructure from JSON: {e.Message}", e);
    }
  }
