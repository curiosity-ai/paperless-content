// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:ff14ce4a5a8e264d0bc7851a141a0ddf9f5f5e567028698857fd416e6c1a7cb5
// To regenerate: alef generate
// To verify freshness: alef verify
#nullable enable

using System;
using System.Collections.Generic;
using System.Text.Json;
using System.Text.Json.Serialization;

namespace Xberg;

/// <summary>
/// Image dimensions in pixels.
/// </summary>
public sealed record ImageDimensions {
  /// <summary>
  /// Width in pixels.
  /// </summary>
  [JsonPropertyName("width")]
  public uint Width { get; init; } = 0;

  /// <summary>
  /// Height in pixels.
  /// </summary>
  [JsonPropertyName("height")]
  public uint Height {
    get; init;
  } = 0;

  /// <summary>
  /// Parse a <see cref="ImageDimensions"/> from a JSON string.
  /// </summary>
  /// <exception cref="XbergException">When the JSON cannot be
  /// deserialised.</exception>
  public static ImageDimensions FromJson(string json) {
    try {
      return JsonSerializer.Deserialize<ImageDimensions>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse ImageDimensions from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse ImageDimensions from JSON: {e.Message}", e);
    }
  }

  private static readonly JsonSerializerOptions JsonOptions = new() {
    DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingDefault,
    Converters = { new JsonStringEnumConverter(JsonNamingPolicy.SnakeCaseLower),
                   new ByteArrayJsonConverter() },
  };

  /// <summary>Options for serializing config/input objects to FFI. Strips nulls
  /// (nullable C# fields default to null and would override required Rust
  /// fields with non-deserialisable nulls) but preserves explicit false/0 so
  /// caller intent is kept.</summary>
  private static readonly JsonSerializerOptions JsonSerializationOptions =
      new() {
        DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingNull,
        Converters = { new JsonStringEnumConverter(
                           JsonNamingPolicy.SnakeCaseLower),
                       new ByteArrayJsonConverter() },
      };
}
