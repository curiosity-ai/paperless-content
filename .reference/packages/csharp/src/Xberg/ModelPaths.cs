// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:f2e3df1030d086919eb9ec39420c4795292bda244e7091f1b4f8f27b42406b7e
// To regenerate: alef generate
// To verify freshness: alef verify
#nullable enable

using System;
using System.Collections.Generic;
using System.Text.Json;
using System.Text.Json.Serialization;

namespace Xberg;

/// <summary>
/// Combined paths to all models needed for OCR (backward compatibility).
/// </summary>
public sealed record ModelPaths {
  /// <summary>
  /// Exact path to the detection ONNX model in the Hugging Face snapshot.
  /// </summary>
  [JsonPropertyName("det_model")]
  public required string DetModel { get; init; }

  /// <summary>
  /// Exact path to the classification ONNX model in the Hugging Face snapshot.
  /// </summary>
  [JsonPropertyName("cls_model")]
  public required string ClsModel {
    get; init;
  }

  /// <summary>
  /// Exact path to the recognition ONNX model in the Hugging Face snapshot.
  /// </summary>
  [JsonPropertyName("rec_model")]
  public required string RecModel {
    get; init;
  }

  /// <summary>
  /// Path to the character dictionary file.
  /// </summary>
  [JsonPropertyName("dict_file")]
  public required string DictFile {
    get; init;
  }

  /// <summary>
  /// Parse a <see cref="ModelPaths"/> from a JSON string.
  /// </summary>
  /// <exception cref="XbergException">When the JSON cannot be
  /// deserialised.</exception>
  public static ModelPaths FromJson(string json) {
    try {
      return JsonSerializer.Deserialize<ModelPaths>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse ModelPaths from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse ModelPaths from JSON: {e.Message}", e);
    }
  }

  private static readonly JsonSerializerOptions JsonOptions = new() {
    DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingDefault,
    Converters = { new JsonStringEnumConverter(JsonNamingPolicy.SnakeCaseLower),
                   new ByteArrayJsonConverter() },
  };

  /// <summary>Options for serializing config/input objects to FFI. Strips nulls
  /// (nullable C# fields default to null and would override required Rust
  /// fields with non-deserialisable nulls) but preserves explicit false/0 so
  /// caller intent is kept.</summary>
  private static readonly JsonSerializerOptions JsonSerializationOptions =
      new() {
        DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingNull,
        Converters = { new JsonStringEnumConverter(
                           JsonNamingPolicy.SnakeCaseLower),
                       new ByteArrayJsonConverter() },
      };
}
