...
// alef:hash:dc05e72857482f973cbc74019d2720ffa99492b42a01488d5d4b02527dae0485
...
public sealed record RedactionPattern {
  ...
  /// <summary>
  /// When `true`, match case-sensitively; otherwise prepend `(?i)` to the
  /// regex.
  /// </summary>
    ...
    try {
      return JsonSerializer.Deserialize<RedactionPattern>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse RedactionPattern from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse RedactionPattern from JSON: {e.Message}", e);
    }
  }
  ...
  public static RedactionPattern Labeled(string label, string pattern) {
    ...
    return JsonSerializer.Deserialize<RedactionPattern>(json ?? "null",
                                                        JsonOptions)!;
  }
}
