...
// alef:hash:101fa6d63ca1b873d3d1f6717f54c1c54acd6e307cd555149c185fd1dccffc48
...
public sealed record LlmConfig {
  ...
  /// <summary>
  /// Stop sequence(s) that halt token generation, applied to individual
  ...
  /// (`types.chat.ReasoningEffort`). A request-time parameter like
  /// `temperature`/ `max_tokens` above, not a client-level setting —
  /// `into_client_builder` does not map it. Accepted as a plain string — one of
  /// `"low"`, `"medium"`, `"high"`,
  ...
  /// (guardrails, safety settings, grounding config, etc.), applied to
  /// individual requests built from this config.
  ///
  ...
  /// <summary>
  /// Whether liter-llm should load provider credentials from environment
  /// variables.
  ///
  /// Mirrors liter-llm's `ClientConfigBuilder.load_env`. When `None`,
  /// liter-llm's own default behavior applies.
  /// </summary>
  ...
  ///
  /// Mirrors liter-llm's `ClientConfigBuilder.header`, for gateways or
  /// providers that require custom auth/routing headers.
  /// </summary>
  ...
  /// <summary>
  /// Custom provider configurations, in addition to liter-llm's built-in
  /// providers.
  ///
  /// Mirrors liter-llm's `LlmConfig.providers`, for OpenAI-compatible gateways
  /// and self-hosted model servers that are not in the built-in provider
  /// catalog.
  /// </summary>
  [JsonPropertyName("providers")]
  public List<LlmProviderConfig>
      ? Providers {
          get; init;
        } = null;
  ...
  /// `tower` feature is compiled in; otherwise the value is accepted but
  /// unused.
  ///
  /// Boxed for the same reason as `bedrock` (a4579589ac): `LlmConfig` is the
  /// payload of `EmbeddingModelType.Llm` and `RerankerModelType.Llm`, whose
  /// other variants are tens of bytes. Inlining this and the two sub-configs
  /// below pushed that variant to 480 bytes and tripped
  /// `clippy.large_enum_variant` on the
  /// `--features full` leg.
  /// </summary>
  [JsonPropertyName("cache")]
  public LlmCacheConfig
      ? Cache {
          get; init;
        } = null;
  ...
  ///
  /// Mirrors liter-llm's `LlmConfig.rate_limit`. Only takes effect when
  /// liter-llm's `tower` feature is compiled in; otherwise the value is
  /// accepted but unused.
  /// </summary>
  [JsonPropertyName("rate_limit")]
  public LlmRateLimitConfig
      ? RateLimit {
          get; init;
        } = null;
  ...
  ///
  /// Mirrors liter-llm's `LlmConfig.tracing`. Only takes effect when
  /// liter-llm's `tower` feature is compiled in; otherwise the value is
  /// accepted but unused.
  /// </summary>
  ...
  /// individual field inside it is `None` — liter-llm falls back to the
  /// standard AWS environment variables and the default credential chain.
  /// </summary>
  ...
  /// <summary>
  /// Managed OAuth2/STS credential provider for auth modes liter-llm cannot
  /// express via a static `api_key` — Azure AD, Vertex AI OAuth2, Vertex AI
  /// Application Default Credentials, and AWS STS Web Identity (EKS IRSA) for
  /// Bedrock.
  ///
  /// Mirrors liter-llm's `client.ClientConfigBuilder.credential_provider`,
  /// which takes an `Arc&lt;dyn liter_llm.auth.CredentialProvider&gt;` trait
  /// object — that cannot appear in a serde DTO. Every
  /// `CredentialProviderConfig` variant is plain data instead, so it
  /// round-trips through TOML/JSON/YAML and every language binding like the
  /// rest of `LlmConfig`.
  ///
  /// Managed credential providers are unavailable on `wasm32`, where liter-llm
  /// uses browser HTTP rather than its native authentication modules.
  /// `LlmConfig.validate` rejects a configured provider on that target instead
  /// of silently ignoring it.
  ///
  /// GitHub Copilot's device-flow provider has no variant here: it takes no
  /// configuration at all
  /// (`liter_llm.auth.github_copilot.GithubCopilotCredentialProvider.new`
  /// accepts only an HTTP client) and drives an interactive terminal prompt, so
  /// it cannot be expressed as data. A Rust embedder who needs it — or any
  /// other fully custom `CredentialProvider` — can call
  /// `xberg.llm.client.create_client_with_credential_provider` directly with a
  ...
  /// <summary>
  /// Maximum number of simultaneously in-flight requests to the LLM provider
  /// this config resolves to.
  ///
  /// This is a real, global bound on *provider* concurrency, not a
  /// per-extraction allowance: `xberg.llm.client.create_client` shares one
  /// process-wide client instance per distinct resolved config, so every
  /// concurrent extraction that resolves to the same config shares the one
  /// in-flight-request limit this value configures, instead of each minting its
  /// own (GH#1465). `None` means unlimited.
  ///
  /// PDF and image OCR batch sizing are **not** derived from this field, even
  /// when the configured OCR backend or `vlm_fallback` policy can reach a VLM —
  /// those call sites mix CPU-bound raster/OCR work with, at most, occasional
  /// remote requests, so they size their batches from the general thread budget
  /// (`max_threads`) unconditionally (GH#1465). Captioning is
  /// the one feature that *additionally* uses this value to bound its own
  /// per-extraction async request fan-out (issuing only VLM requests, with no
  /// CPU batching to protect), on top of the global provider-side limit
  /// described above; that per-extraction bound clamps a value below 1 up to 1.
  /// The global provider-side limit does not clamp: `Some(0)` reaches liter-llm
  /// as-is, and liter-llm rejects it when building the client (zero permitted
  /// in-flight requests is never useful), so it surfaces as a `create_client`
  /// error rather than a silent clamp to 1.
  ///
    ...
    try {
      return JsonSerializer.Deserialize<LlmConfig>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse LlmConfig from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse LlmConfig from JSON: {e.Message}", e);
    }
  }
