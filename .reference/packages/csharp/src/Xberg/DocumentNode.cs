...
// alef:hash:78750443b84587b8767c016b296b9dca9f8456a3b413a204a6620321ebd78a19
...
///
/// Each node has deterministic `id`, typed `content`, optional
/// `parent`/`children` for tree structure, and metadata like page number,
/// bounding box, and content layer.
/// </summary>
public sealed record DocumentNode {
  ...
  public required NodeContent Content {
    get; init;
  }
  ...
  ///
  /// Extensible bag for miscellaneous data without a dedicated typed field: CSS
  /// classes, LaTeX environment names, Excel cell formulas, slide layout names,
  /// etc.
  /// </summary>
    ...
    try {
      return JsonSerializer.Deserialize<DocumentNode>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse DocumentNode from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse DocumentNode from JSON: {e.Message}", e);
    }
  }
