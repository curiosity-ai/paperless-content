...
// alef:hash:bae346d566674abacca9702ea35de17cdeb5932635179d97a3be51ba0a372d59
...
public sealed record ChunkClassificationEnrichmentConfig {
  /// <summary>
  /// Label-definition set and LLM/batching settings for the
  /// chunk-classification stage.
  /// </summary>
  [JsonPropertyName("config")]
  public ChunkClassificationConfig Config {
    get; init;
  } = new ChunkClassificationConfig();

  /// <summary>
  /// Parse a <see cref="ChunkClassificationEnrichmentConfig"/> from a JSON
  /// string.
  /// </summary>
  ...
  public static ChunkClassificationEnrichmentConfig FromJson(string json) {
    try {
      return JsonSerializer.Deserialize<ChunkClassificationEnrichmentConfig>(
                 json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse ChunkClassificationEnrichmentConfig from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse ChunkClassificationEnrichmentConfig from JSON: {e.Message}",
          e);
    }
  }
