...
// alef:hash:eb0cc7126631f8ed88a5d4bef4a5792d0e1c7c52a9dcf0e367906988385ac676
...
/// <summary>
/// A custom provider configuration entry, in addition to liter-llm's built-in
/// providers.
///
...
public sealed record LlmProviderConfig {
  ...
  /// <summary>
  /// Header name used to carry the API key (defaults to `Authorization` when
  /// unset).
  /// </summary>
    ...
    try {
      return JsonSerializer.Deserialize<LlmProviderConfig>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse LlmProviderConfig from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse LlmProviderConfig from JSON: {e.Message}", e);
    }
  }
