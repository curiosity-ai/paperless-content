...
// alef:hash:e89a6a000289558e4a2e0f1e538b1a8a877e11555f40a6e4ec22d3e36a67c161
...
public sealed record RedactionTerm {
  ...
  /// <summary>
  /// When `true`, match the value as-is; otherwise match
    ...
    try {
      return JsonSerializer.Deserialize<RedactionTerm>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse RedactionTerm from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse RedactionTerm from JSON: {e.Message}", e);
    }
  }
  ...
  public static RedactionTerm Literal(string value) {
    ...
    return JsonSerializer.Deserialize<RedactionTerm>(json ?? "null",
                                                     JsonOptions)!;
  }
