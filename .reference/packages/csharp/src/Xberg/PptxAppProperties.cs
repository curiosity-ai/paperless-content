...
// alef:hash:8e3559a31d0db9e6dc74b4e3548057425373bca59e014cdd60503f3b3e6bdc4c
...
public sealed record PptxAppProperties {
  ...
  public List<string> SlideTitles {
    get; init;
  } = [];
    ...
    try {
      return JsonSerializer.Deserialize<PptxAppProperties>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse PptxAppProperties from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse PptxAppProperties from JSON: {e.Message}", e);
    }
  }
