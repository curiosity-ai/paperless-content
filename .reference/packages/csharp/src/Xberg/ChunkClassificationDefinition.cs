...
// alef:hash:30a8a1216159c420261dd35294035a31717e30977765e7f691f5c1514cca2748
...
public sealed record ChunkClassificationDefinition {
  ...
  public static ChunkClassificationDefinition FromJson(string json) {
    try {
      return JsonSerializer.Deserialize<ChunkClassificationDefinition>(
                 json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse ChunkClassificationDefinition from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse ChunkClassificationDefinition from JSON: {e.Message}",
          e);
    }
  }
