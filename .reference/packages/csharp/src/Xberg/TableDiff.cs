...
// alef:hash:9ff44de9932d346771fffca8354e76314820736d34290990b08e6e61ec3f6865
...
public sealed record TableDiff {
  ...
  /// <summary>
  /// Zero-based index in `b.tables` (equal to `from_index` for same-dimension
  /// tables).
  /// </summary>
    ...
    try {
      return JsonSerializer.Deserialize<TableDiff>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse TableDiff from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse TableDiff from JSON: {e.Message}", e);
    }
  }
