// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:23cfad3cea5d1e6c7e2a9c3ef32dcb5f566ed1b6c0f7e12dfa56d496ee6f1f30
// To regenerate: alef generate
// To verify freshness: alef verify
#nullable enable

using System;
using System.Collections.Generic;
using System.Text.Json;
using System.Text.Json.Serialization;

namespace Xberg;

/// <summary>
/// Configuration for the NER post-processor.
/// </summary>
public sealed record NerConfig {
  /// <summary>
  /// Backend that runs the entity detection.
  /// </summary>
  [JsonPropertyName("backend")]
  public NerBackendKind Backend { get; init; } = NerBackendKind.Onnx;

  /// <summary>
  /// Entity categories to detect. Defaults to a sensible
  /// PERSON/ORG/LOCATION/EMAIL set when empty.
  /// </summary>
  [JsonPropertyName("categories")]
  public List<EntityCategory> Categories {
    get; init;
  } = [];

  /// <summary>
  /// Override the default model — only used by `NerBackendKind.Onnx`.
  /// `None` lets the backend pick its pinned default xberg GLiNER model alias.
  /// </summary>
  [JsonPropertyName("model")]
  public string
      ? Model {
          get; init;
        } = null;

  /// <summary>
  /// Optional LLM configuration — only used by `NerBackendKind.Llm`. Token
  /// usage for LLM backends is recorded in `ExtractedDocument.llm_usage`.
  /// </summary>
  [JsonPropertyName("llm")]
  public LlmConfig
      ? Llm {
          get; init;
        } = null;

  /// <summary>
  /// Arbitrary user-supplied entity labels for zero-shot detection.
  ///
  /// `xberg-gliner` natively supports zero-shot inference over caller-supplied
  /// labels. The LLM backend also honours these
  /// labels by including them in the structured-output schema. Custom labels
  /// surface as `EntityCategory.Custom` in the resulting `Entity` stream.
  ///
  /// Use this when you need domain-specific entity types (e.g. `"Treatment"`,
  /// `"Product"`, `"Vessel"`) without forking GLiNER's taxonomy.
  /// </summary>
  [JsonPropertyName("custom_labels")]
  public List<string> CustomLabels {
    get; init;
  } = [];

  /// <summary>
  /// Parse a <see cref="NerConfig"/> from a JSON string.
  /// </summary>
  /// <exception cref="XbergException">When the JSON cannot be
  /// deserialised.</exception>
  public static NerConfig FromJson(string json) {
    try {
      return JsonSerializer.Deserialize<NerConfig>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse NerConfig from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse NerConfig from JSON: {e.Message}", e);
    }
  }

  private static readonly JsonSerializerOptions JsonOptions = new() {
    DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingDefault,
    Converters = { new JsonStringEnumConverter(JsonNamingPolicy.SnakeCaseLower),
                   new ByteArrayJsonConverter() },
  };

  /// <summary>Options for serializing config/input objects to FFI. Strips nulls
  /// (nullable C# fields default to null and would override required Rust
  /// fields with non-deserialisable nulls) but preserves explicit false/0 so
  /// caller intent is kept.</summary>
  private static readonly JsonSerializerOptions JsonSerializationOptions =
      new() {
        DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingNull,
        Converters = { new JsonStringEnumConverter(
                           JsonNamingPolicy.SnakeCaseLower),
                       new ByteArrayJsonConverter() },
      };
}
