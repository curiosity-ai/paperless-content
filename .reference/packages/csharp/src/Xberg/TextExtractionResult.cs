...
// alef:hash:09d6fc49316646fe9650702f949e3f1b6ddce1e283318a0415cbda12dc88957e
...
public sealed record TextExtractionResult {
  ...
  public static TextExtractionResult FromJson(string json) {
    try {
      return JsonSerializer.Deserialize<TextExtractionResult>(json,
                                                              JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse TextExtractionResult from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse TextExtractionResult from JSON: {e.Message}", e);
    }
  }
