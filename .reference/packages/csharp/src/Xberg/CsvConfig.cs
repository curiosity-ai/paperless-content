...
// alef:hash:1e98747bc56fc1c4be7a308365ea3f19bc8cac91d3acf268802d4683db1331e4
...
public sealed record CsvConfig {
  ...
  public List<string> CommentPrefixes {
    get; init;
  } = [];
    ...
    try {
      return JsonSerializer.Deserialize<CsvConfig>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse CsvConfig from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse CsvConfig from JSON: {e.Message}", e);
    }
  }
