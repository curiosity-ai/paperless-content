...
// alef:hash:680a99a60681dbd01c708d0663159f29b33e8981ce0aa8d2503429ff70abb4f5
...
public sealed record PageSignals {
  ...
  public required string TextExcerpt {
    get; init;
  }

  /// <summary>
  /// `true` if page starts with letterhead-like content (ALL CAPS line in first
  /// 5 lines or a logo-image bbox at top).
  /// </summary>
  [JsonPropertyName("starts_with_letterhead_like")]
  public bool StartsWithLetterheadLike {
    get; init;
  } = false;
  ...
  public bool HasPageNumberOneMarker {
    get; init;
  } = false;
  ...
  public float LayoutTextDensity {
    get; init;
  } = 0.0f;
    ...
    try {
      return JsonSerializer.Deserialize<PageSignals>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse PageSignals from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse PageSignals from JSON: {e.Message}", e);
    }
  }
  ...
  /// </summary>
  public static PageSignals FromPageText(uint pageNumber, string text,
    ...
    return JsonSerializer.Deserialize<PageSignals>(json ?? "null",
                                                   JsonOptions)!;
  }
}
