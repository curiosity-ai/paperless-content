...
// alef:hash:75f291e2deadcc7aed788de6c33ff4877437645515c9328d546678cc89e81722
...
public sealed record MultiVectorEmbedding {
  ...
  public static MultiVectorEmbedding FromJson(string json) {
    try {
      return JsonSerializer.Deserialize<MultiVectorEmbedding>(json,
                                                              JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse MultiVectorEmbedding from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse MultiVectorEmbedding from JSON: {e.Message}", e);
    }
  }
