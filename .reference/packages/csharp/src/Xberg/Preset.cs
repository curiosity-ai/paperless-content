// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:37285d80fb57765ee3e900739822d28321794766c07af2a8f9cf351498263000
// To regenerate: alef generate
// To verify freshness: alef verify
#nullable enable

using System;
using System.Collections.Generic;
using System.Text.Json;
using System.Text.Json.Serialization;

namespace Xberg;

/// <summary>
/// A curated structured-extraction preset loaded from the embedded library.
///
/// Each preset is a JSON file under `src/presets/library/&lt;id&gt;/v1.json`
/// that validates against the meta-schema in `src/presets/preset.schema.json`.
///
/// Downstream catalog consumers can inject presets via
/// `extend_from_dir`. The embedded OSS library
/// ships only the `generic_document` toy preset.
/// </summary>
public sealed record Preset {
  /// <summary>
  /// Stable, URL-safe preset identifier (lowercase snake_case).
  /// </summary>
  [JsonPropertyName("id")]
  public required string Id { get; init; }

  /// <summary>
  /// Monotonic version string (e.g. `v1`).
  /// </summary>
  [JsonPropertyName("version")]
  public required string Version {
    get; init;
  }

  /// <summary>
  /// Human-readable schema name forwarded to the LLM as the response/tool name.
  /// </summary>
  [JsonPropertyName("schema_name")]
  public required string SchemaName {
    get; init;
  }

  /// <summary>
  /// One-line preset description shown in the registry UI.
  /// </summary>
  [JsonPropertyName("description")]
  public required string Description {
    get; init;
  }

  /// <summary>
  /// Top-level category for grouping in the playground.
  /// </summary>
  [JsonPropertyName("category")]
  public required PresetCategory Category {
    get; init;
  }

  /// <summary>
  /// Free-form tags used for search/filtering. May be empty.
  /// </summary>
  [JsonPropertyName("tags")]
  public List<string> Tags {
    get; init;
  } = [];

  /// <summary>
  /// JSON Schema (Draft 2020-12) describing the structured output shape.
  /// </summary>
  [JsonPropertyName("schema")]
  public required JsonElement Schema {
    get; init;
  }

  /// <summary>
  /// Instruction primer sent to the model.
  /// </summary>
  [JsonPropertyName("system_prompt")]
  public required string SystemPrompt {
    get; init;
  }

  /// <summary>
  /// Optional mustache-style template merged with caller-supplied context.
  /// </summary>
  [JsonPropertyName("context_template")]
  public string
      ? ContextTemplate {
          get; init;
        } = null;

  /// <summary>
  /// Strategy for merging per-batch outputs across paginated calls.
  /// </summary>
  [JsonPropertyName("merge_mode")]
  public required MergeMode MergeMode {
    get; init;
  }

  /// <summary>
  /// Default call mode suggested for this preset; heuristics may override.
  /// </summary>
  [JsonPropertyName("preferred_call_mode")]
  public required CallMode PreferredCallMode {
    get; init;
  }

  /// <summary>
  /// When true, the prompt asks the model to wrap each field as
  /// `{value, page, bbox, confidence}` for downstream citation overlays.
  /// </summary>
  [JsonPropertyName("emit_citations")]
  public bool EmitCitations {
    get; init;
  } = false;

  /// <summary>
  /// Optional bundled sample (input file + reference output) for preview.
  /// </summary>
  [JsonPropertyName("sample")]
  public PresetSample
      ? Sample {
          get; init;
        } = null;

  /// <summary>
  /// Stable sha256 fingerprint of the canonical preset file contents.
  ///
  /// Populated at registry load — not present in the on-disk JSON files.
  /// Used as a cache-invalidation token by the worker pipeline.
  /// </summary>
  [JsonPropertyName("fingerprint")]
  public string Fingerprint {
    get; init;
  } = "";

  /// <summary>
  /// Parse a <see cref="Preset"/> from a JSON string.
  /// </summary>
  /// <exception cref="XbergException">When the JSON cannot be
  /// deserialised.</exception>
  public static Preset FromJson(string json) {
    try {
      return JsonSerializer.Deserialize<Preset>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse Preset from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException($"Failed to parse Preset from JSON: {e.Message}",
                               e);
    }
  }

  private static readonly JsonSerializerOptions JsonOptions = new() {
    DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingDefault,
    Converters = { new JsonStringEnumConverter(JsonNamingPolicy.SnakeCaseLower),
                   new ByteArrayJsonConverter() },
  };

  /// <summary>Options for serializing config/input objects to FFI. Strips nulls
  /// (nullable C# fields default to null and would override required Rust
  /// fields with non-deserialisable nulls) but preserves explicit false/0 so
  /// caller intent is kept.</summary>
  private static readonly JsonSerializerOptions JsonSerializationOptions =
      new() {
        DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingNull,
        Converters = { new JsonStringEnumConverter(
                           JsonNamingPolicy.SnakeCaseLower),
                       new ByteArrayJsonConverter() },
      };
}
