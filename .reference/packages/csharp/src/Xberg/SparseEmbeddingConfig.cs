...
// alef:hash:3a686842e51ccd9884de1dd9303fd1fbdd12494077bb1f840504142898fcbf22
...
public sealed record SparseEmbeddingConfig {
  /// <summary>
  /// The sparse-embedding model to use (defaults to the "opensearch-v3-distill"
  /// preset).
  /// </summary>
  ...
  /// reports nothing. Ignored by `SparseEmbeddingModelType.Plugin`, which
  /// downloads no model.
  /// </summary>
  ...
  public static SparseEmbeddingConfig FromJson(string json) {
    try {
      return JsonSerializer.Deserialize<SparseEmbeddingConfig>(json,
                                                               JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse SparseEmbeddingConfig from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse SparseEmbeddingConfig from JSON: {e.Message}", e);
    }
  }
  ...
  public static SparseEmbeddingConfig Default() {
    ...
    return JsonSerializer.Deserialize<SparseEmbeddingConfig>(json ?? "null",
                                                             JsonOptions)!;
  }
}
