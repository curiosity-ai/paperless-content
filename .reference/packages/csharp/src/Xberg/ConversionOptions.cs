// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:737d39e6b30827b4634b7707bc2de62e705661585b9901d41303a5bb34659602
// To regenerate: alef generate
// To verify freshness: alef verify
#nullable enable

using System;
using System.Collections.Generic;
using System.Text.Json;
using System.Text.Json.Serialization;

namespace Xberg;

/// <summary>
/// Main conversion options for HTML to Markdown conversion.
///
/// Use `ConversionOptions.builder()` to construct, or `Default.default()` for
/// defaults.
/// </summary>
public sealed record ConversionOptions {
  /// <summary>
  /// Heading style to use in Markdown output (ATX `#` or Setext underline).
  /// </summary>
  [JsonPropertyName("heading_style")]
  public HeadingStyle HeadingStyle { get; init; } = HeadingStyle.Atx;

  /// <summary>
  /// How to indent nested list items (spaces or tab).
  /// </summary>
  [JsonPropertyName("list_indent_type")]
  public ListIndentType ListIndentType {
    get; init;
  } = ListIndentType.Spaces;

  /// <summary>
  /// Number of spaces (or tabs) to use for each level of list indentation.
  /// </summary>
  [JsonPropertyName("list_indent_width")]
  public ulong ListIndentWidth {
    get; init;
  } = 2;

  /// <summary>
  /// Bullet character(s) to use for unordered list items (e.g. `"-"`, `"*"`).
  /// </summary>
  [JsonPropertyName("bullets")]
  public string Bullets {
    get; init;
  } = "-*+";

  /// <summary>
  /// Character used for bold/italic emphasis markers (`*` or `_`).
  /// </summary>
  [JsonPropertyName("strong_em_symbol")]
  public string StrongEmSymbol {
    get; init;
  } = "*";

  /// <summary>
  /// Escape `*` characters in plain text to avoid unintended bold/italic.
  /// </summary>
  [JsonPropertyName("escape_asterisks")]
  public bool EscapeAsterisks {
    get; init;
  } = false;

  /// <summary>
  /// Escape `_` characters in plain text to avoid unintended bold/italic.
  /// </summary>
  [JsonPropertyName("escape_underscores")]
  public bool EscapeUnderscores {
    get; init;
  } = false;

  /// <summary>
  /// Escape miscellaneous Markdown metacharacters (`[]()#` etc.) in plain text.
  /// </summary>
  [JsonPropertyName("escape_misc")]
  public bool EscapeMisc {
    get; init;
  } = false;

  /// <summary>
  /// Escape ASCII characters that have special meaning in certain Markdown
  /// dialects.
  /// </summary>
  [JsonPropertyName("escape_ascii")]
  public bool EscapeAscii {
    get; init;
  } = false;

  /// <summary>
  /// Default language annotation for fenced code blocks that have no language
  /// hint.
  /// </summary>
  [JsonPropertyName("code_language")]
  public string CodeLanguage {
    get; init;
  } = "";

  /// <summary>
  /// Automatically convert bare URLs into Markdown autolinks.
  /// </summary>
  [JsonPropertyName("autolinks")]
  public bool Autolinks {
    get; init;
  } = true;

  /// <summary>
  /// Emit a default title when no `&lt;title&gt;` tag is present.
  /// </summary>
  [JsonPropertyName("default_title")]
  public bool DefaultTitle {
    get; init;
  } = false;

  /// <summary>
  /// Render `&lt;br&gt;` elements inside table cells as literal line breaks.
  /// </summary>
  [JsonPropertyName("br_in_tables")]
  public bool BrInTables {
    get; init;
  } = false;

  /// <summary>
  /// Emit tables without column padding (compact GFM format).
  ///
  /// When `true`, column widths are not computed and cells are emitted with
  /// no trailing spaces. Separator rows use exactly `---` per column.
  /// Produces token-efficient output suitable for RAG / LLM contexts.
  ///
  /// Default `false` (aligned padding preserved).
  /// </summary>
  [JsonPropertyName("compact_tables")]
  public bool CompactTables {
    get; init;
  } = false;

  /// <summary>
  /// Style used for `&lt;mark&gt;` / highlighted text (e.g. `==text==`).
  /// </summary>
  [JsonPropertyName("highlight_style")]
  public HighlightStyle HighlightStyle {
    get; init;
  } = HighlightStyle.DoubleEqual;

  /// <summary>
  /// Populate `result.metadata` with `&lt;head&gt;` / `&lt;meta&gt;` extraction
  /// (title, description, Open Graph, Twitter Card, JSON-LD, …).
  ///
  /// Default `true`. Disabling skips the metadata pass only — table
  /// extraction into `result.tables` runs unconditionally.
  /// </summary>
  [JsonPropertyName("extract_metadata")]
  public bool ExtractMetadata {
    get; init;
  } = true;

  /// <summary>
  /// Controls how whitespace sequences are normalised in the converted output.
  ///
  /// - `WhitespaceMode.Normalized` (default) — collapses consecutive whitespace
  /// characters
  ///   (spaces, tabs, newlines) to a single space, matching browser rendering
  ///   behaviour.
  /// - `WhitespaceMode.Strict` — preserves all whitespace exactly as it appears
  /// in the
  ///   source HTML, including runs of spaces and embedded newlines.
  ///
  /// Choose `Strict` only when the source HTML uses deliberate whitespace (e.g.
  /// pre-formatted content outside `&lt;pre&gt;` tags). For most documents
  /// `Normalized` produces cleaner output.
  /// </summary>
  [JsonPropertyName("whitespace_mode")]
  public WhitespaceMode WhitespaceMode {
    get; init;
  } = WhitespaceMode.Normalized;

  /// <summary>
  /// Strip all newlines from the output, producing a single-line result.
  /// </summary>
  [JsonPropertyName("strip_newlines")]
  public bool StripNewlines {
    get; init;
  } = false;

  /// <summary>
  /// Wrap long lines at `wrap_width` characters.
  /// </summary>
  [JsonPropertyName("wrap")]
  public bool Wrap {
    get; init;
  } = false;

  /// <summary>
  /// Maximum output line width in characters when `wrap` is `true` (default
  /// `80`).
  ///
  /// Lines are broken at word boundaries so that no line exceeds this length. A
  /// value of `0` is treated as "no limit" — equivalent to leaving `wrap`
  /// disabled. Has no effect when `wrap` is `false`.
  /// </summary>
  [JsonPropertyName("wrap_width")]
  public ulong WrapWidth {
    get; init;
  } = 80;

  /// <summary>
  /// Treat the entire document as inline content (no block-level wrappers).
  /// </summary>
  [JsonPropertyName("convert_as_inline")]
  public bool ConvertAsInline {
    get; init;
  } = false;

  /// <summary>
  /// Markdown notation for subscript text (e.g. `"~"`).
  /// </summary>
  [JsonPropertyName("sub_symbol")]
  public string SubSymbol {
    get; init;
  } = "";

  /// <summary>
  /// Markdown notation for superscript text (e.g. `"^"`).
  /// </summary>
  [JsonPropertyName("sup_symbol")]
  public string SupSymbol {
    get; init;
  } = "";

  /// <summary>
  /// How to encode hard line breaks (`&lt;br&gt;`) in Markdown.
  /// </summary>
  [JsonPropertyName("newline_style")]
  public NewlineStyle NewlineStyle {
    get; init;
  } = NewlineStyle.Spaces;

  /// <summary>
  /// Style used for fenced code blocks (backticks or tilde).
  /// </summary>
  [JsonPropertyName("code_block_style")]
  public CodeBlockStyle CodeBlockStyle {
    get; init;
  } = CodeBlockStyle.Backticks;

  /// <summary>
  /// HTML tag names whose `&lt;img&gt;` children are kept inline instead of
  /// block.
  /// </summary>
  [JsonPropertyName("keep_inline_images_in")]
  public List<string> KeepInlineImagesIn {
    get; init;
  } = [];

  /// <summary>
  /// Options for the HTML pre-processing pass applied before conversion begins.
  ///
  /// Pre-processing runs before the HTML is handed to the converter and can
  /// perform operations such as unwrapping redundant wrapper elements, removing
  /// tracking pixels, and normalising vendor-specific markup. See
  /// `PreprocessingOptions` for the full set of knobs.
  ///
  /// Defaults to `PreprocessingOptions.default()`, which enables the standard
  /// cleaning passes. Set individual fields on `PreprocessingOptions` (or
  /// construct via `ConversionOptions.builder`) to opt in or out of specific
  /// passes.
  /// </summary>
  [JsonPropertyName("preprocessing")]
  public PreprocessingOptions Preprocessing {
    get; init;
  } = new PreprocessingOptions();

  /// <summary>
  /// Expected character encoding of the input HTML (default `"utf-8"`).
  /// </summary>
  [JsonPropertyName("encoding")]
  public string Encoding {
    get; init;
  } = "utf-8";

  /// <summary>
  /// Emit debug information during conversion.
  /// </summary>
  [JsonPropertyName("debug")]
  public bool Debug {
    get; init;
  } = false;

  /// <summary>
  /// HTML tag names whose content is stripped from the output entirely.
  /// </summary>
  [JsonPropertyName("strip_tags")]
  public List<string> StripTags {
    get; init;
  } = [];

  /// <summary>
  /// HTML tag names that are preserved verbatim in the output.
  /// </summary>
  [JsonPropertyName("preserve_tags")]
  public List<string> PreserveTags {
    get; init;
  } = [];

  /// <summary>
  /// Skip conversion of `&lt;img&gt;` elements (omit images from output).
  /// </summary>
  [JsonPropertyName("skip_images")]
  public bool SkipImages {
    get; init;
  } = false;

  /// <summary>
  /// URL encoding strategy for link and image destinations.
  ///
  /// Controls how special characters in URL destinations are escaped:
  /// - `UrlEscapeStyle.Angle` (default) — wraps the destination in angle
  /// brackets when it
  ///   contains spaces or newlines. parsers misinterpret `&gt;` inside such a
  ///   destination.
  /// - `UrlEscapeStyle.Percent` — percent-encodes every character that is not
  /// an RFC 3986
  ///   unreserved character or `/`, producing a destination that all Markdown
  ///   parsers handle correctly even when the URL contains `&lt;`, `&gt;`,
  ///   spaces, or parentheses.
  /// </summary>
  [JsonPropertyName("url_escape_style")]
  public UrlEscapeStyle UrlEscapeStyle {
    get; init;
  } = UrlEscapeStyle.Angle;

  /// <summary>
  /// Link rendering style (inline or reference).
  /// </summary>
  [JsonPropertyName("link_style")]
  public LinkStyle LinkStyle {
    get; init;
  } = LinkStyle.Inline;

  /// <summary>
  /// Target output format (Markdown, plain text, etc.).
  /// </summary>
  [JsonPropertyName("output_format")]
  public HtmlToMarkdownRsOutputFormat OutputFormat {
    get; init;
  } = HtmlToMarkdownRsOutputFormat.Markdown;

  /// <summary>
  /// Include structured document tree in result.
  /// </summary>
  [JsonPropertyName("include_document_structure")]
  public bool IncludeDocumentStructure {
    get; init;
  } = false;

  /// <summary>
  /// Extract inline images from data URIs and SVGs.
  /// </summary>
  [JsonPropertyName("extract_images")]
  public bool ExtractImages {
    get; init;
  } = false;

  /// <summary>
  /// Maximum decoded image size in bytes (default 5MB).
  /// </summary>
  [JsonPropertyName("max_image_size")]
  public ulong MaxImageSize {
    get; init;
  } = 5242880;

  /// <summary>
  /// Capture SVG elements as images.
  /// </summary>
  [JsonPropertyName("capture_svg")]
  public bool CaptureSvg {
    get; init;
  } = false;

  /// <summary>
  /// Infer image dimensions from data.
  /// </summary>
  [JsonPropertyName("infer_dimensions")]
  public bool InferDimensions {
    get; init;
  } = true;

  /// <summary>
  /// Maximum DOM traversal depth.
  ///
  /// `None` uses the library's internal native-stack safety limit. Explicit
  /// values above that safety limit are clamped to prevent process-aborting
  /// stack overflows on pathologically deep DOM trees.
  /// </summary>
  [JsonPropertyName("max_depth")]
  public ulong
      ? MaxDepth {
          get; init;
        } = null;

  /// <summary>
  /// CSS selectors for elements to exclude entirely (element + all content).
  ///
  /// Unlike `strip_tags` (which removes the tag wrapper but keeps children),
  /// excluded elements and all their descendants are dropped from the output.
  /// Supports any CSS selector that `tl` supports: tag names, `.class`,
  /// `#id`, `[attribute]`, etc.
  ///
  /// Invalid selectors are silently skipped at conversion time.
  ///
  /// Example: `vec![".cookie-banner".into(), "#ad-container".into(),
  /// "[role='complementary']".into()]`
  /// </summary>
  [JsonPropertyName("exclude_selectors")]
  public List<string> ExcludeSelectors {
    get; init;
  } = [];

  /// <summary>
  /// Which conversion tier to use.
  ///
  /// - `TierStrategy.Auto` (default) — automatically choose the best path.
  /// - `TierStrategy.Tier2` — always use the Tier-2 DOM-walk path.
  /// </summary>
  [JsonPropertyName("tier_strategy")]
  public TierStrategy TierStrategy {
    get; init;
  } = TierStrategy.Auto;

  /// <summary>
  /// Parse a <see cref="ConversionOptions"/> from a JSON string.
  /// </summary>
  /// <exception cref="XbergException">When the JSON cannot be
  /// deserialised.</exception>
  public static ConversionOptions FromJson(string json) {
    try {
      return JsonSerializer.Deserialize<ConversionOptions>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse ConversionOptions from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse ConversionOptions from JSON: {e.Message}", e);
    }
  }

  private static readonly JsonSerializerOptions JsonOptions = new() {
    DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingDefault,
    Converters = { new JsonStringEnumConverter(JsonNamingPolicy.SnakeCaseLower),
                   new ByteArrayJsonConverter() },
  };

  /// <summary>Options for serializing config/input objects to FFI. Strips nulls
  /// (nullable C# fields default to null and would override required Rust
  /// fields with non-deserialisable nulls) but preserves explicit false/0 so
  /// caller intent is kept.</summary>
  private static readonly JsonSerializerOptions JsonSerializationOptions =
      new() {
        DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingNull,
        Converters = { new JsonStringEnumConverter(
                           JsonNamingPolicy.SnakeCaseLower),
                       new ByteArrayJsonConverter() },
      };
}
