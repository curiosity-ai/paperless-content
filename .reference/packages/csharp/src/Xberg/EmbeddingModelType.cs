...
// alef:hash:ecdc27c5670c659f2d51b6cadd9ee8a831485dfd58c77befe77c25fe6af262c1
...
public abstract record EmbeddingModelType {
  ...
  /// </summary>
  public sealed record Llm([property : JsonPropertyName("llm")] LlmConfig Value)
      : EmbeddingModelType;
  ...
  /// (e.g. a wrapper around an already-loaded `llama-cpp-python`,
  /// `sentence-transformers`, or tuned ONNX model), then references it by name
  /// in config. Xberg calls back into the registered backend during chunking
  /// and standalone embed requests — no HuggingFace download, no ONNX Runtime
  /// requirement, no HTTP sidecar.
  ///
  /// When this variant is selected, only the following `EmbeddingConfig` fields
  /// apply: `normalize` (post-call L2 normalization) and
  /// `max_embed_duration_secs` (dispatcher timeout). Model-loading fields
  /// (`batch_size`, `cache_dir`, `show_download_progress`, `acceleration`) are
  /// ignored — the host owns the model lifecycle, so there is no download to
  /// report progress for.
  ///
  /// Semantic chunking falls back to `ChunkingConfig.max_characters` when this
  /// variant is used, since there is no preset to look a chunk-size ceiling up
  /// against — size your context window via `max_characters` directly.
  ///
  /// See `register_embedding_backend`.
  /// </summary>
  public sealed record Plugin([property : JsonPropertyName("name")] string Name)
      : EmbeddingModelType;
}

/// <summary>
/// Custom converter for EmbeddingModelType sealed union with flattened variant
/// fields.
/// </summary>
/// <remarks>
...
/// </remarks>
public sealed class EmbeddingModelTypeJsonConverter
    : JsonConverter<EmbeddingModelType> {
  public override EmbeddingModelType Read(ref Utf8JsonReader reader,
                                          Type typeToConvert,
    ...
    return tagValue switch {
      "preset" => JsonSerializer.Deserialize<EmbeddingModelType.Preset>(
                      flatJson, options) ??
                  throw new JsonException("Failed to deserialize variant"),
      "custom" => JsonSerializer.Deserialize<EmbeddingModelType.Custom>(
                      flatJson, options) ??
                  throw new JsonException("Failed to deserialize variant"),
      "llm" => JsonSerializer.Deserialize<EmbeddingModelType.Llm>(flatJson,
                                                                  options) ??
               throw new JsonException("Failed to deserialize variant"),
      "plugin" => JsonSerializer.Deserialize<EmbeddingModelType.Plugin>(
                      flatJson, options) ??
                  throw new JsonException("Failed to deserialize variant"),
      // Unknown discriminator: forward compatibility for unknown variants
      _ => throw new JsonException(
          $"Unknown EmbeddingModelType discriminator: {tagValue}")
    };
  }

  public override void Write(Utf8JsonWriter writer, EmbeddingModelType value,
                             JsonSerializerOptions options) {
    ...
    default:
      throw new JsonException(
          $"Unknown EmbeddingModelType variant: {value.GetType().Name}");
    }
