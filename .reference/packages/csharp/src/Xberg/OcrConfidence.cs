// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:167d241c3d553fb7b54e6839d86fbfe531d91e30cd6deeb2bec8fb1ec1b2d99f
// To regenerate: alef generate
// To verify freshness: alef verify
#nullable enable

using System;
using System.Collections.Generic;
using System.Text.Json;
using System.Text.Json.Serialization;

namespace Xberg;

/// <summary>
/// Confidence scores for an OCR element.
///
/// Separates detection confidence (how confident that text exists at this
/// location) from recognition confidence (how confident about the actual text
/// content).
/// </summary>
public sealed record OcrConfidence {
  /// <summary>
  /// Detection confidence: how confident the OCR engine is that text exists
  /// here.
  ///
  /// PaddleOCR provides this as `box_score`, Tesseract doesn't have a direct
  /// equivalent. Range: 0.0 to 1.0 (or null if not available).
  /// </summary>
  [JsonPropertyName("detection")]
  public double ? Detection { get; init; } = null;

  /// <summary>
  /// Recognition confidence: how confident about the text content.
  ///
  /// Range: 0.0 to 1.0.
  /// </summary>
  [JsonPropertyName("recognition")]
  public double Recognition {
    get; init;
  } = 0.0;

  /// <summary>
  /// Parse a <see cref="OcrConfidence"/> from a JSON string.
  /// </summary>
  /// <exception cref="XbergException">When the JSON cannot be
  /// deserialised.</exception>
  public static OcrConfidence FromJson(string json) {
    try {
      return JsonSerializer.Deserialize<OcrConfidence>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse OcrConfidence from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse OcrConfidence from JSON: {e.Message}", e);
    }
  }

  private static readonly JsonSerializerOptions JsonOptions = new() {
    DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingDefault,
    Converters = { new JsonStringEnumConverter(JsonNamingPolicy.SnakeCaseLower),
                   new ByteArrayJsonConverter() },
  };

  /// <summary>Options for serializing config/input objects to FFI. Strips nulls
  /// (nullable C# fields default to null and would override required Rust
  /// fields with non-deserialisable nulls) but preserves explicit false/0 so
  /// caller intent is kept.</summary>
  private static readonly JsonSerializerOptions JsonSerializationOptions =
      new() {
        DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingNull,
        Converters = { new JsonStringEnumConverter(
                           JsonNamingPolicy.SnakeCaseLower),
                       new ByteArrayJsonConverter() },
      };
}
