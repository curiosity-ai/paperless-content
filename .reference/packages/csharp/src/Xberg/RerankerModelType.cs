...
// alef:hash:e3c7dcaf727a824bec008038610294bae216e2dfe20f7ee1851f74d1571c14e6
...
public abstract record RerankerModelType {
  ...
  /// </summary>
    ...
    /// wrapper around a `sentence-transformers` cross-encoder or a provider
    /// client), then references it by name in config. Xberg calls back into the
    /// registered backend — no HuggingFace download, no ONNX Runtime
    /// requirement.
    ///
    /// When this variant is selected, only `max_rerank_duration_secs` applies.
    /// Model-loading fields (`batch_size`, `cache_dir`,
    /// `show_download_progress`, `acceleration`) are ignored — the host owns
    /// the model lifecycle, so there is no download to report progress for.
    ///
...
/// <summary>
/// Custom converter for RerankerModelType sealed union with flattened variant
/// fields.
/// </summary>
/// <remarks>
...
/// </remarks>
public sealed class RerankerModelTypeJsonConverter
    : JsonConverter<RerankerModelType> {
  public override RerankerModelType Read(ref Utf8JsonReader reader,
                                         Type typeToConvert,
    ...
    return tagValue switch {
      "preset" => JsonSerializer.Deserialize<RerankerModelType.Preset>(
                      flatJson, options) ??
                  throw new JsonException("Failed to deserialize variant"),
      "custom" => JsonSerializer.Deserialize<RerankerModelType.Custom>(
                      flatJson, options) ??
                  throw new JsonException("Failed to deserialize variant"),
      "llm" => JsonSerializer.Deserialize<RerankerModelType.Llm>(flatJson,
                                                                 options) ??
               throw new JsonException("Failed to deserialize variant"),
      "plugin" => JsonSerializer.Deserialize<RerankerModelType.Plugin>(
                      flatJson, options) ??
                  throw new JsonException("Failed to deserialize variant"),
      // Unknown discriminator: forward compatibility for unknown variants
      _ => throw new JsonException(
          $"Unknown RerankerModelType discriminator: {tagValue}")
    };
  }

  public override void Write(Utf8JsonWriter writer, RerankerModelType value,
                             JsonSerializerOptions options) {
    ...
    case RerankerModelType.Preset v_preset:
      ...
      inner = v_preset;
      break;
    case RerankerModelType.Custom v_custom:
      tag = "custom";
      inner = v_custom;
      break;
    case RerankerModelType.Llm v_llm:
      tag = "llm";
      inner = v_llm;
      break;
    case RerankerModelType.Plugin v_plugin:
      tag = "plugin";
      inner = v_plugin;
      break;
    default:
      throw new JsonException(
          $"Unknown RerankerModelType variant: {value.GetType().Name}");
    }
