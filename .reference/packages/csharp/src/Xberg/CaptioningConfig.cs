...
// alef:hash:f03fd6cd66e06bbea2f4df8a6da7c2469cf2424228dc95c04c694ce2ae0ef7df
...
public sealed record CaptioningConfig {
  ...
  public LlmConfig Llm { get; init; } = new LlmConfig();

  /// <summary>
  /// Optional custom caption prompt. `None` uses the default
  /// `RegionKind.Caption` prompt that ships with `crate.llm.region_extractor`.
  /// </summary>
    ...
    try {
      return JsonSerializer.Deserialize<CaptioningConfig>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse CaptioningConfig from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse CaptioningConfig from JSON: {e.Message}", e);
    }
  }
