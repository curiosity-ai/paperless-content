...
// alef:hash:85e275e17307a171d6d6f13132b40ec238ae5da5dc799fac33d43d1d3768391f
...
public sealed record XmlExtractionResult {
  ...
  public List<string> UniqueElements {
    get; init;
  } = [];
  ...
  public static XmlExtractionResult FromJson(string json) {
    try {
      return JsonSerializer.Deserialize<XmlExtractionResult>(json,
                                                             JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse XmlExtractionResult from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse XmlExtractionResult from JSON: {e.Message}", e);
    }
  }
