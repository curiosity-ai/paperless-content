...
// alef:hash:f5a7abbd6d679cd406b6f0fbb6221d1b2a5bf07d63ab73651948cf859e4ba4f3
...
public abstract record HostMatcher {
  ...
  /// </summary>
  public sealed record Cidr([property : JsonPropertyName("value")] string Value)
      : HostMatcher;
}
...
public sealed class HostMatcherJsonConverter : JsonConverter<HostMatcher> {
  public override HostMatcher Read(ref Utf8JsonReader reader,
                                   Type typeToConvert,
    ...
    return tagValue switch {
      "exact" =>
          JsonSerializer.Deserialize<HostMatcher.Exact>(flatJson, options) ??
          throw new JsonException("Failed to deserialize variant"),
      "suffix" =>
          JsonSerializer.Deserialize<HostMatcher.Suffix>(flatJson, options) ??
          throw new JsonException("Failed to deserialize variant"),
      "cidr" =>
          JsonSerializer.Deserialize<HostMatcher.Cidr>(flatJson, options) ??
          throw new JsonException("Failed to deserialize variant"),
      // Unknown discriminator: forward compatibility for unknown variants
      _ => throw new JsonException(
          $"Unknown HostMatcher discriminator: {tagValue}")
    };
  }

  public override void Write(Utf8JsonWriter writer, HostMatcher value,
                             JsonSerializerOptions options) {
    ...
    default:
      throw new JsonException(
          $"Unknown HostMatcher variant: {value.GetType().Name}");
    }
