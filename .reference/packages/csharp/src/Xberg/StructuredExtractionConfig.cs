...
// alef:hash:db1db0235ac6883a62a3656004012ce2b9768f66beecc5173208758c4b321b73
...
public sealed record StructuredExtractionConfig {
  ...
  /// <summary>
  /// Custom Jinja2 extraction prompt template. When `None`, a default template
  /// is used.
  ///
  ...
  public static StructuredExtractionConfig FromJson(string json) {
    try {
      return JsonSerializer.Deserialize<StructuredExtractionConfig>(
                 json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse StructuredExtractionConfig from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse StructuredExtractionConfig from JSON: {e.Message}",
          e);
    }
  }
