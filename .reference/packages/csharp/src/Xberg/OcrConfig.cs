...
// alef:hash:56859f1d98cdaf2f0980c69b9f98a94189a760b0a9a2768e52aa85afe7cc8cbe
...
public sealed record OcrConfig {
  ...
  ///
  /// Setting `enabled: false` is a shorthand for `disable_ocr: true` on the
  /// parent `ExtractionConfig`. Images return metadata only; PDFs use native
  /// text extraction without OCR fallback.
  ///
  ...
  /// Sceptre uses ONNX Runtime on desktop/server and tract on supported mobile
  /// builds. Browser WebAssembly uses the separate byte-fed Sceptre worker API.
  /// </summary>
  ...
  /// <summary>
  /// Language code(s) for OCR recognition. Defaults to `["eng"]`. For
  /// Tesseract, languages are joined with "+".
  ///
  ...
  /// overridden here — most notably `model_version` (`"pp-ocrv6"` default /
  /// `"pp-ocrv5"`) and `model_tier`. In TOML:
  ///
  /// ```toml
  ...
  /// ```
  ///
  /// The `XBERG_OCR_MODEL_VERSION` / `XBERG_OCR_MODEL_TIER` environment
  /// variables set the same two keys for env-configured servers (issue #1279).
  /// </summary>
  ...
  /// automatically:
  ///
  /// - `VlmFallbackPolicy.OnLowQuality` → `[classical_stage, vlm_stage]` with
  /// the
  ///   `quality_threshold` mapped onto
  ///   `OcrQualityThresholds.pipeline_min_quality`.
  ...
  public VlmFallbackPolicy VlmFallback {
    get; init;
  } = new VlmFallbackPolicy.Disabled();
  ...
  /// <summary>
  /// Hardware acceleration for ONNX Runtime models (e.g. PaddleOCR, layout
  /// detection).
  ///
  ...
  ///
  /// Not user-configurable via config files — injected at runtime from
  /// `ExtractionConfig.security_limits` before each `process_image` call, the
  /// same pattern `Self.acceleration` uses. `ExtractionConfig.security_limits`
  /// is the single source of truth: this field only ever holds a copy the
  /// caller placed here immediately before dispatch, so the two cannot drift. A
  /// backend consulting a `backend_options` override for this call may still
  /// let that override win, but in the absence of one this field is what
  /// backends should fall back to instead of `SecurityLimits.default()`. `None`
  /// means "use `SecurityLimits.default()`", never "disable the check".
  /// </summary>
  ...
  /// the standard resolution chain: TESSDATA_PREFIX env, cache dir, system
  /// paths.
  /// </summary>
    ...
    try {
      return JsonSerializer.Deserialize<OcrConfig>(json, JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse OcrConfig from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse OcrConfig from JSON: {e.Message}", e);
    }
  }
