// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:2b3fe6abe195d5c5af11649e466bc9f940038817f91155a34add7f9747ca0687
// To regenerate: alef generate
// To verify freshness: alef verify
#nullable enable

using System;
using System.Collections.Generic;
using System.Text.Json;
using System.Text.Json.Serialization;

namespace Xberg;

/// <summary>
/// Bounding box for an OCR-detected table in pixel coordinates.
/// </summary>
public sealed record OcrTableBoundingBox {
  /// <summary>
  /// Left x-coordinate (pixels)
  /// </summary>
  [JsonPropertyName("left")]
  public uint Left { get; init; } = 0;

  /// <summary>
  /// Top y-coordinate (pixels)
  /// </summary>
  [JsonPropertyName("top")]
  public uint Top {
    get; init;
  } = 0;

  /// <summary>
  /// Right x-coordinate (pixels)
  /// </summary>
  [JsonPropertyName("right")]
  public uint Right {
    get; init;
  } = 0;

  /// <summary>
  /// Bottom y-coordinate (pixels)
  /// </summary>
  [JsonPropertyName("bottom")]
  public uint Bottom {
    get; init;
  } = 0;

  /// <summary>
  /// Parse a <see cref="OcrTableBoundingBox"/> from a JSON string.
  /// </summary>
  /// <exception cref="XbergException">When the JSON cannot be
  /// deserialised.</exception>
  public static OcrTableBoundingBox FromJson(string json) {
    try {
      return JsonSerializer.Deserialize<OcrTableBoundingBox>(json,
                                                             JsonOptions) ??
             throw new XbergException(
                 $"Failed to parse OcrTableBoundingBox from JSON: deserializer returned null");
    } catch (XbergException) {
      throw;
    } catch (Exception e) {
      throw new XbergException(
          $"Failed to parse OcrTableBoundingBox from JSON: {e.Message}", e);
    }
  }

  private static readonly JsonSerializerOptions JsonOptions = new() {
    DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingDefault,
    Converters = { new JsonStringEnumConverter(JsonNamingPolicy.SnakeCaseLower),
                   new ByteArrayJsonConverter() },
  };

  /// <summary>Options for serializing config/input objects to FFI. Strips nulls
  /// (nullable C# fields default to null and would override required Rust
  /// fields with non-deserialisable nulls) but preserves explicit false/0 so
  /// caller intent is kept.</summary>
  private static readonly JsonSerializerOptions JsonSerializationOptions =
      new() {
        DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingNull,
        Converters = { new JsonStringEnumConverter(
                           JsonNamingPolicy.SnakeCaseLower),
                       new ByteArrayJsonConverter() },
      };
}
