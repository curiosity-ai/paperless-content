...
// alef:hash:995bc3cab12891a427ee9991918d0c2c02918ce0a7f59eedf4418ab5c69afa89
...
/// <summary>
/// Custom converter for ChunkingReason sealed union with flattened variant
/// fields.
/// </summary>
/// <remarks>
...
/// </remarks>
public sealed class ChunkingReasonJsonConverter
    : JsonConverter<ChunkingReason> {
  public override ChunkingReason Read(ref Utf8JsonReader reader,
                                      Type typeToConvert,
    ...
    return tagValue switch {
      "LargeFile" => JsonSerializer.Deserialize<ChunkingReason.LargeFile>(
                         flatJson, options) ??
                     throw new JsonException("Failed to deserialize variant"),
      "ManyPages" => JsonSerializer.Deserialize<ChunkingReason.ManyPages>(
                         flatJson, options) ??
                     throw new JsonException("Failed to deserialize variant"),
      "OcrRequired" => JsonSerializer.Deserialize<ChunkingReason.OcrRequired>(
                           flatJson, options) ??
                       throw new JsonException("Failed to deserialize variant"),
      "LargeAndManyPages" =>
          JsonSerializer.Deserialize<ChunkingReason.LargeAndManyPages>(
              flatJson, options) ??
          throw new JsonException("Failed to deserialize variant"),
      // Unknown discriminator: forward compatibility for unknown variants
      _ => throw new JsonException(
          $"Unknown ChunkingReason discriminator: {tagValue}")
    };
  }

  public override void Write(Utf8JsonWriter writer, ChunkingReason value,
                             JsonSerializerOptions options) {
    ...
    case ChunkingReason.LargeFile v_largefile:
      ...
      inner = v_largefile;
      break;
    case ChunkingReason.ManyPages v_manypages:
      tag = "ManyPages";
      inner = v_manypages;
      break;
    case ChunkingReason.OcrRequired v_ocrrequired:
      tag = "OcrRequired";
      inner = v_ocrrequired;
      break;
    case ChunkingReason.LargeAndManyPages v_largeandmanypages:
      tag = "LargeAndManyPages";
      inner = v_largeandmanypages;
      break;
    default:
      throw new JsonException(
          $"Unknown ChunkingReason variant: {value.GetType().Name}");
    }
