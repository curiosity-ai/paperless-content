...
// alef:hash:e733c4f9554fd712e8df8e6350b1be51204f9d0b70b055e45b2d6a7aa389ff6b
...
fn frb_regeneration_opted_in() -> bool {
    matches!(
        std::env::var(ALEF_FRB_REGENERATE_ON_BUILD).as_deref(),
        Ok("1") | Ok("true")
    )
}
...
fn fix_handler_executor_calls() {
    ...
    if out != source
        && let Err(err) = std::fs::write(path, &out)
    {
