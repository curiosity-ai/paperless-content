...
This only covers glyph indices that decode into the PUA/replacement/control range. The related case where a broken CID-to-Unicode mapping resolves onto *plausible* ASCII (so the text layer "looks" readable but is actually wrong) is not detected — detecting it requires new support in xberg's native PDF engine (`crates/xberg-native-pdf`), which is not implemented yet.
:::
