...
<!-- alef:hash:ebf66ecd46267c404260aaee8e9d1405844d6821b2a6e1f264a5b1b60702a57c -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Kotlin (Android) API Reference <span class="version-badge">v1.1.3</span>
...
```kotlin
val result = mapUrl("value", UrlExtractionConfig())
```
...
```kotlin
val result = listTokenizerBackends()
```
...
```kotlin
val result = classifyChunksOwned(ExtractedDocument(), ChunkClassificationConfig())
```
...
```kotlin
fun maxSimScore(query: MultiVectorEmbedding, doc: MultiVectorEmbedding): Double
```

**Example:**

```kotlin
val result = maxSimScore(MultiVectorEmbedding(), MultiVectorEmbedding())
```
...
```kotlin
val result = maxSimRank(MultiVectorEmbedding(), [])
```
...
```kotlin
val result = doctor(ExtractionConfig())
```
...
| `maxDepth` | `Long?` | `null` | Maximum DOM traversal depth. `null` uses the library's internal native-stack safety limit. Explicit values above that safety limit are clamped to prevent process-aborting stack overflows on pathologically deep DOM trees. |
...
| `maxLinksPerPage` | `Long?` | `null` | Maximum links enqueued from a single page. Defaults to 10000. Bounds the work one hostile or pathological page can create; links past the cap are dropped and a warning is logged. |
...
```kotlin
@Throws(XbergError::class)
fun extract(input: ExtractInput, config: ExtractionConfig): ExtractedDocument
```

**Example:**

```kotlin
val result = instance.extract(ExtractInput(), ExtractionConfig())
```
...
| `cacheDir` | `Path?` | `null` | Optional alternate Hugging Face cache root for model files. When unset, hf-hub follows `HF_HUB_CACHE`, `HUGGINGFACE_HUB_CACHE`, `HF_HOME`, XDG, and platform defaults. Prefer those environment variables when configuring the cache process-wide. |
...
| `extractionTimeoutSecs` | `Long?` | `600` | Default per-file timeout in seconds for batch extraction. When set, each file in a batch will be canceled after this duration unless overridden by `FileExtractionConfig.timeout_secs`. Defaults to `Some(600)` (10 minutes) to prevent pathological files (e.g. deeply nested archives, documents with millions of cells) from running indefinitely and exhausting caller resources, while still giving slow paths (VLM-based OCR, large scanned documents) enough headroom to finish. Set to `null` to disable the timeout for trusted input or long-running workloads. |
| `maxConcurrentExtractions` | `Long?` | `null` | Maximum concurrent document extractions in batch operations. This is a ceiling within the configured total thread budget, not an independent pool size. When unset, the scheduler derives document and per-document concurrency from `ConcurrencyConfig.max_threads`. |
...
| `qrCodes` | `Boolean?` | `null` | Override QR-code detection for this file. |
...
| `cacheDir` | `Path?` | `null` | Optional alternate Hugging Face cache root for model files. When unset, hf-hub follows the standard Hugging Face environment and platform cache conventions. |
| `acceleration` | `AccelerationConfig?` | `null` | Hardware acceleration for the late-interaction ONNX model. |
| `maxEmbedDurationSecs` | `Long?` | `60` | Maximum wall-clock duration (in seconds) for a single embed call when using `LateInteractionModelType.Plugin`. `null` disables the timeout. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `maxEntries` | `Long?` | `null` | Maximum number of cached entries. |
| `ttlSeconds` | `Long?` | `null` | Cache entry time-to-live, in seconds. |
...
| `timeoutSecs` | `Long?` | `null` | Request timeout in seconds. When `null`, liter-llm's built-in 60s default applies, except the VLM OCR path which uses a 300s default (a single page image transcription routinely exceeds 60s). Set explicitly to override. |
...
| `extraBody` | `Any?` | `null` | Provider-specific extra parameters merged into the request body (guardrails, safety settings, grounding config, etc.), applied to individual requests built from this config. Mirrors liter-llm's `ChatCompletionRequest.extra_body`. A request-time parameter like `temperature`/`max_tokens` above, not a client-level setting. |
| `loadEnv` | `Boolean?` | `null` | Whether liter-llm should load provider credentials from environment variables. Mirrors liter-llm's `ClientConfigBuilder.load_env`. When `null`, liter-llm's own default behavior applies. |
...
| `costTracking` | `Boolean?` | `null` | Enable per-request cost tracking. Mirrors liter-llm's `LlmConfig.cost_tracking`. Only takes effect when liter-llm's `tower` feature is compiled in; otherwise the value is accepted but unused. |
| `tracing` | `Boolean?` | `null` | Enable OpenTelemetry-compatible tracing spans. Mirrors liter-llm's `LlmConfig.tracing`. Only takes effect when liter-llm's `tower` feature is compiled in; otherwise the value is accepted but unused. |
| `cooldownSecs` | `Long?` | `null` | Cooldown duration after transient errors, in seconds. Mirrors liter-llm's `LlmConfig.cooldown_secs`. Only takes effect when liter-llm's `tower` feature is compiled in; otherwise the value is accepted but unused. |
| `healthCheckSecs` | `Long?` | `null` | Background health check interval, in seconds. Mirrors liter-llm's `LlmConfig.health_check_secs`. Only takes effect when liter-llm's `tower` feature is compiled in; otherwise the value is accepted but unused. |
...
| `tpm` | `Long?` | `null` | Tokens per minute limit. |
| `windowSeconds` | `Long?` | `null` | Rate limit window, in seconds. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `detModel` | `Path` | — | Exact path to the detection ONNX model in the Hugging Face snapshot. |
| `clsModel` | `Path` | — | Exact path to the classification ONNX model in the Hugging Face snapshot. |
| `recModel` | `Path` | — | Exact path to the recognition ONNX model in the Hugging Face snapshot. |
...
```kotlin
@Throws(XbergError::class)
fun processImage(imageBytes: ByteArray, config: OcrConfig): ExtractedDocument
```
...
```kotlin
@Throws(XbergError::class)
fun processImageFile(path: Path, config: OcrConfig): ExtractedDocument
```
...
```kotlin
fun confidenceSemantics(): ConfidenceSemantics
```

**Example:**

```kotlin
val result = instance.confidenceSemantics()
```
...
```kotlin
fun pageOrientationHandling(): PageOrientationHandling
```

**Example:**

```kotlin
val result = instance.pageOrientationHandling()
```
...
```kotlin
@Throws(XbergError::class)
fun processDocument(path: Path, config: OcrConfig): ExtractedDocument
```
...
| `cacheDir` | `Path?` | `null` | Optional Hugging Face Hub cache root for model files. When unset, the standard `HF_HUB_CACHE`, legacy `HUGGINGFACE_HUB_CACHE`, and `HF_HOME` conventions are used. |
...
```kotlin
@Throws(XbergError::class)
fun process(result: ExtractedDocument, config: ExtractionConfig)
```
...
```kotlin
val result = instance.shouldProcess(ExtractedDocument(), ExtractionConfig())
```
...
```kotlin
val result = instance.estimatedDurationMs(ExtractedDocument())
```
...
```kotlin
fun get_(id: String): Preset?
```

**Example:**

```kotlin
val result = instance.get_("value")
```
...
```kotlin
val result = instance.renderResult(ExtractedDocument())
```
...
| `maxRerankDurationSecs` | `Long?` | `60` | Maximum wall-clock duration (in seconds) for a single `rerank()` call when using `RerankerModelType.Plugin`. Applies only to the in-process plugin path — protects against hung host-language backends. On timeout, the dispatcher returns `Plugin` instead of blocking forever. `null` disables the timeout. The default (60 seconds) is conservative for common in-process inference; increase for large document sets on slow hardware. |
...
| `maxEmbedDurationSecs` | `Long?` | `60` | Maximum wall-clock duration (in seconds) for a single embed call when using `SparseEmbeddingModelType.Plugin`. `null` disables the timeout. |
...
| `value` | `Any?` | `null` | The parsed document as a canonical `serde_json.Value` tree, when the source format could be represented as one. `null` only for TOML inputs whose `toml.Value` fails to round-trip through `serde_json.Value` (xberg-io/xberg#155): the extractor falls back to a raw code block in that case. |
...
```kotlin
val result = instance.countTokens("value")
```
...
| `maxDurationMs` | `Long?` | `1800000` | Hard safety limit on input duration (milliseconds). Files longer than this are rejected after decode, before model work. Default: 30 minutes. Set to `null` to disable (not recommended for untrusted input). |
| `maxBytes` | `Long?` | `536870912` | Hard safety limit on input size (bytes). Default: 512 MiB. Protects against pathological or malicious uploads. |
| `timeoutMs` | `Long?` | `600000` | Wall-clock timeout for the entire transcription operation (ms). Bounds audio decode, model resolution/download, and inference together. On expiry the extraction fails with an `XbergError.Transcription`. `null` disables the bound and lets the operation run unbounded (not recommended for untrusted input). Enforced on the async extraction path only; the size and duration caps (`max_bytes`, `max_duration_ms`) are checked on every path. Default: 10 minutes. |
| `modelCacheDir` | `Path?` | `null` | Optional alternate Hugging Face cache root for Whisper models. When unset, hf-hub follows `HF_HUB_CACHE`, `HUGGINGFACE_HUB_CACHE`, `HF_HOME`, XDG, and platform defaults. Files remain in the standard content-addressed snapshot layout and are not copied into an Xberg cache. |
...
```kotlin
@Throws(XbergError::class)
fun validate(result: ExtractedDocument, config: ExtractionConfig)
```
...
```kotlin
val result = instance.shouldValidate(ExtractedDocument(), ExtractionConfig())
```
...
| `Tokenizer` | Size measured in tokens from a HuggingFace tokenizer or a registered tokenizer backend. — Fields: `model`: `String`, `cacheDir`: `Path` |
...
|-------|----------|------------|-------|
| `OcrBackend` | OcrBackendBridge.register | OcrBackendBridge.unregister | OcrBackendBridge.clearAll |
| `PostProcessor` | PostProcessorBridge.register | PostProcessorBridge.unregister | PostProcessorBridge.clearAll |
| `Validator` | ValidatorBridge.register | ValidatorBridge.unregister | ValidatorBridge.clearAll |
| `DocumentExtractor` | DocumentExtractorBridge.register | DocumentExtractorBridge.unregister | DocumentExtractorBridge.clearAll |
| `EmbeddingBackend` | EmbeddingBackendBridge.register | EmbeddingBackendBridge.unregister | EmbeddingBackendBridge.clearAll |
| `Renderer` | RendererBridge.register | RendererBridge.unregister | RendererBridge.clearAll |
| `RerankerBackend` | RerankerBackendBridge.register | RerankerBackendBridge.unregister | RerankerBackendBridge.clearAll |
| `TokenizerBackend` | TokenizerBackendBridge.register | TokenizerBackendBridge.unregister | TokenizerBackendBridge.clearAll |
