...
<!-- alef:hash:07cd25ae0c2354954b4d60a8b457fb897c9aae764b2639353724240e0a78d09d -->
...
```python
def extract(input: ExtractInput, config: ExtractionConfig) -> ExtractionResult
```
...
```python
def extract_batch(inputs: list[ExtractInput], config: ExtractionConfig) -> ExtractionResult
```
...
```python
def map_url(uri: str, config: UrlExtractionConfig) -> MapResult
```

**Example:**

```python
result = map_url("value", UrlExtractionConfig())
```
...
```python
def classify_chunks_owned(result: ExtractedDocument, config: ChunkClassificationConfig) -> ExtractedDocument
```

**Example:**

```python
result = classify_chunks_owned(ExtractedDocument(), ChunkClassificationConfig())
```
...
```python
result = max_sim_score(MultiVectorEmbedding(), MultiVectorEmbedding())
```
...
```python
def max_sim_rank(query: MultiVectorEmbedding, docs: list[MultiVectorEmbedding]) -> list[LateInteractionMatch]
```

**Example:**

```python
result = max_sim_rank(MultiVectorEmbedding(), [])
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `label` | `str` | — | Label name returned in `ChunkMetadata.classifications`. |
| `description` | `str` | — | Semantic description of when this label applies. Injected verbatim into the classification prompt next to the label name. |
...
| `node_ids` | `list\[str\]` | `/* serde(default) */` | Ids of the `DocumentNode`s this chunk was derived from. Joins a chunk back to the structured document tree via `DocumentNode.id`. Populated from exact node provenance when available, with a textual containment fallback for rendered chunks that do not retain byte offsets. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `text` | `str` | — | The raw source text of this chunk. |
| `context_path` | `list\[str\]` | — | Hierarchical path of enclosing structural items (e.g. `["MyClass", "my_method"]`). |
| `node_types` | `list\[str\]` | — | Tree-sitter node kinds that appear at the top level of this chunk (e.g. `"function_definition"`, `"class_definition"`). |
| `byte_start` | `int` | — | Inclusive start byte offset of this chunk in the original source. |
| `byte_end` | `int` | — | Exclusive end byte offset of this chunk in the original source. |
...
| `byte_start` | `int` | — | Inclusive start byte offset of the `name="value"` attribute token. |
| `byte_end` | `int` | — | Exclusive end byte offset of the `name="value"` attribute token. |
...
| `key` | `str \| None` | `/* serde(default) */` | Key, attribute name, tag name, or positional index (`"0"`, `"1"`, …). `None` at the document root. |
| `value` | `str \| None` | `/* serde(default) */` | Leaf scalar value, if any. `None` for containers (objects, arrays, XML elements with child elements). |
...
| `byte_start` | `int` | — | Inclusive start byte offset of this node in the original source. |
| `byte_end` | `int` | — | Exclusive end byte offset of this node in the original source. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `str` | — | Check identifier, e.g. `ocr.tesseract` or `layout.rtdetr`. |
| `status` | `ProbeStatus` | — | Pass / warn / fail / skip verdict. |
| `message` | `str` | — | One-line reason or detail (e.g. missing language, resolved path, error). |
...
```python
def extract(self, input: ExtractInput, config: ExtractionConfig) -> ExtractedDocument
```

**Example:**

```python
result = instance.extract(ExtractInput(), ExtractionConfig())
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `element_id` | `str` | `/* serde(default) */` | Deterministic element identifier. Empty only when deserializing legacy payloads that predate this field's wire representation. |
...
| `bbox` | `HierarchicalBoundingBox \| None` | `None` | Bounding box information for the block Contains left, top, right, and bottom coordinates in PDF units. |
...
| `dimensions` | `ImageDimensions \| None` | `None` | Image dimensions if available. |
...
| `new_dimensions` | `PixelDimensions \| None` | `None` | New dimensions after resizing (if resized). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `language` | `str` | — | ISO 639-3 language code, matching the corresponding entry in `detected_languages`. |
...
| `script` | `str` | — | Writing system whatlang detected for this language (e.g. `"Latin"`, `"Cyrillic"`). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `str` | — | Stable preset name referenced from config. |
| `model_repo` | `str` | — | HuggingFace repository hosting the ONNX model. |
| `model_file` | `str` | — | Path to the ONNX file within the repo. |
| `additional_files` | `list\[str\]` | — | Sibling files that must be downloaded alongside `model_file`. |
| `max_length` | `int` | — | Maximum document token sequence length. |
| `query_max_length` | `int` | — | Fixed padded query length (ColBERT query augmentation). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `det_model` | `str` | — | Exact path to the detection ONNX model in the Hugging Face snapshot. |
| `cls_model` | `str` | — | Exact path to the classification ONNX model in the Hugging Face snapshot. |
| `rec_model` | `str` | — | Exact path to the recognition ONNX model in the Hugging Face snapshot. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `num_tokens` | `int` | — | Number of attention-live token rows (padding rows are dropped, not zeroed — see `engine.normalize_tokens`). |
...
```python
def confidence_semantics(self) -> ConfidenceSemantics
```
...
```python
def page_orientation_handling(self) -> PageOrientationHandling
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `backend` | `str` | — | Backend name: "tesseract", "paddleocr", "paddle-ocr", "sceptre", "vlm", or a custom registered name. Sceptre uses ONNX Runtime on desktop/server and tract on Android/iOS; browser WebAssembly has a separate byte-fed engine because the normal async OCR registry assumes native model storage. |
| `priority` | `int` | `100` | Priority weight (higher = tried first). Stages are sorted by priority descending. |
| `language` | `list\[str\] \| None` | `/* serde(default) */` | Language override for this stage (None = use parent OcrConfig.language). A list is the canonical form and the only form accepted by the binding object APIs: `["eng", "deu"]`. When deserializing from a config file, JSON body, or the REST/MCP API, a single string is also accepted, either as one code ("eng") or "+"-joined ("eng+deu"). |
...
| `image_indices` | `list\[int\]` | `/* serde(default) */` | Indices into `ExtractedDocument.images` for images found on this page. Each value is a zero-based index into the top-level `images` collection. Only populated when `extract_images = true` in the extraction config. |
| `image_preprocessing` | `ImagePreprocessingMetadata \| None` | `/* serde(default) */` | OCR image preprocessing applied to this page's raster. |
...
| `ocr_confidence` | `PageOcrConfidence \| None` | `/* serde(default) */` | Aggregate OCR confidence for this page. `None` when the page was not OCR'd. |
...
| `dimensions` | `PageDimensions \| None` | `/* serde(default) */` | Dimensions in points (PDF) or pixels (images). |
...
| `bbox` | `BoundingBox \| None` | `None` | Bounding box on this page, if known. |
...
| `author` | `str \| None` | `/* serde(default) */` | Author/creator of the annotation (PDF `/T` entry). |
| `modified` | `str \| None` | `/* serde(default) */` | Last modification date of the annotation (PDF `/M` entry), as a raw PDF date string (e.g. `"D:20240115120000Z"`). |
| `color` | `str \| None` | `/* serde(default) */` | Annotation colour (PDF `/C` entry), normalised to a CSS-compatible `#rrggbb` hex string. Gray and CMYK colour spaces are converted to RGB. |
| `subject` | `str \| None` | `/* serde(default) */` | Subject of the annotation (PDF `/Subj` entry). |
| `quad_points` | `list\[BoundingBox\] \| None` | `/* serde(default) */` | Per-line bounding boxes derived from the annotation's `/QuadPoints` entry. Present for text markup annotations (Highlight, Underline, StrikeOut, Squiggly), one box per marked line/run of text. |
| `marked_text` | `str \| None` | `/* serde(default) */` | The document text covered by `Self.quad_points`, recovered from the page content underneath the marked-up region. Populated for Highlight, Underline, StrikeOut, and Squiggly annotations when the underlying text could be recovered. |
...
```python
instance.process(ExtractedDocument(), ExtractionConfig())
```
...
```python
result = instance.should_process(ExtractedDocument(), ExtractionConfig())
```
...
```python
def estimated_duration_ms(self, result: ExtractedDocument) -> int
```

**Example:**

```python
result = instance.estimated_duration_ms(ExtractedDocument())
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `str` | — | Property name, such as `"bold"`, `"italic"`, `"font_size"`, or `"font_color"`. |
| `from` | `str \| None` | `None` | Value before the change, when available. |
| `to` | `str \| None` | `None` | Value after the change, when available. |
...
```python
def render_result(self, result: ExtractedDocument) -> str
```

**Example:**

```python
result = instance.render_result(ExtractedDocument())
```
...
| `value` | `dict\[str, Any\] \| None` | `None` | The parsed document as a canonical `serde_json.Value` tree, when the source format could be represented as one. `None` only for TOML inputs whose `toml.Value` fails to round-trip through `serde_json.Value` (xberg-io/xberg#155): the extractor falls back to a raw code block in that case. |
| `flattened` | `list\[str\]` | — | Flattened `path: value` renderings for every leaf field, in traversal order. Previously computed and discarded (xberg-io/xberg#166); now surfaced so callers get a full-text view even when the structured renderer only emits headings/lists for a subset of fields. |
...
| `links` | `list\[MarkdownLink\] \| None` | `None` | Markdown links (Markdown files only). |
| `code_blocks` | `list\[MarkdownCodeBlock\] \| None` | `None` | Code blocks (Markdown files only). |
...
| `content` | `str` | — | Translated plain-text body. Matches the shape of `ExtractedDocument.content`. |
...
```python
instance.validate(ExtractedDocument(), ExtractionConfig())
```
...
```python
result = instance.should_validate(ExtractedDocument(), ExtractionConfig())
```
...
| Value | Description |
|-------|-------------|
| `PRESET` | Use a preset ColBERT model (recommended). — Fields: `name`: `str` |
| `CUSTOM` | Use a custom ColBERT ONNX model from HuggingFace. — Fields: `model_id`: `str`, `model_file`: `str`, `additional_files`: `list[str]`, `max_length`: `int` |
| `PLUGIN` | In-process late-interaction backend registered via the plugin system. — Fields: `name`: `str` |
...
| Value | Description |
|-------|-------------|
| `AZURE_AD` | Azure AD OAuth2 client-credentials flow (Azure OpenAI / Azure Cognitive Services). — Fields: `tenant_id`: `str`, `client_id`: `str`, `client_secret`: `str`, `scope`: `str` |
...
| `VERTEX_ADC` | Google Vertex AI Application Default Credentials, resolved from the GCE/GKE/Cloud Run metadata server. Carries no secret material at all. — Fields: `scope`: `str` |
| `BEDROCK_WEB_IDENTITY` | AWS STS `AssumeRoleWithWebIdentity` (EKS IRSA / OIDC federation) for Bedrock. — Fields: `role_arn`: `str`, `token_file`: `str`, `session_name`: `str`, `region`: `str` |
...
| `TOKENIZER` | Size measured in tokens from a HuggingFace tokenizer or a registered tokenizer backend. — Fields: `model`: `str`, `cache_dir`: `str` |
...
| `CUSTOM` | Use a custom ONNX cross-encoder from HuggingFace. — Fields: `model_id`: `str`, `model_file`: `str`, `additional_files`: `list[str]`, `max_length`: `int`, `head`: `RerankerHead` |
...
| Value | Description |
|-------|-------------|
| `PRESET` | Use a preset SPLADE model (recommended). — Fields: `name`: `str` |
| `CUSTOM` | Use a custom SPLADE (`BertForMaskedLM`) ONNX model from HuggingFace. — Fields: `model_id`: `str`, `model_file`: `str`, `additional_files`: `list[str]`, `max_length`: `int` |
| `PLUGIN` | In-process sparse-embedding backend registered via the plugin system. — Fields: `name`: `str` |
...
| `COMMENT` | Reviewer/editor comment content (e.g. DOCX comments). Distinct from `NodeContent.Footnote` (xberg-io/xberg#300): comments and footnotes both reach the internal document via a marker/definition pair, but a consumer needs to tell a reviewer comment apart from an authored footnote. — Fields: `text`: `str` — **Since:** `v1.1` |
...
| Value | Description |
|-------|-------------|
| `EXACT` | Exact hostname match (case-insensitive). — Fields: `value`: `str` |
| `SUFFIX` | Suffix match: ".xberg.io" matches "api.xberg.io" and "xberg.io". — Fields: `value`: `str` |
| `CIDR` | CIDR match: "10.0.0.0/8" matches IP addresses in that range. — Fields: `value`: `str` |
...
|-----------|-------------|
| `IoError(XbergError)` | A file system or I/O operation failed. These errors always bubble up unchanged. |
| `ParsingError(XbergError)` | Document parsing failed (e.g. corrupt file, unsupported format feature). |
| `OcrError(XbergError)` | An OCR engine returned an error or produced unusable output. |
| `ValidationError(XbergError)` | Invalid configuration or input parameters were supplied. |
| `CacheError(XbergError)` | A cache read or write operation failed. |
| `ImageProcessingError(XbergError)` | An image manipulation operation (resize, decode, DPI conversion) failed. |
| `SerializationError(XbergError)` | JSON or MessagePack serialization/deserialization failed. |
| `MissingDependencyError(XbergError)` | A required optional system dependency (e.g. `tesseract`) was not found. |
| `PluginError(XbergError)` | A registered plugin returned an error during extraction. |
| `LockPoisonedError(XbergError)` | An internal `Mutex` or `RwLock` was found in a poisoned state. |
| `UnsupportedFormatError(XbergError)` | The document's MIME type is not supported by any registered extractor. |
| `EmbeddingError(XbergError)` | The embedding model or embedding pipeline returned an error. |
| `RerankingError(XbergError)` | The reranker model or reranking pipeline returned an error. |
| `TranscriptionError(XbergError)` | Audio/video transcription failed. |
| `XbergTimeoutError(XbergError)` | The extraction operation exceeded the configured time limit. |
| `CancelledError(XbergError)` | The extraction stopped after a cooperative cancellation request. Rust callers can request cancellation through `CancellationToken`. The REST async-jobs API uses the same mechanism. Generated language bindings do not currently expose in-process cancellation. |
| `SecurityError(XbergError)` | A security policy was violated (e.g. zip bomb, oversized archive). |
| `OtherError(XbergError)` | A catch-all for uncommon errors that do not fit another variant. |
...
| Exception | Description |
|-----------|-------------|
| `ParseError(LoadError)` | The file is not valid JSON. |
| `SchemaValidationError(LoadError)` | The file parses as JSON but does not validate against the meta-schema. |
| `DeserializeError(LoadError)` | The file validates but cannot be deserialized into `Preset`. |
| `IdMismatchError(LoadError)` | The preset's declared `id` does not match its file-system location. |
| `BadMetaSchemaError(LoadError)` | The meta-schema itself failed to compile. |
| `IoError(LoadError)` | A filesystem I/O error occurred while reading a preset directory. |
...
| Exception | Description |
|-----------|-------------|
| `SchemaNotObjectError(ResolveError)` | A custom schema override was supplied but is not a JSON object. |
