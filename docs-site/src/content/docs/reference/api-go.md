...
<!-- alef:hash:2e388b78c09e794b651a4d1764cdaebb5bc05bb2a633d36d346d0aac1a616089 -->
...
```go
func ClassifyChunksOwned(result ExtractedDocument, config ChunkClassificationConfig) (*ExtractedDocument, error)
```

**Example:**

```go
result, err := ClassifyChunksOwned(ExtractedDocument{}, ChunkClassificationConfig{})
if err != nil {
    return err
}
```
...
|-------|------|---------|-------------|
| `Provider` | `ExecutionProviderType` | `ExecutionProviderTypeAuto` | Execution provider to use for ONNX inference. |
...
| `KeyValues` | `\[\]KeyValueAttribute` | `nil` | Key-value pairs (key="value") |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `EntryCount` | `uint` | — | Number of entries in the bibliography. |
...
| `EntryTypes` | `*map\[string\]uint` | `nil` | Count of entries grouped by BibTeX entry type (e.g. `"article"` → 5). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `Mode` | `BrowserMode` | `BrowserModeAuto` | When to use the headless browser fallback. |
| `Backend` | `BrowserBackend` | `BrowserBackendChromiumoxide` | Browser backend used to render JavaScript-heavy pages. |
...
| `Wait` | `BrowserWait` | `BrowserWaitNetworkIdle` | Wait strategy after browser navigation. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `TotalFiles` | `uint` | — | Total number of files currently in the cache directory. |
...
| `MinImageArea` | `uint32` | `1000` | Skip images whose `width * height` is below this threshold (in pixels). Default `1_000` filters out icons and decorations. |
...
```go
result := CaptioningConfigDefaultMinImageArea()
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `Row` | `uint` | — | Zero-based row index. |
| `Col` | `uint` | — | Zero-based column index. |
...
| `BatchSize` | `uint` | `10` | Number of chunks batched into a single LLM request. Larger batches amortize the fixed prompt cost (definitions block) across more chunks, at the risk of exceeding the model's context window for very large taxonomies or chunk texts. Defaults to `DEFAULT_BATCH_SIZE`. |
| `MaxConcurrency` | `uint` | `4` | Maximum number of in-flight batch requests. Bounds concurrency against the configured LLM provider. Defaults to `DEFAULT_MAX_CONCURRENCY`. |
...
```go
result := ChunkClassificationConfigDefaultBatchSize()
```
...
```go
result := ChunkClassificationConfigDefaultMaxConcurrency()
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `ByteStart` | `uint` | — | Byte offset where this chunk starts in the original text (UTF-8 valid boundary). |
| `ByteEnd` | `uint` | — | Byte offset where this chunk ends in the original text (UTF-8 valid boundary). |
| `TokenCount` | `*uint` | `nil` | Number of tokens in this chunk (if available). This is calculated by the embedding model's tokenizer if embeddings are enabled. |
| `ChunkIndex` | `uint` | — | Zero-based index of this chunk in the document. |
| `TotalChunks` | `uint` | — | Total number of chunks in the document. |
...
| `NodeIds` | `\[\]string` | `/* serde(default) */` | Ids of the `DocumentNode`s this chunk was derived from. Joins a chunk back to the structured document tree via `DocumentNode.id`. Populated from exact node provenance when available, with a textual containment fallback for rendered chunks that do not retain byte offsets. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `MaxCharacters` | `uint` | `1000` | Maximum size per chunk (in units determined by `sizing`). When `sizing` is `Characters` (default), this is the max character count. When using token-based sizing, this is the max token count. Default: 1000 |
| `Overlap` | `uint` | `200` | Overlap between chunks (in units determined by `sizing`). Default: 200 |
| `Trim` | `bool` | `true` | Whether to trim whitespace from chunk boundaries. Default: true |
| `ChunkerType` | `ChunkerType` | `ChunkerTypeText` | Type of chunker to use (Text or Markdown). Default: Text |
...
| `Sizing` | `ChunkSizing` | `ChunkSizingCharacters` | How to measure chunk size. Default: `Characters` (Unicode character count). Enable `chunking-tiktoken` or `chunking-tokenizers` features for token-based sizing. |
| `TopicThreshold` | `*float32` | `nil` | Optional cosine similarity threshold for semantic topic boundary detection. Only used when `chunker_type` is `Semantic` and an `EmbeddingConfig` is provided. You almost never need to set this. When omitted, defaults to `0.75` which works well for most documents. Lower values detect more topic boundaries (more, smaller chunks); higher values detect fewer. Range: `0.0..=1.0`. |
| `TableChunking` | `TableChunkingMode` | `TableChunkingModeSplit` | How to handle markdown tables that exceed the chunk size limit. Only applies when `chunker_type` is `Markdown`. - `Split` (default) — tables are split at row boundaries; continuation chunks do not repeat the header. - `RepeatHeader` — the table header row and separator are prepended to every continuation chunk so each chunk is self-contained. Default: `Split` |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `CitationCount` | `uint` | — | Total number of citation records in the file. |
...
| `ByteStart` | `uint` | — | Inclusive start byte offset of this chunk in the original source. |
| `ByteEnd` | `uint` | — | Exclusive end byte offset of this chunk in the original source. |
...
| `ByteStart` | `uint` | — | Inclusive start byte offset of the `name="value"` attribute token. |
| `ByteEnd` | `uint` | — | Exclusive end byte offset of the `name="value"` attribute token. |
...
| `ByteStart` | `uint` | — | Inclusive start byte offset of this node in the original source. |
| `ByteEnd` | `uint` | — | Exclusive end byte offset of this node in the original source. |
...
| `StripTags` | `\[\]string` | `nil` | HTML tag names to strip (render children only, remove the tag wrapper). Default: `[]`. |
...
| `MaxDepth` | `*uint` | `nil` | Max DOM traversal depth. Prevents stack overflow on deeply nested HTML. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `HeadingStyle` | `HeadingStyle` | `HeadingStyleAtx` | Heading style to use in Markdown output (ATX `#` or Setext underline). |
| `ListIndentType` | `ListIndentType` | `ListIndentTypeSpaces` | How to indent nested list items (spaces or tab). |
| `ListIndentWidth` | `uint` | `2` | Number of spaces (or tabs) to use for each level of list indentation. |
...
| `HighlightStyle` | `HighlightStyle` | `HighlightStyleDoubleEqual` | Style used for `<mark>` / highlighted text (e.g. `==text==`). |
...
| `WrapWidth` | `uint` | `80` | Maximum output line width in characters when `wrap` is `true` (default `80`). Lines are broken at word boundaries so that no line exceeds this length. A value of `0` is treated as "no limit" — equivalent to leaving `wrap` disabled. Has no effect when `wrap` is `false`. |
...
| `NewlineStyle` | `NewlineStyle` | `NewlineStyleSpaces` | How to encode hard line breaks (`<br>`) in Markdown. |
| `CodeBlockStyle` | `CodeBlockStyle` | `CodeBlockStyleBackticks` | Style used for fenced code blocks (backticks or tilde). |
...
| `UrlEscapeStyle` | `UrlEscapeStyle` | `URLEscapeStyleAngle` | URL encoding strategy for link and image destinations. Controls how special characters in URL destinations are escaped: - `UrlEscapeStyle.Angle` (default) — wraps the destination in angle brackets when it contains spaces or newlines. Some parsers misinterpret `>` inside such a destination. - `UrlEscapeStyle.Percent` — percent-encodes every character that is not an RFC 3986 unreserved character or `/`, producing a destination that all Markdown parsers handle correctly even when the URL contains `<`, `>`, spaces, or parentheses. |
| `LinkStyle` | `LinkStyle` | `LinkStyleInline` | Link rendering style (inline or reference). |
| `OutputFormat` | `HtmlToMarkdownRsOutputFormat` | `HTMLToMarkdownRsOutputFormatMarkdown` | Target output format (Markdown, plain text, etc.). |
...
| `MaxDepth` | `*uint` | `nil` | Maximum DOM traversal depth. `nil` uses the library's internal native-stack safety limit. Explicit values above that safety limit are clamped to prevent process-aborting stack overflows on pathologically deep DOM trees. |
...
| `TierStrategy` | `TierStrategy` | `TierStrategyAuto` | Which conversion tier to use. - `TierStrategy.Auto` (default) — automatically choose the best path. - `TierStrategy.Tier2` — always use the Tier-2 DOM-walk path. - `TierStrategy.Tier1` — always attempt Tier-1 (testkit only). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `MaxDepth` | `*uint` | `nil` | Maximum crawl depth (number of link hops from the start URL). |
| `MaxPages` | `*uint` | `nil` | Maximum number of pages to crawl. |
| `MaxLinksPerPage` | `*uint` | `nil` | Maximum links enqueued from a single page. Defaults to 10000. Bounds the work one hostile or pathological page can create; links past the cap are dropped and a warning is logged. |
| `MaxConcurrent` | `*uint` | `nil` | Maximum number of concurrent requests. |
| `CrawlStrategy` | `CrawlStrategyKind` | `CrawlStrategyKindBfs` | Traversal order. Defaults to breadth-first. A frontier or strategy set explicitly on `CrawlEngineBuilder` takes precedence over this field. |
| `ContentFilter` | `*ContentFilterKind` | `nil` | Content filter applied to each page. `nil` keeps every page. A content filter set explicitly on `CrawlEngineBuilder` takes precedence. |
| `Bm25Query` | `*string` | `nil` | Query the BM25 content filter scores pages against. Required by `ContentFilterKind.Bm25`. |
| `Bm25Threshold` | `*float64` | `nil` | Minimum BM25 score a page must reach to be kept. Defaults to `0.0`. |
...
| `MapLimit` | `*uint` | `nil` | Maximum number of URLs to return from a map operation. |
...
| `MaxAssetSize` | `*uint` | `nil` | Maximum size in bytes for individual asset downloads. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `RecordCount` | `uint` | — | Total number of data records in the DBF file. |
| `FieldCount` | `uint` | — | Number of field (column) definitions. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `FromLine` | `uint` | — | Starting line number in the old content (0-indexed). |
| `FromCount` | `uint` | — | Number of lines from the old content in this hunk. |
| `ToLine` | `uint` | — | Starting line number in the new content (0-indexed). |
| `ToCount` | `uint` | — | Number of lines from the new content in this hunk. |
...
| `MaxContentChars` | `*uint` | `nil` | Truncate content to this many characters before diffing. Useful for very large documents where only the first N characters matter. `nil` means no truncation. |
...
| `Attributes` | `\[\]DjotAttributeGroup` | `/* serde(default) */` | Attributes mapped by element identifier (if present) |
...
```go
result := DoctorCheckPass("value", "value")
```
...
```go
result := DoctorCheckWarn("value", "value")
```
...
```go
result := DoctorCheckFail("value", "value")
```
...
```go
result := DoctorCheckSkip("value", "value")
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `Pages` | `uint` | — | Total pages in the source document (`0` when not page-addressable). |
| `Tables` | `uint` | — | Tables detected in the document. |
| `Images` | `uint` | — | Images detected in the document. |
...
| `ElementIndex` | `*uint` | `nil` | Position index in the element sequence |
...
| `CompressedSize` | `uint` | — | Compressed byte count of the original stream (before decompression). Used by callers to compute the decompression ratio and detect zip-bomb-style attacks that embed a tiny compressed stream expanding to gigabytes of data. |
...
```go
func (o *EmbeddingBackend) Dimensions() uint
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `Model` | `EmbeddingModelType` | `EmbeddingModelTypePreset { name: "balanced" }` | The embedding model to use (defaults to "gte-modernbert-base" preset if not specified) |
| `Normalize` | `bool` | `true` | Whether to normalize embedding vectors (recommended for cosine similarity) |
| `BatchSize` | `uint` | `32` | Batch size for embedding generation |
...
| `ColCount` | `uint` | — | Number of columns |
| `CellCount` | `uint` | — | Total number of non-empty cells |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `Kind` | `ExtractInputKind` | `ExtractInputKindURI` | Source kind. `bytes` requires `bytes`; `uri` requires `uri`. |
...
```go
result := ExtractInputFromBytes([]byte("data"), "value", "value")
```
...
```go
result := ExtractInputFromUri("value")
```
...
| `Formulas` | `\[\]Formula` | `nil` | Mathematical formulas recognized in the document. Populated from every source that produces formulas: layout-guided OCR (with geometry), VLM OCR (text only), and markup extraction (DOCX, PPTX, ODT, EPUB, HTML, JATS, LaTeX, Markdown, and related formats, without geometry). Empty when the document contains no formulas. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `MimeDetectionPolicy` | `MimeDetectionPolicy` | `MimeDetectionPolicyPreferContent` | Controls whether MIME inference prefers content, a supported extension, or content alone. |
...
| `OcrStrategy` | `OcrStrategy` | `OcrStrategyAuto` | Which pages get OCR'd when neither `force_ocr` nor `force_ocr_pages` applies. Defaults to `OcrStrategy.Auto`, which OCRs only pages whose native text fails a quality check. Only applies to PDF documents. Cannot be `OcrStrategy.ScannedPages` while `disable_ocr` is `true`. |
...
| `ExtractionTimeoutSecs` | `*uint64` | `600` | Default per-file timeout in seconds for batch extraction. When set, each file in a batch will be canceled after this duration unless overridden by `FileExtractionConfig.timeout_secs`. Defaults to `Some(600)` (10 minutes) to prevent pathological files (e.g. deeply nested archives, documents with millions of cells) from running indefinitely and exhausting caller resources, while still giving slow paths (VLM-based OCR, large scanned documents) enough headroom to finish. Set to `nil` to disable the timeout for trusted input or long-running workloads. |
| `MaxConcurrentExtractions` | `*uint` | `nil` | Maximum concurrent document extractions in batch operations. This is a ceiling within the configured total thread budget, not an independent pool size. When unset, the scheduler derives document and per-document concurrency from `ConcurrencyConfig.max_threads`. |
| `ResultFormat` | `ResultFormat` | `ResultFormatUnified` | Result structure format Controls whether results are returned in unified format (default) with all content in the `content` field, or element-based format with semantic elements (for Unstructured-compatible output). |
...
| `JupyterCellRendering` | `JupyterCellRendering` | `JupyterCellRenderingBoth` | Controls how Jupyter notebook (`.ipynb`) code cells are rendered. - `Both` (default): code source plus the notebook's saved outputs - `Source`: only the code source (fenced code blocks) - `Outputs`: only the saved outputs Cells are never executed; `Outputs`/`Both` surface only outputs already stored in the notebook. |
...
| `MaxArchiveDepth` | `uint` | `3` | Maximum recursion depth for archive extraction (default: 3). Set to 0 to disable recursive extraction (legacy behavior). |
...
```go
result := ExtractionConfigDefaultArchiveDepth()
```

**Returns:** `uint`

###### DefaultMaxEmbeddedFileBytes()
...
```go
result := ExtractionConfigDefaultMaxEmbeddedFileBytes()
```
...
```go
result := ExtractionConfigDefaultExtractionTimeout()
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `Index` | `uint` | — | Input index in the original request. |
...
| `CrawlRedirectCount` | `uint` | — | Total redirects followed while fetching or crawling URLs. |
...
```go
func ExtractionResultSingle(result ExtractedDocument) *ExtractionResult
```

**Example:**

```go
result := ExtractionResultSingle(ExtractedDocument{})
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `Inputs` | `uint` | — | Number of inputs submitted by the caller. |
| `Results` | `uint` | — | Number of extraction results produced. |
| `Errors` | `uint` | — | Number of per-input errors. |
| `RemoteUrls` | `uint` | — | Number of URI inputs that resolved to remote HTTP(S) URLs. |
| `PagesCrawled` | `uint` | — | Number of HTML pages crawled or scraped. |
| `DocumentsDownloaded` | `uint` | — | Number of downloaded non-HTML documents extracted from URLs. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `MimeDetectionPolicy` | `*MimeDetectionPolicy` | `nil` | Override MIME inference policy for this file. |
...
| `Offset` | `uint` | — | Byte offset of the anchor in the markdown text. |
...
| `Offset` | `uint` | — | Byte offset of the definition line in the markdown text. |
...
| `Level` | `*uint` | `nil` | Heading level (1-6) for headings, or nesting level for lists |
...
| `Page` | `*uint32` | `/* serde(default) */` | 1-indexed page number the formula appears on. `nil` when the source format has no page concept. The C FFI reports an absent page as `0`. |
...
| `RowSpan` | `uint32` | `1` | Number of rows this cell spans. |
| `ColSpan` | `uint32` | `1` | Number of columns this cell spans. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `Left` | `float32` | — | Left coordinate. |
| `Top` | `float32` | — | Top coordinate. |
| `Right` | `float32` | — | Right coordinate. |
| `Bottom` | `float32` | — | Bottom coordinate. |
...
| `KClusters` | `uint` | `3` | Number of font size clusters to use for hierarchy levels (1-7) Default: 3, which provides two heading levels plus body text. Larger values create more fine-grained hierarchy levels. |
...
| `Theme` | `HtmlTheme` | `HTMLThemeUnstyled` | Built-in colour/typography theme. Default: `HtmlTheme.Unstyled`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `Horizontal` | `float64` | — | Horizontal resolution. |
| `Vertical` | `float64` | — | Vertical resolution. |
...
| `OutputFormat` | `ImageOutputFormat` | `ImageOutputFormatNative` | Target format for re-encoding extracted images. When set to anything other than `Native`, each extracted image is re-encoded to the requested format before being returned. This lets callers receive uniform output without duplicating encode logic downstream. Defaults to `Native` — no re-encode pass is performed and `ExtractedImage.format` reflects the source extractor's output. |
...
| `Attributes` | `\[\]KeyValueAttribute` | — | Additional attributes as key-value pairs. |
...
| `NewDimensions` | `*PixelDimensions` | `nil` | New dimensions after resizing (if resized). |
...
| `Positions` | `*\[\]uint` | `nil` | Optional positions where keyword appears in text (character offsets). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `Algorithm` | `KeywordAlgorithm` | `KeywordAlgorithmYake` | Algorithm to use for extraction. |
| `MaxKeywords` | `uint` | `10` | Maximum number of keywords to extract (default: 10). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `Model` | `LateInteractionModelType` | `LateInteractionModelTypePreset` | The late-interaction model to use (defaults to the "gte-moderncolbert" preset). |
| `BatchSize` | `uint` | `16` | Batch size for local ONNX inference. ColBERT emits a `[seq, dim]` multi-vector embedding per document, so memory scales with batch size — keep this modest. |
| `MaxLength` | `uint` | `512` | Maximum token sequence length for the tokenizer (documents). |
| `QueryMaxLength` | `uint` | `32` | Fixed padded length for query augmentation. ColBERT queries are padded (with the mask token, kept attention-live) to exactly this many tokens rather than truncated/left as-is — this is the "query augmentation" trick from the ColBERT paper. |
...
| `MaxEmbedDurationSecs` | `*uint64` | `60` | Maximum wall-clock duration (in seconds) for a single embed call when using `LateInteractionModelType.Plugin`. `nil` disables the timeout. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `Index` | `uint` | — | Position of this document in the original input slice. |
...
| `MaxLength` | `uint` | — | Maximum document token sequence length. |
| `QueryMaxLength` | `uint` | — | Fixed padded query length (ColBERT query augmentation). |
| `Dim` | `uint` | — | Per-token embedding dimensionality. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `Strategy` | `LayoutStrategy` | `LayoutStrategyAlways` | Which pages the layout model runs on. Defaults to `LayoutStrategy.Always`, the historical behavior: every page is rendered and inferred. `LayoutStrategy.Auto` pre-screens pages with cheap signals and skips the model where it cannot help. |
...
| `TableModel` | `TableModel` | `TableModelTatr` | Table structure recognition model. Controls which model is used for table cell detection within layout-detected table regions. Defaults to `TableModel.Tatr`. |
| `FormulaModel` | `*FormulaModel` | `nil` | Formula recognition model for layout-detected formula regions. `nil` (the default) keeps the plain OCR text of the region. Setting a model converts each formula region crop to LaTeX. Requires the `formula-recognition` feature; without it the setting is ignored. |
| `TableOverlapPreference` | `TableOverlapPreference` | `TableOverlapPreferenceContent` | How to resolve overlapping native vs layout tables. When a native table and a layout (TATR/SLANeXT) table overlap on the same region, this controls which one is kept. Defaults to `TableOverlapPreference.Content` (historical behavior: keep the table with more content). Set to `TableOverlapPreference.Native` to favor source reading order (higher text F1) over the model's cell reflow. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `MaxEntries` | `*uint` | `nil` | Maximum number of cached entries. |
...
```go
func MetaSchemaCompile(metaSchemaJson string) (*MetaSchema, error)
```

**Example:**

```go
result, err := MetaSchemaCompile("value")
if err != nil {
    return err
}
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `Backend` | `NerBackendKind` | `NerBackendKindOnnx` | Backend that runs the entity detection. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `Min` | `uint` | `1` | Minimum number of words in a candidate. |
| `Max` | `uint` | `3` | Maximum number of words in a candidate. |
...
```go
func (o *OcrBackend) ConfidenceSemantics() *ConfidenceSemantics
```

**Example:**

```go
result := instance.ConfidenceSemantics()
```
...
```go
func (o *OcrBackend) PageOrientationHandling() *PageOrientationHandling
```

**Example:**

```go
result := instance.PageOrientationHandling()
```
...
| `SecurityLimits` | `*SecurityLimits` | `nil` | Security limits applied when decoding raw image bytes for OCR (GH#1554). Not user-configurable via config files — injected at runtime from `ExtractionConfig.security_limits` before each `process_image` call, the same pattern `Self.acceleration` uses. `ExtractionConfig.security_limits` is the single source of truth: this field only ever holds a copy the caller placed here immediately before dispatch, so the two cannot drift. A backend consulting a `backend_options` override for this call may still let that override win, but in the absence of one this field is what backends should fall back to instead of `SecurityLimits.default()`. `nil` means "use `SecurityLimits.default()`", never "disable the check". |
...
| `Geometry` | `OcrBoundingGeometry` | `OcrBoundingGeometryRectangle` | Bounding geometry (rectangle or quadrilateral). |
| `Confidence` | `OcrConfidence` | — | Confidence scores for detection and recognition. |
| `Level` | `OcrElementLevel` | `OcrElementLevelLine` | Hierarchical level (word, line, block, page). |
| `Rotation` | `*OcrRotation` | `nil` | Rotation information (if detected). |
| `PageNumber` | `uint32` | `1` | Page number (1-indexed). |
| `ParentId` | `*string` | `nil` | Parent element ID for hierarchical relationships. When hierarchy output is enabled, this resolves to another emitted element's `backend_metadata["element_id"]` value. |
...
| `MinLevel` | `OcrElementLevel` | `OcrElementLevelLine` | Minimum hierarchical level to include. Elements below this level (e.g., words when min_level is Line) will be excluded. |
...
| `Priority` | `uint32` | `100` | Priority weight (higher = tried first). Stages are sorted by priority descending. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `X` | `uint32` | — | Horizontal coordinate. |
| `Y` | `uint32` | — | Vertical coordinate. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `MinTotalNonWhitespace` | `uint` | `64` | Minimum total non-whitespace characters to consider text substantive. |
| `MinNonWhitespacePerPage` | `float64` | `32` | Minimum non-whitespace characters per page on average. |
| `MinMeaningfulWordLen` | `uint` | `4` | Minimum character count for a word to be "meaningful". |
| `MinMeaningfulWords` | `uint` | `3` | Minimum count of meaningful words before text is accepted. |
| `MinAlnumRatio` | `float64` | `0.3` | Minimum alphanumeric ratio (non-whitespace chars that are alphanumeric). |
| `MinGarbageChars` | `uint` | `5` | Minimum Unicode replacement characters (U+FFFD) to trigger OCR fallback. |
...
| `MinWordsForAvgLengthCheck` | `uint` | `50` | Minimum word count before average word length check applies. |
| `MinConsecutiveRepeatRatio` | `float64` | `0.08` | Minimum consecutive word repetition ratio to detect column scrambling. |
| `MinWordsForRepeatCheck` | `uint` | `50` | Minimum word count before consecutive repetition check is applied. |
| `SubstantiveMinChars` | `uint` | `100` | Minimum character count for "substantive markdown" OCR skip gate. |
| `NonTextMinChars` | `uint` | `20` | Minimum character count for "non-text content" OCR skip gate. |
...
| `MinProvenanceFallbackRatio` | `float64` | `0.5` | Minimum fraction of a page's non-whitespace characters with `MappingProvenance.Fallback` provenance before the page is treated as having a fabricated text layer and routed to OCR (issue #1254). Gated by `min_total_non_whitespace` so a short page with a stray fallback character cannot trip it. Only used when `enable_provenance_ocr_routing` is `true`. |
...
| `DetDbThresh` | `float32` | `0.3` | Database threshold for text detection (default: 0.3) Range: 0.0-1.0, higher values require more confident detections |
| `DetDbBoxThresh` | `float32` | `0.5` | Box threshold for text bounding box refinement (default: 0.5) Range: 0.0-1.0 |
| `DetDbUnclipRatio` | `float32` | `1.6` | Unclip ratio for expanding text bounding boxes (default: 1.6) Controls the expansion of detected text regions |
| `DetLimitSideLen` | `uint32` | `1024` | Maximum side length for detection image (default: 1024) Larger images may be resized to this limit for faster inference |
| `RecBatchNum` | `uint32` | `6` | Batch size for recognition inference (default: 6) Number of text regions to process simultaneously |
| `Padding` | `uint32` | `10` | Padding in pixels added around the image before detection (default: 10). Large values can include surrounding content like table gridlines. |
| `DropScore` | `float32` | `0.5` | Minimum recognition confidence score for text lines (default: 0.5). Text regions with recognition confidence below this threshold are discarded. Matches PaddleOCR Python's `drop_score` parameter. Range: 0.0-1.0 |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `ByteStart` | `uint` | — | Byte offset where this page starts in the content string (UTF-8 valid boundary, inclusive) |
| `ByteEnd` | `uint` | — | Byte offset where this page ends in the content string (UTF-8 valid boundary, exclusive) |
...
| `OcrConfidence` | `*PageOcrConfidence` | `/* serde(default) */` | Aggregate OCR confidence for this page. `nil` when the page was not OCR'd. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `Width` | `float64` | — | Page width in points or pixels. |
| `Height` | `float64` | — | Page height in points or pixels. |
...
```go
result := PageSignalsFromPageText(42, "value", 0.5)
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `Start` | `uint` | — | Inclusive byte-offset start of the match in the source text. |
| `End` | `uint` | — | Exclusive byte-offset end of the match. |
...
| `Backend` | `PdfBackend` | `PdfBackendNative` | Which engine parses and renders this PDF. Defaults to `PdfBackend.Native`. Selecting `PdfBackend.Pdfium` requires the `pdf-pdfium` feature and is rejected otherwise; the pdfium engine is also deliberately narrower in scope than `Native` -- see `PdfBackend` and `extractors.pdf.pdfium_engine` for details. |
...
```go
func (o *PdfConfig) Validate() error
```
...
| `SlideCount` | `uint` | — | Total number of slides |
| `ImageCount` | `uint` | — | Total number of embedded images |
| `TableCount` | `uint` | — | Total number of tables |
...
| `Hyperlinks` | `\[\]PresentationHyperlink` | `/* serde(default) */` | Hyperlinks discovered in slides. |
...
| `Preset` | `PreprocessingPreset` | `PreprocessingPresetStandard` | Preprocessing preset level (Minimal, Standard, Aggressive) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `MessageCount` | `uint` | — | Total number of email messages found in the PST archive. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `MinWordLength` | `uint` | `1` | Minimum word length to consider (default: 1). |
| `MaxWordsPerPhrase` | `uint` | `3` | Maximum words in a keyword phrase (default: 3). |
...
| `Strategy` | `RedactionStrategy` | `RedactionStrategyMask` | Strategy applied to every match. |
...
```go
result := RedactionPatternLabeled("value", "value")
```
...
```go
result := RedactionTermLiteral("value")
```
...
```go
result := RedactionTermLabeled("value", "value")
```
...
```go
func RegistryLoadEmbedded() (*Registry, error)
```

**Example:**

```go
result, err := RegistryLoadEmbedded()
if err != nil {
    return err
}
```
...
```go
func RegistryGlobal() *Registry
```

**Example:**

```go
result := RegistryGlobal()
```
...
```go
func (o *Registry) ExtendFromDir(dir string) (uint, error)
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `Index` | `uint` | — | Position of this document in the original input `documents` slice. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `Model` | `RerankerModelType` | `RerankerModelTypePreset { name: "balanced" }` | The reranker model to use (defaults to "balanced" preset if not specified). |
| `TopK` | `*uint` | `nil` | Return at most this many documents. `nil` returns all. Applied after sorting by score, so the highest-scoring documents are kept. |
| `BatchSize` | `uint` | `32` | Batch size for local ONNX cross-encoder inference. |
...
| `Port` | `uint16` | `8000` | Server port number |
...
| `MaxRequestBodyBytes` | `uint` | `104857600` | Maximum size of request body in bytes (default: 100 MB) |
| `MaxMultipartFieldBytes` | `uint` | `104857600` | Maximum size of multipart fields in bytes (default: 100 MB) |
...
```go
func (o *ServerConfig) MaxRequestBodyMb() uint
```
...
```go
func (o *ServerConfig) MaxMultipartFieldMb() uint
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `Model` | `SparseEmbeddingModelType` | `SparseEmbeddingModelTypePreset` | The sparse-embedding model to use (defaults to the "opensearch-v3-distill" preset). |
| `BatchSize` | `uint` | `16` | Batch size for local ONNX inference. SPLADE emits a `[seq, vocab]` logit tensor per document, so memory scales with batch size — keep this modest. |
| `MaxLength` | `uint` | `256` | Maximum token sequence length for the tokenizer. |
...
| `MaxEmbedDurationSecs` | `*uint64` | `60` | Maximum wall-clock duration (in seconds) for a single embed call when using `SparseEmbeddingModelType.Plugin`. `nil` disables the timeout. |
...
| `ModelFile` | `string` | — | Path to the ONNX file within the repo. |
| `AdditionalFiles` | `\[\]string` | — | Sibling files that must be downloaded alongside `model_file`. |
| `MaxLength` | `uint` | — | Maximum token sequence length. |
| `Description` | `string` | — | Human-readable description. |
...
| `SchemeAllowlist` | `\[\]string` | `["http", "https"]` | Allowed URI schemes. Default: `["http", "https"]`. Only `http` and `https` are supported. An empty list denies every URL. |
...
```go
result := StructuredExtractionConfigDefaultSchemaName()
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `Strategy` | `SummaryStrategy` | `SummaryStrategyExtractive` | Summarisation strategy. |
...
| `RowSpan` | `uint32` | `1` | Row span (number of rows this cell spans) |
| `ColSpan` | `uint32` | `1` | Column span (number of columns this cell spans) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `Row` | `uint32` | — | Zero-indexed row of the cell this style belongs to. |
| `Col` | `uint32` | — | Zero-indexed column of the cell this style belongs to. |
| `HeadingLevel` | `*uint8` | `nil` | Outline level 1-6 when the style resolves to a heading, otherwise `nil`. |
| `StyleName` | `*string` | `nil` | Human-readable style name, e.g. `heading 2`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `FromIndex` | `uint` | — | Zero-based index of the table in both `a.tables` and `b.tables`. |
| `ToIndex` | `uint` | — | Zero-based index in `b.tables` (equal to `from_index` for same-dimension tables). |
...
| `LineCount` | `uint` | — | Number of lines |
...
| `Links` | `*\[\]MarkdownLink` | `nil` | Markdown links (Markdown files only). |
| `CodeBlocks` | `*\[\]MarkdownCodeBlock` | `nil` | Code blocks (Markdown files only). |
...
| `Links` | `*\[\]MarkdownLink` | `nil` | Markdown links (for Markdown files). |
| `CodeBlocks` | `*\[\]MarkdownCodeBlock` | `nil` | Code blocks (for Markdown files). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `Level` | `ReductionLevel` | `ReductionLevelModerate` | Reduction intensity level. |
...
```go
func (o *TokenizerBackend) CountTokens(text string) uint
```
...
| `Model` | `WhisperModel` | `WhisperModelTiny` | Whisper model size to use. Smaller = faster + lower memory. `tiny` is the pragmatic default for first-time users and CI. |
...
| `MaxDurationMs` | `*uint64` | `1800000` | Hard safety limit on input duration (milliseconds). Files longer than this are rejected after decode, before model work. Default: 30 minutes. Set to `nil` to disable (not recommended for untrusted input). |
| `MaxBytes` | `*uint64` | `536870912` | Hard safety limit on input size (bytes). Default: 512 MiB. Protects against pathological or malicious uploads. |
| `TimeoutMs` | `*uint64` | `600000` | Wall-clock timeout for the entire transcription operation (ms). Bounds audio decode, model resolution/download, and inference together. On expiry the extraction fails with an `XbergError.Transcription`. `nil` disables the bound and lets the operation run unbounded (not recommended for untrusted input). Enforced on the async extraction path only; the size and duration caps (`max_bytes`, `max_duration_ms`) are checked on every path. Default: 10 minutes. |
...
| `ChunkMaxSize` | `*uint` | `nil` | Maximum chunk size in bytes. `nil` disables chunking. |
| `ContentMode` | `CodeContentMode` | `CodeContentModeChunks` | Content rendering mode for code extraction. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `Mode` | `UrlExtractionMode` | `URLExtractionModeAuto` | URL extraction mode. |
...
```go
result := UrlExtractionConfigDefaultXbergCrawlConfig()
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `WindowSize` | `uint` | `2` | Window size for co-occurrence analysis (default: 2). Controls the context window for computing co-occurrence statistics. |
...
| `Custom` | Use a custom ONNX model from HuggingFace — Fields: `ModelId`: `string`, `Dimensions`: `uint` |
...
| Value | Description |
|-------|-------------|
| `Legibility` | Validated to track legibility on a known scale — usable as an absolute quality gate. — Fields: `ScaleMax`: `float64` |
...
| `MetadataBlock` | Structured metadata block (email headers, YAML frontmatter, etc.). — Fields: `Entries`: `[]KeyValueAttribute` |
...
| `Quadrilateral` | 4-point quadrilateral for rotated/skewed text (PaddleOCR). Points are in clockwise order starting from top-left: `[top_left, top_right, bottom_right, bottom_left]` — Fields: `Points`: `[]OcrPoint` |
...
| Value | Description |
|-------|-------------|
| `Paragraph` | Body paragraph, identified by its zero-based index in the document flow. — Fields: `Index`: `uint` |
| `TableCell` | Cell inside a table. — Fields: `Row`: `uint`, `Col`: `uint`, `TableIndex`: `uint` |
| `Page` | Page, identified by its zero-based index. — Fields: `Index`: `uint` |
| `Slide` | Presentation slide, identified by its zero-based index. — Fields: `Index`: `uint` |
| `Sheet` | Spreadsheet cell or range, identified by sheet index and optional name. — Fields: `Index`: `uint`, `Name`: `string` |
...
| Variant | Description |
|---------|-------------|
| `ErrXbergIo` | A file system or I/O operation failed. These errors always bubble up unchanged. |
| `ErrParsing` | Document parsing failed (e.g. corrupt file, unsupported format feature). |
| `ErrOcr` | An OCR engine returned an error or produced unusable output. |
| `ErrValidation` | Invalid configuration or input parameters were supplied. |
| `ErrCache` | A cache read or write operation failed. |
| `ErrImageProcessing` | An image manipulation operation (resize, decode, DPI conversion) failed. |
| `ErrSerialization` | JSON or MessagePack serialization/deserialization failed. |
| `ErrMissingDependency` | A required optional system dependency (e.g. `tesseract`) was not found. |
| `ErrPlugin` | A registered plugin returned an error during extraction. |
| `ErrLockPoisoned` | An internal `Mutex` or `RwLock` was found in a poisoned state. |
| `ErrUnsupportedFormat` | The document's MIME type is not supported by any registered extractor. |
| `ErrEmbedding` | The embedding model or embedding pipeline returned an error. |
| `ErrReranking` | The reranker model or reranking pipeline returned an error. |
| `ErrTranscription` | Audio/video transcription failed. |
| `ErrTimeout` | The extraction operation exceeded the configured time limit. |
| `ErrCancelled` | The extraction stopped after a cooperative cancellation request. Rust callers can request cancellation through `CancellationToken`. The REST async-jobs API uses the same mechanism. Generated language bindings do not currently expose in-process cancellation. |
| `ErrSecurity` | A security policy was violated (e.g. zip bomb, oversized archive). |
| `ErrOther` | A catch-all for uncommon errors that do not fit another variant. |
...
| Variant | Description |
|---------|-------------|
| `ErrConfigError` | Invalid configuration value. |
| `ErrPdfAnalysisError` | PDF analysis step failed (only when `heuristics-pdf` feature is active). |
...
| Variant | Description |
|---------|-------------|
| `ErrParse` | The file is not valid JSON. |
| `ErrSchemaValidation` | The file parses as JSON but does not validate against the meta-schema. |
| `ErrDeserialize` | The file validates but cannot be deserialized into `Preset`. |
| `ErrIdMismatch` | The preset's declared `id` does not match its file-system location. |
| `ErrBadMetaSchema` | The meta-schema itself failed to compile. |
| `ErrLoadIo` | A filesystem I/O error occurred while reading a preset directory. |
...
| Variant | Description |
|---------|-------------|
| `ErrSchemaNotObject` | A custom schema override was supplied but is not a JSON object. |
