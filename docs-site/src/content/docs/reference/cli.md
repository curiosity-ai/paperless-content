...
<!-- alef:hash:6c26e46ec949756623ac570c6fcf95ffae017cb831b7ec910817b0809fcb4ce9 -->
...
| `ocr` | `--ocr` | `Option<bool>` |  | Enable or disable OCR. When true, configures an OCR backend (default: tesseract). When false, hard-disables OCR and removes its configuration. |
| `ocr_backend` | `--ocr-backend` | `Option<String>` |  | OCR backend to use when --ocr is enabled (tesseract, paddle-ocr, sceptre, vlm, or candle-*). |
...
| `ocr_no_cache` | `--ocr-no-cache` | `Option<bool>` |  | Bypass the on-disk OCR result cache for this run: neither read nor write it. |
...
| `layout_formula_model` | `--layout-formula-model` | `Option<String>` |  | Formula recognition model for layout-detected formula regions: latex_ocr. |
...
| `pdf_extract_tables` | `--pdf-extract-tables` | `Option<bool>` |  | Extract tables from PDF (native engine grid + heuristic text-layer fallback). Default: true. |
...
| `pdf_backend` | `--pdf-backend` | `Option<String>` |  | PDF extraction backend to use: "native" (default) or "pdfium". |
...
| `csv_delimiter` | `--csv-delimiter` | `Option<String>` |  | CSV/TSV field delimiter (single ASCII character, e.g. ";", "\|", "\t"). When unset, the delimiter is auto-detected from the file. |
| `csv_comment_prefix` | `--csv-comment-prefix` | `Vec<String>` |  | Line prefix marking a CSV/TSV comment line to skip entirely (e.g. "#"). Can be specified multiple times. Default: no comment filtering. |
...
| `config` | `--config` | `Option<PathBuf>` |  | Path to config file (TOML, YAML, or JSON). If not specified, searches for xberg.toml/yaml/json in current and parent directories. |
| `no_config_discovery` | `--no-config-discovery` | `bool` |  | Skip project and user config discovery and start from built-in defaults. |
| `format` | `--format` | `WireFormat` | `text` | Output format |
| `clean` | `--clean` | `bool` |  | Delete stray files reported in xberg-owned cache directories after probing |
...
| Name | Flags | Type | Default | Description |
| --- | --- | --- | --- | --- |
| `languages` |  | `Vec<String>` |  | Language names to download (e.g., python rust go) |
...
| `groups` | `--groups` | `Option<Vec<String>>` |  | Download specific language groups (comma-separated: web,systems,scripting,data,jvm,functional) |
| `cache_dir` | `--cache-dir` | `Option<PathBuf>` |  | Grammar cache directory. CLI arg overrides the config file's tree_sitter.cache_dir. |
| `from_config` | `--from-config` | `bool` |  | Resolve cache_dir/languages/groups from the auto-discovered xberg config's [tree_sitter] section. Explicit CLI args (--cache-dir, languages, --groups) still win. |
| `format` | `--format` | `WireFormat` | `text` | Output format (text or json) |

#### `xberg tree-sitter list`
...
| Name | Flags | Type | Default | Description |
| --- | --- | --- | --- | --- |
| `downloaded` | `--downloaded` | `bool` |  | Only list already-downloaded languages |
| `filter` | `--filter` | `Option<String>` |  | Filter languages by name substring |
| `format` | `--format` | `WireFormat` | `text` | Output format (text or json) |
