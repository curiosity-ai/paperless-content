...
<!-- alef:hash:f0c6f8b91b8472318347324fc6911a7c7b8acd3dcd58ee8822d5d43d4cf48ad6 -->
...
```ruby
def self.map_url(uri, config)
```

**Example:**

```ruby
result = map_url("value", UrlExtractionConfig.new)
```
...
```ruby
def self.register_document_extractor(extractor)
```

**Example:**

```ruby
register_document_extractor(DocumentExtractor.new)
```
...
```ruby
def self.unregister_document_extractor(name)
```
...
```ruby
register_renderer(Renderer.new)
```
...
```ruby
def self.clear_tokenizer_backends()
```
...
```ruby
def self.list_tokenizer_backends()
```
...
```ruby
def self.classify_chunks_owned(result, config)
```

**Example:**

```ruby
result = classify_chunks_owned(ExtractedDocument.new, ChunkClassificationConfig.new)
```
...
```ruby
def self.max_sim_score(query, doc)
```

**Example:**

```ruby
result = max_sim_score(MultiVectorEmbedding.new, MultiVectorEmbedding.new)
```
...
```ruby
def self.max_sim_rank(query, docs)
```

**Example:**

```ruby
result = max_sim_rank(MultiVectorEmbedding.new, [])
```
...
|------|------|----------|-------------|
| `query` | `MultiVectorEmbedding` | Yes | The multi vector embedding |
| `docs` | `Array<MultiVectorEmbedding>` | Yes | The docs |

**Returns:** `Array<LateInteractionMatch>`
...
```ruby
def self.doctor(config)
```

**Example:**

```ruby
result = doctor(ExtractionConfig.new)
```
...
| `key_values` | `Array<KeyValueAttribute>` | `[]` | Key-value pairs (key="value") |
...
| `cross_region_prefix` | `String?` | `nil` | Cross-region inference profile prefix (e.g. `"us"`). |
| `access_key_id` | `String?` | `nil` | Explicit AWS access key ID. Secret — never logged. |
| `secret_access_key` | `String?` | `nil` | Explicit AWS secret access key. Secret — never logged. |
| `session_token` | `String?` | `nil` | Explicit AWS session token for temporary credentials. Secret — never logged. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `prompt_template` | `String?` | `nil` | Minijinja prompt template. Receives `{{ definitions }}` (rendered label + description list) and `{{ chunks }}` (a numbered list of chunk texts in the current batch) variables. `nil` lets the backend pick a sensible default. |
...
| `node_types` | `Array<String>` | — | Tree-sitter node kinds that appear at the top level of this chunk (e.g. `"function_definition"`, `"class_definition"`). |
...
| `attributes` | `Array<CodeDataAttribute>` | `/* serde(default) */` | Attributes on element-shape nodes (XML `STag` attributes). Empty for all other kinds. |
| `children` | `Array<CodeDataNode>` | `/* serde(default) */` | Children for nested containers and XML element bodies. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `chunks` | `Array<CodeChunkInfo>` | `[]` | Structural code chunks (function/class/module boundaries). |
...
| `max_depth` | `Integer?` | `nil` | Maximum DOM traversal depth. `nil` uses the library's internal native-stack safety limit. Explicit values above that safety limit are clamped to prevent process-aborting stack overflows on pathologically deep DOM trees. |
...
| `max_links_per_page` | `Integer?` | `nil` | Maximum links enqueued from a single page. Defaults to 10000. Bounds the work one hostile or pathological page can create; links past the cap are dropped and a warning is logged. |
...
| `content_filter` | `ContentFilterKind?` | `nil` | Content filter applied to each page. `nil` keeps every page. A content filter set explicitly on `CrawlEngineBuilder` takes precedence. |
| `bm25_query` | `String?` | `nil` | Query the BM25 content filter scores pages against. Required by `ContentFilterKind.Bm25`. |
| `bm25_threshold` | `Float?` | `nil` | Minimum BM25 score a page must reach to be kept. Defaults to `0.0`. |
...
| `comment_prefixes` | `Array<String>` | `[]` | Line prefixes that mark a comment line to skip entirely during row parsing (e.g. `["#"]`). A line is treated as a comment when its trimmed start matches any of these prefixes exactly. Default: empty, meaning no line is treated as a comment (matches the pre-existing extractor behavior). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `model_path` | `String?` | `nil` | Local DeepSeek-OCR model directory. The backend requires this option. |
| `device` | `CandleDevicePreference?` | `nil` | Optional per-call device override. |
| `version` | `Integer?` | `nil` | DeepSeek-OCR model generation, either 1 or 2. Defaults to 2. |
...
```ruby
result = instance.extract(ExtractInput.new, ExtractionConfig.new)
```
...
| `changed` | `Array<EmbeddedDiff>` | `[]` | Children present in both but with differing content (matched by `path`). Each entry holds the diff of the nested `ExtractedDocument`. |
...
| `cache_dir` | `String?` | `nil` | Optional alternate Hugging Face cache root for model files. When unset, hf-hub follows `HF_HUB_CACHE`, `HUGGINGFACE_HUB_CACHE`, `HF_HOME`, XDG, and platform defaults. Prefer those environment variables when configuring the cache process-wide. |
...
| `detected_language_confidences` | `Array<LanguageConfidence>?` | `[]` | Structured per-language detection results: confidence, document share, script, and reliability, alongside the ISO-code-only `detected_languages` (#261). One entry per language in `detected_languages`, in the same order. `nil` under the same conditions as `detected_languages`: detection disabled, empty input text, or no language met the configured `min_confidence`. |
...
| `ocr_result` | `ExtractedDocument?` | `nil` | Nested OCR extraction result (if image was OCRed) When OCR is performed on this image, the result is embedded here rather than in a separate collection, making the relationship explicit. |
...
| `extraction_timeout_secs` | `Integer?` | `600` | Default per-file timeout in seconds for batch extraction. When set, each file in a batch will be canceled after this duration unless overridden by `FileExtractionConfig.timeout_secs`. Defaults to `Some(600)` (10 minutes) to prevent pathological files (e.g. deeply nested archives, documents with millions of cells) from running indefinitely and exhausting caller resources, while still giving slow paths (VLM-based OCR, large scanned documents) enough headroom to finish. Set to `nil` to disable the timeout for trusted input or long-running workloads. |
| `max_concurrent_extractions` | `Integer?` | `nil` | Maximum concurrent document extractions in batch operations. This is a ceiling within the configured total thread budget, not an independent pool size. When unset, the scheduler derives document and per-document concurrency from `ConcurrencyConfig.max_threads`. |
...
| `geojson` | `GeoJsonExtractionConfig?` | `nil` | GeoJSON extraction configuration (None = bounded summary). By default, GeoJSON coordinates are replaced by aggregate counts and bounds so large geometry arrays do not become unbounded rendered output. Set `include_full_coordinates` explicitly to retain the legacy full-coordinate output. |
| `concurrency` | `ConcurrencyConfig?` | `nil` | Concurrency limits for constrained environments (None = use defaults). Controls Rayon thread pool size, ONNX Runtime intra-op threads, and the combined document/inner-task budget for batch extraction. See `ConcurrencyConfig` for details. |
...
| `structured_extraction` | `StructuredExtractionConfig?` | `nil` | Structured extraction via LLM (None = disabled). When set, the extracted document content is sent to an LLM with the provided JSON schema. The structured response is stored in `ExtractedDocument.structured_output`. |
...
| `page_classification` | `PageClassificationConfig?` | `nil` | Per-page classification configuration. When set, the classification post-processor runs at the Middle stage and populates `ExtractedDocument.page_classifications`. |
| `chunk_classification` | `ChunkClassificationConfig?` | `nil` | Per-chunk multi-label classification configuration. When set, the chunk-classification post-processor runs at the Middle stage (after chunking) and populates `ChunkMetadata.classifications` on every chunk. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `mime_detection_policy` | `MimeDetectionPolicy?` | `nil` | Override MIME inference policy for this file. |
...
| `ocr_strategy` | `OcrStrategy?` | `nil` | Override the OCR page-selection strategy for this file. |
...
| `html_output` | `HtmlOutputConfig?` | `nil` | Override styled HTML output configuration for this file. |
...
| `page_classification` | `PageClassificationConfig?` | `nil` | Override per-page classification configuration for this file. |
| `chunk_classification` | `ChunkClassificationConfig?` | `nil` | Override per-chunk classification configuration for this file. |
| `captioning` | `CaptioningConfig?` | `nil` | Override VLM captioning configuration for this file. |
| `qr_codes` | `Boolean?` | `nil` | Override QR-code detection for this file. |
...
| `page` | `Integer?` | `/* serde(default) */` | 1-indexed page number the formula appears on. `nil` when the source format has no page concept. The C FFI reports an absent page as `0`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `task` | `GlmOcrTaskKind?` | `nil` | Optional recognition task; the backend constructor's task is used when absent. |
| `device` | `CandleDevicePreference?` | `nil` | Optional per-call device override. |
| `layout_mode` | `GlmOcrLayoutMode?` | `nil` | Optional whole-page or layout-paired dispatch mode. |
| `enable_chart_understanding` | `Boolean?` | `nil` | Whether chart regions use chart understanding instead of captioning. |
| `cache_dir` | `String?` | `nil` | Optional Hugging Face cache root. |
...
| `bbox` | `HierarchicalBoundingBox?` | `nil` | Bounding box information for the block Contains left, top, right, and bottom coordinates in PDF units. |
...
| `dimensions` | `ImageDimensions?` | `nil` | Image dimensions if available. |
...
| `new_dimensions` | `PixelDimensions?` | `nil` | New dimensions after resizing (if resized). |
...
| `cache_dir` | `String?` | `nil` | Optional alternate Hugging Face cache root for model files. When unset, hf-hub follows the standard Hugging Face environment and platform cache conventions. |
| `acceleration` | `AccelerationConfig?` | `nil` | Hardware acceleration for the late-interaction ONNX model. |
| `max_embed_duration_secs` | `Integer?` | `60` | Maximum wall-clock duration (in seconds) for a single embed call when using `LateInteractionModelType.Plugin`. `nil` disables the timeout. |
...
| `additional_files` | `Array<String>` | — | Sibling files that must be downloaded alongside `model_file`. |
...
| `formula_model` | `FormulaModel?` | `nil` | Formula recognition model for layout-detected formula regions. `nil` (the default) keeps the plain OCR text of the region. Setting a model converts each formula region crop to LaTeX. Requires the `formula-recognition` feature; without it the setting is ignored. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `global_limit` | `Float?` | `nil` | Global spend limit in USD. |
| `model_limits` | `Hash{String=>Float}?` | `{}` | Per-model spend limits in USD, keyed by model name. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `max_entries` | `Integer?` | `nil` | Maximum number of cached entries. |
| `ttl_seconds` | `Integer?` | `nil` | Cache entry time-to-live, in seconds. |
| `backend` | `String?` | `nil` | Cache backend name (e.g. `"memory"`, or an `opendal` scheme). |
| `backend_config` | `Hash{String=>String}?` | `{}` | Backend-specific configuration key/value pairs. |
...
| `timeout_secs` | `Integer?` | `nil` | Request timeout in seconds. When `nil`, liter-llm's built-in 60s default applies, except the VLM OCR path which uses a 300s default (a single page image transcription routinely exceeds 60s). Set explicitly to override. |
...
| `stop` | `Array<String>?` | `[]` | Stop sequence(s) that halt token generation, applied to individual requests built from this config. Mirrors liter-llm's `ChatCompletionRequest.stop` (`types.common.StopSequence`), which liter-llm represents as either a single string or a list of strings via an untagged enum. Always expressed here as a list — even one stop sequence is `["..."]` — so the field has a single, FFI-friendly shape across every language binding instead of a single-or-list union type. Converted to liter-llm's `StopSequence.Multiple` at each request-building call site; see `llm.client.to_stop_sequence`. |
| `seed` | `Integer?` | `nil` | Random seed for reproducible outputs, applied to individual requests built from this config. Provider support varies — some silently ignore it. Mirrors liter-llm's `ChatCompletionRequest.seed`. |
...
| `frequency_penalty` | `Float?` | `nil` | Frequency penalty for generation tasks, applied to individual requests built from this config. Positive values discourage the model from repeating the same tokens verbatim. Validated to `[-2.0, 2.0]` by `LlmConfig.validate`. Mirrors liter-llm's `ChatCompletionRequest.frequency_penalty`. |
| `reasoning_effort` | `String?` | `nil` | Reasoning effort level for extended-thinking models, applied to individual requests built from this config. Mirrors liter-llm's `ChatCompletionRequest.reasoning_effort` (`types.chat.ReasoningEffort`). A request-time parameter like `temperature`/ `max_tokens` above, not a client-level setting — `into_client_builder` does not map it. Accepted as a plain string — one of `"low"`, `"medium"`, `"high"`, `"minimal"`, `"max"` (case-insensitive; liter-llm's own `#[serde(rename_all = "lowercase")]` spelling) — rather than importing liter-llm's enum, because this module compiles even when the `liter-llm` feature is disabled. See `llm.client.parse_reasoning_effort` for the conversion into `liter_llm.ReasoningEffort`. |
| `extra_body` | `Object?` | `nil` | Provider-specific extra parameters merged into the request body (guardrails, safety settings, grounding config, etc.), applied to individual requests built from this config. Mirrors liter-llm's `ChatCompletionRequest.extra_body`. A request-time parameter like `temperature`/`max_tokens` above, not a client-level setting. |
| `load_env` | `Boolean?` | `nil` | Whether liter-llm should load provider credentials from environment variables. Mirrors liter-llm's `ClientConfigBuilder.load_env`. When `nil`, liter-llm's own default behavior applies. |
| `headers` | `Hash{String=>String}?` | `{}` | Extra HTTP headers sent with every request to the provider. Mirrors liter-llm's `ClientConfigBuilder.header`, for gateways or providers that require custom auth/routing headers. |
| `providers` | `Array<LlmProviderConfig>?` | `[]` | Custom provider configurations, in addition to liter-llm's built-in providers. Mirrors liter-llm's `LlmConfig.providers`, for OpenAI-compatible gateways and self-hosted model servers that are not in the built-in provider catalog. |
...
| `rate_limit` | `LlmRateLimitConfig?` | `nil` | Per-model rate limiting configuration. Mirrors liter-llm's `LlmConfig.rate_limit`. Only takes effect when liter-llm's `tower` feature is compiled in; otherwise the value is accepted but unused. |
| `cost_tracking` | `Boolean?` | `nil` | Enable per-request cost tracking. Mirrors liter-llm's `LlmConfig.cost_tracking`. Only takes effect when liter-llm's `tower` feature is compiled in; otherwise the value is accepted but unused. |
| `tracing` | `Boolean?` | `nil` | Enable OpenTelemetry-compatible tracing spans. Mirrors liter-llm's `LlmConfig.tracing`. Only takes effect when liter-llm's `tower` feature is compiled in; otherwise the value is accepted but unused. |
| `cooldown_secs` | `Integer?` | `nil` | Cooldown duration after transient errors, in seconds. Mirrors liter-llm's `LlmConfig.cooldown_secs`. Only takes effect when liter-llm's `tower` feature is compiled in; otherwise the value is accepted but unused. |
| `health_check_secs` | `Integer?` | `nil` | Background health check interval, in seconds. Mirrors liter-llm's `LlmConfig.health_check_secs`. Only takes effect when liter-llm's `tower` feature is compiled in; otherwise the value is accepted but unused. |
...
| `auth_header` | `String?` | `nil` | Header name used to carry the API key (defaults to `Authorization` when unset). |
| `model_prefixes` | `Array<String>` | `[]` | Model name prefixes routed to this provider (e.g. `["my-provider/"]`). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `rpm` | `Integer?` | `nil` | Requests per minute limit. |
| `tpm` | `Integer?` | `nil` | Tokens per minute limit. |
| `window_seconds` | `Integer?` | `nil` | Rate limit window, in seconds. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `det_model` | `String` | — | Exact path to the detection ONNX model in the Hugging Face snapshot. |
| `cls_model` | `String` | — | Exact path to the classification ONNX model in the Hugging Face snapshot. |
| `rec_model` | `String` | — | Exact path to the recognition ONNX model in the Hugging Face snapshot. |
...
| `data` | `Array<Float>` | — | Flat row-major buffer, length `num_tokens * dim`. |
...
```ruby
result = instance.confidence_semantics()
```
...
```ruby
def page_orientation_handling()
```

**Example:**

```ruby
result = instance.page_orientation_handling()
```
...
| `security_limits` | `SecurityLimits?` | `nil` | Security limits applied when decoding raw image bytes for OCR (GH#1554). Not user-configurable via config files — injected at runtime from `ExtractionConfig.security_limits` before each `process_image` call, the same pattern `Self.acceleration` uses. `ExtractionConfig.security_limits` is the single source of truth: this field only ever holds a copy the caller placed here immediately before dispatch, so the two cannot drift. A backend consulting a `backend_options` override for this call may still let that override win, but in the absence of one this field is what backends should fall back to instead of `SecurityLimits.default()`. `nil` means "use `SecurityLimits.default()`", never "disable the check". |
...
| `parent_id` | `String?` | `nil` | Parent element ID for hierarchical relationships. When hierarchy output is enabled, this resolves to another emitted element's `backend_metadata["element_id"]` value. |
...
| `ocr_elements` | `Array<OcrElement>?` | `[]` | Structured OCR elements with bounding boxes and confidence scores. Available when TSV output is requested or table detection is enabled. |
...
| `cache_dir` | `String?` | `nil` | Optional Hugging Face Hub cache root for model files. When unset, the standard `HF_HUB_CACHE`, legacy `HUGGINGFACE_HUB_CACHE`, and `HF_HOME` conventions are used. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `task` | `PaddleOcrVlTaskKind?` | `nil` | Optional per-call recognition task; the backend constructor's task is used when absent. |
| `model_path` | `String?` | `nil` | Optional local model directory, which takes precedence over `model_id`. |
| `model_id` | `String?` | `nil` | Optional Hugging Face repository identifier. |
| `hf_revision` | `String?` | `nil` | Optional immutable Hugging Face model revision. |
| `cache_dir` | `String?` | `nil` | Optional Hugging Face cache root. |
| `device` | `CandleDevicePreference?` | `nil` | Optional per-call device override. |
...
| `image_indices` | `Array<Integer>` | `/* serde(default) */` | Indices into `ExtractedDocument.images` for images found on this page. Each value is a zero-based index into the top-level `images` collection. Only populated when `extract_images = true` in the extraction config. |
...
| `ocr_confidence` | `PageOcrConfidence?` | `/* serde(default) */` | Aggregate OCR confidence for this page. `nil` when the page was not OCR'd. |
...
| `quad_points` | `Array<BoundingBox>?` | `/* serde(default) */` | Per-line bounding boxes derived from the annotation's `/QuadPoints` entry. Present for text markup annotations (Highlight, Underline, StrikeOut, Squiggly), one box per marked line/run of text. |
| `marked_text` | `String?` | `/* serde(default) */` | The document text covered by `Self.quad_points`, recovered from the page content underneath the marked-up region. Populated for Highlight, Underline, StrikeOut, and Squiggly annotations when the underlying text could be recovered. |
...
| `scanned_confidence` | `Float?` | `nil` | How strongly the document's most scan-like page resembles a scan, in `[0.0, 1.0]`. `nil` when the document could not be inspected. A full-page raster with no visible text scores at least `0.85`; a born-digital slide with a full-bleed background image scores `0.50`. |
| `scanned_pages` | `Array<Integer>?` | `[]` | Pages that look like scans (1-indexed), using the default confidence threshold. `nil` when the document could not be inspected; empty when no page qualifies. |
| `layout_gated_pages` | `Array<Integer>?` | `[]` | Pages the `auto` layout strategy skipped (1-indexed). `nil` unless layout detection ran with `LayoutStrategy.Auto`; empty when the gate selected every page. |
| `layout_gate_reasons` | `Array<String>?` | `[]` | Why the `auto` layout gate selected or skipped each page. Index `i` is page `i + 1`. Snake_case values such as `multi_column`, `table_grid`, or `plain_text` (the skip reason). `nil` unless layout detection ran with `LayoutStrategy.Auto`. |
...
```ruby
instance.process(ExtractedDocument.new, ExtractionConfig.new)
```
...
```ruby
result = instance.should_process(ExtractedDocument.new, ExtractionConfig.new)
```
...
```ruby
result = instance.estimated_duration_ms(ExtractedDocument.new)
```
...
|-------|------|---------|-------------|
| `url` | `String` | — | Link destination. |
| `label` | `String?` | `nil` | Optional visible label. |
...
```ruby
result = instance.render_result(ExtractedDocument.new)
```
...
| `max_rerank_duration_secs` | `Integer?` | `60` | Maximum wall-clock duration (in seconds) for a single `rerank()` call when using `RerankerModelType.Plugin`. Applies only to the in-process plugin path — protects against hung host-language backends. On timeout, the dispatcher returns `Plugin` instead of blocking forever. `nil` disables the timeout. The default (60 seconds) is conservative for common in-process inference; increase for large document sets on slow hardware. |
...
| `property_changes` | `Array<PropertyChange>` | `[]` | Formatting or metadata property changes for this revision. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `indices` | `Array<Integer>` | — | Vocabulary token ids with non-zero weight, ascending. |
| `values` | `Array<Float>` | — | Weights parallel to `SparseEmbedding.indices`. |
...
| `max_embed_duration_secs` | `Integer?` | `60` | Maximum wall-clock duration (in seconds) for a single embed call when using `SparseEmbeddingModelType.Plugin`. `nil` disables the timeout. |
...
| `scheme_allowlist` | `Array<String>` | `["http", "https"]` | Allowed URI schemes. Default: `["http", "https"]`. Only `http` and `https` are supported. An empty list denies every URL. |
...
| `value` | `Object?` | `nil` | The parsed document as a canonical `serde_json.Value` tree, when the source format could be represented as one. `nil` only for TOML inputs whose `toml.Value` fails to round-trip through `serde_json.Value` (xberg-io/xberg#155): the extractor falls back to a raw code block in that case. |
...
Flat rather than a nested `Array<Vec<Option<..>>>`: the nested shape marshals badly across the
...
| `heading_level` | `Integer?` | `nil` | Outline level 1-6 when the style resolves to a heading, otherwise `nil`. |
| `style_name` | `String?` | `nil` | Human-readable style name, e.g. `heading 2`. |
...
`i32`/`Array<String>`).** They are two independent struct definitions bridged only by
...
| `links` | `Array<MarkdownLink>?` | `nil` | Markdown links (Markdown files only). |
| `code_blocks` | `Array<MarkdownCodeBlock>?` | `nil` | Code blocks (Markdown files only). |
...
| `links` | `Array<MarkdownLink>?` | `[]` | Markdown links (for Markdown files). |
| `code_blocks` | `Array<MarkdownCodeBlock>?` | `[]` | Code blocks (for Markdown files). |
...
```ruby
result = instance.count_tokens("value")
```
...
| `max_duration_ms` | `Integer?` | `1800000` | Hard safety limit on input duration (milliseconds). Files longer than this are rejected after decode, before model work. Default: 30 minutes. Set to `nil` to disable (not recommended for untrusted input). |
| `max_bytes` | `Integer?` | `536870912` | Hard safety limit on input size (bytes). Default: 512 MiB. Protects against pathological or malicious uploads. |
| `timeout_ms` | `Integer?` | `600000` | Wall-clock timeout for the entire transcription operation (ms). Bounds audio decode, model resolution/download, and inference together. On expiry the extraction fails with an `XbergError.Transcription`. `nil` disables the bound and lets the operation run unbounded (not recommended for untrusted input). Enforced on the async extraction path only; the size and duration caps (`max_bytes`, `max_duration_ms`) are checked on every path. Default: 10 minutes. |
| `model_cache_dir` | `String?` | `nil` | Optional alternate Hugging Face cache root for Whisper models. When unset, hf-hub follows `HF_HUB_CACHE`, `HUGGINGFACE_HUB_CACHE`, `HF_HOME`, XDG, and platform defaults. Files remain in the standard content-addressed snapshot layout and are not copied into an Xberg cache. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `variant` | `CandleTrocrVariant?` | `nil` | Optional model variant; the backend constructor's variant is used when absent. |
| `device` | `CandleDevicePreference?` | `nil` | Optional per-call device override. |
| `cache_dir` | `String?` | `nil` | Optional Hugging Face cache root. |
| `hf_revision` | `String?` | `nil` | Optional immutable Hugging Face model revision. |
...
| `max_document_urls_per_result` | `Integer?` | `100` | Maximum URLs to follow per extraction result. |
| `max_total_urls` | `Integer?` | `1000` | Maximum URLs followed across the whole extraction call. |
...
```ruby
instance.validate(ExtractedDocument.new, ExtractionConfig.new)
```
...
```ruby
result = instance.should_validate(ExtractedDocument.new, ExtractionConfig.new)
```
...
| `custom` | Use a custom ColBERT ONNX model from HuggingFace. — Fields: `model_id`: `String`, `model_file`: `String`, `additional_files`: `Array<String>`, `max_length`: `Integer` |
...
| Value | Description |
|-------|-------------|
| `azure_ad` | Azure AD OAuth2 client-credentials flow (Azure OpenAI / Azure Cognitive Services). — Fields: `tenant_id`: `String`, `client_id`: `String`, `client_secret`: `String`, `scope`: `String` |
...
| `bedrock_web_identity` | AWS STS `AssumeRoleWithWebIdentity` (EKS IRSA / OIDC federation) for Bedrock. — Fields: `role_arn`: `String`, `token_file`: `String`, `session_name`: `String`, `region`: `String` |
...
| `tokenizer` | Size measured in tokens from a HuggingFace tokenizer or a registered tokenizer backend. — Fields: `model`: `String`, `cache_dir`: `String` |
...
| `custom` | Use a custom ONNX cross-encoder from HuggingFace. — Fields: `model_id`: `String`, `model_file`: `String`, `additional_files`: `Array<String>`, `max_length`: `Integer`, `head`: `RerankerHead` |
...
| `custom` | Use a custom SPLADE (`BertForMaskedLM`) ONNX model from HuggingFace. — Fields: `model_id`: `String`, `model_file`: `String`, `additional_files`: `Array<String>`, `max_length`: `Integer` |
...
| `metadata_block` | Structured metadata block (email headers, YAML frontmatter, etc.). — Fields: `entries`: `Array<KeyValueAttribute>` |
